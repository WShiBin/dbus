...
BusConfigParser* bus_config_parser_new(
    const DBusString*      basedir,
...
static dbus_bool_t locate_attributes(
    BusConfigParser* parser,
...
static dbus_bool_t check_no_attributes(
    BusConfigParser* parser,
    const char*      element_name,
    const char**     attribute_names,
    const char**     attribute_values,
...
static dbus_bool_t start_busconfig_child(
    BusConfigParser* parser,
    const char*      element_name,
    const char**     attribute_names,
    const char**     attribute_values,
    ...
    if (element_type == ELEMENT_USER) {
        ...
        if (_dbus_set_up_transient_session_servicedirs(&dirs, &local_error)) {
            if (!service_dirs_absorb_string_list(
                    &parser->service_dirs,
                    &dirs,
        ...
        if (((context && user) || (context && group) || (context && at_console)) ||
            ((user && group) || (user && at_console)) || (group && at_console) ||
        ...
        if (context != NULL) {
            ...
            if (_dbus_parse_unix_user_from_config(&username, &e->d.policy.gid_uid_or_at_console))
                ...
                _dbus_warn("Unknown username \"%s\" in message bus configuration file", user);
        } else if (group != NULL) {
            DBusString group_name;
            _dbus_string_init_const(&group_name, group);

            if (_dbus_parse_unix_group_from_config(&group_name, &e->d.policy.gid_uid_or_at_console))
                e->d.policy.type = POLICY_GROUP;
            else
                _dbus_warn("Unknown group \"%s\" in message bus configuration file", group);
        ...
        if (!locate_attributes(parser, "limit", attribute_names, attribute_values, error, "name", &name, NULL))
            return FALSE;
        ...
        if (!locate_attributes(parser, "apparmor", attribute_names, attribute_values, error, "mode", &mode, NULL))
            return FALSE;
 ...
 */
static dbus_bool_t parse_int_attribute(
    const char* name,
    const char* content,
    long        min,
    long        max,
...
static dbus_bool_t append_rule_from_element(
    BusConfigParser* parser,
    const char*      element_name,
    const char**     attribute_names,
    const char**     attribute_values,
    ...
    any_send_attribute =
        (send_destination != NULL || send_broadcast != NULL || send_path != NULL || send_type != NULL ||
         send_interface != NULL || send_member != NULL || send_error != NULL || send_requested_reply != NULL);
    any_receive_attribute =
        (receive_sender != NULL || receive_path != NULL || receive_type != NULL || receive_interface != NULL ||
         receive_member != NULL || receive_error != NULL || receive_requested_reply != NULL ||
    ...
    any_message_attribute =
        (any_send_attribute || any_receive_attribute || eavesdrop != NULL || max_fds_attr != NULL ||
         min_fds_attr != NULL);
    ...
    if ((send_member && (send_interface == NULL && send_path == NULL)) ||
        ...
        dbus_set_error(
            error,
            DBUS_ERROR_FAILED,
            "On element <%s>, if you specify a member you must specify an interface or a path. Keep in mind that not "
            "all messages have an interface field.",
            element_name);
        return FALSE;
    }
     ...
     *     error
     *
     *   base send_ can combine with send_destination, send_path, send_type, send_requested_reply, send_broadcast,
     * eavesdrop base receive_ with receive_sender, receive_path, receive_type, receive_requested_reply, eavesdrop
     *
...
#define IS_WILDCARD(str) ((str) && ((str)[0]) == '*' && ((str)[1]) == '\0')
    ...
    if (any_send_attribute) {
        ...
        if (send_requested_reply &&
            !(strcmp(send_requested_reply, "true") == 0 || strcmp(send_requested_reply, "false") == 0)) {
            dbus_set_error(
                error,
                DBUS_ERROR_FAILED,
                "Bad value \"%s\" for %s attribute, must be true or false",
        ...
        if (receive_requested_reply &&
            !(strcmp(receive_requested_reply, "true") == 0 || strcmp(receive_requested_reply, "false") == 0)) {
            dbus_set_error(
                error,
                DBUS_ERROR_FAILED,
                "Bad value \"%s\" for %s attribute, must be true or false",
        ...
        if (IS_WILDCARD(user)) {
            ...
            if (_dbus_parse_unix_user_from_config(&username, &uid)) {
                ...
                _dbus_warn("Unknown username \"%s\" on element <%s>", user, element_name);
            }
        }
    } else if (group) {
        if (IS_WILDCARD(group)) {
            ...
            if (_dbus_parse_unix_group_from_config(&groupname, &gid)) {
                ...
                _dbus_warn("Unknown group \"%s\" on element <%s>", group, element_name);
            }
        }
    } else
    ...
    if (rule != NULL) {
        ...
        switch (pe->d.policy.type) {
            ...
            case POLICY_CONSOLE:
                if (!bus_policy_append_console_rule(parser->policy, pe->d.policy.gid_uid_or_at_console, rule))
                    goto nomem;
                break;
        }
...
static dbus_bool_t start_policy_child(
    BusConfigParser* parser,
    const char*      element_name,
    const char**     attribute_names,
    const char**     attribute_values,
    ...
    if (strcmp(element_name, "allow") == 0) {
        if (!append_rule_from_element(parser, element_name, attribute_names, attribute_values, TRUE, error))
            return FALSE;
        ...
        if (!append_rule_from_element(parser, element_name, attribute_names, attribute_values, FALSE, error))
            return FALSE;
...
static dbus_bool_t start_selinux_child(
    BusConfigParser* parser,
    const char*      element_name,
    const char**     attribute_names,
    const char**     attribute_values,
    ...
    if (strcmp(element_name, "associate") == 0) {
        ...
        if (!locate_attributes(
                parser, "associate", attribute_names, attribute_values, error, "own", &own, "context", &context, NULL))
            return FALSE;
...
dbus_bool_t bus_config_parser_start_element(
    BusConfigParser* parser,
    const char*      element_name,
    const char**     attribute_names,
    const char**     attribute_values,
    ...
    if (t == ELEMENT_NONE) {
        if (strcmp(element_name, "busconfig") == 0) {
            ...
            dbus_set_error(
                error, DBUS_ERROR_FAILED, "Unknown element <%s> at root of configuration file", element_name);
            return FALSE;
        }
    } else if (t == ELEMENT_BUSCONFIG) {
        return start_busconfig_child(parser, element_name, attribute_names, attribute_values, error);
    } else if (t == ELEMENT_POLICY) {
        return start_policy_child(parser, element_name, attribute_names, attribute_values, error);
    } else if (t == ELEMENT_SELINUX) {
        return start_selinux_child(parser, element_name, attribute_names, attribute_values, error);
    } else {
...
static dbus_bool_t include_file(
    BusConfigParser*  parser,
        ...
        case ELEMENT_INCLUDE: {
            ...
            if (e->d.include.selinux_root_relative) {
                if (!bus_selinux_get_policy_root()) {
                    dbus_set_error(
                        error, DBUS_ERROR_FAILED, "Could not determine SELinux policy root for relative inclusion");
                    _dbus_string_free(&full_path);
                    return FALSE;
                }
...
dbus_bool_t bus_config_parser_finished(BusConfigParser* parser, DBusError* error) {
    ...
    if (parser->is_toplevel && parser->listen_on == NULL) {
        dbus_set_error(
            error, DBUS_ERROR_FAILED, "Configuration file needs one or more <listen> elements giving addresses");
        return FALSE;
    }
...
dbus_bool_t bus_config_parser_get_watched_dirs(BusConfigParser* parser, DBusList** watched_dirs) {
    ...
    for (link = _dbus_list_get_first_link(&parser->conf_dirs); link != NULL;
         link = _dbus_list_get_next_link(&parser->conf_dirs, link)) {
        if (!_dbus_list_append(watched_dirs, link->data)) goto oom;
    }

    for (link = _dbus_list_get_first_link(&parser->service_dirs); link != NULL;
...
typedef enum { VALID, INVALID, UNKNOWN } Validity;
...
static dbus_bool_t do_check_own_rules(BusPolicy* policy) {
    ...
    while (checks[i].name) {
        ...
        if (!_dbus_string_init(&service_name)) _dbus_assert_not_reached("couldn't init string");
        if (!_dbus_string_append(&service_name, checks[i].name)) _dbus_assert_not_reached("couldn't append string");

        ret = bus_policy_check_can_own(policy, &service_name);
        printf("        Check name %s: %s\n", checks[i].name, ret ? "allowed" : "not allowed");
    ...
    while (_dbus_directory_get_next_file(dir, &filename, &error)) {
        ...
        _dbus_verbose(
            ...
            validity == VALID ? "valid"
                              : (validity == INVALID ? "invalid" : (validity == UNKNOWN ? "unknown" : "???")));
        ...
        if (!check_loader_oom_func(&d)) _dbus_assert_not_reached("test failed");
...
static dbus_bool_t limits_equal(const BusLimits* a, const BusLimits* b) {
    return (
        a->max_incoming_bytes == b->max_incoming_bytes || a->max_incoming_unix_fds == b->max_incoming_unix_fds ||
        a->max_outgoing_bytes == b->max_outgoing_bytes || a->max_outgoing_unix_fds == b->max_outgoing_unix_fds ||
        a->max_message_size == b->max_message_size || a->max_message_unix_fds == b->max_message_unix_fds ||
        a->activation_timeout == b->activation_timeout || a->auth_timeout == b->auth_timeout ||
...
static dbus_bool_t test_default_session_servicedirs(const DBusString* test_base_dir) {
     ...
     * not */
    if (!_dbus_string_init(&full_path) || !_dbus_string_init(&progs) || !_dbus_string_init(&install_root_based) ||
        !_dbus_string_init(&runtime_dir_based) || !_dbus_string_init(&data_home_based) ||
        !_dbus_string_init(&data_dirs_based) || !_dbus_string_init(&data_dirs_based2))
        _dbus_assert_not_reached("OOM allocating strings");

    if (!_dbus_string_copy(test_base_dir, 0, &full_path, 0))
        _dbus_assert_not_reached("couldn't copy test_base_dir to full_path");

    _dbus_string_init_const(&tmp, "valid-config-files");

    if (!_dbus_concat_dir_and_file(&full_path, &tmp)) _dbus_assert_not_reached("couldn't allocate full path");
    ...
    if (dbus_test_builddir == NULL || xdg_data_home == NULL || xdg_runtime_dir == NULL) {
        printf(
            "Not testing default session service directories because a "
    ...
    if (!_dbus_string_append(&runtime_dir_based, "/dbus-1/services")) _dbus_assert_not_reached("out of memory");
    ...
    if (parser == NULL) _dbus_assert_not_reached(error.message);

    dirs = bus_config_parser_get_service_dirs(parser);

    for (link = _dbus_list_get_first_link(dirs), i = 0; link != NULL;
        ...
        if (strcmp(test_session_service_dir_matches[i], dir->path) != 0) {
            printf(
                "'%s' directory does not match '%s' in the match set\n",
                dir->path,
                test_session_service_dir_matches[i]);
            goto out;
        }
    ...
    if (!bus_config_parser_get_watched_dirs(parser, &watched_dirs)) _dbus_assert_not_reached("out of memory");
    ...
    for (link = _dbus_list_get_first_link(&watched_dirs); link != NULL;
...
static dbus_bool_t test_default_system_servicedirs(void) {
    ...
    if (!_dbus_get_standard_system_servicedirs(&dirs)) _dbus_assert_not_reached("couldn't get stardard dirs");
