...
static void bus_service_directory_unref(BusServiceDirectory* dir) {
    if (dir == NULL) return;

    _dbus_assert(dir->refcount > 0);
    dir->refcount--;

    if (dir->refcount > 0) return;

    if (dir->entries) _dbus_hash_table_unref(dir->entries);

    dbus_free(dir->dir_c);
    dbus_free(dir);
}

static void bus_pending_activation_entry_free(BusPendingActivationEntry* entry) {
    if (entry->activation_message) dbus_message_unref(entry->activation_message);

    if (entry->connection) dbus_connection_unref(entry->connection);

    dbus_free(entry);
}

static BusPendingActivation* bus_pending_activation_ref(BusPendingActivation* pending_activation) {
...
static void bus_pending_activation_unref(BusPendingActivation* pending_activation) {
    DBusList* link;
    ...
    if (pending_activation->refcount > 0) return;

    if (pending_activation->timeout_added) {
        _dbus_loop_remove_timeout(
            bus_context_get_loop(pending_activation->activation->context), pending_activation->timeout);
        pending_activation->timeout_added = FALSE;
    }

    if (pending_activation->timeout) _dbus_timeout_unref(pending_activation->timeout);

    if (pending_activation->babysitter) {
        if (!_dbus_babysitter_set_watch_functions(
                pending_activation->babysitter, NULL, NULL, NULL, pending_activation->babysitter, NULL))
    ...
    pending_activation->activation->n_pending_activations -= pending_activation->n_entries;
...
static BusActivationEntry* bus_activation_entry_ref(BusActivationEntry* entry) {
...
static void bus_activation_entry_unref(BusActivationEntry* entry) {
...
static dbus_bool_t update_desktop_file_entry(
    BusActivation*       activation,
    BusServiceDirectory* s_dir,
    DBusString*          filename,
    ...
    if (!_dbus_string_append(&file_path, s_dir->dir_c) || !_dbus_concat_dir_and_file(&file_path, filename)) {
        BUS_SET_OOM(error);
        goto out;
    }

    if (!_dbus_stat(&file_path, &stat_buf, NULL)) {
        dbus_set_error(error, DBUS_ERROR_FAILED, "Can't stat the service file\n");
        goto out;
    }

    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION, DBUS_SERVICE_NAME, &name, error)) goto out;

    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION, DBUS_SERVICE_EXEC, &exec_tmp, error)) goto out;

    if (!_dbus_string_init(&str)) goto out;

    if (!_dbus_string_append(&str, exec_tmp) || !_dbus_replace_install_prefix(&str) ||
    ...
    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION, DBUS_SERVICE_USER, &user, &tmp_error)) {
    ...
    if (!bus_desktop_file_get_string(
            desktop_file, DBUS_SERVICE_SECTION, DBUS_SERVICE_SYSTEMD_SERVICE, &systemd_service, &tmp_error)) {
        _DBUS_ASSERT_ERROR_IS_SET(&tmp_error);
        /* if we got OOM, then exit */
        if (dbus_error_has_name(&tmp_error, DBUS_ERROR_NO_MEMORY)) {
            dbus_move_error(&tmp_error, error);
    ...
    if (entry == NULL) /* New file */
    {
        ...
        if (!_dbus_string_append(&expected_name, name) || !_dbus_string_append(&expected_name, ".service")) {
        ...
        if (_dbus_string_equal(&expected_name, filename)) {
            ...
            _dbus_verbose(
                "Name of \"%s\" should canonically be \"%s\"\n",
                _dbus_string_get_const_data(&file_path),
                _dbus_string_get_const_data(&expected_name));
        }
        ...
        if (_dbus_hash_table_lookup_string(activation->entries, name)) {
            dbus_set_error(error, DBUS_ERROR_FAILED, "Service %s already exists in activation entry list\n", name);
            goto out;
        }
...
out:
    ...
    if (entry) bus_activation_entry_unref(entry);

    return retval;
}

static dbus_bool_t check_service_file(
    BusActivation*       activation,
    BusActivationEntry*  entry,
    ...
    if (!_dbus_string_append(&file_path, entry->s_dir->dir_c) || !_dbus_concat_dir_and_file(&file_path, &filename)) {
        BUS_SET_OOM(error);
        retval = FALSE;
        goto out;
    }
    ...
    if (!_dbus_stat(&file_path, &stat_buf, NULL)) {
        ...
        if (stat_buf.mtime > entry->mtime) {
            ...
            if (desktop_file == NULL) {
                _dbus_verbose("Could not load %s: %s\n", _dbus_string_get_const_data(&file_path), tmp_error.message);
                if (dbus_error_has_name(&tmp_error, DBUS_ERROR_NO_MEMORY)) {
                    dbus_move_error(&tmp_error, error);
                    retval = FALSE;
                    goto out;
...
out:
    ...
    if (updated_entry != NULL) *updated_entry = tmp_entry;
    return retval;
}
 ...
 */
static dbus_bool_t update_directory(BusActivation* activation, BusServiceDirectory* s_dir, DBusError* error) {
    DBusDirIter*        iter;
    ...
    while (_dbus_directory_get_next_file(iter, &filename, &tmp_error)) {
        ...
        {
            if (!check_service_file(activation, entry, NULL, error)) goto out;

            continue;
        }

        if (!_dbus_string_append(&full_path, s_dir->dir_c) || !_dbus_concat_dir_and_file(&full_path, &filename)) {
            BUS_SET_OOM(error);
            goto out;
        }
        ...
        if (!update_desktop_file_entry(activation, s_dir, &filename, desktop_file, &tmp_error)) {
            ...
            _dbus_verbose(
                "Could not add %s to activation entry list: %s\n",
...
out:
    ...
    if (iter != NULL) _dbus_directory_close(iter);
    _dbus_string_free(&filename);
...
static dbus_bool_t populate_environment(BusActivation* activation) {
    char**      environment;
...
dbus_bool_t bus_activation_reload(
    BusActivation*    activation,
    ...
    if (activation->server_address != NULL) dbus_free(activation->server_address);
    if (!_dbus_string_copy_data(address, &activation->server_address)) {
        BUS_SET_OOM(error);
        goto failed;
    }

    if (activation->entries != NULL) _dbus_hash_table_unref(activation->entries);
    activation->entries = _dbus_hash_table_new(DBUS_HASH_STRING, NULL, (DBusFreeFunction)bus_activation_entry_unref);
    if (activation->entries == NULL) {
        BUS_SET_OOM(error);
        goto failed;
    }

    _dbus_list_foreach(&activation->directories, (DBusForeachFunction)bus_service_directory_unref, NULL);
    ...
    while (link != NULL) {
        ...
        s_dir->entries = _dbus_hash_table_new(DBUS_HASH_STRING, NULL, (DBusFreeFunction)bus_activation_entry_unref);
...
BusActivation* bus_activation_new(
    BusContext*       context,
    const DBusString* address,
    DBusList**        directories,
    DBusError*        error) {
    ...
    if (!bus_activation_reload(activation, address, directories, error)) goto failed;

    /* Initialize this hash table once, we don't want to lose pending
     * activations on reload. */
    activation->pending_activations =
        _dbus_hash_table_new(DBUS_HASH_STRING, NULL, (DBusFreeFunction)bus_pending_activation_unref);
    ...
    activation->environment =
        _dbus_hash_table_new(DBUS_HASH_STRING, (DBusFreeFunction)dbus_free, (DBusFreeFunction)dbus_free);
...
void bus_activation_unref(BusActivation* activation) {
    ...
    if (activation->refcount > 0) return;

    dbus_free(activation->server_address);
    if (activation->entries) _dbus_hash_table_unref(activation->entries);
    if (activation->pending_activations) _dbus_hash_table_unref(activation->pending_activations);

    _dbus_list_foreach(&activation->directories, (DBusForeachFunction)bus_service_directory_unref, NULL);
    _dbus_list_clear(&activation->directories);

    if (activation->environment) _dbus_hash_table_unref(activation->environment);

    dbus_free(activation);
}

static dbus_bool_t add_bus_environment(BusActivation* activation, DBusError* error) {
    const char* type;

    if (!bus_activation_set_environment_variable(activation, "DBUS_STARTER_ADDRESS", activation->server_address, error))
        return FALSE;

    type = bus_context_get_type(activation->context);
    if (type != NULL) {
        if (!bus_activation_set_environment_variable(activation, "DBUS_STARTER_BUS_TYPE", type, error)) return FALSE;

        if (strcmp(type, "session") == 0) {
            if (!bus_activation_set_environment_variable(
                    activation, "DBUS_SESSION_BUS_ADDRESS", activation->server_address, error))
                return FALSE;
        } else if (strcmp(type, "system") == 0) {
            if (!bus_activation_set_environment_variable(
                    activation, "DBUS_SYSTEM_BUS_ADDRESS", activation->server_address, error))
                return FALSE;
        }
    }
...
static void restore_pending(void* data) {
    ...
    _dbus_hash_table_insert_string_preallocated(
        d->pending_activation->activation->pending_activations,
        d->hash_entry,
        d->pending_activation->service_name,
        d->pending_activation);
...
static void free_restore_pending_data(void* data) {
    RestorePendingData* d = data;

    if (d->hash_entry)
        _dbus_hash_table_free_preallocated_entry(d->pending_activation->activation->pending_activations, d->hash_entry);
...
static dbus_bool_t add_restore_pending_to_transaction(
    ...
    if (d->hash_entry == NULL ||
        !bus_transaction_add_cancel_hook(transaction, restore_pending, d, free_restore_pending_data)) {
...
dbus_bool_t bus_activation_service_created(
    BusActivation*  activation,
    ...
    if (!pending_activation) return TRUE;

    bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO, "Successfully activated service '%s'", service_name);
    ...
    while (link != NULL) {
        ...
        if (entry->connection && dbus_connection_get_is_connected(entry->connection)) {
            ...
            if (!entry->auto_activation) {
                ...
                if (!dbus_message_append_args(message, DBUS_TYPE_UINT32, &result, DBUS_TYPE_INVALID)) {
...
dbus_bool_t bus_activation_send_pending_auto_activation_messages(
    ...
    /* Check if it's a pending activation */
    pending_activation = _dbus_hash_table_lookup_string(activation->pending_activations, bus_service_get_name(service));
    ...
    while (link != NULL) {
        ...
        if (entry->auto_activation &&
            ...
            if (!bus_dispatch_matches(
                    transaction, entry->connection, addressed_recipient, entry->activation_message, &error)) {
                ...
                if (!bus_transaction_send_error_reply(
                        transaction, entry->connection, &error, entry->activation_message)) {
                    bus_connection_send_oom_error(entry->connection, entry->activation_message);
                }
 ...
 */
static dbus_bool_t try_send_activation_failure(BusPendingActivation* pending_activation, const DBusError* how) {
    BusActivation*  activation;
    DBusList*       link;
    BusTransaction* transaction;
    ...
    while (link != NULL) {
        ...
        if (entry->connection && dbus_connection_get_is_connected(entry->connection)) {
            if (!bus_transaction_send_error_reply(transaction, entry->connection, how, entry->activation_message))
                goto error;
        }
...
error:
    if (transaction) bus_transaction_cancel_and_free(transaction);
    return FALSE;
}
 ...
 */
static void pending_activation_failed(BusPendingActivation* pending_activation, const DBusError* how) {
    /* FIXME use preallocated OOM messages instead of bus_wait_for_memory() */
    while (!try_send_activation_failure(pending_activation, how)) _dbus_wait_for_memory();

    /* Destroy this pending activation */
    _dbus_hash_table_remove_string(
        pending_activation->activation->pending_activations, pending_activation->service_name);
}

/**
 * Depending on the exit code of the helper, set the error accordingly
 */
static void handle_servicehelper_exit_error(int exit_code, DBusError* error) {
    switch (exit_code) {
        case BUS_SPAWN_EXIT_CODE_CONFIG_INVALID:
            dbus_set_error(error, DBUS_ERROR_SPAWN_CONFIG_INVALID, "Invalid configuration (missing or empty <user>?)");
            break;
        case BUS_SPAWN_EXIT_CODE_NO_MEMORY:
            dbus_set_error(error, DBUS_ERROR_NO_MEMORY, "Launcher could not run (out of memory)");
            break;
        case BUS_SPAWN_EXIT_CODE_SETUP_FAILED:
            dbus_set_error(error, DBUS_ERROR_SPAWN_SETUP_FAILED, "Failed to setup environment correctly");
            break;
        case BUS_SPAWN_EXIT_CODE_NAME_INVALID:
            dbus_set_error(error, DBUS_ERROR_SPAWN_SERVICE_INVALID, "Bus name is not valid or missing");
            break;
        case BUS_SPAWN_EXIT_CODE_SERVICE_NOT_FOUND:
            dbus_set_error(error, DBUS_ERROR_SPAWN_SERVICE_NOT_FOUND, "Bus name not found in system service directory");
            break;
        case BUS_SPAWN_EXIT_CODE_PERMISSIONS_INVALID:
            dbus_set_error(
                error, DBUS_ERROR_SPAWN_PERMISSIONS_INVALID, "The permission of the setuid helper is not correct");
            break;
        ...
        case BUS_SPAWN_EXIT_CODE_EXEC_FAILED:
            dbus_set_error(
                error, DBUS_ERROR_SPAWN_EXEC_FAILED, "Cannot launch daemon, file not found or permissions invalid");
            break;
        case BUS_SPAWN_EXIT_CODE_INVALID_ARGS:
            dbus_set_error(error, DBUS_ERROR_INVALID_ARGS, "Invalid arguments to command line");
            break;
        case BUS_SPAWN_EXIT_CODE_CHILD_SIGNALED:
            dbus_set_error(error, DBUS_ERROR_SPAWN_CHILD_SIGNALED, "Launched child was signaled, it probably crashed");
            break;
        case BUS_SPAWN_EXIT_CODE_GENERIC_FAILURE:
        default:
            dbus_set_error(
                error, DBUS_ERROR_SPAWN_CHILD_EXITED, "Launch helper exited with unknown return code %i", exit_code);
            break;
    }
}

static void pending_activation_finished_cb(DBusBabysitter* babysitter, void* data) {
    ...
    if (_dbus_babysitter_get_child_exited(babysitter)) {
        ...
        if (activation_failed) {
            bus_context_log(
                pending_activation->activation->context,
            ...
            _dbus_hash_iter_init(pending_activation->activation->pending_activations, &iter);
            while (_dbus_hash_iter_next(&iter)) {
                BusPendingActivation* p = _dbus_hash_iter_get_value(&iter);

                if (p != pending_activation && p->exec != NULL && strcmp(p->exec, pending_activation->exec) == 0)
...
static dbus_bool_t add_babysitter_watch(DBusWatch* watch, void* data) {
    BusPendingActivation* pending_activation = data;

    return _dbus_loop_add_watch(bus_context_get_loop(pending_activation->activation->context), watch);
}

static void remove_babysitter_watch(DBusWatch* watch, void* data) {
    BusPendingActivation* pending_activation = data;

    _dbus_loop_remove_watch(bus_context_get_loop(pending_activation->activation->context), watch);
}

static void toggle_babysitter_watch(DBusWatch* watch, void* data) {
    BusPendingActivation* pending_activation = data;

    _dbus_loop_toggle_watch(bus_context_get_loop(pending_activation->activation->context), watch);
}

static dbus_bool_t pending_activation_timed_out(void* data) {
    BusPendingActivation* pending_activation = data;
    BusContext*           context;
    DBusError             error;
    int                   timeout;
     ...
     */
    if (pending_activation->babysitter) _dbus_babysitter_kill_child(pending_activation->babysitter);
...
static void free_pending_cancel_data(void* data) {
    BusPendingActivation* pending_activation = data;

    bus_pending_activation_unref(pending_activation);
}

static dbus_bool_t add_cancel_pending_to_transaction(
    BusTransaction*       transaction,
    BusPendingActivation* pending_activation) {
    if (!bus_transaction_add_cancel_hook(transaction, cancel_pending, pending_activation, free_pending_cancel_data))
        return FALSE;
...
static dbus_bool_t update_service_cache(BusActivation* activation, DBusError* error) {
    DBusList* iter;

    for (iter = _dbus_list_get_first_link(&activation->directories); iter != NULL;
...
static BusActivationEntry* activation_find_entry(
    BusActivation* activation,
    const char*    service_name,
    DBusError*     error) {
    BusActivationEntry* entry;
    ...
    if (!entry) {
        if (!update_service_cache(activation, error)) return NULL;

        entry = _dbus_hash_table_lookup_string(activation->entries, service_name);
    } else {
        BusActivationEntry* updated_entry;

        if (!check_service_file(activation, entry, &updated_entry, error)) return NULL;
    ...
    if (!entry) {
        dbus_set_error(
            error, DBUS_ERROR_SERVICE_UNKNOWN, "The name %s was not provided by any .service files", service_name);
        return NULL;
    }
...
static char** bus_activation_get_environment(BusActivation* activation) {
    return _dbus_hash_table_to_array(activation->environment, '=');
}

dbus_bool_t bus_activation_set_environment_variable(
    BusActivation* activation,
    const char*    key,
    const char*    value,
    DBusError*     error) {
    ...
    if (hash_key == NULL) goto out;

    hash_value = _dbus_strdup(value);

    if (hash_value == NULL) goto out;

    if (!_dbus_hash_table_insert_string(activation->environment, hash_key, hash_value)) goto out;
...
static void child_setup(void* user_data) {
#ifdef DBUS_UNIX
    ...
    if (initial_fd_limit != NULL && !_dbus_rlimit_restore_fd_limit(initial_fd_limit, &error)) {
...
dbus_bool_t bus_activation_activate_service(
    BusActivation*  activation,
    DBusConnection* connection,
    BusTransaction* transaction,
    dbus_bool_t     auto_activation,
    ...
    if (was_pending_activation) {
        ...
        pending_activation->timeout = _dbus_timeout_new(
            bus_context_get_activation_timeout(activation->context),
        ...
        if (!_dbus_loop_add_timeout(bus_context_get_loop(activation->context), pending_activation->timeout)) {
        ...
        if (!_dbus_hash_table_insert_string(
                activation->pending_activations, pending_activation->service_name, pending_activation)) {
    ...
    if (was_pending_activation) return TRUE;
    ...
    if (bus_context_get_systemd_activation(activation->context)) {
        ...
        if (entry->systemd_service) {
            ...
            message =
                dbus_message_new_signal(DBUS_PATH_DBUS, "org.freedesktop.systemd1.Activator", "ActivationRequest");
            if (!message) {
            ...
            if (!dbus_message_set_sender(message, DBUS_SERVICE_DBUS) ||
                ...
                !dbus_message_append_args(message, DBUS_TYPE_STRING, &entry->systemd_service, DBUS_TYPE_INVALID)) {
            ...
            if (service) systemd = bus_service_get_primary_owners_connection(service);
            ...
            if (!bus_transaction_capture(activation_transaction, NULL, systemd, message)) {
                dbus_message_unref(message);
                BUS_SET_OOM(error);
                goto cancel_pending_activation;
            }
            ...
            if (service != NULL) {
                bus_context_log(
                    activation->context,
                    DBUS_SYSTEM_LOG_INFO,
                    "Activating via systemd: service name='%s' unit='%s' requested by '%s' (%s)",
                    service_name,
                    entry->systemd_service,
                    bus_connection_get_name(connection),
                    bus_connection_get_loginfo(connection));
                /* Wonderful, systemd is connected, let's just send the msg */
                retval = bus_dispatch_matches(activation_transaction, NULL, systemd, message, error);
            } else {
                bus_context_log(
                    activation->context,
                    DBUS_SYSTEM_LOG_INFO,
                    "Activating systemd to hand-off: service name='%s' unit='%s' requested by '%s' (%s)",
                    service_name,
                    entry->systemd_service,
                    bus_connection_get_name(connection),
                    bus_connection_get_loginfo(connection));
                /* systemd is not around, let's "activate" it. */
                retval = bus_activation_activate_service(
                    activation, NULL, activation_transaction, TRUE, message, "org.freedesktop.systemd1", error);
            }
    ...
    if (servicehelper != NULL) {
        if (entry->user == NULL) {
            ...
            dbus_set_error(error, DBUS_ERROR_SPAWN_FILE_INVALID, "Cannot do system-bus activation with no user\n");
            goto cancel_pending_activation;
        }
    ...
    if (servicehelper != NULL)
        bus_context_log(
            activation->context,
            DBUS_SYSTEM_LOG_INFO,
            "Activating service name='%s' requested by '%s' (%s) (using servicehelper)",
            service_name,
            bus_connection_get_name(connection),
            bus_connection_get_loginfo(connection));
    else
        bus_context_log(
            activation->context,
            DBUS_SYSTEM_LOG_INFO,
            "Activating service name='%s' requested by '%s' (%s)",
            service_name,
            bus_connection_get_name(connection),
            bus_connection_get_loginfo(connection));
    ...
    if (bus_context_get_using_syslog(activation->context)) flags |= DBUS_SPAWN_REDIRECT_OUTPUT;

    if (!_dbus_spawn_async_with_babysitter(
            &pending_activation->babysitter, service_name, argv, envp, flags, child_setup, activation, &tmp_error)) {
    ...
    _dbus_babysitter_set_result_function(
        pending_activation->babysitter, pending_activation_finished_cb, pending_activation);
...
cancel_pending_activation:
    _DBUS_ASSERT_ERROR_IS_SET(error);
    _dbus_hash_table_remove_string(activation->pending_activations, pending_activation->service_name);
    return FALSE;
}

dbus_bool_t bus_activation_list_services(BusActivation* activation, char*** listp, int* array_len) {
    int          i, j, len;
    char**       retval;
    DBusHashIter iter;
    ...
    if (array_len) *array_len = len;

    *listp = retval;
    return TRUE;

error:
    for (j = 0; j < i; j++) dbus_free(retval[j]);
    dbus_free(retval);
...
dbus_bool_t dbus_activation_systemd_failure(BusActivation* activation, DBusMessage* message) {
    ...
    if (unit) {
        ...
        bus_context_log(
            activation->context, DBUS_SYSTEM_LOG_INFO, "Activation via systemd failed for unit '%s': %s", unit, str);
        ...
        while (_dbus_hash_iter_next(&iter)) {
            ...
            if (p->systemd_service && strcmp(p->systemd_service, unit) == 0) pending_activation_failed(p, &error);
        }
    }
...
static dbus_bool_t test_create_service_file(DBusString* dir, const char* filename, const char* name, const char* exec) {
    ...
    if (!_dbus_string_init(&full_path)) return FALSE;
...
static dbus_bool_t test_remove_service_file(DBusString* dir, const char* filename) {
    DBusString  file_name, full_path;
    dbus_bool_t ret_val;
...
static dbus_bool_t test_remove_directory(DBusString* dir) {
    DBusDirIter* iter;
    ...
    if (!_dbus_string_init(&filename)) return FALSE;
...
static dbus_bool_t init_service_reload_test(DBusString* dir) {
    if (!_dbus_create_directory(dir, NULL)) return FALSE;

    /* Create one initial file */
    if (!test_create_service_file(dir, SERVICE_FILE_1, SERVICE_NAME_1, "exec-1")) return FALSE;

    return TRUE;
}

static dbus_bool_t cleanup_service_reload_test(DBusString* dir) {
    if (!test_remove_directory(dir)) return FALSE;
...
static dbus_bool_t check_func(void* data) {
    CheckData*          d;
    BusActivationEntry* entry;
    DBusError           error;
    dbus_bool_t         ret_val;
    ...
    if (entry == NULL) {
        if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
            ...
            if (d->expecting_find) ret_val = FALSE;
        }
...
static dbus_bool_t do_test(const char* description, dbus_bool_t oom_test, CheckData* data) {
    dbus_bool_t err;
    ...
    if (err) _dbus_assert_not_reached("Test failed");

    return TRUE;
}

static dbus_bool_t do_service_reload_test(const DBusString* test_data_dir, DBusString* dir, dbus_bool_t oom_test) {
    BusActivation*      activation;
    ...
    if (!_dbus_list_append(&directories, &config)) return FALSE;
    ...
    if (!do_test("Existing service file", oom_test, &d)) return FALSE;

    /* Check for non-existing service file */
    d.expecting_find = FALSE;
    d.service_name   = SERVICE_NAME_3;

    if (!do_test("Nonexisting service file", oom_test, &d)) return FALSE;

    /* Check for added service file */
    if (!test_create_service_file(dir, SERVICE_FILE_2, SERVICE_NAME_2, "exec-2")) return FALSE;

    d.expecting_find = TRUE;
    d.service_name   = SERVICE_NAME_2;

    if (!do_test("Added service file", oom_test, &d)) return FALSE;

    /* Check for removed service file */
    if (!test_remove_service_file(dir, SERVICE_FILE_2)) return FALSE;

    d.expecting_find = FALSE;
    d.service_name   = SERVICE_FILE_2;

    if (!do_test("Removed service file", oom_test, &d)) return FALSE;

    /* Check for updated service file */

    _dbus_sleep_milliseconds(1000); /* Sleep a second to make sure the mtime is updated */

    if (!test_create_service_file(dir, SERVICE_FILE_1, SERVICE_NAME_3, "exec-3")) return FALSE;

    d.expecting_find = TRUE;
    d.service_name   = SERVICE_NAME_3;

    if (!do_test("Updated service file, part 1", oom_test, &d)) return FALSE;

    d.expecting_find = FALSE;
    d.service_name   = SERVICE_NAME_1;

    if (!do_test("Updated service file, part 2", oom_test, &d)) return FALSE;
...
dbus_bool_t bus_activation_service_reload_test(const DBusString* test_data_dir) {
    ...
    if (!_dbus_string_init(&directory)) return FALSE;

    tmp = _dbus_get_tmpdir();

    if (tmp == NULL) goto out;

    if (!_dbus_string_append(&directory, tmp)) goto out;

    if (!_dbus_string_append(&directory, "/dbus-reload-test-") || !_dbus_generate_random_ascii(&directory, 6, NULL))
        goto out;

    /* Do normal tests */
    if (!init_service_reload_test(&directory)) _dbus_assert_not_reached("could not initiate service reload test");

    if (!do_service_reload_test(test_data_dir, &directory, FALSE)) {
        /* Do nothing? */
    }

    if (!cleanup_service_reload_test(&directory)) goto out;
