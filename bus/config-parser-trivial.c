...
static dbus_bool_t service_dirs_find_dir(DBusList** service_dirs, const char* dir) {
    DBusList* link;
    ...
    for (link = *service_dirs; link; link = _dbus_list_get_next_link(service_dirs, link)) {
        ...
        if (strcmp(dir, link_dir) == 0) return TRUE;
    }
...
BusConfigParser* bus_config_parser_new(
    ...
    /* init the strings */
    if (!_dbus_string_init(&parser->user)) goto failed_parser;
    if (!_dbus_string_init(&parser->bus_type)) goto failed_type;
    if (!_dbus_string_init(&parser->service_helper)) goto failed_helper;
...
void bus_config_parser_unref(BusConfigParser* parser) {
    ...
    _dbus_list_foreach(&parser->service_dirs, (DBusForeachFunction)dbus_free, NULL);
...
dbus_bool_t bus_config_parser_content(BusConfigParser* parser, const DBusString* content, DBusError* error) {
    ...
    switch (parser->type) {
        ...
        default: {
            ...
            _dbus_verbose(
                " CONTENTS We don't care about '%s' type '%i'\n",
                _dbus_string_get_const_data(&content_sane),
                parser->type);
            break;
        }
    }
...
static dbus_bool_t check_return_values(const DBusString* full_path) {
    BusConfigParser* parser;
    DBusError        error;
    dbus_bool_t      retval;
    const char*      user;
    ...
    if (parser == NULL) {
        ...
        if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) _dbus_verbose("Failed to load valid file due to OOM\n");
        goto finish;
    }
    _DBUS_ASSERT_ERROR_IS_CLEAR(&error);
...
static dbus_bool_t do_load(const DBusString* full_path, Validity validity, dbus_bool_t oom_possible) {
    BusConfigParser* parser;
    DBusError        error;
...
static dbus_bool_t make_full_path(
...
static dbus_bool_t check_file_valid(DBusString* full_path, Validity validity) {
    dbus_bool_t retval;
...
dbus_bool_t bus_config_parser_trivial_test(const DBusString* test_data_dir) {
    ...
    if (!process_test_valid_subdir(test_data_dir, "valid-config-files", VALID)) goto finish;

#ifndef DBUS_WIN
    /* We already test default_session_servicedirs and default_system_servicedirs
     * in bus_config_parser_test() */
    if (!process_test_valid_subdir(test_data_dir, "valid-config-files-system", VALID)) goto finish;
#endif
    ...
    if (!make_full_path(test_data_dir, "invalid-config-files", "not-well-formed.conf", &full_path)) goto finish;
    if (!check_file_valid(&full_path, INVALID)) goto finish;
    _dbus_string_free(&full_path);

#ifndef DBUS_WIN
    /* just test if the check_file_valid works okay and we got sane values */
    if (!make_full_path(test_data_dir, "valid-config-files-system", "system.conf", &full_path)) goto finish;
    if (!check_file_valid(&full_path, VALID)) goto finish;
    /* check to see if we got the correct values from the parser */
    if (!check_return_values(&full_path)) goto finish;
#endif
