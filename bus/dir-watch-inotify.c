...
static dbus_bool_t _handle_inotify_watch(DBusWatch* passed_watch, unsigned int flags, void* data) {
    ...
    while (i < ret) {
        ...
        if (ev->len) _dbus_verbose("event name: '%s'\n", ev->name);
...
static void _set_watched_dirs_internal(DBusList** directories) {
...
#include <stdio.h>
static void _shutdown_inotify(void* data) {
    DBusList* empty = NULL;
...
static int _init_inotify(BusContext* context) {
    int ret = 0;
    ...
    if (inotify_fd == -1) {
         ...
         */
        if (inotify_fd < 0) inotify_fd = inotify_init();
#else
        ...
        watch = _dbus_watch_new(inotify_fd, DBUS_WATCH_READABLE, TRUE, _handle_inotify_watch, NULL, NULL);
...
void bus_set_watched_dirs(BusContext* context, DBusList** directories) {
    if (!_init_inotify(context)) return;
