...
typedef enum { BUS_POLICY_TRISTATE_ANY = 0, BUS_POLICY_TRISTATE_FALSE, BUS_POLICY_TRISTATE_TRUE } BusPolicyTristate;

/** determines whether the rule affects a connection, or some global item */
#define BUS_POLICY_RULE_IS_PER_CLIENT(rule) \
    (!((rule)->type == BUS_POLICY_RULE_USER || (rule)->type == BUS_POLICY_RULE_GROUP))
