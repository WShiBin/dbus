...
static dbus_bool_t list_allows_user(
    dbus_bool_t          def,
    DBusList**           list,
    unsigned long        uid,
...
void bus_client_policy_optimize(BusClientPolicy* policy) {
    ...
    while (link != NULL) {
        ...
        switch (rule->type) {
            case BUS_POLICY_RULE_SEND:
                remove_preceding = rule->d.send.message_type == DBUS_MESSAGE_TYPE_INVALID &&
                                   rule->d.send.path == NULL && rule->d.send.interface == NULL &&
                                   rule->d.send.member == NULL && rule->d.send.error == NULL &&
                                   rule->d.send.destination == NULL;
                break;
            case BUS_POLICY_RULE_RECEIVE:
                remove_preceding = rule->d.receive.message_type == DBUS_MESSAGE_TYPE_INVALID &&
                                   rule->d.receive.path == NULL && rule->d.receive.interface == NULL &&
                                   rule->d.receive.member == NULL && rule->d.receive.error == NULL &&
                                   rule->d.receive.origin == NULL;
                break;
            case BUS_POLICY_RULE_OWN:
                remove_preceding = rule->d.own.service_name == NULL;
                break;
...
dbus_bool_t bus_client_policy_check_can_send(
    BusClientPolicy* policy,
    ...
    while (link != NULL) {
        ...
        if (dbus_message_get_reply_serial(message) != 0) {
            ...
            if (!requested_reply && rule->allow && rule->d.send.requested_reply && !rule->d.send.eavesdrop) {
                _dbus_verbose(
                    "  (policy) skipping allow rule since it only applies to requested replies and does not allow "
                    "eavesdropping\n");
                continue;
            }
        ...
        if (rule->d.send.path != NULL) {
            if (dbus_message_get_path(message) != NULL &&
                strcmp(dbus_message_get_path(message), rule->d.send.path) != 0) {
        ...
        if (rule->d.send.interface != NULL) {
            ...
            if ((no_interface && rule->allow) ||
                (!no_interface && strcmp(dbus_message_get_interface(message), rule->d.send.interface) != 0)) {
        ...
        if (rule->d.send.member != NULL) {
            if (dbus_message_get_member(message) != NULL &&
                strcmp(dbus_message_get_member(message), rule->d.send.member) != 0) {
        ...
        if (rule->d.send.error != NULL) {
            if (dbus_message_get_error_name(message) != NULL &&
                strcmp(dbus_message_get_error_name(message), rule->d.send.error) != 0) {
        ...
        if (rule->d.send.destination != NULL) {
            ...
            if (receiver == NULL) {
                if (!dbus_message_has_destination(message, rule->d.send.destination)) {
                    _dbus_verbose(
                        "  (policy) skipping rule because message dest is not %s\n", rule->d.send.destination);
                    continue;
                }
            } else {
                DBusString  str;
                ...
                if (!bus_service_has_owner(service, receiver)) {
                    _dbus_verbose(
                        "  (policy) skipping rule because dest %s isn't owned by receiver\n", rule->d.send.destination);
                    continue;
                }
            }
        }
        ...
        if (rule->d.send.min_fds > 0 || rule->d.send.max_fds < DBUS_MAXIMUM_MESSAGE_UNIX_FDS) {
            ...
            if (n_fds < rule->d.send.min_fds || n_fds > rule->d.send.max_fds) {
                _dbus_verbose(
                    "  (policy) skipping rule because message has %u fds "
 ...
 * comment
 */
dbus_bool_t bus_client_policy_check_can_receive(
    BusClientPolicy* policy,
    BusRegistry*     registry,
    dbus_bool_t      requested_reply,
    ...
    *toggles = 0;
    ...
    while (link != NULL) {
        ...
        if (rule->d.receive.path != NULL) {
            if (dbus_message_get_path(message) != NULL &&
                strcmp(dbus_message_get_path(message), rule->d.receive.path) != 0) {
                _dbus_verbose("  (policy) skipping rule for different path\n");
                continue;
            }
        }
        ...
        if (rule->d.receive.interface != NULL) {
            ...
            if ((no_interface && rule->allow) ||
                (!no_interface && strcmp(dbus_message_get_interface(message), rule->d.receive.interface) != 0)) {
                _dbus_verbose("  (policy) skipping rule for different interface\n");
                continue;
            }
        }
        ...
        if (rule->d.receive.member != NULL) {
            if (dbus_message_get_member(message) != NULL &&
                strcmp(dbus_message_get_member(message), rule->d.receive.member) != 0) {
                _dbus_verbose("  (policy) skipping rule for different member\n");
                continue;
            }
        }
        ...
        if (rule->d.receive.error != NULL) {
            if (dbus_message_get_error_name(message) != NULL &&
                strcmp(dbus_message_get_error_name(message), rule->d.receive.error) != 0) {
                _dbus_verbose("  (policy) skipping rule for different error name\n");
                continue;
            }
        }
        ...
        if (rule->d.receive.origin != NULL) {
            ...
            if (sender == NULL) {
                if (!dbus_message_has_sender(message, rule->d.receive.origin)) {
                    _dbus_verbose(
                        "  (policy) skipping rule because message sender is not %s\n", rule->d.receive.origin);
                    continue;
                }
            } else {
                BusService* service;
                ...
                if (!bus_service_has_owner(service, sender)) {
                    _dbus_verbose(
                        "  (policy) skipping rule because origin %s isn't owned by sender\n", rule->d.receive.origin);
                    continue;
                }
            }
        }
...
static dbus_bool_t bus_rules_check_can_own(DBusList* rules, const DBusString* service_name) {
    ...
    while (link != NULL) {
        ...
        if (!rule->d.own.prefix && rule->d.own.service_name != NULL) {
            if (!_dbus_string_equal_c_str(service_name, rule->d.own.service_name)) continue;
