...
typedef enum { ACTION_RELOAD = 'r', ACTION_QUIT = 'q' } SignalAction;
    ...
    switch (action) {
        case ACTION_RELOAD:
            if (!bus_context_reload_config(context, &error)) {
                ...
                _dbus_assert(
                    dbus_error_has_name(&error, DBUS_ERROR_FAILED) ||
...
static void setup_reload_pipe(DBusLoop* loop) {
    ...
    watch = _dbus_watch_new(
        _dbus_socket_get_pollable(reload_pipe[RELOAD_READ_END]),
        DBUS_WATCH_READABLE,
        TRUE,
        handle_reload_watch,
        NULL,
...
int main(int argc, char** argv) {
    ...
    if (print_address) {
        ...
        if (_dbus_string_get_length(&addr_fd) > 0) {
            long val;
            int  end;
            if (!_dbus_string_parse_int(&addr_fd, 0, &val, &end) || end != _dbus_string_get_length(&addr_fd) ||
    ...
    if (print_pid) {
        ...
        if (_dbus_string_get_length(&pid_fd) > 0) {
            long val;
            int  end;
            if (!_dbus_string_parse_int(&pid_fd, 0, &val, &end) || end != _dbus_string_get_length(&pid_fd) || val < 0 ||
                val > _DBUS_INT_MAX) {
                fprintf(stderr, "Invalid file descriptor: \"%s\"\n", _dbus_string_get_const_data(&pid_fd));
                exit(1);
            }
