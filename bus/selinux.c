...
static const struct avc_memory_callback mem_cb    = {.func_malloc = dbus_malloc, .func_free = dbus_free};
static const struct avc_log_callback    log_cb    = {.func_log = log_callback, .func_audit = log_audit_callback};
 ...
 */
static int policy_reload_callback(
    u_int32_t        event,
    security_id_t    ssid,
    security_id_t    tsid,
...
#ifdef HAVE_SELINUX
static struct security_class_mapping dbus_map[] = {{"dbus", {"acquire_svc", "send_msg", NULL}}, {NULL}};
#endif /* HAVE_SELINUX */
 ...
 */
#ifdef HAVE_SELINUX
static dbus_bool_t bus_selinux_check(
    ...
    if (avc_has_perm(
            SELINUX_SID_FROM_BUS(sender_sid),
            override_sid ? SELINUX_SID_FROM_BUS(override_sid) : bus_sid,
            target_class,
            requested,
            &aeref,
            auxdata) < 0) {
 ...
 */
dbus_bool_t bus_selinux_allows_acquire_service(
 ...
 */
dbus_bool_t bus_selinux_allows_send(
...
dbus_bool_t bus_selinux_append_context(DBusMessage* message, BusSELinuxID* sid, DBusError* error) {
    ...
    if (avc_sid_to_context(SELINUX_SID_FROM_BUS(sid), &context) < 0) {
        if (errno == ENOMEM)
            ...
            dbus_set_error(error, DBUS_ERROR_FAILED, "Error getting context from SID: %s\n", _dbus_strerror(errno));
        return FALSE;
    }
    if (!dbus_message_append_args(
            message, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE, &context, strlen(context), DBUS_TYPE_INVALID)) {
        _DBUS_SET_OOM(error);
        return FALSE;
    }
    freecon(context);
...
BusSELinuxID* bus_selinux_init_connection_id(DBusConnection* connection, DBusError* error) {
    ...
    if (avc_context_to_sid(con, &sid) < 0) {
        if (errno == ENOMEM)
            ...
            dbus_set_error(
                error, DBUS_ERROR_FAILED, "Error getting SID from context \"%s\": %s\n", con, _dbus_strerror(errno));
 ...
 */
dbus_bool_t bus_selinux_id_table_insert(
    DBusHashTable* service_table,
    const char*    service_name,
