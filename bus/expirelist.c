...
BusExpireList* bus_expire_list_new(DBusLoop* loop, int expire_after, BusExpireFunc expire_func, void* data) {
    ...
    list->timeout = _dbus_timeout_new(
        ...
        list,
        NULL);
    if (list->timeout == NULL) goto failed;

    _dbus_timeout_disable(list->timeout);

    if (!_dbus_loop_add_timeout(list->loop, list->timeout)) goto failed;

    return list;

failed:
    if (list->timeout) _dbus_timeout_unref(list->timeout);
...
static int do_expiration_with_monotonic_time(BusExpireList* list, long tv_sec, long tv_usec) {
    DBusList* link;
    ...
    while (link != NULL) {
        ...
        elapsed = ELAPSED_MILLISECONDS_SINCE(item->added_tv_sec, item->added_tv_usec, tv_sec, tv_usec);
        ...
        if (((item->added_tv_sec == 0) && (item->added_tv_usec == 0)) ||
            ...
            if (min_wait_time > to_wait) min_wait_time = to_wait;
        }

        link = next;
    }

    if (next_interval < 0 && items_to_expire) next_interval = min_wait_time;

    return next_interval;
}

static void bus_expirelist_expire(BusExpireList* list) {
    int next_interval;
...
dbus_bool_t bus_expire_list_add(BusExpireList* list, BusExpireItem* item) {
    ...
    if (ret && !dbus_timeout_get_enabled(list->timeout)) bus_expire_timeout_set_interval(list->timeout, 0);
...
void bus_expire_list_add_link(BusExpireList* list, DBusList* link) {
    ...
    if (!dbus_timeout_get_enabled(list->timeout)) bus_expire_timeout_set_interval(list->timeout, 0);
}
...
static dbus_bool_t test_expire_func(BusExpireList* list, DBusList* link, void* data) {
    TestExpireItem* t;
...
static void time_add_milliseconds(long* tv_sec, long* tv_usec, int milliseconds) {
...
#define EXPIRE_AFTER 100
    ...
    list = bus_expire_list_new(loop, EXPIRE_AFTER, test_expire_func, NULL);
    ...
    time_add_milliseconds(&tv_sec_not_expired, &tv_usec_not_expired, EXPIRE_AFTER - 1);

    tv_sec_expired  = tv_sec;
    tv_usec_expired = tv_usec;
    time_add_milliseconds(&tv_sec_expired, &tv_usec_expired, EXPIRE_AFTER);
    ...
    if (item == NULL) goto oom;

    item->item.added_tv_sec  = tv_sec;
    item->item.added_tv_usec = tv_usec;
    if (!bus_expire_list_add(list, &item->item)) _dbus_assert_not_reached("out of memory");

    next_interval = do_expiration_with_monotonic_time(list, tv_sec_not_expired, tv_usec_not_expired);
    _dbus_assert(item->expire_count == 0);
    _dbus_verbose("next_interval = %d\n", next_interval);
    _dbus_assert(next_interval == 1);

    next_interval = do_expiration_with_monotonic_time(list, tv_sec_expired, tv_usec_expired);
    _dbus_assert(item->expire_count == 1);
    _dbus_verbose("next_interval = %d\n", next_interval);
    _dbus_assert(next_interval == -1);

    next_interval = do_expiration_with_monotonic_time(list, tv_sec_past, tv_usec_past);
    _dbus_assert(item->expire_count == 1);
    _dbus_verbose("next_interval = %d\n", next_interval);
