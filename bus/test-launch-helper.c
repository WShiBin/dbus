...
static dbus_bool_t bus_activation_helper_oom_test(void* data) {
    const char* service;
    DBusError   error;
    dbus_bool_t retval;
...
int main(int argc, char** argv) {
    ...
    if (!_dbus_string_append(&config_file, dir)) return 1;
    if (!_dbus_string_append(&config_file, "/valid-config-files-system/debug-allow-all-pass.conf")) return 1;
