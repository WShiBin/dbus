...
static dbus_bool_t send_one_message(
    ...
    if (!bus_context_check_security_policy(
            context, transaction, sender, addressed_recipient, connection, message, NULL, &stack_error)) {
        if (!bus_transaction_capture_error_reply(transaction, sender, &stack_error, message)) {
            bus_context_log(
                context,
    ...
    if (dbus_message_contains_unix_fds(message) && !dbus_connection_can_send_type(connection, DBUS_TYPE_UNIX_FD)) {
        dbus_set_error(
            &stack_error,
...
dbus_bool_t bus_dispatch_matches(
    BusTransaction* transaction,
    DBusConnection* sender,
    DBusConnection* addressed_recipient,
    DBusMessage*    message,
    ...
    if (addressed_recipient != NULL) {
        if (!bus_context_check_security_policy(
                context, transaction, sender, addressed_recipient, addressed_recipient, message, NULL, error))
            return FALSE;
    ...
    recipients = NULL;
    if (!bus_matchmaker_get_recipients(matchmaker, connections, sender, addressed_recipient, message, &recipients)) {
        BUS_SET_OOM(error);
        return FALSE;
    }
    ...
    while (link != NULL) {
        ...
        if (!send_one_message(dest, context, sender, addressed_recipient, message, transaction, &tmp_error)) break;
...
static DBusHandlerResult bus_dispatch(DBusConnection* connection, DBusMessage* message) {
     ...
     * until we can.
     */
    while (!bus_connection_preallocate_oom_error(connection)) _dbus_wait_for_memory();
    ...
    if (service_name && strcmp(service_name, DBUS_SERVICE_DBUS) == 0) /* to bus driver */
    {
        ...
        if (!bus_driver_handle_message(connection, transaction, message, &error)) goto out;
    ...
    } else if (service_name != NULL) /* route to named service */
    {
        ...
        if (service == NULL && dbus_message_get_auto_start(message)) {
             ...
             */
            if (!bus_activation_activate_service(
                    activation, connection, transaction, TRUE, message, service_name, &error)) {
     ...
     * match rules.
     */
    if (!bus_dispatch_matches(transaction, connection, addressed_recipient, message, &error)) goto out;
...
static DBusHandlerResult bus_dispatch_message_filter(
    DBusConnection* connection,
    DBusMessage*    message,
    void*           user_data) {
...
dbus_bool_t bus_dispatch_add_connection(DBusConnection* connection) {
    if (!dbus_connection_add_filter(connection, bus_dispatch_message_filter, NULL, NULL)) return FALSE;
...
void bus_dispatch_remove_connection(DBusConnection* connection) {
    ...
    dbus_connection_remove_filter(connection, bus_dispatch_message_filter, NULL);
}
...
typedef dbus_bool_t (*Check2Func)(BusContext* context, DBusConnection* connection);

static dbus_bool_t check_no_leftovers(BusContext* context);

static void block_connection_until_message_from_bus(
    BusContext*     context,
    DBusConnection* connection,
    const char*     what_is_expected) {
    _dbus_verbose("expecting: %s\n", what_is_expected);

    while (dbus_connection_get_dispatch_status(connection) == DBUS_DISPATCH_COMPLETE &&
...
static void spin_connection_until_authenticated(BusContext* context, DBusConnection* connection) {
    _dbus_verbose("Spinning to auth connection %p\n", connection);
    while (!dbus_connection_get_is_authenticated(connection) && dbus_connection_get_is_connected(connection)) {
...
static DBusMessage* pop_message_waiting_for_memory(DBusConnection* connection) {
    while (dbus_connection_get_dispatch_status(connection) == DBUS_DISPATCH_NEED_MEMORY) _dbus_wait_for_memory();

    return dbus_connection_pop_message(connection);
}

static DBusMessage* borrow_message_waiting_for_memory(DBusConnection* connection) {
    while (dbus_connection_get_dispatch_status(connection) == DBUS_DISPATCH_NEED_MEMORY) _dbus_wait_for_memory();

    return dbus_connection_borrow_message(connection);
}

static void warn_unexpected_real(
    DBusConnection* connection,
    DBusMessage*    message,
    const char*     expected,
    const char*     function,
    ...
    if (message)
        _dbus_warn(
            "%s:%d received message interface \"%s\" member \"%s\" error name \"%s\" on %p, expecting %s",
            function,
            line,
            dbus_message_get_interface(message) ? dbus_message_get_interface(message) : "(unset)",
            dbus_message_get_member(message) ? dbus_message_get_member(message) : "(unset)",
            dbus_message_get_error_name(message) ? dbus_message_get_error_name(message) : "(unset)",
            connection,
            expected);
    else
        _dbus_warn("%s:%d received no message on %p, expecting %s", function, line, connection, expected);
}
...
static void verbose_message_received(DBusConnection* connection, DBusMessage* message) {
    _dbus_verbose(
        "Received message interface \"%s\" member \"%s\" error name \"%s\" on %p\n",
        dbus_message_get_interface(message) ? dbus_message_get_interface(message) : "(unset)",
        dbus_message_get_member(message) ? dbus_message_get_member(message) : "(unset)",
        dbus_message_get_error_name(message) ? dbus_message_get_error_name(message) : "(unset)",
        connection);
...
typedef enum { SERVICE_CREATED, OWNER_CHANGED, SERVICE_DELETED } ServiceInfoKind;
...
static dbus_bool_t check_service_owner_changed_foreach(DBusConnection* connection, void* data) {
    CheckServiceOwnerChangedData* d = data;
    DBusMessage*                  message;
    DBusError                     error;
    const char *                  service_name, *old_owner, *new_owner;

    if (d->expected_kind == SERVICE_CREATED && connection == d->skip_connection) return TRUE;
    ...
    if (message == NULL) {
        ...
        message = pop_message_waiting_for_memory(connection);
        if (message == NULL) {
            _dbus_warn("Did not receive a message on %p, expecting %s", connection, "NameOwnerChanged");
            goto out;
        }
    ...
    reget_service_info_data:
        ...
        if (strcmp(service_name, d->expected_service_name) != 0) {
            _dbus_warn("expected info on service %s, got info on %s", d->expected_service_name, service_name);
            goto out;
        }

        if (*service_name == ':' && new_owner[0] && strcmp(service_name, new_owner) != 0) {
            _dbus_warn(
                "inconsistent ServiceOwnedChanged message (\"%s\" [ %s -> %s ])", service_name, old_owner, new_owner);
            goto out;
        }
    }
...
static void kill_client_connection(BusContext* context, DBusConnection* connection) {
    char*                        base_service;
    const char*                  s;
    ...
    while ((base_service = _dbus_strdup(s)) == NULL) _dbus_wait_for_memory();
    ...
    bus_test_clients_foreach(check_service_owner_changed_foreach, &socd);

    dbus_free(base_service);

    if (socd.failed) _dbus_assert_not_reached("didn't get the expected NameOwnerChanged (deletion) messages");

    if (!check_no_leftovers(context))
        _dbus_assert_not_reached("stuff left in message queues after disconnecting a client");
}

static void kill_client_connection_unchecked(DBusConnection* connection) {
...
static dbus_bool_t check_no_messages_foreach(DBusConnection* connection, void* data) {
...
static dbus_bool_t check_no_leftovers(BusContext* context) {
    ...
    nmd.failed = FALSE;
    bus_test_clients_foreach(check_no_messages_foreach, &nmd);
 ...
 */
static dbus_bool_t check_hello_message(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    ...
    message = pop_message_waiting_for_memory(connection);
    if (message == NULL) {
        _dbus_warn("Did not receive a reply to %s %d on %p", "Hello", serial, connection);
        goto out;
    }
    ...
    if (!dbus_message_has_sender(message, DBUS_SERVICE_DBUS)) {
        _dbus_warn(
            ...
            dbus_message_get_sender(message) ? dbus_message_get_sender(message) : "(none)");
        goto out;
    }
    ...
    retry_get_hello_name:
        ...
        while (!dbus_bus_set_unique_name(connection, name)) _dbus_wait_for_memory();
        ...
        if (message == NULL) {
            ...
            message = pop_message_waiting_for_memory(connection);
            if (message == NULL) {
                _dbus_warn("Expecting %s, got nothing", "NameAcquired");
                goto out;
            }
        }
        if (!dbus_message_is_signal(message, DBUS_INTERFACE_DBUS, "NameAcquired")) {
            _dbus_warn("Expecting %s, got smthg else", "NameAcquired");
            goto out;
        }

    retry_get_acquired_name:
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &acquired, DBUS_TYPE_INVALID)) {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
        ...
        if (strcmp(acquired, name) != 0) {
            _dbus_warn("Acquired name is %s but expected %s", acquired, name);
            goto out;
        }
        acquired = NULL;
    }

    if (!check_no_leftovers(context)) goto out;
...
out:
    ...
    if (name_message) dbus_message_unref(name_message);
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_double_hello_message(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;
    DBusError     error;
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_get_connection_unix_user(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;
    DBusError     error;
    ...
    if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &base_service_name, DBUS_TYPE_INVALID)) {
        dbus_message_unref(message);
        return TRUE;
    }
    ...
    message = pop_message_waiting_for_memory(connection);
    if (message == NULL) {
        _dbus_warn("Did not receive a reply to %s %d on %p", "GetConnectionUnixUser", serial, connection);
        goto out;
    }
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR) {
        ...
        if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_METHOD_RETURN) {
            ...
            warn_unexpected(connection, message, "method_return for GetConnectionUnixUser");
    ...
    retry_get_property:
        ...
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_UINT32, &uid, DBUS_TYPE_INVALID)) {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_get_connection_unix_process_id(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;
    DBusError     error;
    ...
    message = pop_message_waiting_for_memory(connection);
    if (message == NULL) {
        _dbus_warn("Did not receive a reply to %s %d on %p", "GetConnectionUnixProcessID", serial, connection);
        goto out;
    }
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR) {
...
#if defined(__FreeBSD__) || defined(__FreeBSD_kernel__) || defined(__linux__) || defined(__OpenBSD__)
        ...
        if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_METHOD_RETURN) {
            ...
            warn_unexpected(connection, message, "method_return for GetConnectionUnixProcessID");
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_add_match(BusContext* context, DBusConnection* connection, const char* rule) {
    DBusMessage*  message;
    dbus_bool_t   retval;
    dbus_uint32_t serial;
    DBusError     error;
    ...
    message = dbus_message_new_method_call(DBUS_SERVICE_DBUS, DBUS_PATH_DBUS, DBUS_INTERFACE_DBUS, "AddMatch");

    if (message == NULL) return TRUE;

    if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &rule, DBUS_TYPE_INVALID)) {
        dbus_message_unref(message);
        return TRUE;
    }
    ...
    message = pop_message_waiting_for_memory(connection);
    if (message == NULL) {
        _dbus_warn("Did not receive a reply to %s %d on %p", "AddMatch", serial, connection);
        goto out;
    }
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_get_all_match_rules(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_bool_t   retval;
    dbus_uint32_t serial;
    DBusError     error;
    ...
    message = dbus_message_new_method_call(DBUS_SERVICE_DBUS, DBUS_PATH_DBUS, BUS_INTERFACE_STATS, "GetAllMatchRules");
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_hello_connection(BusContext* context) {
    DBusConnection* connection;
    DBusError       error;
    ...
    if (!check_hello_message(context, connection)) return FALSE;
    ...
    if (dbus_bus_get_unique_name(connection) == NULL) {
        ...
        if (!check_add_match(context, connection, "")) return FALSE;
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_nonexistent_service_no_auto_start(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;
    ...
    message =
        dbus_message_new_method_call(DBUS_SERVICE_DBUS, DBUS_PATH_DBUS, DBUS_INTERFACE_DBUS, "StartServiceByName");
    ...
    if (!dbus_message_append_args(
            message, DBUS_TYPE_STRING, &nonexistent, DBUS_TYPE_UINT32, &flags, DBUS_TYPE_INVALID)) {
        dbus_message_unref(message);
        return TRUE;
    }
    ...
    message = pop_message_waiting_for_memory(connection);
    if (message == NULL) {
        _dbus_warn("Did not receive a reply to %s %d on %p", "StartServiceByName", serial, connection);
        goto out;
    }
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR) {
        ...
        if (dbus_message_is_error(message, DBUS_ERROR_NO_MEMORY)) {
            ; /* good, this is a valid response */
        } else if (dbus_message_is_error(message, DBUS_ERROR_SERVICE_UNKNOWN)) {
        ...
        _dbus_warn("Did not expect to successfully activate %s", NONEXISTENT_SERVICE_NAME);
        goto out;
    }
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_nonexistent_service_auto_start(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;

    message = dbus_message_new_method_call(
        NONEXISTENT_SERVICE_NAME, "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Echo");
    ...
    if (message == NULL) {
        _dbus_warn("Did not receive a reply to %s %d on %p", "Echo message (auto activation)", serial, connection);
        goto out;
    }
...
static dbus_bool_t check_base_service_activated(
    BusContext*     context,
    DBusConnection* connection,
    ...
    reget_service_name_arg:
        ...
        if (!dbus_message_get_args(
            ...
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
                ...
                _dbus_warn("Message %s doesn't have a service name: %s", "NameOwnerChanged (creation)", error.message);
                goto out;
            }
        }

        if (*base_service != ':') {
            _dbus_warn("Expected base service activation, got \"%s\" instead", base_service);
            goto out;
        }

        if (strcmp(base_service, base_service_from_bus) != 0) {
            _dbus_warn(
                "Expected base service activation, got \"%s\" instead with owner \"%s\"",
                base_service,
                base_service_from_bus);
            goto out;
        }
        ...
        if (old_owner[0]) {
            _dbus_warn("Received an old_owner argument during base service activation, \"%s\"", old_owner);
            goto out;
        }
    ...
    if (base_service_p) *base_service_p = base_service;
...
static dbus_bool_t check_service_activated(
    BusContext*     context,
    DBusConnection* connection,
    ...
    reget_service_name_arg:
        ...
        if (strcmp(service_name, activated_name) != 0) {
            _dbus_warn("Expected to see service %s created, saw %s instead", activated_name, service_name);
            goto out;
        }

        if (strcmp(base_service_name, base_service_from_bus) != 0) {
            _dbus_warn(
                "NameOwnerChanged reports wrong base service: %s owner, expected %s instead",
                base_service_from_bus,
                base_service_name);
            goto out;
        }
        ...
        if (old_owner[0]) {
            _dbus_warn("expected a %s, got a %s", "NameOwnerChanged (creation)", "NameOwnerChanged (change)");
            goto out;
        }
        ...
        message = pop_message_waiting_for_memory(connection);
        if (message == NULL) {
            _dbus_warn("Expected a reply to %s, got nothing", "StartServiceByName");
            goto out;
        }
    } else {
    ...
    activation_result = 0;
    if (!dbus_message_get_args(message, &error, DBUS_TYPE_UINT32, &activation_result, DBUS_TYPE_INVALID)) {
        if (!dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
            _dbus_warn("Did not have activation result first argument to %s: %s", "StartServiceByName", error.message);
            goto out;
        }
        ...
        if (activation_result == DBUS_START_REPLY_SUCCESS)
            ...
            _dbus_warn("Activation result was %u, no good.", activation_result);
            goto out;
        }
    }
...
static dbus_bool_t check_service_auto_activated(
    BusContext*     context,
    DBusConnection* connection,
    const char*     activated_name,
    const char*     base_service_name,
    ...
    reget_service_name_arg:
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &service_name, DBUS_TYPE_INVALID)) {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
                ...
                _dbus_warn("Message %s doesn't have a service name: %s", "NameOwnerChanged", error.message);
                dbus_error_free(&error);
                goto out;
            }
        }
...
static dbus_bool_t check_service_deactivated(
    BusContext*     context,
    DBusConnection* connection,
    const char*     activated_name,
...
static dbus_bool_t check_send_exit_to_service(
    BusContext*     context,
    DBusConnection* connection,
    const char*     service_name,
    const char*     base_service) {
    ...
    message =
        dbus_message_new_method_call(service_name, "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Exit");

    if (message == NULL) {
        /* Do this again; we still need the service to exit... */
        if (!check_send_exit_to_service(context, connection, service_name, base_service)) goto out;
    ...
    if (got_error) {
        ...
        if (dbus_message_get_reply_serial(message) != serial) {
            warn_unexpected(connection, message, "error with the correct reply serial");
            goto out;
        }

        if (!dbus_message_is_error(message, DBUS_ERROR_NO_MEMORY)) {
            warn_unexpected(connection, message, "a no memory error from asking test service to exit");
            goto out;
        }
        ...
        if (!check_service_deactivated(context, connection, service_name, base_service)) goto out;
        ...
        if (message == NULL) {
            warn_unexpected(connection, NULL, "reply to Exit() method call");
            goto out;
        }
        if (!dbus_message_is_error(message, DBUS_ERROR_NO_REPLY)) {
            warn_unexpected(connection, message, "NoReply error from Exit() method call");
            goto out;
        }
        ...
        if (!check_no_leftovers(context)) {
            _dbus_warn("Messages were left over after %s", _DBUS_FUNCTION_NAME);
            goto out;
        }
    }
...
static dbus_bool_t check_got_error(BusContext* context, DBusConnection* connection, const char* first_error_name, ...) {
    DBusMessage* message;
    dbus_bool_t  retval;
    va_list      ap;
    ...
    if (!error_found) {
        _dbus_warn(
            "Expected error %s or other, got %s instead", first_error_name, dbus_message_get_error_name(message));
        goto out;
    }
...
typedef enum { GOT_SERVICE_CREATED, GOT_SERVICE_DELETED, GOT_ERROR, GOT_SOMETHING_ELSE } GotServiceInfo;

static GotServiceInfo check_got_service_info(DBusMessage* message) {
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_existent_service_no_auto_start(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    ...
    flags = 0;
    if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &existent, DBUS_TYPE_UINT32, &flags, DBUS_TYPE_INVALID)) {
        dbus_message_unref(message);
        return TRUE;
    }
    ...
    message = pop_message_waiting_for_memory(connection);
    if (message == NULL) {
        _dbus_warn("Did not receive any messages after %s %d on %p", "StartServiceByName", serial, connection);
        goto out;
    }
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR) {
        ...
        if (dbus_message_is_error(message, DBUS_ERROR_NO_MEMORY)) {
            ...
            dbus_message_is_error(message, DBUS_ERROR_SPAWN_CHILD_SIGNALED) ||
            dbus_message_is_error(message, DBUS_ERROR_SPAWN_EXEC_FAILED)) {
            ; /* good, this is expected also */
        } else {
            _dbus_warn("Did not expect error %s", dbus_message_get_error_name(message));
            goto out;
        }
    } else {
        GotServiceInfo message_kind;

        if (!check_base_service_activated(context, connection, message, &base_service)) goto out;
        ...
        switch (message_kind) {
            ...
            default:
                _dbus_warn(
                    "Unexpected message after ActivateService "
                    "(should be an error or a service announcement");
                goto out;

            case GOT_ERROR:
                if (!check_got_error(context, connection, DBUS_ERROR_SPAWN_CHILD_EXITED, DBUS_ERROR_NO_MEMORY, NULL))
                    goto out;
            ...
            case GOT_SERVICE_DELETED: {
                ...
                if (message_kind != GOT_ERROR) {
                    ...
                    if (!check_got_error(
                            context, connection, DBUS_ERROR_SPAWN_CHILD_EXITED, DBUS_ERROR_NO_MEMORY, NULL))
                        goto out;
                }
                break;
            }
            ...
            case GOT_SERVICE_CREATED:
                ...
                if (message == NULL) {
                    _dbus_warn(
                        "Failed to pop message we just put back! "
                        "should have been a NameOwnerChanged (creation)");
                    goto out;
                }

                if (!check_service_activated(context, connection, EXISTENT_SERVICE_NAME, base_service, message))
                    goto out;
                ...
                if (!check_send_exit_to_service(context, connection, EXISTENT_SERVICE_NAME, base_service)) goto out;
...
out:
    ...
    if (base_service_message) dbus_message_unref(base_service_message);
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_segfault_service_no_auto_start(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;
    ...
    flags        = 0;
    if (!dbus_message_append_args(
            message, DBUS_TYPE_STRING, &segv_service, DBUS_TYPE_UINT32, &flags, DBUS_TYPE_INVALID)) {
        dbus_message_unref(message);
        return TRUE;
    }
...
#ifdef DBUS_WIN
        else if (dbus_message_is_error(message, DBUS_ERROR_SPAWN_CHILD_EXITED)) {
            /* unhandled exceptions are normal exit codes */
        }
#else
        else if (dbus_message_is_error(message, DBUS_ERROR_SPAWN_CHILD_SIGNALED)) {
            ; /* good, this is expected also */
        }
#endif
        else {
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_segfault_service_auto_start(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_existent_hello_from_self(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    const char*   text;

    message = dbus_message_new_method_call(
        EXISTENT_SERVICE_NAME, "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "RunHelloFromSelf");

    if (message == NULL) return TRUE;

    text = TEST_RUN_HELLO_FROM_SELF_MESSAGE;
    if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &text, DBUS_TYPE_INVALID)) {
        dbus_message_unref(message);
        return TRUE;
    }
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_existent_ping(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    message = dbus_message_new_method_call(
        EXISTENT_SERVICE_NAME, "/org/freedesktop/TestSuite", "org.freedesktop.DBus.Peer", "Ping");
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_existent_get_machine_id(BusContext* context, DBusConnection* connection) {
    ...
    message = dbus_message_new_method_call(
        EXISTENT_SERVICE_NAME, "/org/freedesktop/TestSuite", "org.freedesktop.DBus.Peer", "GetMachineId");
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_existent_service_auto_start(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    DBusMessage*  base_service_message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;
    ...
    message = dbus_message_new_method_call(
        EXISTENT_SERVICE_NAME, "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Echo");
    ...
    message = pop_message_waiting_for_memory(connection);
    if (message == NULL) {
        _dbus_warn("Did not receive any messages after auto start %d on %p", serial, connection);
        goto out;
    }
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_SIGNAL) {
        ...
        if (message == NULL) {
            _dbus_warn(
                "No message after auto activation "
        ...
        switch (message_kind) {
            case GOT_SERVICE_CREATED:
                ...
                if (!check_service_auto_activated(context, connection, EXISTENT_SERVICE_NAME, base_service, message))
                    goto out;
    ...
    if (!check_existent_ping(context, connection)) goto out;

    if (!check_existent_get_machine_id(context, connection)) goto out;

    if (!check_existent_hello_from_self(context, connection)) goto out;
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_launch_service_file_missing(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;

    message = dbus_message_new_method_call(
        SERVICE_FILE_MISSING_NAME, "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Echo");
    ...
    block_connection_until_message_from_bus(
        context, connection, "reply to service file missing should fail to auto-start");
    bus_test_run_everything(context);
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_launch_service_user_missing(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;

    message = dbus_message_new_method_call(
        SERVICE_USER_MISSING_NAME, "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Echo");
    ...
    bus_test_run_everything(context);
    block_connection_until_message_from_bus(
        context, connection, "reply to service which should fail to auto-start (missing User)");
    bus_test_run_everything(context);
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR) {
        ...
        if (dbus_message_is_error(message, DBUS_ERROR_NO_MEMORY)) {
            ; /* good, this is a valid response */
        } else if (dbus_message_is_error(message, DBUS_ERROR_SPAWN_FILE_INVALID)) {
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_launch_service_exec_missing(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;

    message = dbus_message_new_method_call(
        SERVICE_EXEC_MISSING_NAME, "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Echo");
    ...
    bus_test_run_everything(context);
    block_connection_until_message_from_bus(
        context, connection, "reply to service which should fail to auto-start (missing Exec)");
    bus_test_run_everything(context);
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_launch_service_service_missing(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;

    message = dbus_message_new_method_call(
        SERVICE_SERVICE_MISSING_NAME, "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Echo");
    ...
    bus_test_run_everything(context);
    block_connection_until_message_from_bus(
        context, connection, "reply to service which should fail to auto-start (missing Service)");
    bus_test_run_everything(context);
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_shell_fail_service_auto_start(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;

    message = dbus_message_new_method_call(
        SHELL_FAIL_SERVICE_NAME, "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Echo");
    ...
    bus_test_run_everything(context);
    block_connection_until_message_from_bus(
        context, connection, "reply to shell Echo on service which should fail to auto-start");
    bus_test_run_everything(context);
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR) {
        ...
        if (dbus_message_is_error(message, DBUS_ERROR_NO_MEMORY)) {
            ; /* good, this is a valid response */
        } else if (dbus_message_is_error(message, DBUS_ERROR_INVALID_ARGS)) {
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_shell_service_success_auto_start(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    DBusMessage*  base_service_message;
    dbus_uint32_t serial;
    dbus_bool_t   retval;
    ...
    message = dbus_message_new_method_call(
        SHELL_SUCCESS_SERVICE_NAME, "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Echo");
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_SIGNAL) {
        ...
        switch (message_kind) {
            case GOT_SERVICE_CREATED:
                ...
                /* Check that ServiceOwnerChanged (creation) was correctly received */
                if (!check_service_auto_activated(
                        context, connection, SHELL_SUCCESS_SERVICE_NAME, base_service, message))
                    goto out;
    ...
    */
    if (strcmp("-test", argv[1]) != 0) {
        _dbus_warn("Unexpected argv[1] in shell success service test (expected: %s, got: %s)", "-test", argv[1]);
        goto out;
    }

    if (strcmp("that", argv[2]) != 0) {
        _dbus_warn("Unexpected argv[2] in shell success service test (expected: %s, got: %s)", "that", argv[2]);
        goto out;
    }

    if (strcmp("we get", argv[3]) != 0) {
        _dbus_warn("Unexpected argv[3] in shell success service test (expected: %s, got: %s)", "we get", argv[3]);
        goto out;
    }

    if (strcmp("back", argv[4]) != 0) {
        _dbus_warn("Unexpected argv[4] in shell success service test (expected: %s, got: %s)", "back", argv[4]);
        goto out;
    }

    if (strcmp("--what", argv[5]) != 0) {
        _dbus_warn("Unexpected argv[5] in shell success service test (expected: %s, got: %s)", "--what", argv[5]);
        goto out;
    }

    if (strcmp("we put in", argv[6]) != 0) {
        _dbus_warn("Unexpected argv[6] in shell success service test (expected: %s, got: %s)", "we put in", argv[6]);
        goto out;
    }
    ...
    if (!check_send_exit_to_service(context, connection, SHELL_SUCCESS_SERVICE_NAME, base_service)) goto out;
...
static dbus_bool_t check_oom_check1_func(void* data) {
    Check1Data* d = data;

    if (!(*d->func)(d->context)) return FALSE;
...
static void check1_try_iterations(BusContext* context, const char* description, Check1Func func) {
    Check1Data d;

    d.func    = func;
    d.context = context;

    if (!_dbus_test_oom_handling(description, check_oom_check1_func, &d)) _dbus_assert_not_reached("test failed");
}

static dbus_bool_t check_get_services(
    BusContext*     context,
    DBusConnection* connection,
    const char*     method,
    char***         services,
    ...
    message = dbus_message_new_method_call(DBUS_SERVICE_DBUS, DBUS_PATH_DBUS, DBUS_INTERFACE_DBUS, method);
    ...
    message = pop_message_waiting_for_memory(connection);
    if (message == NULL) {
        _dbus_warn("Did not receive a reply to %s %d on %p", method, serial, connection);
        goto out;
    }
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR) {
        ...
        if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_METHOD_RETURN) {
            ...
            warn_unexpected(connection, message, "method_return for ListActivatableNames/ListNames");
    ...
    retry_get_property:
        ...
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_ARRAY, DBUS_TYPE_STRING, &srvs, &l, DBUS_TYPE_INVALID)) {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_list_services(BusContext* context, DBusConnection* connection) {
    DBusMessage*  message;
    DBusMessage*  base_service_message;
    const char*   base_service;
    dbus_uint32_t serial;
...
static dbus_bool_t check_oom_check2_func(void* data) {
    Check2Data* d = data;

    if (!(*d->func)(d->context, d->connection)) return FALSE;
...
static void check2_try_iterations(
    BusContext*     context,
    DBusConnection* connection,
    ...
    if (!_dbus_test_oom_handling(description, check_oom_check2_func, &d)) {
        _dbus_warn("%s failed during oom", description);
        _dbus_assert_not_reached("test failed");
    }
}

static dbus_bool_t setenv_TEST_LAUNCH_HELPER_CONFIG(const DBusString* test_data_dir, const char* filename) {
    DBusString full;
    DBusString file;

    if (!_dbus_string_init(&full)) return FALSE;
...
static dbus_bool_t bus_dispatch_test_conf(
    ...
    if (foo == NULL) _dbus_assert_not_reached("could not alloc connection");

    if (!bus_setup_debug_client(foo)) _dbus_assert_not_reached("could not set up connection");

    spin_connection_until_authenticated(context, foo);

    if (!check_hello_message(context, foo)) _dbus_assert_not_reached("hello message failed");

    if (!check_double_hello_message(context, foo)) _dbus_assert_not_reached("double hello message failed");

    if (!check_add_match(context, foo, "")) _dbus_assert_not_reached("AddMatch message failed");

    bar = dbus_connection_open_private(TEST_DEBUG_PIPE, &error);
    if (bar == NULL) _dbus_assert_not_reached("could not alloc connection");

    if (!bus_setup_debug_client(bar)) _dbus_assert_not_reached("could not set up connection");

    spin_connection_until_authenticated(context, bar);

    if (!check_hello_message(context, bar)) _dbus_assert_not_reached("hello message failed");

    if (!check_add_match(context, bar, "")) _dbus_assert_not_reached("AddMatch message failed");

    baz = dbus_connection_open_private(TEST_DEBUG_PIPE, &error);
    if (baz == NULL) _dbus_assert_not_reached("could not alloc connection");

    if (!bus_setup_debug_client(baz)) _dbus_assert_not_reached("could not set up connection");

    spin_connection_until_authenticated(context, baz);

    if (!check_hello_message(context, baz)) _dbus_assert_not_reached("hello message failed");

    if (!check_add_match(context, baz, "")) _dbus_assert_not_reached("AddMatch message failed");

    if (!check_add_match(context, baz, "interface='com.example'")) _dbus_assert_not_reached("AddMatch message failed");

#ifdef DBUS_ENABLE_STATS
    if (!check_get_all_match_rules(context, baz)) _dbus_assert_not_reached("GetAllMatchRules message failed");
#endif

    if (!check_get_connection_unix_user(context, baz)) _dbus_assert_not_reached("GetConnectionUnixUser message failed");
    ...
    if (!check_list_services(context, baz)) _dbus_assert_not_reached("ListActivatableNames message failed");
    ...
    check1_try_iterations(context, "create_and_hello", check_hello_connection);

    check2_try_iterations(context, foo, "nonexistent_service_no_auto_start", check_nonexistent_service_no_auto_start);

    check2_try_iterations(context, foo, "segfault_service_no_auto_start", check_segfault_service_no_auto_start);

    check2_try_iterations(context, foo, "existent_service_no_auto_start", check_existent_service_no_auto_start);

    check2_try_iterations(context, foo, "nonexistent_service_auto_start", check_nonexistent_service_auto_start);

    /* only do the segfault test if we are not using the launcher */
    check2_try_iterations(context, foo, "segfault_service_auto_start", check_segfault_service_auto_start);

    /* only do the shell fail test if we are not using the launcher */
    check2_try_iterations(context, foo, "shell_fail_service_auto_start", check_shell_fail_service_auto_start);
...
#ifndef DBUS_WIN
static dbus_bool_t bus_dispatch_test_conf_fail(const DBusString* test_data_dir, const char* filename) {
    BusContext*     context;
    DBusConnection* foo;
    DBusError       error;
...
dbus_bool_t bus_dispatch_test(const DBusString* test_data_dir) {
    ...
    if (!bus_dispatch_test_conf(test_data_dir, "valid-config-files/debug-allow-all.conf", FALSE)) return FALSE;

#ifndef DBUS_WIN
    /* run launch-helper activation tests */
    _dbus_verbose("Launch helper activation tests\n");
    if (!bus_dispatch_test_conf(test_data_dir, "valid-config-files-system/debug-allow-all-pass.conf", TRUE))
        return FALSE;

    /* run select launch-helper activation tests on broken service files */
    if (!bus_dispatch_test_conf_fail(test_data_dir, "valid-config-files-system/debug-allow-all-fail.conf"))
        return FALSE;
#endif
...
dbus_bool_t bus_dispatch_sha1_test(const DBusString* test_data_dir) {
    ...
    context = bus_context_new_test(test_data_dir, "valid-config-files/debug-allow-all-sha1.conf");
    if (context == NULL) return FALSE;
    ...
    check1_try_iterations(context, "create_and_hello_sha1", check_hello_connection);
...
dbus_bool_t bus_unix_fds_passing_test(const DBusString* test_data_dir) {
    ...
    if (context == NULL) _dbus_assert_not_reached("could not alloc context");
    ...
    if (!(m = dbus_message_new_signal("/", "a.b.c", "d"))) _dbus_assert_not_reached("could not alloc message");

    if (!(_dbus_socketpair(one, one + 1, TRUE, &error))) _dbus_assert_not_reached("Failed to allocate pipe #1");

    if (!(_dbus_socketpair(two, two + 1, TRUE, &error))) _dbus_assert_not_reached("Failed to allocate pipe #2");

    if (!dbus_message_append_args(
            m, DBUS_TYPE_UNIX_FD, one, DBUS_TYPE_UNIX_FD, two, DBUS_TYPE_UNIX_FD, two, DBUS_TYPE_INVALID))
        _dbus_assert_not_reached("Failed to attach fds.");

    if (!_dbus_close_socket(one[0], &error)) _dbus_assert_not_reached("Failed to close pipe #1 ");
    if (!_dbus_close_socket(two[0], &error)) _dbus_assert_not_reached("Failed to close pipe #2 ");
    ...
    if (!dbus_connection_send(foo, m, NULL)) _dbus_assert_not_reached("Failed to send fds");
    ...
    if (!(m = pop_message_waiting_for_memory(foo))) _dbus_assert_not_reached("Failed to receive msg");

    if (!dbus_message_is_signal(m, "a.b.c", "d")) _dbus_assert_not_reached("bogus message received");

    dbus_message_unref(m);

    block_connection_until_message_from_bus(context, bar, "unix fd reception on bar");

    if (!(m = pop_message_waiting_for_memory(bar))) _dbus_assert_not_reached("Failed to receive msg");

    if (!dbus_message_is_signal(m, "a.b.c", "d")) _dbus_assert_not_reached("bogus message received");

    if (!dbus_message_get_args(
            m, &error, DBUS_TYPE_UNIX_FD, &x, DBUS_TYPE_UNIX_FD, &y, DBUS_TYPE_UNIX_FD, &z, DBUS_TYPE_INVALID))
        _dbus_assert_not_reached("Failed to parse fds.");

    dbus_message_unref(m);

    if (write(x, "X", 1) != 1) _dbus_assert_not_reached("Failed to write to pipe #1");
    if (write(y, "Y", 1) != 1) _dbus_assert_not_reached("Failed to write to pipe #2");
    if (write(z, "Z", 1) != 1) _dbus_assert_not_reached("Failed to write to pipe #2/2nd fd");

    if (!_dbus_close(x, &error)) _dbus_assert_not_reached("Failed to close pipe #1/other side ");
    if (!_dbus_close(y, &error)) _dbus_assert_not_reached("Failed to close pipe #2/other side ");
    if (!_dbus_close(z, &error)) _dbus_assert_not_reached("Failed to close pipe #2/other size 2nd fd ");

    if (read(one[1].fd, &r, 1) != 1 || r != 'X') _dbus_assert_not_reached("Failed to read value from pipe.");
    if (read(two[1].fd, &r, 1) != 1 || r != 'Y') _dbus_assert_not_reached("Failed to read value from pipe.");
    if (read(two[1].fd, &r, 1) != 1 || r != 'Z') _dbus_assert_not_reached("Failed to read value from pipe.");

    if (!_dbus_close_socket(one[1], &error)) _dbus_assert_not_reached("Failed to close pipe #1 ");
    if (!_dbus_close_socket(two[1], &error)) _dbus_assert_not_reached("Failed to close pipe #2 ");
