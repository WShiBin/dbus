    ...
    if (rule->refcount == 0) {
        ...
        if (rule->args) {
            ...
            while (i < rule->args_len) {
                if (rule->args[i]) dbus_free(rule->args[i]);
                ++i;
            }
...
static dbus_bool_t append_key_and_escaped_value(DBusString* str, const char* token, const char* value) {
    ...
    if (!_dbus_string_append_printf(str, "%s='", token)) return FALSE;

    while (*p != '\0') {
        const char* next = strchr(p, '\'');

        if (next) {
            if (!_dbus_string_append_printf(str, "%.*s", (int)(next - p), p)) return FALSE;
             ...
             */
            if (!_dbus_string_append_printf(str, "'\\''")) return FALSE;
            p = next + 1;
        } else {
            if (!_dbus_string_append_printf(str, "%s", p)) return FALSE;
            break;
        }
    }

    if (!_dbus_string_append_byte(str, '\'')) return FALSE;
...
static char* match_rule_to_string(BusMatchRule* rule) {
    ...
    if (rule->flags & BUS_MATCH_MESSAGE_TYPE) {
        if (!append_key_and_escaped_value(&str, "type", dbus_message_type_to_string(rule->message_type))) goto nomem;
    }

    if (rule->flags & BUS_MATCH_INTERFACE) {
        if (_dbus_string_get_length(&str) > 0) {
            if (!_dbus_string_append(&str, ",")) goto nomem;
        }

        if (!append_key_and_escaped_value(&str, "interface", rule->interface)) goto nomem;
    }
    ...
    if (rule->flags & BUS_MATCH_MEMBER) {
        ...
        if (!append_key_and_escaped_value(&str, "member", rule->member)) goto nomem;
    }
    ...
    if (rule->flags & BUS_MATCH_PATH) {
        ...
        if (!append_key_and_escaped_value(&str, "path", rule->path)) goto nomem;
    }
    ...
    if (rule->flags & BUS_MATCH_PATH_NAMESPACE) {
        ...
        if (!append_key_and_escaped_value(&str, "path_namespace", rule->path)) goto nomem;
    }
    ...
    if (rule->flags & BUS_MATCH_SENDER) {
        ...
        if (!append_key_and_escaped_value(&str, "sender", rule->sender)) goto nomem;
    }
    ...
    if (rule->flags & BUS_MATCH_DESTINATION) {
        ...
        if (!append_key_and_escaped_value(&str, "destination", rule->destination)) goto nomem;
    }
    ...
    if (rule->flags & BUS_MATCH_CLIENT_IS_EAVESDROPPING) {
        ...
        if (!append_key_and_escaped_value(
                &str, "eavesdrop", (rule->flags & BUS_MATCH_CLIENT_IS_EAVESDROPPING) ? "true" : "false"))
            goto nomem;
    }
        ...
        while (i < rule->args_len) {
            if (rule->args[i] != NULL) {
                ...
                if (!_dbus_string_append_printf(&str, "arg%d%s", i, is_path ? "path" : is_namespace ? "namespace" : ""))
                    goto nomem;
                if (!append_key_and_escaped_value(&str, "", rule->args[i])) goto nomem;
            }
    ...
    if (!_dbus_string_steal_data(&str, &ret)) goto nomem;
...
dbus_bool_t bus_match_rule_set_arg(
    BusMatchRule*     rule,
    int               arg,
    ...
    if (arg >= rule->args_len) {
        ...
        if (new_arg_lens == NULL) return FALSE;
    ...
    if (!_dbus_string_copy_data(value, &new)) return FALSE;
    ...
    if (is_path) rule->arg_lens[arg] |= BUS_MATCH_ARG_IS_PATH;

    if (is_namespace) rule->arg_lens[arg] |= BUS_MATCH_ARG_NAMESPACE;
...
static dbus_bool_t tokenize_rule(const DBusString* rule_text, RuleToken tokens[MAX_RULE_TOKENS], DBusError* error) {
    ...
    while (i < MAX_RULE_TOKENS && pos < _dbus_string_get_length(rule_text)) {
        ...
        if (!find_key(rule_text, pos, &key, &pos, error)) goto out;

        if (_dbus_string_get_length(&key) == 0) goto next;
        ...
        if (!find_value(rule_text, pos, tokens[i].key, &value, &pos, error)) goto out;
...
static dbus_bool_t bus_match_rule_parse_arg_match(
    BusMatchRule*     rule,
    const char*       key,
    const DBusString* value,
    DBusError*        error) {
    ...
    if (_dbus_string_get_length(&key_str) < 4) {
        dbus_set_error(
            ...
            "Key '%s' in match rule starts with 'arg' but lacks an arg number. Should be 'arg0' or 'arg7' for "
            "example.\n",
            key);
        goto failed;
    }
    ...
    if (!_dbus_string_parse_uint(&key_str, 3, &arg, &end)) {
        dbus_set_error(
            error,
            DBUS_ERROR_MATCH_RULE_INVALID,
            "Key '%s' in match rule starts with 'arg' but could not parse arg number. Should be 'arg0' or 'arg7' for "
            "example.\n",
            key);
        goto failed;
    }
    ...
    if (end != length) {
        ...
        if ((end + len1) == length && _dbus_string_ends_with_c_str(&key_str, "path")) {
            ...
            dbus_set_error(
                error,
                DBUS_ERROR_MATCH_RULE_INVALID,
                "Key '%s' in match rule contains junk after argument number (%lu). Only 'arg%lupath' (for example) or "
                "'arg0namespace' are valid",
                key,
                arg,
                arg);
            goto failed;
...
BusMatchRule* bus_match_rule_parse(DBusConnection* matches_go_to, const DBusString* rule_text, DBusError* error) {
    ...
    if (!tokenize_rule(rule_text, tokens, error)) goto failed;
    ...
    while (tokens[i].key != NULL) {
        ...
        if (strcmp(key, "type") == 0) {
            ...
            if (t == DBUS_MESSAGE_TYPE_INVALID) {
                dbus_set_error(
                    error, DBUS_ERROR_MATCH_RULE_INVALID, "Invalid message type (%s) in match rule\n", value);
                goto failed;
            }
            ...
            if (rule->flags & (BUS_MATCH_PATH | BUS_MATCH_PATH_NAMESPACE)) {
                dbus_set_error(
                    error, DBUS_ERROR_MATCH_RULE_INVALID, "path or path_namespace specified twice in match rule\n");
                goto failed;
            }
            ...
            if (!bus_match_rule_parse_arg_match(rule, key, &tmp_str, error)) goto failed;
        } else {
...
BusMatchmaker* bus_matchmaker_new(void) {
    ...
    for (i = DBUS_MESSAGE_TYPE_INVALID; i < DBUS_NUM_MESSAGE_TYPES; i++) {
        ...
        if (p->rules_by_iface == NULL) goto nomem;
    }
...
static DBusList** bus_matchmaker_get_rules(
    BusMatchmaker* matchmaker,
    int            message_type,
    ...
    _dbus_verbose(
        "Looking up rules for message_type %d, interface %s\n", message_type, interface != NULL ? interface : "<null>");
...
static void bus_matchmaker_gc_rules(
    BusMatchmaker* matchmaker,
    int            message_type,
    const char*    interface,
    DBusList**     rules) {
...
dbus_bool_t bus_matchmaker_add_rule(BusMatchmaker* matchmaker, BusMatchRule* rule) {
    ...
    _dbus_verbose(
        "Adding rule with message_type %d, interface %s\n",
        rule->message_type,
    ...
    if (!_dbus_list_append(rules, rule)) return FALSE;
...
static dbus_bool_t match_rule_equal(BusMatchRule* a, BusMatchRule* b) {
    ...
    if (a->matches_go_to != b->matches_go_to) return FALSE;

    if ((a->flags & BUS_MATCH_MESSAGE_TYPE) && a->message_type != b->message_type) return FALSE;

    if ((a->flags & BUS_MATCH_MEMBER) && strcmp(a->member, b->member) != 0) return FALSE;

    if ((a->flags & BUS_MATCH_PATH) && strcmp(a->path, b->path) != 0) return FALSE;

    if ((a->flags & BUS_MATCH_INTERFACE) && strcmp(a->interface, b->interface) != 0) return FALSE;

    if ((a->flags & BUS_MATCH_SENDER) && strcmp(a->sender, b->sender) != 0) return FALSE;

    if ((a->flags & BUS_MATCH_DESTINATION) && strcmp(a->destination, b->destination) != 0) return FALSE;
    ...
    if (a->flags & BUS_MATCH_ARGS) {
        ...
        if (a->args_len != b->args_len) return FALSE;

        i = 0;
        while (i < a->args_len) {
            int length;

            if ((a->args[i] != NULL) != (b->args[i] != NULL)) return FALSE;

            if (a->arg_lens[i] != b->arg_lens[i]) return FALSE;

            length = a->arg_lens[i] & ~BUS_MATCH_ARG_FLAGS;

            if (a->args[i] != NULL) {
                _dbus_assert(b->args[i] != NULL);
                if (memcmp(a->args[i], b->args[i], length) != 0) return FALSE;
            }
...
void bus_matchmaker_remove_rule(BusMatchmaker* matchmaker, BusMatchRule* rule) {
    ...
    _dbus_verbose(
        "Removing rule with message_type %d, interface %s\n",
        rule->message_type,
        rule->interface != NULL ? rule->interface : "<null>");
...
dbus_bool_t bus_matchmaker_remove_rule_by_value(BusMatchmaker* matchmaker, BusMatchRule* value, DBusError* error) {
    ...
    _dbus_verbose(
        "Removing rule by value with message_type %d, interface %s\n",
        value->message_type,
    ...
    if (link == NULL) {
        dbus_set_error(
            error, DBUS_ERROR_MATCH_RULE_NOT_FOUND, "The given match rule wasn't found and can't be removed");
        return FALSE;
    }
...
static void rule_list_remove_by_connection(DBusList** rules, DBusConnection* connection) {
    ...
    while (link != NULL) {
        ...
        if (rule->matches_go_to == connection) {
            ...
            ((rule->flags & BUS_MATCH_SENDER) && *rule->sender == ':') ||
...
void bus_matchmaker_disconnected(BusMatchmaker* matchmaker, DBusConnection* connection) {
    ...
    for (i = DBUS_MESSAGE_TYPE_INVALID; i < DBUS_NUM_MESSAGE_TYPES; i++) {
        ...
        while (_dbus_hash_iter_next(&iter)) {
            ...
            if (*items == NULL) _dbus_hash_iter_remove_entry(&iter);
        }
    }
}
...
static dbus_bool_t connection_is_primary_owner(DBusConnection* connection, const char* service_name) {
    ...
    if (service == NULL) return FALSE; /* Service doesn't exist so connection can't own it. */
...
static dbus_bool_t match_rule_matches(
    BusMatchRule*   rule,
    ...
    if (flags & BUS_MATCH_CLIENT_IS_EAVESDROPPING) wants_to_eavesdrop = TRUE;

    if (flags & BUS_MATCH_MESSAGE_TYPE) {
        _dbus_assert(rule->message_type != DBUS_MESSAGE_TYPE_INVALID);

        if (rule->message_type != dbus_message_get_type(message)) return FALSE;
    }
    ...
    if (flags & BUS_MATCH_INTERFACE) {
        ...
        if (strcmp(iface, rule->interface) != 0) return FALSE;
    }
    ...
    if (flags & BUS_MATCH_MEMBER) {
        ...
        if (strcmp(member, rule->member) != 0) return FALSE;
    }
    ...
    if (flags & BUS_MATCH_SENDER) {
        ...
        if (sender == NULL) {
            if (strcmp(rule->sender, DBUS_SERVICE_DBUS) != 0) return FALSE;
        } else {
            if (!connection_is_primary_owner(sender, rule->sender)) return FALSE;
        }
    }
    ...
    if (flags & BUS_MATCH_DESTINATION) {
        ...
        if (destination == NULL) /* broadcast, but this rule specified a destination: no match */
            return FALSE;

        /* rule owner does not intend to eavesdrop: we'll deliver only msgs
         * directed to it, NOT MATCHING */
        if (!wants_to_eavesdrop) return FALSE;
        ...
        if (addressed_recipient == NULL) {
             ...
             */
            if (strcmp(rule->destination, destination) != 0) return FALSE;
        } else {
            if (!connection_is_primary_owner(addressed_recipient, rule->destination)) return FALSE;
        }
        ...
        if (!wants_to_eavesdrop && !msg_is_broadcast) return FALSE;
    ...
    if (flags & BUS_MATCH_PATH) {
        ...
        if (strcmp(path, rule->path) != 0) return FALSE;
    }
    ...
    if (flags & BUS_MATCH_PATH_NAMESPACE) {
        ...
        if (!str_has_prefix(path, rule->path)) return FALSE;
         ...
         * matches everything.
         */
        if (len > 1 && path[len] != '\0' && path[len] != '/') return FALSE;
    }
    ...
    if (flags & BUS_MATCH_ARGS) {
        ...
        while (i < rule->args_len) {
            ...
            if (expected_arg != NULL) {
                ...
                if (is_path) {
                    if (actual_length < expected_length && actual_arg[actual_length - 1] != '/') return FALSE;

                    if (expected_length < actual_length && expected_arg[expected_length - 1] != '/') return FALSE;

                    if (memcmp(actual_arg, expected_arg, MIN(actual_length, expected_length)) != 0) return FALSE;
                     ...
                     */
                    if (memcmp(expected_arg, actual_arg, expected_length) != 0) return FALSE;
                    ...
                    if (expected_length < actual_length) {
                         ...
                         */
                        if (actual_arg[expected_length] != '.') return FALSE;
                    }
            ...
            if (current_type != DBUS_TYPE_INVALID) dbus_message_iter_next(&iter);
...
static dbus_bool_t get_recipients_from_list(
    DBusList**      rules,
    DBusConnection* sender,
    DBusConnection* addressed_recipient,
    DBusMessage*    message,
    ...
    while (link != NULL) {
...
#ifdef DBUS_ENABLE_VERBOSE_MODE
        {
            ...
            _dbus_verbose(
                "Checking whether message matches rule %s for connection %p\n", s ? s : "nomem", rule->matches_go_to);
            dbus_free(s);
        }
#endif

        if (match_rule_matches(
                rule, sender, addressed_recipient, message, BUS_MATCH_MESSAGE_TYPE | BUS_MATCH_INTERFACE)) {
            _dbus_verbose("Rule matched\n");

            /* Append to the list if we haven't already */
            if (bus_connection_mark_stamp(rule->matches_go_to)) {
                if (!_dbus_list_append(recipients_p, rule->matches_go_to)) return FALSE;
            } else {
...
dbus_bool_t bus_matchmaker_get_recipients(
    BusMatchmaker*  matchmaker,
     ...
     */
    if (addressed_recipient != NULL) bus_connection_mark_stamp(addressed_recipient);
    ...
    if (type > DBUS_MESSAGE_TYPE_INVALID && type < DBUS_NUM_MESSAGE_TYPES) {
        ...
        if (interface != NULL) both = bus_matchmaker_get_rules(matchmaker, type, interface, FALSE);
    }
...
static dbus_bool_t test_parsing(void* data) {
    BusMatchRule* rule;

    rule = check_parse(
        TRUE,
        "type='signal',sender='org.freedesktop.DBusSender',interface='org.freedesktop.DBusInterface',member='Foo',path="
        "'/bar/foo',destination=':452345.34'");
    if (rule != NULL) {
    ...
    rule = check_parse(
        TRUE,
        "    type='signal',  \tsender='org.freedes''ktop.DBusSender',   interface='org.freedesktop.DBusInterface''''', "
    ...
    while (i < _DBUS_N_ELEMENTS(equality_tests)) {
        ...
        while (j < _DBUS_N_ELEMENTS(equality_tests)) {
            if (i != j) {
                ...
                if (match_rule_equal(first, second)) {
                    _dbus_warn(
                        "rule %s and %s should not have been equal", equality_tests[i].first, equality_tests[j].second);
                    exit(1);
                }
...
static void check_matches(dbus_bool_t expected_to_match, int number, DBusMessage* message, const char* rule_text) {
    ...
    if (matched != expected_to_match) {
        _dbus_warn(
            "Expected rule %s to %s message %d, failed", rule_text, expected_to_match ? "match" : "not match", number);
        exit(1);
    }
...
static void test_matching(void) {
    ...
    if (!dbus_message_set_member(message1, "Frobated")) _dbus_assert_not_reached("oom");
    ...
    if (!dbus_message_set_member(message2, "NameOwnerChanged")) _dbus_assert_not_reached("oom");

    /* Obviously this isn't really a NameOwnerChanged signal. */
    v_STRING = EXAMPLE_NAME;
    if (!dbus_message_append_args(message2, DBUS_TYPE_STRING, &v_STRING, NULL)) _dbus_assert_not_reached("oom");
...
static void test_path_match(
    ...
    if (!dbus_message_set_member(message, "Foo")) _dbus_assert_not_reached("oom");

    if (!dbus_message_append_args(message, type, &path, NULL)) _dbus_assert_not_reached("oom");

    matched = match_rule_matches(rule, NULL, NULL, message, 0);

    if (matched != should_match) {
        _dbus_warn(
            "Expected rule %s to %s message "
...
static const char* path_namespace_should_match_message_1[] = {
...
static void test_matching_path_namespace(void) {
    ...
    message1 = dbus_message_new(DBUS_MESSAGE_TYPE_SIGNAL);
    _dbus_assert(message1 != NULL);
    if (!dbus_message_set_path(message1, "/foo/TheObjectManager")) _dbus_assert_not_reached("oom");

    message2 = dbus_message_new(DBUS_MESSAGE_TYPE_SIGNAL);
    _dbus_assert(message2 != NULL);
    if (!dbus_message_set_path(message2, "/foo/TheObjectManager/child_object")) _dbus_assert_not_reached("oom");

    message3 = dbus_message_new(DBUS_MESSAGE_TYPE_SIGNAL);
    _dbus_assert(message3 != NULL);
    if (!dbus_message_set_path(message3, "/foo/TheObjectManagerOther")) _dbus_assert_not_reached("oom");

    message4 = dbus_message_new(DBUS_MESSAGE_TYPE_SIGNAL);
    _dbus_assert(message4 != NULL);
    if (!dbus_message_set_path(message4, "/")) _dbus_assert_not_reached("oom");
