...
static dbus_bool_t bus_pending_reply_expired(BusExpireList* list, DBusList* link, void* data);

static void bus_connection_drop_pending_replies(BusConnections* connections, DBusConnection* connection);

static dbus_bool_t expire_incomplete_timeout(void* data);

#define BUS_CONNECTION_DATA(connection) (dbus_connection_get_data((connection), connection_data_slot))

static DBusLoop* connection_get_loop(DBusConnection* connection) {
    BusConnectionData* d;
...
static int get_connections_for_uid(BusConnections* connections, dbus_uid_t uid) {
    void* val;
    int   current_count;

    /* val is NULL is 0 when it isn't in the hash yet */

    val = _dbus_hash_table_lookup_uintptr(connections->completed_by_user, uid);
...
static dbus_bool_t adjust_connections_for_uid(BusConnections* connections, dbus_uid_t uid, int adjustment) {
    int current_count;

    current_count = get_connections_for_uid(connections, uid);

    _dbus_verbose(
        "Adjusting connection count for UID " DBUS_UID_FORMAT ": was %d adjustment %d making %d\n",
        uid,
        current_count,
        adjustment,
    ...
    if (current_count == 0) {
        ...
        retval =
            _dbus_hash_table_insert_uintptr(connections->completed_by_user, uid, _DBUS_INT_TO_POINTER(current_count));

        /* only positive adjustment can fail as otherwise
         * a hash entry should already exist
         */
        _dbus_assert(adjustment > 0 || (adjustment <= 0 && retval));
...
void bus_connection_disconnected(DBusConnection* connection) {
    ...
    retry:
        ...
        while ((transaction = bus_transaction_new(d->connections->context)) == NULL) _dbus_wait_for_memory();
    ...
    /* no more watching */
    if (!dbus_connection_set_watch_functions(connection, NULL, NULL, NULL, connection, NULL))
        _dbus_assert_not_reached("setting watch functions to NULL failed");

    if (!dbus_connection_set_timeout_functions(connection, NULL, NULL, NULL, connection, NULL))
    ...
    if (d->pending_unix_fds_timeout) {
        _dbus_loop_remove_timeout(bus_context_get_loop(d->connections->context), d->pending_unix_fds_timeout);
    ...
    if (d->link_in_monitors != NULL) {
        ...
        if (mm != NULL) bus_matchmaker_disconnected(mm, connection);
    ...
    dbus_connection_set_data(connection, connection_data_slot, NULL, NULL);
...
static void remove_connection_watch(DBusWatch* watch, void* data) {
    DBusConnection* connection = data;
...
static void remove_connection_timeout(DBusTimeout* timeout, void* data) {
    DBusConnection* connection = data;
...
static dbus_bool_t allow_unix_user_function(DBusConnection* connection, unsigned long uid, void* data) {
    BusConnectionData* d;
...
static void free_connection_data(void* data) {
    ...
    if (d->oom_preallocated) dbus_connection_free_preallocated_send(d->connection, d->oom_preallocated);

    if (d->oom_message) dbus_message_unref(d->oom_message);

    if (d->policy) bus_client_policy_unref(d->policy);

    if (d->apparmor_confinement) bus_apparmor_confinement_unref(d->apparmor_confinement);
...
BusConnections* bus_connections_new(BusContext* context) {
    ...
    if (!dbus_connection_allocate_data_slot(&connection_data_slot)) goto failed_0;

    connections = dbus_new0(BusConnections, 1);
    if (connections == NULL) goto failed_1;

    connections->completed_by_user = _dbus_hash_table_new(DBUS_HASH_UINTPTR, NULL, NULL);
    if (connections->completed_by_user == NULL) goto failed_2;

    connections->expire_timeout = _dbus_timeout_new(
        100, /* irrelevant */
        expire_incomplete_timeout,
        connections,
        NULL);
    if (connections->expire_timeout == NULL) goto failed_3;

    _dbus_timeout_disable(connections->expire_timeout);

    connections->pending_replies = bus_expire_list_new(
        bus_context_get_loop(context), bus_context_get_reply_timeout(context), bus_pending_reply_expired, connections);
    if (connections->pending_replies == NULL) goto failed_4;

    if (!_dbus_loop_add_timeout(bus_context_get_loop(context), connections->expire_timeout)) goto failed_5;
...
void bus_connections_unref(BusConnections* connections) {
    ...
    if (connections->refcount == 0) {
        ...
        _dbus_loop_remove_timeout(bus_context_get_loop(connections->context), connections->expire_timeout);

        _dbus_timeout_unref(connections->expire_timeout);

        _dbus_hash_table_unref(connections->completed_by_user);

        if (connections->monitor_matchmaker != NULL) bus_matchmaker_unref(connections->monitor_matchmaker);
...
/* Used for logging */
static dbus_bool_t cache_peer_loginfo_string(BusConnectionData* d, DBusConnection* connection) {
    ...
    if (!_dbus_string_init(&loginfo_buf)) return FALSE;
    ...
    if (dbus_connection_get_unix_process_id(connection, &pid)) {
        if (prev_added) {
            if (!_dbus_string_append_byte(&loginfo_buf, ' ')) goto oom;
        }
        if (!_dbus_string_append_printf(&loginfo_buf, "pid=%ld comm=\"", pid)) goto oom;
    ...
    if (dbus_connection_get_windows_user(connection, &windows_sid)) {
        ...
        did_append = _dbus_string_append_printf(&loginfo_buf, "sid=\"%s\"", windows_sid);
    ...
    if (_dbus_connection_get_linux_security_label(connection, &security_label)) {
        ...
        did_append = _dbus_string_append_printf(&loginfo_buf, "label=\"%s\"", security_label);
    ...
    if (!_dbus_string_steal_data(&loginfo_buf, &(d->cached_loginfo_string))) goto oom;
...
oom:
    _dbus_string_free(&loginfo_buf);
    if (security_label != NULL) dbus_free(security_label);
    if (windows_sid != NULL) dbus_free(windows_sid);

    return FALSE;
}

static void check_pending_fds_cb(DBusConnection* connection) {
    ...
    _dbus_verbose(
        "Pending fds count changed on connection %p: %d -> %d\n",
        connection,
        n_pending_unix_fds_old,
        n_pending_unix_fds_new);

    if (n_pending_unix_fds_old == 0 && n_pending_unix_fds_new > 0) {
        _dbus_timeout_restart(d->pending_unix_fds_timeout, bus_context_get_pending_fd_timeout(d->connections->context));
    }
...
static dbus_bool_t pending_unix_fds_timeout_cb(void* data) {
    DBusConnection*    connection = data;
    BusConnectionData* d          = BUS_CONNECTION_DATA(connection);
    int                limit;
...
dbus_bool_t bus_connections_setup_connection(BusConnections* connections, DBusConnection* connection) {
    ...
    if (dbus_error_is_set(&error)) {
        bus_context_log(
            connections->context, DBUS_SYSTEM_LOG_WARNING, "Unable to set up new connection: %s", error.message);
        dbus_error_free(&error);
        goto error;
    }
    ...
    if (!dbus_connection_set_watch_functions(
            connection, add_connection_watch, remove_connection_watch, toggle_connection_watch, connection, NULL))
        goto oom;

    if (!dbus_connection_set_timeout_functions(
            connection, add_connection_timeout, remove_connection_timeout, NULL, connection, NULL))
        goto oom;
    ...
    dbus_connection_set_dispatch_status_function(
        connection, dispatch_status_function, bus_context_get_loop(connections->context), NULL);

    d->link_in_connection_list = _dbus_list_alloc_link(connection);
    if (d->link_in_connection_list == NULL) goto oom;

    /* Setup the connection with the dispatcher */
    if (!bus_dispatch_add_connection(connection)) goto oom;
    ...
    d->pending_unix_fds_timeout = _dbus_timeout_new(
        100, /* irrelevant */
        pending_unix_fds_timeout_cb,
        connection,
        NULL);
    if (d->pending_unix_fds_timeout == NULL) goto oom;

    _dbus_timeout_disable(d->pending_unix_fds_timeout);
    if (!_dbus_loop_add_timeout(bus_context_get_loop(connections->context), d->pending_unix_fds_timeout)) goto oom;

    _dbus_connection_set_pending_fds_function(
        connection, (DBusPendingFdsChangeFunction)check_pending_fds_cb, connection);
...
error:
    if (d != NULL) {
        ...
        if (d->pending_unix_fds_timeout) _dbus_timeout_unref(d->pending_unix_fds_timeout);
        ...
        if (!dbus_connection_set_data(connection, connection_data_slot, NULL, NULL))
...
dbus_bool_t bus_connection_get_unix_groups(
    DBusConnection* connection,
...
dbus_bool_t bus_connection_is_in_unix_group(DBusConnection* connection, unsigned long gid) {
    int            i;
    unsigned long* group_ids;
    int            n_group_ids;

    if (!bus_connection_get_unix_groups(connection, &group_ids, &n_group_ids, NULL)) return FALSE;
...
const char* bus_connection_get_loginfo(DBusConnection* connection) {
    ...
    if (!bus_connection_is_active(connection)) return "inactive";
...
static dbus_bool_t foreach_active(BusConnections* connections, BusConnectionForeachFunction function, void* data) {
    DBusList* link;
    ...
    while (link != NULL) {
        ...
        if (!(*function)(connection, data)) return FALSE;
...
static dbus_bool_t foreach_inactive(BusConnections* connections, BusConnectionForeachFunction function, void* data) {
    DBusList* link;
 ...
 */
void bus_connections_foreach_active(BusConnections* connections, BusConnectionForeachFunction function, void* data) {
 ...
 * @param data data to pass to it as a second arg
 */
void bus_connections_foreach(BusConnections* connections, BusConnectionForeachFunction function, void* data) {
    if (!foreach_active(connections, function, data)) return;
...
dbus_bool_t bus_connection_preallocate_oom_error(DBusConnection* connection) {
    ...
    if (d->oom_preallocated != NULL) return TRUE;
    ...
    if (!dbus_message_set_error_name(message, DBUS_ERROR_NO_MEMORY) ||
        !dbus_message_set_destination(message, d->name) || !dbus_message_set_sender(message, DBUS_SERVICE_DBUS)) {
        dbus_connection_free_preallocated_send(connection, preallocated);
...
void bus_connection_send_oom_error(DBusConnection* connection, DBusMessage* in_reply_to) {
    BusConnectionData* d;
    ...
    if (!dbus_message_set_reply_serial(d->oom_message, dbus_message_get_serial(in_reply_to)))
    ...
    dbus_connection_send_preallocated(connection, d->oom_preallocated, d->oom_message, NULL);
...
#ifdef DBUS_ENABLE_STATS
static void update_peak(int* peak, int n) {
    if (*peak < n) *peak = n;
}
#endif

void bus_connection_add_match_rule_link(DBusConnection* connection, DBusList* link) {
    BusConnectionData* d;
    ...
    update_peak(&d->connections->peak_match_rules, d->connections->total_match_rules);
#endif
}

dbus_bool_t bus_connection_add_match_rule(DBusConnection* connection, BusMatchRule* rule) {
    DBusList* link;
...
void bus_connection_remove_match_rule(DBusConnection* connection, BusMatchRule* rule) {
    BusConnectionData* d;
...
void bus_connection_add_owned_service_link(DBusConnection* connection, DBusList* link) {
    BusConnectionData* d;
    ...
    update_peak(&d->connections->peak_bus_names_per_conn, d->n_services_owned);

    d->connections->total_bus_names += 1;
    update_peak(&d->connections->peak_bus_names, d->connections->total_bus_names);
#endif
}

dbus_bool_t bus_connection_add_owned_service(DBusConnection* connection, BusService* service) {
    DBusList* link;
...
void bus_connection_remove_owned_service(DBusConnection* connection, BusService* service) {
    BusConnectionData* d;
...
dbus_bool_t bus_connection_complete(DBusConnection* connection, const DBusString* name, DBusError* error) {
    BusConnectionData* d;
    unsigned long      uid;
    ...
    if (dbus_connection_get_unix_user(connection, &uid)) {
        if (!adjust_connections_for_uid(d->connections, uid, 1)) goto fail;
    }

    /* Create and cache a string which holds information about the
     * peer process; used for logging purposes.
     */
    if (!cache_peer_loginfo_string(d, connection)) goto fail;

    /* Now the connection is active, move it between lists */
    _dbus_list_unlink(&d->connections->incomplete, d->link_in_connection_list);
    d->connections->n_incomplete -= 1;
    _dbus_list_append_link(&d->connections->completed, d->link_in_connection_list);
    d->connections->n_completed += 1;
...
dbus_bool_t bus_connections_reload_policy(BusConnections* connections, DBusError* error) {
    BusConnectionData* d;
    DBusConnection*    connection;
    DBusList*          link;
    ...
    for (link = _dbus_list_get_first_link(&(connections->completed)); link;
        ...
        d->policy = bus_context_create_client_policy(connections->context, connection, error);
        if (d->policy == NULL) {
            _dbus_verbose("Failed to create security policy for connection %p\n", connection);
            _DBUS_ASSERT_ERROR_IS_SET(error);
            return FALSE;
 ...
 */
dbus_bool_t bus_connections_check_limits(
    BusConnections* connections,
    ...
    if (connections->n_completed >= limit) {
        if (limit_name_out != NULL) *limit_name_out = "max_completed_connections";

        if (limit_out != NULL) *limit_out = limit;

        dbus_set_error(error, DBUS_ERROR_LIMITS_EXCEEDED, "The maximum number of active connections has been reached");
        return FALSE;
    }
    ...
    if (dbus_connection_get_unix_user(requesting_completion, &uid)) {
        ...
        if (get_connections_for_uid(connections, uid) >= limit) {
            if (limit_name_out != NULL) *limit_name_out = "max_connections_per_user";
...
static void bus_pending_reply_free(BusPendingReply* pending) {
    _dbus_verbose(
        "Freeing pending reply %p, replier %p receiver %p serial %u\n",
        pending,
...
static dbus_bool_t bus_pending_reply_send_no_reply(
    BusConnections*  connections,
    BusTransaction*  transaction,
    ...
    if (!dbus_message_set_reply_serial(message, pending->reply_serial)) goto out;

    if (!dbus_message_set_error_name(message, DBUS_ERROR_NO_REPLY)) goto out;
    ...
    if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_STRING, &errmsg)) goto out;

    if (!bus_transaction_send_from_driver(transaction, pending->will_get_reply, message)) goto out;
...
static dbus_bool_t bus_pending_reply_expired(BusExpireList* list, DBusList* link, void* data) {
    ...
    _dbus_verbose(
        "Expiring pending reply %p, replier %p receiver %p serial %u\n",
        pending,
        pending->will_send_reply,
        pending->will_get_reply,
        pending->reply_serial);
    ...
    if (!bus_pending_reply_send_no_reply(connections, transaction, pending)) {
        bus_transaction_cancel_and_free(transaction);
        return FALSE;
    }
...
static void bus_connection_drop_pending_replies(BusConnections* connections, DBusConnection* connection) {
    ...
    while (link != NULL) {
        ...
        next    = bus_expire_list_get_next_link(connections->pending_replies, link);
        pending = link->data;
        ...
        if (pending->will_get_reply == connection) {
            ...
            _dbus_verbose(
                "Dropping pending reply %p, replier %p receiver %p serial %u\n",
                pending,
                pending->will_send_reply,
                pending->will_get_reply,
                pending->reply_serial);
             ...
             */
            _dbus_verbose(
                "Will expire pending reply %p, replier %p receiver %p serial %u\n",
                pending,
                pending->will_send_reply,
                pending->will_get_reply,
                pending->reply_serial);
...
static void cancel_pending_reply(void* data) {
    CancelPendingReplyData* d = data;

    _dbus_verbose("d = %p\n", d);

    if (!bus_expire_list_remove(d->connections->pending_replies, &d->pending->expire_item))
...
static void cancel_pending_reply_data_free(void* data) {
    CancelPendingReplyData* d = data;
 ...
 */
dbus_bool_t bus_connections_expect_reply(
    BusConnections* connections,
    BusTransaction* transaction,
    DBusConnection* will_get_reply,
    DBusConnection* will_send_reply,
    ...
    while (link != NULL) {
        ...
        if (pending->reply_serial == reply_serial && pending->will_get_reply == will_get_reply &&
        ...
        link = bus_expire_list_get_next_link(connections->pending_replies, link);
        if (pending->will_get_reply == will_get_reply) ++count;
    }
    ...
    if (count >= limit) {
        ...
        dbus_set_error(
            error, DBUS_ERROR_LIMITS_EXCEEDED, "The maximum number of pending replies per connection has been reached");
        return FALSE;
    }
    ...
    if (!bus_expire_list_add(connections->pending_replies, &pending->expire_item)) {
        BUS_SET_OOM(error);
        dbus_free(cprd);
        bus_pending_reply_free(pending);
        return FALSE;
    ...
    if (!bus_transaction_add_cancel_hook(transaction, cancel_pending_reply, cprd, cancel_pending_reply_data_free)) {
        BUS_SET_OOM(error);
    ...
    _dbus_get_monotonic_time(&pending->expire_item.added_tv_sec, &pending->expire_item.added_tv_usec);

    _dbus_verbose(
        "Added pending reply %p, replier %p receiver %p serial %u\n",
        pending,
        pending->will_send_reply,
        pending->will_get_reply,
        pending->reply_serial);
...
static void cancel_check_pending_reply(void* data) {
    CheckPendingReplyData* d = data;

    _dbus_verbose("d = %p\n", d);

    bus_expire_list_add_link(d->connections->pending_replies, d->link);
    d->link = NULL;
}

static void check_pending_reply_data_free(void* data) {
    CheckPendingReplyData* d = data;
    ...
    if (d->link != NULL) {
        ...
        _dbus_assert(!bus_expire_list_contains_item(d->connections->pending_replies, &pending->expire_item));
 ...
 */
dbus_bool_t bus_connections_check_reply(
    BusConnections* connections,
    BusTransaction* transaction,
    ...
    while (link != NULL) {
        ...
        if (pending->reply_serial == reply_serial && pending->will_get_reply == receiving_reply &&
    ...
    if (!bus_transaction_add_cancel_hook(
            transaction, cancel_check_pending_reply, cprd, check_pending_reply_data_free)) {
        BUS_SET_OOM(error);
        dbus_free(cprd);
        return FALSE;
    }
    ...
    bus_expire_list_unlink(connections->pending_replies, link);
...
static void message_to_send_free(DBusConnection* connection, MessageToSend* to_send) {
    if (to_send->message) dbus_message_unref(to_send->message);

    if (to_send->preallocated) dbus_connection_free_preallocated_send(connection, to_send->preallocated);

    dbus_free(to_send);
}

static void cancel_hook_cancel(void* element, void* data) {
    CancelHook* ch = element;

    _dbus_verbose("Running transaction cancel hook\n");

    if (ch->cancel_function) (*ch->cancel_function)(ch->data);
}

static void cancel_hook_free(void* element, void* data) {
    CancelHook* ch = element;

    if (ch->free_data_function) (*ch->free_data_function)(ch->data);

    dbus_free(ch);
}

static void free_cancel_hooks(BusTransaction* transaction) {
    _dbus_list_foreach(&transaction->cancel_hooks, cancel_hook_free, NULL);
 ...
 */
dbus_bool_t bus_transaction_capture(
    BusTransaction* transaction,
    ...
    if (connections->monitors == NULL) return TRUE;
    ...
    if (!bus_matchmaker_get_recipients(mm, connections, sender, addressed_recipient, message, &recipients)) goto out;
    ...
    for (link = _dbus_list_get_first_link(&recipients); link != NULL;
        ...
        if (!bus_transaction_send(transaction, recipient, message)) goto out;
    }
...
dbus_bool_t bus_transaction_capture_error_reply(
    BusTransaction*  transaction,
    DBusConnection*  addressed_recipient,
    ...
    reply = dbus_message_new_error(in_reply_to, error->name, error->message);

    if (reply == NULL) return FALSE;

    if (!dbus_message_set_sender(reply, DBUS_SERVICE_DBUS)) goto out;
...
dbus_bool_t bus_transaction_send_from_driver(
    BusTransaction* transaction,
    DBusConnection* connection,
    DBusMessage*    message) {
    ...
    _dbus_verbose(
        ...
        dbus_message_get_error_name(message) ? dbus_message_get_error_name(message) : "(no error name)");

    if (!dbus_message_set_sender(message, DBUS_SERVICE_DBUS)) return FALSE;

    if (bus_connection_is_active(connection)) {
        if (!dbus_message_set_destination(message, bus_connection_get_name(connection))) return FALSE;
    }
     ...
     */
    if (!bus_transaction_capture(transaction, NULL, connection, message)) return FALSE;
    ...
    if (!bus_context_check_security_policy(
            ...
            NULL,
            &error)) {
        if (!bus_transaction_capture_error_reply(transaction, connection, &error, message)) {
            bus_context_log(
                transaction->context,
                DBUS_SYSTEM_LOG_WARNING,
...
dbus_bool_t bus_transaction_send(BusTransaction* transaction, DBusConnection* connection, DBusMessage* message) {
    MessageToSend*     to_send;
    BusConnectionData* d;
    DBusList*          link;

    _dbus_verbose(
        "  trying to add %s interface=%s member=%s error=%s to transaction%s\n",
        ...
        dbus_connection_get_is_connected(connection) ? "" : " (disconnected)");

    _dbus_assert(dbus_message_get_sender(message) != NULL);

    if (!dbus_connection_get_is_connected(connection)) return TRUE; /* silently ignore disconnected connections */
    ...
    while (link != NULL) {
        ...
        if (m->transaction == transaction) break;
...
static void transaction_free(BusTransaction* transaction) {
...
static void connection_cancel_transaction(DBusConnection* connection, BusTransaction* transaction) {
    DBusList*          link;
    BusConnectionData* d;
    ...
    while (link != NULL) {
        ...
        if (m->transaction == transaction) {
            _dbus_list_remove_link(&d->transaction_messages, link);
...
void bus_transaction_cancel_and_free(BusTransaction* transaction) {
    ...
    _dbus_list_foreach(&transaction->cancel_hooks, cancel_hook_cancel, NULL);

    transaction_free(transaction);
}

static void connection_execute_transaction(DBusConnection* connection, BusTransaction* transaction) {
    DBusList*          link;
    BusConnectionData* d;
    ...
    while (link != NULL) {
        ...
        if (m->transaction == transaction) {
            ...
            dbus_connection_send_preallocated(connection, m->preallocated, m->message, NULL);
...
static void bus_connection_remove_transactions(DBusConnection* connection) {
    ...
    while ((to_send = _dbus_list_get_first(&d->transaction_messages))) {
        ...
        _dbus_list_remove(&to_send->transaction->connections, connection);
 ...
 */
dbus_bool_t bus_transaction_send_error_reply(
    BusTransaction*  transaction,
    DBusConnection*  connection,
    const DBusError* error,
    DBusMessage*     in_reply_to) {
...
dbus_bool_t bus_transaction_add_cancel_hook(
    BusTransaction*              transaction,
    BusTransactionCancelFunction cancel_function,
    void*                        data,
    DBusFreeFunction             free_data_function) {
...
static dbus_bool_t bcd_add_monitor_rules(BusConnectionData* d, DBusConnection* connection, DBusList** rules) {
    BusMatchmaker* mm = d->connections->monitor_matchmaker;
    DBusList*      iter;
...
static void bcd_drop_monitor_rules(BusConnectionData* d, DBusConnection* connection) {
    BusMatchmaker* mm = d->connections->monitor_matchmaker;

    if (mm != NULL) bus_matchmaker_disconnected(mm, connection);
}

dbus_bool_t bus_connection_be_monitor(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusList**      rules,
    DBusError*      error) {
    ...
    bus_context_log(
        ...
        "Connection %s (%s) became a monitor.",
        d->name,
