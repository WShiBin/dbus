...
static DBusConnection* bus_driver_get_owner_of_name(DBusConnection* connection, const char* name) {
...
BusDriverFound bus_driver_get_conn_helper(
    ...
    if (name_p != NULL) *name_p = name;

    if (strcmp(name, DBUS_SERVICE_DBUS) == 0) return BUS_DRIVER_FOUND_SELF;

    conn = bus_driver_get_owner_of_name(connection, name);

    if (conn == NULL) {
        dbus_set_error(
            error, DBUS_ERROR_NAME_HAS_NO_OWNER, "Could not get %s of name '%s': no such name", what_we_want, name);
        return BUS_DRIVER_FOUND_ERROR;
    }

    if (peer_conn_p != NULL) *peer_conn_p = conn;
 ...
 * to exist.
 */
static dbus_bool_t bus_driver_check_caller_is_privileged(
    DBusConnection* connection,
    ...
    if (!dbus_connection_get_windows_user(connection, &windows_sid)) {
        ...
        bus_context_log_and_set_error(
            ...
            error,
            DBUS_ERROR_ACCESS_DENIED,
            "rejected attempt to call %s by unknown uid",
            method);
        goto out;
    }
    ...
    if (!_dbus_windows_user_is_process_owner(windows_sid)) {
        ...
        bus_context_log_and_set_error(
            ...
            error,
            DBUS_ERROR_ACCESS_DENIED,
            "rejected attempt to call %s by uid %s",
            method,
            windows_sid);
        goto out;
    }
...
static dbus_bool_t bus_driver_send_welcome_message(
    DBusConnection* connection,
    DBusMessage*    hello_message,
    BusTransaction* transaction,
    DBusError*      error);

dbus_bool_t bus_driver_send_service_owner_changed(
    ...
    null_service = "";
    _dbus_verbose(
        "sending name owner changed: %s [%s -> %s]\n",
        service_name,
    ...
    if (!dbus_message_set_sender(message, DBUS_SERVICE_DBUS)) goto oom;
    ...
    if (!bus_transaction_capture(transaction, NULL, NULL, message)) goto oom;
...
dbus_bool_t bus_driver_send_service_lost(
    DBusConnection* connection,
    const char*     service_name,
    BusTransaction* transaction,
    DBusError*      error) {
...
dbus_bool_t bus_driver_send_service_acquired(
    DBusConnection* connection,
    const char*     service_name,
    BusTransaction* transaction,
    DBusError*      error) {
...
static dbus_bool_t create_unique_client_name(BusRegistry* registry, DBusString* str) {
    ...
    while (TRUE) {
        ...
        if (next_minor_number <= 0) {
            next_major_number += 1;
            next_minor_number = 0;
            if (next_major_number <= 0) _dbus_assert_not_reached("INT_MAX * INT_MAX clients were added");
        }
        ...
        if (!_dbus_string_append(str, ":")) return FALSE;

        if (!_dbus_string_append_int(str, next_major_number)) return FALSE;

        if (!_dbus_string_append(str, ".")) return FALSE;

        if (!_dbus_string_append_int(str, next_minor_number)) return FALSE;

        next_minor_number += 1;

        /* Check if a client with the name exists */
        if (bus_registry_lookup(registry, str) == NULL) break;
...
static dbus_bool_t bus_driver_handle_hello(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (!bus_driver_send_welcome_message(connection, message, transaction, error)) goto out_0;

    /* Create the service */
    service = bus_registry_ensure(registry, &unique_name, connection, 0, transaction, error);
    if (service == NULL) goto out_0;
...
static dbus_bool_t bus_driver_handle_list_services(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
...
static dbus_bool_t bus_driver_handle_list_activatable_services(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
...
static dbus_bool_t bus_driver_handle_acquire_service(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
...
static dbus_bool_t bus_driver_handle_release_service(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (!dbus_message_get_args(message, error, DBUS_TYPE_STRING, &name, DBUS_TYPE_INVALID)) return FALSE;
...
static dbus_bool_t bus_driver_handle_service_exists(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
...
static dbus_bool_t bus_driver_handle_activate_service(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
...
dbus_bool_t bus_driver_send_ack_reply(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (dbus_message_get_no_reply(message)) return TRUE;
...
static dbus_bool_t bus_driver_handle_update_activation_environment(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    while ((array_type = dbus_message_iter_get_arg_type(&dict_iter)) == DBUS_TYPE_DICT_ENTRY) {
        ...
        while ((key_type = dbus_message_iter_get_arg_type(&dict_entry_iter)) == DBUS_TYPE_STRING) {
            ...
            if (value_type != DBUS_TYPE_STRING) break;
        ...
        if (key_type != DBUS_TYPE_INVALID) break;

        dbus_message_iter_next(&dict_iter);
    }

    if (array_type != DBUS_TYPE_INVALID) goto out;
    ...
    if (bus_context_get_systemd_activation(bus_connection_get_context(connection))) {
        ...
        systemd_message = dbus_message_new_method_call(
     ...
     */
    if (key_link != NULL) {
        if (systemd_message != NULL) dbus_message_iter_abandon_container(&iter, &systemd_iter);
        goto out;
    }
    ...
    if (!bus_driver_send_ack_reply(connection, transaction, message, error)) goto out;

    retval = TRUE;

out:
    if (systemd_message != NULL) dbus_message_unref(systemd_message);
    _dbus_list_clear(&keys);
    _dbus_list_clear(&values);
    return retval;
}

static dbus_bool_t bus_driver_handle_add_match(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
...
failed:
    _DBUS_ASSERT_ERROR_IS_SET(error);
    if (rule) bus_match_rule_unref(rule);
    return FALSE;
}

static dbus_bool_t bus_driver_handle_remove_match(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
...
static dbus_bool_t bus_driver_handle_get_service_owner(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (!dbus_message_get_args(message, error, DBUS_TYPE_STRING, &text, DBUS_TYPE_INVALID)) goto failed;
    ...
    if (!dbus_message_append_args(reply, DBUS_TYPE_STRING, &base_name, DBUS_TYPE_INVALID)) goto oom;
...
static dbus_bool_t bus_driver_handle_list_queued_owners(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (service == NULL && _dbus_string_equal_c_str(&str, DBUS_SERVICE_DBUS)) {
        /* ORG_FREEDESKTOP_DBUS owns itself */
        if (!_dbus_list_append(&base_names, (char*)dbus_service_name)) goto oom;
    } else if (service == NULL) {
        dbus_set_error(error, DBUS_ERROR_NAME_HAS_NO_OWNER, "Could not get owners of name '%s': no such name", text);
        goto failed;
    } else {
        if (!bus_service_list_queued_owners(service, &base_names, error)) goto failed;
    }
    ...
    dbus_message_iter_init_append(reply, &iter);
    if (!dbus_message_iter_open_container(&iter, DBUS_TYPE_ARRAY, DBUS_TYPE_STRING_AS_STRING, &array_iter)) goto oom;
    ...
    while (link != NULL) {
        ...
        if (!dbus_message_iter_append_basic(&array_iter, DBUS_TYPE_STRING, &uname)) goto oom;

        link = _dbus_list_get_next_link(&base_names, link);
    }

    if (!dbus_message_iter_close_container(&iter, &array_iter)) goto oom;
...
failed:
    ...
    if (base_names) _dbus_list_clear(&base_names);
...
static dbus_bool_t bus_driver_handle_get_connection_unix_user(
    ...
    switch (found) {
        ...
        case BUS_DRIVER_FOUND_PEER:
            if (!dbus_connection_get_unix_user(conn, &uid)) uid = DBUS_UID_UNSET;
            break;
    ...
    uid32 = uid;
    if (!dbus_message_append_args(reply, DBUS_TYPE_UINT32, &uid32, DBUS_TYPE_INVALID)) goto oom;
...
static dbus_bool_t bus_driver_handle_get_connection_unix_process_id(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    switch (found) {
        case BUS_DRIVER_FOUND_SELF:
            ...
            break;
        case BUS_DRIVER_FOUND_PEER:
            if (!dbus_connection_get_unix_process_id(conn, &pid)) pid = DBUS_PID_UNSET;
            break;
        case BUS_DRIVER_FOUND_ERROR:
            /* fall through */
        default:
    ...
    pid32 = pid;
    if (!dbus_message_append_args(reply, DBUS_TYPE_UINT32, &pid32, DBUS_TYPE_INVALID)) goto oom;
...
static dbus_bool_t bus_driver_handle_get_adt_audit_session_data(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (found == BUS_DRIVER_FOUND_ERROR) goto failed;
    ...
    if (found != BUS_DRIVER_FOUND_PEER || !dbus_connection_get_adt_audit_session_data(conn, &data, &data_size) ||
        data == NULL) {
        dbus_set_error(
            error, DBUS_ERROR_ADT_AUDIT_DATA_UNKNOWN, "Could not determine audit session data for '%s'", service);
        goto failed;
    }
...
static dbus_bool_t bus_driver_handle_get_connection_selinux_security_context(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (!bus_selinux_append_context(reply, context, error)) goto failed;
...
static dbus_bool_t bus_driver_handle_get_connection_credentials(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    switch (found) {
        ...
        case BUS_DRIVER_FOUND_PEER:
            if (!dbus_connection_get_unix_process_id(conn, &ulong_pid)) ulong_pid = DBUS_PID_UNSET;
            if (!dbus_connection_get_unix_user(conn, &ulong_uid)) ulong_uid = DBUS_UID_UNSET;
            break;
        case BUS_DRIVER_FOUND_ERROR:
            /* fall through */
        default:
    ...
    if (found == BUS_DRIVER_FOUND_PEER && dbus_connection_get_windows_user(conn, &s)) {
        DBusString  str;
        dbus_bool_t result;
    ...
    if (found == BUS_DRIVER_FOUND_PEER && _dbus_connection_get_linux_security_label(conn, &s)) {
        if (s == NULL) goto oom;

        /* use the GVariant bytestring convention for strings of unknown
         * encoding: include the \0 in the payload, for zero-copy reading */
        if (!_dbus_asv_add_byte_array(&array_iter, "LinuxSecurityLabel", s, strlen(s) + 1)) {
            dbus_free(s);
            goto oom;
        }
    ...
    if (!_dbus_asv_close(&reply_iter, &array_iter)) goto oom;
...
static dbus_bool_t bus_driver_handle_reload_config(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    context = bus_connection_get_context(connection);
    if (!bus_context_reload_config(context, error)) goto failed;
...
static dbus_bool_t bus_driver_handle_enable_verbose(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
...
static dbus_bool_t bus_driver_handle_disable_verbose(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
...
static dbus_bool_t bus_driver_handle_get_id(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    context = bus_connection_get_context(connection);
    if (!bus_context_get_id(context, &uuid)) goto oom;

    reply = dbus_message_new_method_return(message);
    if (reply == NULL) goto oom;

    v_STRING = _dbus_string_get_const_data(&uuid);
    if (!dbus_message_append_args(reply, DBUS_TYPE_STRING, &v_STRING, DBUS_TYPE_INVALID)) goto oom;
...
static dbus_bool_t bus_driver_handle_become_monitor(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (!bus_apparmor_allows_eavesdropping(connection, bustype, error)) goto out;
    ...
    if (!bus_connection_be_monitor(connection, transaction, &rules, error)) goto out;
...
static dbus_bool_t bus_driver_handle_get_machine_id(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (!_dbus_get_local_machine_uuid_encoded(&uuid, error)) goto fail;
    ...
    if (!dbus_message_append_args(reply, DBUS_TYPE_STRING, &str, DBUS_TYPE_INVALID)) goto oom;
...
static dbus_bool_t bus_driver_handle_ping(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
...
static dbus_bool_t bus_driver_handle_get(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error);

static dbus_bool_t bus_driver_handle_get_all(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error);

static dbus_bool_t bus_driver_handle_set(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error);
...
typedef struct {
    ...
    const char* out_args;
    dbus_bool_t (
        *handler)(DBusConnection* connection, BusTransaction* transaction, DBusMessage* message, DBusError* error);
    MethodFlags flags;
} MessageHandler;
...
static const MessageHandler monitoring_message_handlers[] = {
    {"BecomeMonitor", "asu", "", bus_driver_handle_become_monitor, METHOD_FLAG_PRIVILEGED},
    {NULL, NULL, NULL, NULL}};

#ifdef DBUS_ENABLE_VERBOSE_MODE
static const MessageHandler verbose_message_handlers[] = {
    {"EnableVerbose", "", "", bus_driver_handle_enable_verbose, METHOD_FLAG_NONE},
    {"DisableVerbose", "", "", bus_driver_handle_disable_verbose, METHOD_FLAG_NONE},
    {NULL, NULL, NULL, NULL}};
#endif

#ifdef DBUS_ENABLE_STATS
static const MessageHandler stats_message_handlers[] = {
    {"GetStats", "", "a{sv}", bus_stats_handle_get_stats, METHOD_FLAG_NONE},
    {"GetConnectionStats", "s", "a{sv}", bus_stats_handle_get_connection_stats, METHOD_FLAG_NONE},
    {"GetAllMatchRules", "", "a{sas}", bus_stats_handle_get_all_match_rules, METHOD_FLAG_NONE},
    {NULL, NULL, NULL, NULL}};
#endif
...
static InterfaceHandler interface_handlers[] = {
    ...
    {DBUS_INTERFACE_INTROSPECTABLE,
     introspectable_message_handlers,
     NULL,
...
static dbus_bool_t write_args_for_direction(DBusString* xml, const char* signature, dbus_bool_t in) {
    ...
    while ((current_type = _dbus_type_reader_get_current_type(&typereader)) != DBUS_TYPE_INVALID) {
        ...
        if (!_dbus_string_append_printf(xml, "      <arg direction=\"%s\" type=\"", in ? "in" : "out")) goto oom;
        if (!_dbus_string_append_len(xml, _dbus_string_get_const_data(subsig) + start, len)) goto oom;
...
dbus_bool_t bus_driver_generate_introspect_string(
    ...
    if (!_dbus_string_append(xml, DBUS_INTROSPECT_1_0_XML_DOCTYPE_DECL_NODE)) return FALSE;
    if (!_dbus_string_append(xml, "<node>\n")) return FALSE;

    for (ih = interface_handlers; ih->name != NULL; ih++) {
        if (!(is_canonical_path || (ih->flags & INTERFACE_FLAG_ANY_PATH))) continue;

        if (!_dbus_string_append_printf(xml, "  <interface name=\"%s\">\n", ih->name)) return FALSE;

        for (mh = ih->message_handlers; mh->name != NULL; mh++) {
            if (!_dbus_string_append_printf(xml, "    <method name=\"%s\">\n", mh->name)) return FALSE;

            if (!write_args_for_direction(xml, mh->in_args, TRUE)) return FALSE;

            if (!write_args_for_direction(xml, mh->out_args, FALSE)) return FALSE;
        ...
        for (ph = ih->property_handlers; ph != NULL && ph->name != NULL; ph++) {
            ...
            if (!_dbus_string_append_printf(
                    xml, "    <property name=\"%s\" type=\"%s\" access=\"read\">\n", ph->name, ph->type))
                return FALSE;
        ...
        if (ih->extra_introspection != NULL && !_dbus_string_append(xml, ih->extra_introspection)) return FALSE;
...
static dbus_bool_t bus_driver_handle_introspect(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (!bus_driver_generate_introspect_string(&xml, is_canonical_path, message)) goto oom;
...
dbus_bool_t bus_driver_handle_message(
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (dbus_message_is_signal(message, "org.freedesktop.systemd1.Activator", "ActivationFailure")) {
        ...
        if (!bus_driver_check_caller_is_privileged(connection, transaction, message, error)) return FALSE;
    ...
    for (ih = interface_handlers; ih->name != NULL; ih++) {
        ...
        if (interface != NULL && strcmp(interface, ih->name) != 0) continue;
        ...
        for (mh = ih->message_handlers; mh->name != NULL; mh++) {
            ...
            if ((mh->flags & METHOD_FLAG_PRIVILEGED) &&
                !bus_driver_check_caller_is_privileged(connection, transaction, message, error)) {
                _DBUS_ASSERT_ERROR_IS_SET(error);
                return FALSE;
            }
    ...
    dbus_set_error(
        error,
        found_interface ? DBUS_ERROR_UNKNOWN_METHOD : DBUS_ERROR_UNKNOWN_INTERFACE,
...
static dbus_bool_t features_getter(BusContext* context, DBusMessageIter* variant_iter) {
    ...
    if (bus_apparmor_enabled()) {
        ...
        if (!dbus_message_iter_append_basic(&arr_iter, DBUS_TYPE_STRING, &s)) goto abandon;
    }
...
static dbus_bool_t interfaces_getter(BusContext* context, DBusMessageIter* variant_iter) {
    ...
    for (ih = interface_handlers; ih->name != NULL; ih++) {
        if (ih->flags & INTERFACE_FLAG_UNINTERESTING) continue;

        if (!dbus_message_iter_append_basic(&arr_iter, DBUS_TYPE_STRING, &ih->name)) goto abandon;
    }
...
static const InterfaceHandler* bus_driver_find_interface(
    const char* name,
    ...
    for (ih = interface_handlers; ih->name != NULL; ih++) {
        if (!(canonical_path || (ih->flags & INTERFACE_FLAG_ANY_PATH))) continue;

        if (strcmp(name, ih->name) == 0) return ih;
    }
...
static const PropertyHandler* interface_handler_find_property(
    const InterfaceHandler* ih,
    const char*             name,
    DBusError*              error) {
    const PropertyHandler* ph;
    ...
    for (ph = ih->property_handlers; ph != NULL && ph->name != NULL; ph++) {
        if (strcmp(name, ph->name) == 0) return ph;
    }
...
static dbus_bool_t bus_driver_handle_get(
    ...
    if (!dbus_message_iter_open_container(&iter, DBUS_TYPE_VARIANT, handler->type, &var_iter)) goto oom;
    ...
    if (!dbus_message_iter_close_container(&iter, &var_iter)) goto oom;
...
static dbus_bool_t bus_driver_handle_get_all(
    ...
    /* The message signature has already been checked for us,
     * so this should always succeed. */
    if (!dbus_message_get_args(message, error, DBUS_TYPE_STRING, &iface, DBUS_TYPE_INVALID)) return FALSE;
    ...
    for (ph = ih->property_handlers; ph != NULL && ph->name != NULL; ph++) {
        ...
        if (!_dbus_asv_open_entry(&array_iter, &entry_iter, ph->name, ph->type, &var_iter)) goto oom_abandon_message;
        ...
        if (!_dbus_asv_close_entry(&array_iter, &entry_iter, &var_iter)) goto oom_abandon_message;
    }
...
static dbus_bool_t bus_driver_handle_set(
    ...
    if (!dbus_message_iter_next(&iter)) _dbus_assert_not_reached("Message type was already checked to be 'ssv'");
