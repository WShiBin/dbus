...
static unsigned char valid[256] = {
    ...
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
    0x0, 0x3, 0x2, 0x0, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3,
    0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3,
    0x3, 0x3, 0x3, 0x0, 0x0, 0x0, 0x0, 0x2, 0x0, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3,
    0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x3, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
...
static void report_error(BusDesktopFileParser* parser, const char* message, const char* error_name, DBusError* error);

static void parser_free(BusDesktopFileParser* parser) {
    bus_desktop_file_free(parser->desktop_file);

    _dbus_string_free(&parser->data);
}

static void bus_desktop_file_line_free(BusDesktopFileLine* line) {
    dbus_free(line->key);
    dbus_free(line->value);
}

static void bus_desktop_file_section_free(BusDesktopFileSection* section) {
    int i;

    for (i = 0; i < section->n_lines; i++) bus_desktop_file_line_free(&section->lines[i]);

    dbus_free(section->lines);
    dbus_free(section->section_name);
}

void bus_desktop_file_free(BusDesktopFile* desktop_file) {
    int i;

    for (i = 0; i < desktop_file->n_sections; i++) bus_desktop_file_section_free(&desktop_file->sections[i]);
    dbus_free(desktop_file->sections);

    dbus_free(desktop_file);
}

static dbus_bool_t grow_lines_in_section(BusDesktopFileSection* section) {
    BusDesktopFileLine* lines;
    ...
    lines = dbus_realloc(section->lines, sizeof(BusDesktopFileLine) * new_n_lines);
...
static dbus_bool_t grow_sections(BusDesktopFile* desktop_file) {
    int                    new_n_sections;
    BusDesktopFileSection* sections;
    ...
    sections = dbus_realloc(desktop_file->sections, sizeof(BusDesktopFileSection) * new_n_sections);
...
static char* unescape_string(
    ...
    while (pos < end_pos) {
        if (_dbus_string_get_byte(str, pos) == 0) {
            ...
            dbus_free(retval);
            report_error(
                parser, "Text to be unescaped contains embedded nul", BUS_DESKTOP_PARSE_ERROR_INVALID_ESCAPES, error);
            return NULL;
        }
        ...
        if (_dbus_string_get_byte(str, pos) == '\\') {
            ...
            if (pos >= end_pos) {
                ...
                dbus_free(retval);
                report_error(
                    parser, "Text to be unescaped ended in \\", BUS_DESKTOP_PARSE_ERROR_INVALID_ESCAPES, error);
                return NULL;
            }
...
static BusDesktopFileSection* new_section(BusDesktopFile* desktop_file, const char* name) {
    int   n;
    char* name_copy;

    if (desktop_file->n_allocated_sections == desktop_file->n_sections) {
        if (!grow_sections(desktop_file)) return NULL;
    }
...
static BusDesktopFileSection* open_section(BusDesktopFileParser* parser, char* name) {
...
static BusDesktopFileLine* new_line(BusDesktopFileParser* parser) {
    BusDesktopFileSection* section;
    ...
    if (section->n_allocated_lines == section->n_lines) {
        if (!grow_lines_in_section(section)) return NULL;
    }
...
static dbus_bool_t is_blank_line(BusDesktopFileParser* parser) {
    int  p;
    char c;
...
static void parse_comment_or_blank(BusDesktopFileParser* parser) {
    int line_end, eol_len;

    if (!_dbus_string_find_eol(&parser->data, parser->pos, &line_end, &eol_len)) line_end = parser->len;
...
static dbus_bool_t is_valid_section_name(const char* name) {
    ...
    while (*name) {
        if (*name <= 0x1f || *name >= 0x7f || *name == '[' || *name == ']') return FALSE;
...
static dbus_bool_t parse_section_start(BusDesktopFileParser* parser, DBusError* error) {
    int   line_end, eol_len;
    char* section_name;
    ...
    if (line_end - parser->pos <= 2 || _dbus_string_get_byte(&parser->data, line_end - 1) != ']') {
        report_error(parser, "Invalid syntax for section header", BUS_DESKTOP_PARSE_ERROR_INVALID_SYNTAX, error);
        parser_free(parser);
        return FALSE;
    }

    section_name = unescape_string(parser, &parser->data, parser->pos + 1, line_end - 1, error);
...
static dbus_bool_t parse_key_value(BusDesktopFileParser* parser, DBusError* error) {
    int                 line_end, eol_len;
    ...
    p         = parser->pos;
    key_start = p;
    while (p < line_end && (valid[_dbus_string_get_byte(&parser->data, p)] & VALID_KEY_CHAR)) p++;
    key_end = p;
    ...
    /* Skip space before '=' */
    while (p < line_end && _dbus_string_get_byte(&parser->data, p) == ' ') p++;
    ...
    if (!_dbus_string_copy_len(&parser->data, key_start, key_end - key_start, &key, 0)) {
...
static void report_error(BusDesktopFileParser* parser, const char* message, const char* error_name, DBusError* error) {
    ...
    if (section_name)
        dbus_set_error(
            error, error_name, "Error in section %s at line %d: %s\n", section_name, parser->line_num, message);
    else
        dbus_set_error(error, error_name, "Error at line %d: %s\n", parser->line_num, message);
}
...
BusDesktopFile* bus_desktop_file_load(DBusString* filename, DBusError* error) {
     ...
     */
    if (!_dbus_stat(filename, &sb, error)) return NULL;

    if (sb.size > _DBUS_ONE_KILOBYTE * 128) {
        dbus_set_error(error, DBUS_ERROR_FAILED, "Desktop file size (%ld bytes) is too large", (long)sb.size);
        return NULL;
    }
    ...
    if (!_dbus_string_validate_utf8(&str, 0, _dbus_string_get_length(&str))) {
        _dbus_string_free(&str);
        dbus_set_error(error, DBUS_ERROR_FAILED, "invalid UTF-8");
        return NULL;
    }
    ...
    while (parser.pos < parser.len) {
        ...
        } else if (is_blank_line(&parser) || _dbus_string_get_byte(&parser.data, parser.pos) == '#')
            parse_comment_or_blank(&parser);
        else if (parser.current_section < 0) {
            dbus_set_error(error, DBUS_ERROR_FAILED, "invalid service file: key=value before [Section]");
            return NULL;
        } else {
...
static BusDesktopFileSection* lookup_section(BusDesktopFile* desktop_file, const char* section_name) {
    BusDesktopFileSection* section;
    int                    i;
    ...
    for (i = 0; i < desktop_file->n_sections; i++) {
        ...
        if (strcmp(section->section_name, section_name) == 0) return section;
    }

    return NULL;
}

static BusDesktopFileLine* lookup_line(
    ...
    for (i = 0; i < section->n_lines; i++) {
        ...
        if (strcmp(line->key, keyname) == 0) return line;
    }

    return NULL;
}

dbus_bool_t bus_desktop_file_get_raw(
    BusDesktopFile* desktop_file,
    ...
    *val = NULL;
    ...
    line = lookup_line(desktop_file, section, keyname);
...
dbus_bool_t bus_desktop_file_get_string(
    BusDesktopFile* desktop_file,
    const char*     section,
    const char*     keyname,
    char**          val,
    ...
    *val = NULL;
    ...
    if (!bus_desktop_file_get_raw(desktop_file, section, keyname, &raw)) {
        dbus_set_error(error, DBUS_ERROR_FAILED, "No \"%s\" key in .service file\n", keyname);
        return FALSE;
    }
