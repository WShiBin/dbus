...
static dbus_bool_t setup_server(BusContext* context, DBusServer* server, char** auth_mechanisms, DBusError* error) {
    ...
    if (!dbus_server_set_watch_functions(
            server, add_server_watch, remove_server_watch, toggle_server_watch, server, NULL)) {
        BUS_SET_OOM(error);
        return FALSE;
    }
 ...
 */
static dbus_bool_t process_config_first_time_only(
    BusContext*       context,
...
BusContext* bus_context_new(
    ...
    /* Now become a daemon if appropriate and write out pid file in any case */
    {
        ...
        if (((flags & BUS_CONTEXT_FLAG_FORK_NEVER) == 0 && context->fork) || (flags & BUS_CONTEXT_FLAG_FORK_ALWAYS)) {
             ...
             */
            if (!_dbus_write_pid_to_file_and_pipe(
                    context->pidfile ? &u : NULL, print_pid_pipe, _dbus_getpid(), error)) {
                _DBUS_ASSERT_ERROR_IS_SET(error);
                goto failed;
            }
        }
    ...
    if (print_pid_pipe && _dbus_pipe_is_valid(print_pid_pipe) && !_dbus_pipe_is_stdout_or_stderr(print_pid_pipe))
...
void bus_context_log_and_set_error(
    BusContext*           context,
 ...
 */
static void complain_about_message(
    BusContext*     context,
 ...
 */
dbus_bool_t bus_context_check_security_policy(
    BusContext*         context,
    ...
    _dbus_assert(
        dest != NULL || type == DBUS_MESSAGE_TYPE_SIGNAL ||
        (sender == NULL && !bus_connection_is_active(proposed_recipient)));
    _dbus_assert(
        type == DBUS_MESSAGE_TYPE_SIGNAL || addressed_recipient != NULL || activation_entry != NULL ||
    ...
    if (sender != NULL) {
        if (bus_connection_is_active(sender)) {
            ...
            if (dbus_message_get_reply_serial(message) != 0) {
                if (proposed_recipient != NULL /* not to the bus driver */ &&
                    ...
                    requested_reply = bus_connections_check_reply(
                        bus_connection_get_connections(sender),
                        transaction,
                        sender,
                        addressed_recipient,
                        message,
        ...
        if (!bus_connection_is_active(sender)) {
             ...
             */
            if (proposed_recipient == NULL && dbus_message_is_method_call(message, DBUS_INTERFACE_DBUS, "Hello")) {
    ...
    if (proposed_recipient != NULL) {
        ...
        if (bus_connection_is_active(proposed_recipient)) {
            ...
            _dbus_assert_not_reached(
                "a message was somehow sent to an inactive recipient from a source other than the message bus");
            recipient_policy = NULL;
        }
    } else
        recipient_policy = NULL;
    ...
    if (sender_policy &&
        !bus_client_policy_check_can_send(
            sender_policy, context->registry, requested_reply, proposed_recipient, message, &toggles, &log)) {
        complain_about_message(
            context,
            DBUS_ERROR_ACCESS_DENIED,
     ...
     */
    if (type == DBUS_MESSAGE_TYPE_METHOD_CALL && sender && addressed_recipient &&
        addressed_recipient == proposed_recipient && /* not eavesdropping */
        !bus_connections_expect_reply(
            bus_connection_get_connections(sender), transaction, sender, addressed_recipient, message, error)) {
