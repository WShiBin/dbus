...
static void die(const char* failure) {
    fprintf(stderr, "Unit test failed: %s\n", failure);
    exit(1);
}

static void check_memleaks(const char* name) {
    dbus_shutdown();

    printf("%s: checking for memleaks\n", name);
    if (_dbus_get_malloc_blocks_outstanding() != 0) {
        _dbus_warn("%d dbus_malloc blocks were not freed", _dbus_get_malloc_blocks_outstanding());
        die("memleaks");
    }
}
...
int main(int argc, char** argv) {
    ...
    if (!_dbus_threads_init_debug()) die("initializing debug threads");

    if (only == NULL || strcmp(only, "expire-list") == 0) {
        test_pre_hook();
        printf("%s: Running expire list test\n", argv[0]);
        if (!bus_expire_list_test(&test_data_dir)) die("expire list");
        test_post_hook();
    }
    ...
    if (only == NULL || strcmp(only, "config-parser") == 0) {
        ...
        if (!bus_config_parser_test(&test_data_dir)) die("parser");
        test_post_hook();
    }
    ...
    if (only == NULL || strcmp(only, "signals") == 0) {
        ...
        if (!bus_signals_test(&test_data_dir)) die("signals");
        test_post_hook();
    }
    ...
    if (only == NULL || strcmp(only, "dispatch-sha1") == 0) {
        ...
        if (!bus_dispatch_sha1_test(&test_data_dir)) die("sha1");
        test_post_hook();
    }
    ...
    if (only == NULL || strcmp(only, "dispatch") == 0) {
        ...
        if (!bus_dispatch_test(&test_data_dir)) die("dispatch");
        test_post_hook();
    }
    ...
    if (only == NULL || strcmp(only, "activation-service-reload") == 0) {
        ...
        if (!bus_activation_service_reload_test(&test_data_dir)) die("service reload");
        test_post_hook();
    }
    ...
    if (only == NULL || strcmp(only, "unix-fds-passing") == 0) {
        ...
        if (!bus_unix_fds_passing_test(&test_data_dir)) die("unix fd passing");
        test_post_hook();
    }
#endif
