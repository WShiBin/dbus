...
BusRegistry* bus_registry_new(BusContext* context) {
    ...
    registry->service_hash = _dbus_hash_table_new(DBUS_HASH_STRING, NULL, NULL);
    if (registry->service_hash == NULL) goto failed;

    registry->service_pool = _dbus_mem_pool_new(sizeof(BusService), TRUE);

    if (registry->service_pool == NULL) goto failed;

    registry->owner_pool = _dbus_mem_pool_new(sizeof(BusOwner), TRUE);

    if (registry->owner_pool == NULL) goto failed;
...
void bus_registry_unref(BusRegistry* registry) {
    ...
    if (registry->refcount == 0) {
        if (registry->service_hash) _dbus_hash_table_unref(registry->service_hash);
        if (registry->service_pool) _dbus_mem_pool_free(registry->service_pool);
        if (registry->owner_pool) _dbus_mem_pool_free(registry->owner_pool);
        if (registry->service_sid_table) _dbus_hash_table_unref(registry->service_sid_table);

        dbus_free(registry);
    }
}

BusService* bus_registry_lookup(BusRegistry* registry, const DBusString* service_name) {
    BusService* service;

    service = _dbus_hash_table_lookup_string(registry->service_hash, _dbus_string_get_const_data(service_name));

    return service;
}

static DBusList* _bus_service_find_owner_link(BusService* service, DBusConnection* connection) {
    DBusList* link;
    ...
    while (link != NULL) {
        ...
        if (bus_owner->conn == connection) break;
...
static void bus_owner_set_flags(BusOwner* owner, dbus_uint32_t flags) {
    owner->allow_replacement = (flags & DBUS_NAME_FLAG_ALLOW_REPLACEMENT) != FALSE;

    owner->do_not_queue = (flags & DBUS_NAME_FLAG_DO_NOT_QUEUE) != FALSE;
}

static BusOwner* bus_owner_new(BusService* service, DBusConnection* conn, dbus_uint32_t flags) {
    BusOwner* result;
...
static BusOwner* bus_owner_ref(BusOwner* owner) {
...
static void bus_owner_unref(BusOwner* owner) {
    _dbus_assert(owner->refcount > 0);
    owner->refcount -= 1;
...
BusService* bus_registry_ensure(
    BusRegistry*      registry,
    ...
    _dbus_verbose(
        "copied string %p '%s' to '%s'\n", service_name, _dbus_string_get_const_data(service_name), service->name);

    if (!bus_driver_send_service_owner_changed(
            service->name, NULL, bus_connection_get_name(owner_connection_if_created), transaction, error)) {
        bus_service_unref(service);
        return NULL;
    }

    if (!bus_activation_service_created(
            bus_context_get_activation(registry->context), service->name, transaction, error)) {
        bus_service_unref(service);
        return NULL;
    }

    if (!bus_service_add_owner(service, owner_connection_if_created, flags, transaction, error)) {
        bus_service_unref(service);
        return NULL;
    }

    if (!_dbus_hash_table_insert_string(registry->service_hash, service->name, service)) {
...
void bus_registry_foreach(BusRegistry* registry, BusServiceForeachFunction function, void* data) {
    DBusHashIter iter;
...
dbus_bool_t bus_registry_list_services(BusRegistry* registry, char*** listp, int* array_len) {
    int          i, j, len;
    char**       retval;
    DBusHashIter iter;
...
dbus_bool_t bus_registry_acquire_service(
    ...
    if (!_dbus_validate_bus_name(service_name, 0, _dbus_string_get_length(service_name))) {
        dbus_set_error(
            error,
    ...
    if (_dbus_string_get_byte(service_name, 0) == ':') {
        ...
        _dbus_verbose("Attempt to acquire invalid base service name \"%s\"", _dbus_string_get_const_data(service_name));
     ...
     */
    sid = bus_selinux_id_table_lookup(registry->service_sid_table, service_name);

    if (!bus_selinux_allows_acquire_service(connection, sid, _dbus_string_get_const_data(service_name), error)) {
    ...
    if (!bus_apparmor_allows_acquire_service(
            connection, bus_context_get_type(registry->context), _dbus_string_get_const_data(service_name), error))
        goto out;
    ...
    if (bus_connection_get_n_services_owned(connection) >= limit) {
        ...
        dbus_set_error(
            ...
            bus_connection_is_active(connection) ? bus_connection_get_name(connection) : "(inactive)",
            limit);
        bus_context_log(registry->context, DBUS_SYSTEM_LOG_WARNING, "%s", tmp_error.message);
    ...
    if (service == NULL) {
        service = bus_registry_ensure(registry, service_name, connection, flags, transaction, error);
        if (service == NULL) goto out;
    }

    primary_owner = bus_service_get_primary_owner(service);
    if (primary_owner == NULL) goto out;
    ...
    if (old_owner_conn == NULL) {
        ...
        *result = DBUS_REQUEST_NAME_REPLY_ALREADY_OWNER;
        ...
        ((flags & DBUS_NAME_FLAG_DO_NOT_QUEUE) && !(bus_service_get_allow_replacement(service))) ||
        ((flags & DBUS_NAME_FLAG_DO_NOT_QUEUE) && !(flags & DBUS_NAME_FLAG_REPLACE_EXISTING))) {
        DBusList* link;
        BusOwner* temp_owner;
        ...
        *result = DBUS_REQUEST_NAME_REPLY_EXISTS;
        ...
        (!(flags & DBUS_NAME_FLAG_REPLACE_EXISTING) || !(bus_service_get_allow_replacement(service)))) {
        /* Queue the connection */
        if (!bus_service_add_owner(service, connection, flags, transaction, error)) goto out;
        ...
        *result = DBUS_REQUEST_NAME_REPLY_IN_QUEUE;
        ...
        if (primary_owner->do_not_queue) {
            if (!bus_service_remove_owner(service, old_owner_conn, transaction, error)) goto out;
        } else {
            if (!bus_service_swap_owner(service, old_owner_conn, transaction, error)) goto out;
        }
    ...
    retval     = bus_activation_send_pending_auto_activation_messages(activation, service, transaction);
    if (!retval) BUS_SET_OOM(error);

out:
    return retval;
}

dbus_bool_t bus_registry_release_service(
    BusRegistry*      registry,
    DBusConnection*   connection,
    const DBusString* service_name,
    dbus_uint32_t*    result,
    ...
    if (_dbus_string_get_byte(service_name, 0) == ':') {
        ...
        _dbus_verbose("Attempt to release invalid base service name \"%s\"", _dbus_string_get_const_data(service_name));
    ...
    if (_dbus_string_equal_c_str(service_name, DBUS_SERVICE_DBUS)) {
        ...
        _dbus_verbose("Attempt to release service name \"%s\"", DBUS_SERVICE_DBUS);
    ...
    if (service == NULL) {
        ...
        *result = DBUS_RELEASE_NAME_REPLY_NOT_OWNER;
        ...
        if (!bus_service_remove_owner(service, connection, transaction, error)) goto out;
...
dbus_bool_t bus_registry_set_service_context_table(BusRegistry* registry, DBusHashTable* table) {
    ...
    while (_dbus_hash_iter_next(&iter)) {
        ...
        if (!bus_selinux_id_table_insert(new_table, service, context)) return FALSE;
    }
...
static void bus_service_unlink_owner(BusService* service, BusOwner* owner) {
    _dbus_list_remove_last(&service->owners, owner);
    bus_owner_unref(owner);
}

static void bus_service_unlink(BusService* service) {
...
static void bus_service_relink(BusService* service, DBusPreallocatedHash* preallocated) {
    _dbus_assert(service->owners == NULL);
    _dbus_assert(preallocated != NULL);

    _dbus_hash_table_insert_string_preallocated(service->registry->service_hash, preallocated, service->name, service);
...
static void cancel_ownership(void* data) {
    ...
    if (d->service->owners == NULL) bus_service_unlink(d->service);
}

static void free_ownership_cancel_data(void* data) {
    OwnershipCancelData* d = data;
...
static dbus_bool_t add_cancel_ownership_to_transaction(
    ...
    if (!bus_transaction_add_cancel_hook(transaction, cancel_ownership, d, free_ownership_cancel_data)) {
        dbus_free(d);
        return FALSE;
    }
...
dbus_bool_t bus_service_add_owner(
    BusService*     service,
    DBusConnection* connection,
    dbus_uint32_t   flags,
    BusTransaction* transaction,
    ...
    if (service->owners == NULL) {
        if (!bus_driver_send_service_acquired(connection, service->name, transaction, error)) return FALSE;
    }
    ...
    if (bus_owner_link == NULL) {
        ...
        if (!(flags & DBUS_NAME_FLAG_REPLACE_EXISTING) || service->owners == NULL) {
            if (!_dbus_list_append(&service->owners, bus_owner)) {
                bus_owner_unref(bus_owner);
                BUS_SET_OOM(error);
                return FALSE;
            }
        } else {
            if (!_dbus_list_insert_after(&service->owners, _dbus_list_get_first_link(&service->owners), bus_owner)) {
                bus_owner_unref(bus_owner);
                BUS_SET_OOM(error);
                return FALSE;
            }
    ...
    if (!add_cancel_ownership_to_transaction(transaction, service, bus_owner)) {
...
static void restore_ownership(void* data) {
    ...
    while (link != NULL) {
        if (link->data == d->before_owner) break;
...
static void free_ownership_restore_data(void* data) {
    OwnershipRestoreData* d = data;

    if (d->service_link) _dbus_list_free_link(d->service_link);
    if (d->owner_link) _dbus_list_free_link(d->owner_link);
    if (d->hash_entry) _dbus_hash_table_free_preallocated_entry(d->service->registry->service_hash, d->hash_entry);
...
static dbus_bool_t add_restore_ownership_to_transaction(
    ...
    while (link != NULL) {
        if (link->data == owner) {
            ...
            if (link) d->before_owner = link->data;
    ...
    if (d->service_link == NULL || d->owner_link == NULL || d->hash_entry == NULL ||
        !bus_transaction_add_cancel_hook(transaction, restore_ownership, d, free_ownership_restore_data)) {
...
dbus_bool_t bus_service_swap_owner(
    BusService*     service,
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusError*      error) {
    ...
    if (!bus_driver_send_service_lost(connection, service->name, transaction, error)) return FALSE;
    ...
    if (service->owners == NULL) {
        ...
        if (!bus_driver_send_service_acquired(new_owner_conn, service->name, transaction, error)) return FALSE;
    }
    ...
    _dbus_list_insert_after_link(&service->owners, _dbus_list_get_first_link(&service->owners), swap_link);

    return TRUE;
}

/* this function is self-cancelling if you cancel the transaction */
dbus_bool_t bus_service_remove_owner(
    BusService*     service,
    DBusConnection* connection,
    BusTransaction* transaction,
    DBusError*      error) {
    ...
    if (service->owners == NULL) {
        ...
        if (!bus_driver_send_service_owner_changed(
                service->name, bus_connection_get_name(connection), NULL, transaction, error))
            return FALSE;
    } else {
        DBusList*       link;
        BusOwner*       new_owner;
    ...
    if (service->owners == NULL) bus_service_unlink(service);
...
dbus_bool_t bus_service_has_owner(BusService* service, DBusConnection* connection) {
    DBusList* link;
...
dbus_bool_t bus_service_list_queued_owners(BusService* service, DBusList** return_list, DBusError* error) {
    DBusList* link;
    ...
    while (link != NULL) {
        ...
        if (!_dbus_list_append(return_list, (char*)uname)) goto oom;
