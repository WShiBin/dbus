...
typedef enum { APPARMOR_DISABLED, APPARMOR_ENABLED, APPARMOR_REQUIRED } AppArmorConfigMode;
...
static dbus_bool_t _bus_apparmor_detect_aa_dbus_support(dbus_bool_t* is_supported) {
    ...
    *is_supported = FALSE;
     ...
     */
    if (!_dbus_string_append(&aa_dbus, aa_securityfs) || !_dbus_string_append(&aa_dbus, "/features/dbus/mask"))
        goto out;
...
static void log_message(dbus_bool_t allow, const char* op, DBusString* data) {
    ...
    if (audit_fd >= 0) {
        ...
        audit_log_user_avc_message(
            audit_fd, AUDIT_USER_AVC, _dbus_string_get_const_data(&avc), NULL, NULL, NULL, getuid());
        _dbus_string_free(&avc);
        return;
    }
...
static dbus_bool_t _dbus_append_pair_uint(DBusString* auxdata, const char* name, unsigned long value) {
    return _dbus_string_append(auxdata, " ") && _dbus_string_append(auxdata, name) &&
           _dbus_string_append(auxdata, "=") && _dbus_string_append_uint(auxdata, value);
}

static dbus_bool_t _dbus_append_pair_str(DBusString* auxdata, const char* name, const char* value) {
    return _dbus_string_append(auxdata, " ") && _dbus_string_append(auxdata, name) &&
           _dbus_string_append(auxdata, "=\"") && _dbus_string_append(auxdata, value) &&
...
static dbus_bool_t is_unconfined(const char* con, const char* mode) {
    /* treat con == NULL as confined as it is going to result in a denial */
    if ((!mode && con && strcmp(con, "unconfined") == 0) || strcmp(mode, "unconfined") == 0) {
        return TRUE;
    }
...
static dbus_bool_t build_common_query(DBusString* query, const char* con, const char* bustype) {
     ...
     */
    return _dbus_string_insert_bytes(query, 0, AA_QUERY_CMD_LABEL_SIZE, 0) && _dbus_string_append(query, con) &&
           _dbus_string_append_byte(query, '\0') && _dbus_string_append_byte(query, AA_CLASS_DBUS) &&
...
static dbus_bool_t build_message_query(
    DBusString* query,
    const char* src_con,
    const char* bustype,
    const char* name,
    ...
    const char* member) {
    return build_common_query(query, src_con, bustype) && query_append(query, name) && query_append(query, dst_con) &&
           query_append(query, path) && query_append(query, interface) && query_append(query, member);
}
...
static void set_error_from_query_errno(DBusError* error, int error_number) {
    dbus_set_error(
        error,
        _dbus_error_from_errno(error_number),
        "Failed to query AppArmor policy: %s",
        _dbus_strerror(error_number));
}

static void set_error_from_denied_message(
    DBusError*      error,
    ...
    if (apparmor_enabled) {
        ...
        if (bus_con == NULL) {
            if (aa_getcon(&label, &mode) == -1) {
                dbus_set_error(
                    error, DBUS_ERROR_FAILED, "Error getting AppArmor context of bus: %s", _dbus_strerror(errno));
                return FALSE;
            }
 ...
 */
dbus_bool_t bus_apparmor_allows_acquire_service(
 ...
 */
dbus_bool_t bus_apparmor_allows_send(
    DBusConnection*     sender,
    DBusConnection*     proposed_recipient,
    dbus_bool_t         requested_reply,
    const char*         bustype,
    ...
    if (is_unconfined(src_con->label, src_con->mode)) {
        ...
        res = aa_query_label(
            src_perm, _dbus_string_get_data(&qstr), _dbus_string_get_length(&qstr), &src_allow, &src_audit);
        _dbus_string_free(&qstr);
        if (res == -1) {
            src_errno = errno;
    ...
    if (activation_entry != NULL || is_unconfined(dst_label, dst_mode)) {
        ...
        res = aa_query_label(
            dst_perm, _dbus_string_get_data(&qstr), _dbus_string_get_length(&qstr), &dst_allow, &dst_audit);
        _dbus_string_free(&qstr);
        if (res == -1) {
            dst_errno = errno;
    ...
    if ((!src_audit && !dst_audit) ||
        (msgtype == DBUS_MESSAGE_TYPE_METHOD_RETURN || msgtype == DBUS_MESSAGE_TYPE_ERROR))
        goto out;
...
audit:
    ...
    if (src_audit) {
        ...
        if (proposed_recipient && dbus_connection_get_unix_process_id(proposed_recipient, &pid) &&
...
dbus_bool_t bus_apparmor_allows_eavesdropping(DBusConnection* connection, const char* bustype, DBusError* error) {
    ...
    res =
        aa_query_label(AA_DBUS_EAVESDROP, _dbus_string_get_data(&qstr), _dbus_string_get_length(&qstr), &allow, &audit);
    _dbus_string_free(&qstr);
    if (res == -1) {
        serrno = errno;
        set_error_from_query_errno(error, serrno);
