...
static dbus_bool_t process_content(ExpatParseContext* context) {
    if (context->failed) return FALSE;

    if (_dbus_string_get_length(&context->content) > 0) {
        if (!bus_config_parser_content(context->parser, &context->content, context->error)) {
            context->failed = TRUE;
            return FALSE;
        }
...
static void expat_StartElementHandler(void* userData, const XML_Char* name, const XML_Char** atts) {
    ...
    if (!process_content(context)) return;
    ...
    if (!bus_config_parser_start_element(
            context->parser, name, (const char**)names, (const char**)values, context->error)) {
        dbus_free(names);
        dbus_free(values);
        context->failed = TRUE;
        return;
...
static void expat_EndElementHandler(void* userData, const XML_Char* name) {
    ...
    if (!bus_config_parser_end_element(context->parser, name, context->error)) {
        context->failed = TRUE;
        return;
    }
}
...
static void expat_CharacterDataHandler(void* userData, const XML_Char* s, int len) {
    ...
    if (!_dbus_string_append_len(&context->content, s, len)) {
        dbus_set_error(context->error, DBUS_ERROR_NO_MEMORY, NULL);
        context->failed = TRUE;
        return;
    }
...
BusConfigParser* bus_config_load(
    ...
    XML_SetElementHandler(expat, expat_StartElementHandler, expat_EndElementHandler);
    XML_SetCharacterDataHandler(expat, expat_CharacterDataHandler);

    {
        ...
        if (XML_Parse(expat, data_str, _dbus_string_get_length(&data), TRUE) == XML_STATUS_ERROR) {
            if (context.error != NULL && !dbus_error_is_set(context.error)) {
                enum XML_Error e;
    ...
    if (!bus_config_parser_finished(parser, error)) goto failed;
...
failed:
    ...
    if (parser) bus_config_parser_unref(parser);
    return NULL;
}
