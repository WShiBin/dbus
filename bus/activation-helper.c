...
static BusDesktopFile* desktop_file_for_name(BusConfigParser* parser, const char* name, DBusError* error) {
    ...
    for (link = _dbus_list_get_first_link(service_dirs); link != NULL;
    ...
    if (desktop_file == NULL) {
        dbus_set_error(
            error, DBUS_ERROR_SPAWN_SERVICE_NOT_FOUND, "The name %s was not provided by any .service files", name);
    }
...
static dbus_bool_t check_service_name(BusDesktopFile* desktop_file, const char* service_name, DBusError* error) {
    ...
    /* try to get Name */
    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION, DBUS_SERVICE_NAME, &name_tmp, error))
        goto failed;
...
static dbus_bool_t get_parameters_for_service(
    BusDesktopFile* desktop_file,
