...
dbus_bool_t bus_stats_handle_get_stats(
    ...
    if (!_dbus_asv_add_uint32(&arr_iter, "ActiveConnections", bus_connections_get_n_active(connections)) ||
        !_dbus_asv_add_uint32(&arr_iter, "IncompleteConnections", bus_connections_get_n_incomplete(connections)) ||
        !_dbus_asv_add_uint32(&arr_iter, "MatchRules", bus_connections_get_total_match_rules(connections)) ||
        !_dbus_asv_add_uint32(&arr_iter, "PeakMatchRules", bus_connections_get_peak_match_rules(connections)) ||
        !_dbus_asv_add_uint32(
            &arr_iter, "PeakMatchRulesPerConnection", bus_connections_get_peak_match_rules_per_conn(connections)) ||
        !_dbus_asv_add_uint32(&arr_iter, "BusNames", bus_connections_get_total_bus_names(connections)) ||
        !_dbus_asv_add_uint32(&arr_iter, "PeakBusNames", bus_connections_get_peak_bus_names(connections)) ||
        !_dbus_asv_add_uint32(
            &arr_iter, "PeakBusNamesPerConnection", bus_connections_get_peak_bus_names_per_conn(connections))) {
        _dbus_asv_abandon(&iter, &arr_iter);
        goto oom;
    }
    ...
    if (!_dbus_asv_close(&iter, &arr_iter)) goto oom;
...
dbus_bool_t bus_stats_handle_get_connection_stats(
    DBusConnection* caller_connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (!_dbus_asv_add_uint32(&arr_iter, "Serial", stats_serial++) ||
        !_dbus_asv_add_uint32(&arr_iter, "MatchRules", bus_connection_get_n_match_rules(stats_connection)) ||
        !_dbus_asv_add_uint32(&arr_iter, "PeakMatchRules", bus_connection_get_peak_match_rules(stats_connection)) ||
        !_dbus_asv_add_uint32(&arr_iter, "BusNames", bus_connection_get_n_services_owned(stats_connection)) ||
        !_dbus_asv_add_uint32(&arr_iter, "PeakBusNames", bus_connection_get_peak_bus_names(stats_connection)) ||
        !_dbus_asv_add_string(&arr_iter, "UniqueName", bus_connection_get_name(stats_connection))) {
        _dbus_asv_abandon(&iter, &arr_iter);
        goto oom;
    }
    ...
    if (!bus_transaction_send_from_driver(transaction, caller_connection, reply)) goto oom;
...
dbus_bool_t bus_stats_handle_get_all_match_rules(
    DBusConnection* caller_connection,
    BusTransaction* transaction,
    DBusMessage*    message,
    DBusError*      error) {
    ...
    if (!bus_registry_list_services(registry, &services, &services_len)) return FALSE;
    ...
    if (!dbus_message_iter_open_container(&iter, DBUS_TYPE_ARRAY, "{sas}", &hash_iter)) goto oom;
    ...
    for (i = 0; i < services_len; i++) {
        ...
        if (!dbus_message_iter_open_container(&hash_iter, DBUS_TYPE_DICT_ENTRY, NULL, &entry_iter)) {
        ...
        if (!dbus_message_iter_open_container(&entry_iter, DBUS_TYPE_ARRAY, "s", &arr_iter)) {
            dbus_message_iter_abandon_container(&hash_iter, &entry_iter);
            dbus_message_iter_abandon_container(&iter, &hash_iter);
            goto oom;
        }
    ...
    if (!dbus_message_iter_close_container(&iter, &hash_iter)) goto oom;
