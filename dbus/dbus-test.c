...
static void run_test(const char* test_name, const char* specific_test, TestFunc test) {
    if (!specific_test || strcmp(specific_test, test_name) == 0) {
        ...
        if (!test()) die(test_name);

        check_memleaks();
    }
}

static void run_data_test(
    ...
    if (!specific_test || strcmp(specific_test, test_name) == 0) {
        printf("%s: running %s tests\n", "test-dbus", test_name);
        if (!test(test_data_dir)) die(test_name);
 ...
 */
void dbus_internal_do_not_use_run_tests(const char* test_data_dir, const char* specific_test) {
#ifdef DBUS_ENABLE_EMBEDDED_TESTS
    if (!_dbus_threads_init_debug()) die("debug threads init");
