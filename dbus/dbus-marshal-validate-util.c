...
static void run_validity_tests(
    ...
    for (i = 0; i < n_tests; i++) {
        ...
        if (v != tests[i].expected) {
            _dbus_warn("Improper validation result %d for '%s'", v, tests[i].data);
...
dbus_bool_t _dbus_marshal_validate_test(void) {
    ...
    const char* valid_paths[]   = {"/", "/foo/bar", "/foo", "/foo/bar/baz"};
    ...
    const char* valid_unique_names[] = {
        ":0", ":a", ":", ":.a", ":.1", ":0.1", ":000.2222", ":.blah", ":abce.freedesktop.blah"};
    ...
    const char* valid_members[] = {"Hello", "Bar", "foobar", "_foobar", "foo89"};

    const char* invalid_members[] = {
        "9Hello", "10", "1", "foo-bar", "blah.org", ".blah", "blah.", "Hello.", "!foo", "", "   ", "foo bar"};
    ...
    while (i < (int)_DBUS_N_ELEMENTS(valid_paths)) {
        ...
        if (!_dbus_validate_path(&str, 0, _dbus_string_get_length(&str))) {
    ...
    if (_dbus_validate_bus_name(&str, 0, 8)) _dbus_assert_not_reached("validated too-long string");
    if (_dbus_validate_interface(&str, 0, 8)) _dbus_assert_not_reached("validated too-long string");
    if (_dbus_validate_error_name(&str, 0, 8)) _dbus_assert_not_reached("validated too-long string");

    _dbus_string_init_const(&str, "abc");
    if (_dbus_validate_member(&str, 0, 4)) _dbus_assert_not_reached("validated too-long string");
    ...
    while (_dbus_string_get_length(&str) <= DBUS_MAXIMUM_NAME_LENGTH)
        if (!_dbus_string_append(&str, "abc.def")) _dbus_assert_not_reached("no memory");
    ...
    while (_dbus_string_get_length(&str) <= DBUS_MAXIMUM_NAME_LENGTH)
        if (!_dbus_string_append(&str, "abc")) _dbus_assert_not_reached("no memory");
...

    {
        ...
        while (dbus_internal_do_not_use_generate_bodies(sequence, DBUS_LITTLE_ENDIAN, &signature, &body)) {
            ...
            validity = _dbus_validate_body_with_reason(
                &signature, 0, DBUS_LITTLE_ENDIAN, NULL, &body, 0, _dbus_string_get_length(&body));
        ...
        while (dbus_internal_do_not_use_generate_bodies(sequence, DBUS_BIG_ENDIAN, &signature, &body)) {
            ...
            validity = _dbus_validate_body_with_reason(
                &signature, 0, DBUS_BIG_ENDIAN, NULL, &body, 0, _dbus_string_get_length(&body));
            if (validity != DBUS_VALID) {
