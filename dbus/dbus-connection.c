...
static void _dbus_connection_trace_ref(
...
struct DBusConnection {
    ...
    char* server_guid; /**< GUID of server if we are in shared_connections, #NULL if server GUID is unknown or
    ...
    unsigned int shareable : 1; /**< #TRUE if libdbus owns a reference to the connection and can return it from
                                   dbus_connection_open() more than once */

    unsigned int exit_on_disconnect : 1; /**< If #TRUE, exit after handling disconnect signal */

    unsigned int route_peer_messages : 1; /**< If #TRUE, if org.freedesktop.DBus.Peer messages have a bus name, don't
...
void _dbus_connection_unlock(DBusConnection* connection) {
    ...
    for (iter = _dbus_list_pop_first_link(&expired_messages); iter != NULL;
 ...
 *        variable pointer
 */
void _dbus_connection_test_get_locks(
    DBusConnection* connection,
    DBusMutex**     mutex_loc,
...
void _dbus_connection_queue_received_message_link(DBusConnection* connection, DBusList* link) {
    ...
    _dbus_verbose(
        ...
        dbus_message_get_path(message) ? dbus_message_get_path(message) : "no path",
...
static dbus_bool_t _dbus_connection_acquire_io_path(DBusConnection* connection, int timeout_milliseconds) {
    ...
    if (connection->io_path_acquired) {
        if (timeout_milliseconds != -1) {
            ...
            if (!_dbus_condvar_wait_timeout(
                    connection->io_path_cond, connection->io_path_mutex, timeout_milliseconds)) {
...
void _dbus_connection_do_iteration_unlocked(
    ...
    if (_dbus_connection_acquire_io_path(connection, (flags & DBUS_ITERATION_BLOCK) ? timeout_milliseconds : 0)) {
        ...
        if ((pending != NULL) && _dbus_pending_call_get_completed_unlocked(pending)) {
            ...
            (pending != NULL) && _dbus_connection_peek_for_reply_unlocked(
                                     connection, _dbus_pending_call_get_reply_serial_unlocked(pending))) {
...
static void connection_forget_shared_unlocked(DBusConnection* connection) {
    ...
    if (connection->server_guid != NULL) {
        ...
        if (!_dbus_hash_table_remove_string(shared_connections, connection->server_guid))
...
static void _dbus_connection_send_preallocated_unlocked_no_update(
    DBusConnection*       connection,
    DBusPreallocatedSend* preallocated,
    DBusMessage*          message,
    ...
    _dbus_verbose(
        ...
        dbus_message_get_member(message) ? dbus_message_get_member(message) : "no member",
        dbus_message_get_signature(message),
        dbus_message_get_destination(message) ? dbus_message_get_destination(message) : "null",
        connection,
        connection->n_outgoing);
...
static void _dbus_connection_send_preallocated_and_unlock(
    DBusConnection*       connection,
    DBusPreallocatedSend* preallocated,
    DBusMessage*          message,
    dbus_uint32_t*        client_serial) {
 ...
 */
dbus_bool_t _dbus_connection_send_and_unlock(
    DBusConnection* connection,
    DBusMessage*    message,
    dbus_uint32_t*  client_serial) {
    DBusPreallocatedSend* preallocated;
...
static void complete_pending_call_and_unlock(
    DBusConnection*  connection,
    DBusPendingCall* pending,
    DBusMessage*     message) {
...
void _dbus_connection_block_pending_call(DBusPendingCall* pending) {
    ...
    if (timeout) {
        ...
        _dbus_verbose(
            "dbus_connection_send_with_reply_and_block(): will block %d milliseconds for reply serial %u from %ld sec "
            "%ld usec\n",
            timeout_milliseconds,
            client_serial,
            start_tv_sec,
    ...
    _dbus_connection_do_iteration_unlocked(
        connection, pending, DBUS_ITERATION_DO_READING | DBUS_ITERATION_BLOCK, timeout_milliseconds);
...
recheck_status:
    ...
    if (!_dbus_connection_get_is_connected_unlocked(connection)) {
        ...
        error_msg = generate_local_error_message(
            client_serial, DBUS_ERROR_DISCONNECTED, "Connection was disconnected before a reply was received");
        ...
        if (status == DBUS_DISPATCH_NEED_MEMORY) {
            ...
            _dbus_connection_do_iteration_unlocked(
                connection,
                pending,
                DBUS_ITERATION_DO_READING | DBUS_ITERATION_BLOCK,
 ...
 */
void _dbus_connection_set_pending_fds_function(
    DBusConnection*              connection,
    ...
    if (old_refcount == 1) {
        ...
        if (_dbus_transport_get_is_connected(connection->transport)) {
            _dbus_warn_check_failed(
                "The last reference on a connection was dropped without closing the connection. This is a bug in an "
                "application. See dbus_connection_unref() documentation for details.\n%s",
                connection->shareable ? "Most likely, the application called unref() too many times and removed a "
                                        "reference belonging to libdbus, since this is a shared connection."
...
void dbus_connection_close(DBusConnection* connection) {
    ...
    if (connection->shareable) {
        ...
        _dbus_warn_check_failed(
            "Applications must not close shared connections - see dbus_connection_close() docs. This is a bug in the "
            "application.");
        return;
    }
#endif
...
void dbus_connection_send_preallocated(
    ...
    _dbus_return_if_fail(
        ...
        (dbus_message_get_interface(message) != NULL && dbus_message_get_member(message) != NULL));
...
static dbus_bool_t _dbus_connection_send_unlocked_no_update(
    DBusConnection* connection,
    DBusMessage*    message,
    dbus_uint32_t*  client_serial) {
    DBusPreallocatedSend* preallocated;
...
static DBusDispatchStatus _dbus_connection_flush_unlocked(DBusConnection* connection) {
    ...
    while (connection->n_outgoing > 0 && _dbus_connection_get_is_connected_unlocked(connection)) {
        ...
        HAVE_LOCK_CHECK(connection);
        _dbus_connection_do_iteration_unlocked(
            connection, NULL, DBUS_ITERATION_DO_READING | DBUS_ITERATION_DO_WRITING | DBUS_ITERATION_BLOCK, -1);
    }
 ...
 */
static dbus_bool_t _dbus_connection_read_write_dispatch(
    DBusConnection* connection,
    int             timeout_milliseconds,
    ...
    if (dispatch && dstatus == DBUS_DISPATCH_DATA_REMAINS) {
        ...
        if (_dbus_connection_get_is_connected_unlocked(connection)) {
            ...
            _dbus_connection_do_iteration_unlocked(
                connection,
                NULL,
                DBUS_ITERATION_DO_READING | DBUS_ITERATION_DO_WRITING | DBUS_ITERATION_BLOCK,
                timeout_milliseconds);
        }
    }
     ...
     */
    if (dispatch)
        progress_possible = connection->n_incoming != 0 || connection->disconnect_message_link != NULL;
    else
...
static void check_disconnected_message_arrived_unlocked(DBusConnection* connection, DBusMessage* head_of_queue) {
    ...
    if (connection->disconnect_message_link == NULL &&
        dbus_message_is_signal(head_of_queue, DBUS_INTERFACE_LOCAL, "Disconnected")) {
...
static DBusList* _dbus_connection_pop_message_link_unlocked(DBusConnection* connection) {
    ...
    if (connection->n_incoming > 0) {
        ...
        _dbus_verbose(
            ...
            dbus_message_get_interface(link->data) ? dbus_message_get_interface(link->data) : "no interface",
            dbus_message_get_member(link->data) ? dbus_message_get_member(link->data) : "no member",
...
static void _dbus_connection_putback_message_link_unlocked(DBusConnection* connection, DBusList* message_link) {
    ...
    _dbus_verbose(
        ...
        dbus_message_get_interface(message_link->data) ? dbus_message_get_interface(message_link->data)
                                                       : "no interface",
        dbus_message_get_member(message_link->data) ? dbus_message_get_member(message_link->data) : "no member",
 ...
 */
static DBusHandlerResult _dbus_connection_peer_filter_unlocked_no_update(
    DBusConnection* connection,
    DBusMessage*    message) {
    ...
    if (dbus_message_is_method_call(message, DBUS_INTERFACE_PEER, "Ping")) {
        ...
        ret = dbus_message_new_error(
            message, DBUS_ERROR_UNKNOWN_METHOD, "Unknown method invoked on org.freedesktop.DBus.Peer interface");
        if (ret == NULL) goto out;
 ...
 **/
static DBusHandlerResult _dbus_connection_run_builtin_filters_unlocked_no_update(
    DBusConnection* connection,
    DBusMessage*    message) {
...
dbus_bool_t dbus_connection_set_watch_functions(
    ...
    retval = _dbus_watch_list_set_functions(
        connection->watches, add_function, remove_function, toggled_function, data, free_data_function);
...
dbus_bool_t dbus_connection_set_timeout_functions(
    ...
    retval = _dbus_timeout_list_set_functions(
        connection->timeouts, add_function, remove_function, toggled_function, data, free_data_function);
...
dbus_bool_t dbus_connection_get_unix_user(DBusConnection* connection, unsigned long* uid) {
    ...
    if (!_dbus_transport_try_to_authenticate(connection->transport))
        ...
        result = _dbus_transport_get_unix_user(connection->transport, uid);
...
dbus_bool_t dbus_connection_get_unix_process_id(DBusConnection* connection, unsigned long* pid) {
    ...
    if (!_dbus_transport_try_to_authenticate(connection->transport))
        ...
        result = _dbus_transport_get_unix_process_id(connection->transport, pid);
...
dbus_bool_t dbus_connection_get_adt_audit_session_data(
    ...
    if (!_dbus_transport_try_to_authenticate(connection->transport))
        ...
        result = _dbus_transport_get_adt_audit_session_data(connection->transport, data, data_size);
    CONNECTION_UNLOCK(connection);
...
void dbus_connection_set_unix_user_function(
    ...
    _dbus_transport_set_unix_user_function(
        connection->transport, function, data, free_data_function, &old_data, &old_free_function);
    CONNECTION_UNLOCK(connection);
...
dbus_bool_t _dbus_connection_get_linux_security_label(DBusConnection* connection, char** label_p) {
    ...
    if (!_dbus_transport_try_to_authenticate(connection->transport))
        ...
        result = _dbus_transport_get_linux_security_label(connection->transport, label_p);
#ifndef __linux__
    _dbus_assert(!result);
#endif
...
dbus_bool_t dbus_connection_get_windows_user(DBusConnection* connection, char** windows_sid_p) {
    ...
    if (!_dbus_transport_try_to_authenticate(connection->transport))
        ...
        result = _dbus_transport_get_windows_user(connection->transport, windows_sid_p);
 ...
 */
static dbus_bool_t _dbus_connection_register_object_path(
    DBusConnection*             connection,
    ...
    retval = _dbus_object_tree_register(
        connection->objects, fallback, (const char**)decomposed_path, vtable, user_data, error);
...
dbus_bool_t dbus_connection_set_data(
    ...
    retval = _dbus_data_slot_list_set(
        &slot_allocator, &connection->slot_list, slot, data, free_data_func, &old_free_func, &old_data);
...
void _dbus_connection_get_stats(
    DBusConnection* connection,
