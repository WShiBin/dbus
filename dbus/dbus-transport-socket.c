...
static void check_write_watch(DBusTransport* transport) {
    ...
    if (_dbus_transport_try_to_authenticate(transport))
        ...
        if (transport->send_credentials_pending)
             ...
             */
            if (auth_state == DBUS_AUTH_STATE_HAVE_BYTES_TO_SEND || auth_state == DBUS_AUTH_STATE_WAITING_FOR_MEMORY)
                needed = TRUE;
            else
                needed = FALSE;
        }
...
static void check_read_watch(DBusTransport* transport) {
    ...
    if (_dbus_transport_try_to_authenticate(transport))
        ...
        if (transport->receive_credentials_pending)
             ...
             * authenticated.
             */
            if (auth_state == DBUS_AUTH_STATE_WAITING_FOR_INPUT || auth_state == DBUS_AUTH_STATE_WAITING_FOR_MEMORY ||
...
static dbus_bool_t exchange_credentials(DBusTransport* transport, dbus_bool_t do_reading, dbus_bool_t do_writing) {
    ...
    if (do_writing && transport->send_credentials_pending) {
        if (_dbus_send_credentials_socket(socket_transport->fd, &error)) {
    ...
    if (do_reading && transport->receive_credentials_pending) {
         ...
         */
        if (_dbus_read_credentials_socket(socket_transport->fd, transport->credentials, &error)) {
...
static dbus_bool_t do_authentication(
      ...
      _dbus_verbose ("writing message %p\n", message);
        ...
        if (_dbus_auth_needs_encoding(transport->auth)) {
          ...
          _dbus_verbose ("encoded message is %d bytes\n",
            ...
            bytes_written = _dbus_write_socket(
                socket_transport->fd,
          ...
          _dbus_verbose ("message is %d bytes\n",
            ...
            if (socket_transport->message_bytes_written <= 0 && DBUS_TRANSPORT_CAN_SEND_UNIX_FD(transport)) {
                ...
                bytes_written = _dbus_write_socket_with_unix_fds_two(
                    socket_transport->fd,
                    header,
                    socket_transport->message_bytes_written,
...
#endif
            {
                if (socket_transport->message_bytes_written < header_len) {
                    bytes_written = _dbus_write_socket_two(
                        socket_transport->fd,
                        header,
                        socket_transport->message_bytes_written,
                        header_len - socket_transport->message_bytes_written,
...
static dbus_bool_t socket_handle_watch(DBusTransport* transport, DBusWatch* watch, unsigned int flags) {
    ...
    if (watch == socket_transport->read_watch && (flags & DBUS_WATCH_READABLE)) {
        ...
        if (watch == socket_transport->read_watch)
            ...
            _dbus_verbose(
                "asked to handle watch %p on fd %" DBUS_SOCKET_FORMAT " that we don't recognize\n",
                watch,
                _dbus_socket_printable(_dbus_watch_get_socket(watch)));
    }
...
static void socket_do_iteration(DBusTransport* transport, unsigned int flags, int timeout_milliseconds) {
    ...
    if (_dbus_transport_try_to_authenticate(transport)) {
         ...
         */
        if ((flags & DBUS_ITERATION_DO_WRITING) && !(flags & (DBUS_ITERATION_DO_READING | DBUS_ITERATION_BLOCK)) &&
            !transport->disconnected && _dbus_connection_has_messages_to_send_unlocked(transport->connection)) {
            do_writing(transport);

            if (transport->disconnected || !_dbus_connection_has_messages_to_send_unlocked(transport->connection))
                goto out;
        }
...
DBusTransport* _dbus_transport_new_for_socket(DBusSocket fd, const DBusString* server_guid, const DBusString* address) {
    ...
    if (!_dbus_string_init(&socket_transport->encoded_outgoing)) goto failed_0;

    if (!_dbus_string_init(&socket_transport->encoded_incoming)) goto failed_1;

    socket_transport->write_watch =
        _dbus_watch_new(_dbus_socket_get_pollable(fd), DBUS_WATCH_WRITABLE, FALSE, NULL, NULL, NULL);
    if (socket_transport->write_watch == NULL) goto failed_2;

    socket_transport->read_watch =
        _dbus_watch_new(_dbus_socket_get_pollable(fd), DBUS_WATCH_READABLE, FALSE, NULL, NULL, NULL);
    if (socket_transport->read_watch == NULL) goto failed_3;

    if (!_dbus_transport_init_base(&socket_transport->base, &socket_vtable, server_guid, address)) goto failed_4;
 ...
 */
DBusTransport* _dbus_transport_new_for_tcp_socket(
    const char* host,
    const char* port,
    const char* family,
    ...
    if (family != NULL && (!_dbus_string_append(&address, ",family=") || !_dbus_string_append(&address, family)))
        goto error;

    if (noncefile != NULL &&
        (!_dbus_string_append(&address, ",noncefile=") || !_dbus_string_append(&address, noncefile)))
        goto error;
 ...
 */
DBusTransportOpenResult _dbus_transport_open_socket(
