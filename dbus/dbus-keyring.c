...
static DBusKeyring* _dbus_keyring_new(void) {
    DBusKeyring* keyring;

    keyring = dbus_new0(DBusKeyring, 1);
    if (keyring == NULL) goto out_0;

    if (!_dbus_string_init(&keyring->directory)) goto out_1;

    if (!_dbus_string_init(&keyring->filename)) goto out_2;

    if (!_dbus_string_init(&keyring->filename_lock)) goto out_3;
...
static void free_keys(DBusKey* keys, int n_keys) {
    int i;
...
static dbus_bool_t _dbus_keyring_lock(DBusKeyring* keyring) {
    int n_timeouts;
    ...
    while (n_timeouts < MAX_LOCK_TIMEOUTS) {
        ...
        if (_dbus_create_file_exclusively(&keyring->filename_lock, &error)) break;
    ...
    if (n_timeouts == MAX_LOCK_TIMEOUTS) {
        ...
        if (!_dbus_create_file_exclusively(&keyring->filename_lock, &error)) {
...
static void _dbus_keyring_unlock(DBusKeyring* keyring) {
    DBusError error = DBUS_ERROR_INIT;

    if (!_dbus_delete_file(&keyring->filename_lock, &error)) {
        _dbus_warn("Failed to delete lock file: %s", error.message);
        dbus_error_free(&error);
    }
}

static DBusKey* find_key_by_id(DBusKey* keys, int n_keys, int id) {
    int i;
...
static dbus_bool_t add_new_key(DBusKey** keys_p, int* n_keys_p, DBusError* error) {
    DBusKey* new;
    DBusString           bytes;
    int                  id;
    long                 timestamp;
...
retry:
    ...
    if (!_dbus_generate_random_bytes(&bytes, 4, error)) goto out;
...
#define KEY_LENGTH_BYTES 24
    ...
    *keys_p = keys; /* otherwise *keys_p ends up invalid */
    ...
    if (!_dbus_string_move(&bytes, 0, &keys[n_keys - 1].secret, 0)) {
        dbus_set_error(error, DBUS_ERROR_NO_MEMORY, NULL);
 ...
 */
static dbus_bool_t _dbus_keyring_reload(DBusKeyring* keyring, dbus_bool_t add_new, DBusError* error) {
    DBusString  contents;
    DBusString  line;
    dbus_bool_t retval;
    ...
    if (!_dbus_check_dir_is_private_to_user(&keyring->directory, error)) return FALSE;
    ...
    if (add_new) {
        if (!_dbus_keyring_lock(keyring)) {
            dbus_set_error(error, DBUS_ERROR_FAILED, "Could not lock keyring file to add to it");
            goto out;
        }
    ...
    if (!_dbus_file_get_contents(&contents, &keyring->filename, &tmp_error)) {
        _dbus_verbose("Failed to load keyring file: %s\n", tmp_error.message);
        /* continue with empty keyring file, so we recreate it */
        dbus_error_free(&tmp_error);
    }

    if (!_dbus_string_validate_ascii(&contents, 0, _dbus_string_get_length(&contents))) {
    ...
    while (_dbus_string_pop_line(&contents, &line)) {
        ...
        if (n_keys >= (add_new ? MAX_KEYS_IN_FILE - 1 : MAX_KEYS_IN_FILE)) break;
        ...
        if (timestamp < 0 || (now + MAX_TIME_TRAVEL_SECONDS) < timestamp ||
        ...
        keys[n_keys - 1].id            = id;
        keys[n_keys - 1].creation_time = timestamp;
        if (!_dbus_string_hex_decode(&line, next, &end, &keys[n_keys - 1].secret, 0)) {
            dbus_set_error(error, DBUS_ERROR_NO_MEMORY, NULL);
            goto out;
        }
    ...
    if (add_new) {
        ...
        i = 0;
        while (i < n_keys) {
            if (!_dbus_string_append_int(&contents, keys[i].id)) goto nomem;

            if (!_dbus_string_append_byte(&contents, ' ')) goto nomem;

            if (!_dbus_string_append_int(&contents, keys[i].creation_time)) goto nomem;

            if (!_dbus_string_append_byte(&contents, ' ')) goto nomem;

            if (!_dbus_string_hex_encode(&keys[i].secret, 0, &contents, _dbus_string_get_length(&contents))) goto nomem;
        ...
        if (!_dbus_string_save_to_file(&contents, &keyring->filename, FALSE, error)) goto out;
    }

    if (keyring->keys) free_keys(keyring->keys, keyring->n_keys);
    keyring->keys   = keys;
...
out:
    if (have_lock) _dbus_keyring_unlock(keyring);

    if (!((retval == TRUE && (error == NULL || error->name == NULL)) ||
          (retval == FALSE && (error == NULL || error->name != NULL)))) {
        if (error && error->name) _dbus_verbose("error is %s: %s\n", error->name, error->message);
        _dbus_warn("returning %d but error pointer %p name %s", retval, error, error->name ? error->name : "(none)");
...
void _dbus_keyring_unref(DBusKeyring* keyring) {
    ...
    if (keyring->refcount == 0) {
        if (keyring->credentials) _dbus_credentials_unref(keyring->credentials);
 ...
 */
DBusKeyring* _dbus_keyring_new_for_credentials(
    ...
    if (_dbus_check_setuid()) {
        dbus_set_error_const(error, DBUS_ERROR_NOT_SUPPORTED, "Unable to create DBus keyring when setuid");
        return NULL;
    }
    ...
    if (our_credentials == NULL) goto failed;

    if (_dbus_credentials_are_anonymous(our_credentials)) {
        if (!_dbus_credentials_add_from_current_process(our_credentials)) goto failed;
    }

    if (!_dbus_append_keyring_directory_for_credentials(&ringdir, our_credentials)) goto failed;

    keyring = _dbus_keyring_new();
    if (keyring == NULL) goto failed;
    ...
    if (!_dbus_keyring_validate_context(context)) {
        error_set = TRUE;
        dbus_set_error_const(error, DBUS_ERROR_FAILED, "Invalid context in keyring creation");
        goto failed;
    }

    /* Save keyring dir in the keyring object */
    if (!_dbus_string_copy(&ringdir, 0, &keyring->directory, 0)) goto failed;

    /* Create keyring->filename based on keyring dir and context */
    if (!_dbus_string_copy(&keyring->directory, 0, &keyring->filename, 0)) goto failed;

    if (!_dbus_concat_dir_and_file(&keyring->filename, context)) goto failed;

    /* Create lockfile name */
    if (!_dbus_string_copy(&keyring->filename, 0, &keyring->filename_lock, 0)) goto failed;

    if (!_dbus_string_append(&keyring->filename_lock, ".lock")) goto failed;
     ...
     */
    dbus_error_init(&tmp_error);
    if (!_dbus_ensure_directory(&keyring->directory, &tmp_error)) {
...
failed:
    if (!error_set) dbus_set_error_const(error, DBUS_ERROR_NO_MEMORY, NULL);
    if (our_credentials) _dbus_credentials_unref(our_credentials);
    if (keyring) _dbus_keyring_unref(keyring);
    _dbus_string_free(&ringdir);
    return NULL;
}
...
dbus_bool_t _dbus_keyring_validate_context(const DBusString* context) {
    ...
    if (!_dbus_string_validate_ascii(context, 0, _dbus_string_get_length(context))) {
...
static DBusKey* find_recent_key(DBusKeyring* keyring) {
    int  i;
    long tv_sec, tv_usec;
    ...
    while (i < keyring->n_keys) {
        ...
        if ((tv_sec - NEW_KEY_TIMEOUT_SECONDS) < key->creation_time) return key;
...
int _dbus_keyring_get_best_key(DBusKeyring* keyring, DBusError* error) {
     ...
     */
    if (!_dbus_keyring_reload(keyring, TRUE, error)) return -1;
    ...
    if (key)
        ...
        dbus_set_error_const(
            error, DBUS_ERROR_FAILED, "No recent-enough key found in keyring, and unable to create a new key");
        return -1;
    }
}
...
dbus_bool_t _dbus_keyring_is_for_credentials(DBusKeyring* keyring, DBusCredentials* credentials) {
    return _dbus_credentials_same_user(keyring->credentials, credentials);
}
...
dbus_bool_t _dbus_keyring_get_hex_key(DBusKeyring* keyring, int key_id, DBusString* hex_key) {
    ...
    key = find_key_by_id(keyring->keys, keyring->n_keys, key_id);
    if (key == NULL) return TRUE; /* had enough memory, so TRUE */

    return _dbus_string_hex_encode(&key->secret, 0, hex_key, _dbus_string_get_length(hex_key));
}
...
dbus_bool_t _dbus_keyring_test(void) {
    ...
    if (!_dbus_string_init(&context)) _dbus_assert_not_reached("no memory");
    if (!_dbus_string_append_byte(&context, '\0')) _dbus_assert_not_reached("no memory");
    _dbus_assert(!_dbus_keyring_validate_context(&context));
    ...
    ring1 = _dbus_keyring_new_for_credentials(NULL, &context, &error);
    ...
    while (i < ring1->n_keys) {
        if (ring1->keys[i].id != ring2->keys[i].id) {
            fprintf(
                stderr, "Keyring 1 has first key ID %d and keyring 2 has %d\n", ring1->keys[i].id, ring2->keys[i].id);
            goto failure;
        }
        ...
        if (ring1->keys[i].creation_time != ring2->keys[i].creation_time) {
            fprintf(
                ...
                ring1->keys[i].creation_time,
                ring2->keys[i].creation_time);
            goto failure;
        }

        if (!_dbus_string_equal(&ring1->keys[i].secret, &ring2->keys[i].secret)) {
...
failure:
    if (ring1) _dbus_keyring_unref(ring1);
    if (ring2) _dbus_keyring_unref(ring2);
