...
dbus_bool_t _dbus_file_get_contents(DBusString* str, const DBusString* filename, DBusError* error) {
    ...
    hnd = CreateFileA(
        filename_c, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    if (fsize == 0xFFFFFFFF && GetLastError() != NO_ERROR) {
        char* emsg = _dbus_win_error_string(GetLastError());
        dbus_set_error(
            error, _dbus_win_error_from_last_error(), "Failed to get file size for \"%s\": %s", filename_c, emsg);
        _dbus_win_free_error_string(emsg);
    ...
    if (fsize > 0) {
        ...
        while (total < fsize) {
            ...
            if (bytes_read <= 0) {
                if (bytes_read == 0) {
                    dbus_set_error(error, DBUS_ERROR_FAILED, "Premature EOF reading \"%s\"", filename_c);
                } else
