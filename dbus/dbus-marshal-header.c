 ...
 * @returns #FALSE if no memory
 */
static dbus_bool_t set_basic_field(
    DBusTypeReader*       reader,
    int                   field,
    int                   type,
    const void*           value,
...
dbus_bool_t _dbus_header_create(
    ...
    _dbus_assert(
        ((interface || message_type != DBUS_MESSAGE_TYPE_SIGNAL) && member) || (error_name) ||
    ...
    _dbus_type_writer_init_values_only(
        &writer, byte_order, &_dbus_header_signature_str, 0, &header->data, HEADER_END_BEFORE_PADDING(header));
    ...
    if (!_dbus_type_writer_recurse(
            &writer, DBUS_TYPE_ARRAY, &_dbus_header_signature_str, FIELDS_ARRAY_SIGNATURE_OFFSET, &array))
        goto oom;
...
#define REQUIRE_FIELD(name)                                                                             \
    do {                                                                                                \
        if (header->fields[DBUS_HEADER_FIELD_##name].value_pos < 0) return DBUS_INVALID_MISSING_##name; \
    } while (0)
...
static DBusValidity load_and_validate_field(DBusHeader* header, int field, DBusTypeReader* variant_reader) {
    ...
    if (expected_type == DBUS_TYPE_UINT32) {
        _dbus_header_get_field_basic(header, field, expected_type, &v_UINT32);
    } else if (
        expected_type == DBUS_TYPE_STRING || expected_type == DBUS_TYPE_OBJECT_PATH ||
...
dbus_bool_t _dbus_header_load(
    ...
    if (mode == DBUS_VALIDATION_MODE_WE_TRUST_THIS_DATA_ABSOLUTELY) {
        ...
        v = _dbus_validate_body_with_reason(&_dbus_header_signature_str, 0, byte_order, &leftover, str, start, len);
    ...
    _dbus_type_reader_init(&reader, byte_order, &_dbus_header_signature_str, 0, str, start);
 ...
 */
static dbus_bool_t find_field_for_modification(
    DBusHeader*     header,
    int             field,
    DBusTypeReader* reader,
...
dbus_bool_t _dbus_header_set_field_basic(DBusHeader* header, int field, int type, const void* value) {
    ...
    if (_dbus_header_cache_check(header, field)) {
        ...
        if (!find_field_for_modification(header, field, &reader, &realign_root))
        ...
        if (!_dbus_type_writer_append_array(
                &writer, &_dbus_header_signature_str, FIELDS_ARRAY_ELEMENT_SIGNATURE_OFFSET, &array))
...
dbus_bool_t _dbus_header_get_field_basic(DBusHeader* header, int field, int type, void* value) {
    ...
    _dbus_marshal_read_basic(
        &header->data, header->fields[field].value_pos, type, value, _dbus_header_get_byte_order(header), NULL);
...
dbus_bool_t _dbus_header_delete_field(DBusHeader* header, int field) {
    ...
    if (_dbus_header_cache_known_nonexistent(header, field)) return TRUE; /* nothing to do */

    /* Scan to the field we want, delete and realign, reappend
     * padding. Field may turn out not to exist.
     */
    if (!find_field_for_modification(header, field, &reader, &realign_root)) return TRUE; /* nothing to do */
