...
static void sha_append(DBusSHAContext* context, const unsigned char* buffer, unsigned int count) {
    ...
    if ((context->count_lo = tmp + ((dbus_uint32_t)count << 3)) < tmp) context->count_hi++; /* Carry from low to high */
...
static dbus_bool_t check_sha_binary(const unsigned char* input, int input_len, const char* expected) {
    ...
    if (!_dbus_string_init(&results)) _dbus_assert_not_reached("no memory for SHA-1 results");

    if (!_dbus_sha_compute(&input_str, &results)) _dbus_assert_not_reached("no memory for SHA-1 results");
...
next_iteration:
    while (_dbus_string_pop_line(results, &line)) {
        ...
        if (_dbus_string_get_length(&line) == 0)
            ...
            /* don't print */
        } else if (_dbus_string_starts_with_c_str(&line, "D>") || _dbus_string_starts_with_c_str(&line, "<D"))
            goto next_iteration;
        else {
            int i;

            if (!_dbus_string_move(&line, 0, result, 0)) _dbus_assert_not_reached("no memory");
...
static dbus_bool_t process_test_data(const char* test_data_dir) {
    ...
    if (!_dbus_string_init(&tests_file)) _dbus_assert_not_reached("no memory");

    if (!_dbus_string_init(&results_file)) _dbus_assert_not_reached("no memory");

    if (!_dbus_string_init(&tests)) _dbus_assert_not_reached("no memory");

    if (!_dbus_string_init(&results)) _dbus_assert_not_reached("no memory");

    if (!_dbus_string_init(&line)) _dbus_assert_not_reached("no memory");

    if (!_dbus_string_append(&tests_file, test_data_dir)) _dbus_assert_not_reached("no memory");

    if (!_dbus_string_append(&results_file, test_data_dir)) _dbus_assert_not_reached("no memory");

    _dbus_string_init_const(&tmp, "sha-1/byte-messages.sha1");
    if (!_dbus_concat_dir_and_file(&tests_file, &tmp)) _dbus_assert_not_reached("no memory");

    _dbus_string_init_const(&tmp, "sha-1/byte-hashes.sha1");
    if (!_dbus_concat_dir_and_file(&results_file, &tmp)) _dbus_assert_not_reached("no memory");

    if (!_dbus_file_get_contents(&tests, &tests_file, &error)) {
        fprintf(
            stderr, "could not load test data file %s: %s\n", _dbus_string_get_const_data(&tests_file), error.message);
    ...
    if (!_dbus_file_get_contents(&results, &results_file, &error)) {
        fprintf(
            ...
            _dbus_string_get_const_data(&results_file),
            error.message);
        dbus_error_free(&error);
        goto out;
    }
...
next_iteration:
    while (_dbus_string_pop_line(&tests, &line)) {
        ...
        if (_dbus_string_get_length(&line) == 0)
            ...
            if (!_dbus_string_init(&next_line)) _dbus_assert_not_reached("no memory");

            if (!_dbus_string_init(&expected)) _dbus_assert_not_reached("no memory");

            if (!_dbus_string_init(&test)) _dbus_assert_not_reached("no memory");

            if (!_dbus_string_init(&result)) _dbus_assert_not_reached("no memory");
            ...
            while (!_dbus_string_find(&line, 0, "^", NULL) && _dbus_string_pop_line(&tests, &next_line)) {
                if (!_dbus_string_append_byte(&line, ' ') ||
                    !_dbus_string_move(&next_line, 0, &line, _dbus_string_get_length(&line)))
            ...
            if (!_dbus_sha_compute(&test, &result)) _dbus_assert_not_reached("no memory for SHA-1 result");
...
#define CHECK(input, expected) \
    if (!check_sha_str(input, expected)) return FALSE
