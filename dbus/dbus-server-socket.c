...
static void socket_finalize(DBusServer* server) {
    ...
    if (socket_server->noncefile) _dbus_noncefile_delete(socket_server->noncefile, NULL);
...
static const DBusServerVTable socket_vtable = {socket_finalize, socket_disconnect};
 ...
 *
 */
DBusServer* _dbus_server_new_for_socket(
    DBusSocket*       fds,
    int               n_fds,
    ...
    if (socket_server == NULL) goto failed_0;

    socket_server->noncefile = noncefile;

    socket_server->fds = dbus_new(DBusSocket, n_fds);
    if (!socket_server->fds) goto failed_0;

    socket_server->watch = dbus_new0(DBusWatch*, n_fds);
    if (!socket_server->watch) goto failed_1;

    for (i = 0; i < n_fds; i++) {
        DBusWatch* watch;

        watch = _dbus_watch_new(
            _dbus_socket_get_pollable(fds[i]), DBUS_WATCH_READABLE, TRUE, socket_handle_watch, socket_server, NULL);
        if (watch == NULL) goto failed_2;
    ...
    if (!_dbus_server_init_base(&socket_server->base, &socket_vtable, address, error)) goto failed_2;
    ...
    for (i = 0; i < n_fds; i++) {
        if (!_dbus_server_add_watch(&socket_server->base, socket_server->watch[i])) {
             ...
             * close them */
            for (j = 0; j < n_fds; j++) _dbus_socket_invalidate(&socket_server->fds[j]);
 ...
 */
DBusServer* _dbus_server_new_for_tcp_socket(
    const char* host,
    const char* bind,
    const char* port,
    const char* family,
    ...
    if (!_dbus_string_append(&address, use_nonce ? "nonce-tcp:host=" : "tcp:host=") ||
        !_dbus_address_append_escaped(&address, &host_str) || !_dbus_string_append(&address, ",port=") ||
    ...
    if (use_nonce) {
        ...
        if (!_dbus_noncefile_create(noncefile, error)) goto failed_3;
...
DBusServerListenResult _dbus_server_listen_socket(DBusAddressEntry* entry, DBusServer** server_p, DBusError* error) {
    ...
    *server_p = NULL;
    ...
    if (strcmp(method, "tcp") == 0 || strcmp(method, "nonce-tcp") == 0) {
        ...
        *server_p = _dbus_server_new_for_tcp_socket(
            host, bind, port, family, error, strcmp(method, "nonce-tcp") == 0 ? TRUE : FALSE);
