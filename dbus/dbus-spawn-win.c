...
void _dbus_babysitter_kill_child(DBusBabysitter* sitter) {
    PING();
    if (sitter->child_handle == NULL) return; /* child is already dead, or we're so hosed we'll never recover */
...
dbus_bool_t _dbus_spawn_async_with_babysitter(
    ...
    sitter->sitter_watch =
        _dbus_watch_new(sitter->socket_to_babysitter, DBUS_WATCH_READABLE, TRUE, handle_watch, sitter, NULL);
    PING();
    if (sitter->sitter_watch == NULL) {
        _DBUS_SET_OOM(error);
        goto out0;
    ...
    PING();
    sitter->thread_handle =
        (HANDLE)CreateThread(NULL, 0, babysitter, _dbus_babysitter_ref(sitter), 0, &sitter_thread_id);
