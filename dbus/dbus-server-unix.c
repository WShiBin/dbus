    ...
    *server_p = NULL;
    ...
    if (strcmp(method, "unix") == 0) {
        ...
        mutually_exclusive_modes =
            (path != NULL) + (tmpdir != NULL) + (abstract != NULL) + (runtime != NULL) + (dir != NULL);
        ...
        if (runtime != NULL) {
            ...
            *server_p =
                _dbus_server_new_for_domain_socket(_dbus_string_get_const_data(&full_path), use_abstract, error);
...
DBusServer* _dbus_server_new_for_domain_socket(const char* path, dbus_bool_t abstract, DBusError* error) {
    ...
    if ((abstract && !_dbus_string_append(&address, "unix:abstract=")) ||
        (!abstract && !_dbus_string_append(&address, "unix:path=")) ||
