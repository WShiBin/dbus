 ...
 */
static dbus_bool_t append_quoted_string(DBusString* dest, const DBusString* quoted) {
    ...
    while (i < _dbus_string_get_length(quoted)) {
        ...
        if (in_backslash) {
            ...
            if (!_dbus_string_append_byte(dest, a)) return FALSE;
            ...
            if (b == '\'')
                ...
                if (!_dbus_string_append_byte(dest, b)) return FALSE;
            }
        } else {
            if (b == '\'')
                in_quotes = TRUE;
...
static dbus_bool_t same_first_word(const DBusString* a, const DBusString* b) {
    int first_a_blank, first_b_blank;

    _dbus_string_find_blank(a, 0, &first_a_blank);
    _dbus_string_find_blank(b, 0, &first_b_blank);

    if (first_a_blank != first_b_blank) return FALSE;

    return _dbus_string_equal_len(a, b, first_a_blank);
}

static DBusAuthState auth_state_from_string(const DBusString* str) {
...
static const char* auth_state_to_string(DBusAuthState state) {
    switch (state) {
...
static char** split_string(DBusString* str) {
    int    i, j, k, count, end;
    char** array;
    ...
    for (k = 0; k < count; k++) {
        ...
        memcpy(array[k], _dbus_string_get_const_data_len(str, i, j - i), j - i);
        array[k][j - i] = '\0';
...
static void auth_set_unix_credentials(DBusAuth* auth, dbus_uid_t uid, dbus_pid_t pid) {
    DBusCredentials* credentials;

    credentials = _dbus_credentials_new();
    if (credentials == NULL) _dbus_assert_not_reached("no memory");

    if (uid != DBUS_UID_UNSET) {
        if (!_dbus_credentials_add_unix_uid(credentials, uid)) _dbus_assert_not_reached("no memory");
    }
    if (pid != DBUS_PID_UNSET) {
        if (!_dbus_credentials_add_pid(credentials, pid)) _dbus_assert_not_reached("no memory");
    }
...
dbus_bool_t _dbus_auth_script_run(const DBusString* filename) {
    ...
    if (!_dbus_string_init(&file)) return FALSE;
    ...
    if (!_dbus_file_get_contents(&file, filename, &error)) {
        _dbus_warn("Getting contents of %s failed: %s", _dbus_string_get_const_data(filename), error.message);
...
next_iteration:
    while (_dbus_string_pop_line(&file, &line)) {
        ...
        if (auth != NULL) {
            while ((state = _dbus_auth_do_work(auth)) == DBUS_AUTH_STATE_HAVE_BYTES_TO_SEND) {
                const DBusString* tmp;
                if (_dbus_auth_get_bytes_to_send(auth, &tmp)) {
                    int count = _dbus_string_get_length(tmp);

                    if (_dbus_string_copy(tmp, 0, &from_auth, _dbus_string_get_length(&from_auth)))
        ...
        if (_dbus_string_get_length(&line) == 0) {
...
#ifdef DBUS_WIN
        else if (_dbus_string_starts_with_c_str(&line, "WIN_ONLY")) {
            /* Ignore this line */
            goto next_iteration;
        } else if (_dbus_string_starts_with_c_str(&line, "UNIX_ONLY")) {
            /* skip this file */
...
#endif
        else if (_dbus_string_starts_with_c_str(&line, "CLIENT")) {
        ...
        } else if (_dbus_string_starts_with_c_str(&line, "NO_CREDENTIALS")) {
            auth_set_unix_credentials(auth, DBUS_UID_UNSET, DBUS_PID_UNSET);
        } else if (_dbus_string_starts_with_c_str(&line, "ROOT_CREDENTIALS")) {
            auth_set_unix_credentials(auth, 0, DBUS_PID_UNSET);
        } else if (_dbus_string_starts_with_c_str(&line, "SILLY_CREDENTIALS")) {
            auth_set_unix_credentials(auth, 4312, DBUS_PID_UNSET);
        } else if (_dbus_string_starts_with_c_str(&line, "ALLOWED_MECHS")) {
            char** mechs;
        ...
        } else if (_dbus_string_starts_with_c_str(&line, "SEND")) {
            DBusString to_send;
            ...
            if (!append_quoted_string(&to_send, &line)) {
                _dbus_warn("failed to append quoted string line %d", line_no);
            ...
            /* Replace USERID_HEX with our username in hex */
            {
                ...
                if (_dbus_string_find(&to_send, 0, "WRONG_USERID_HEX", &where)) {
                    ...
                    _dbus_string_delete(&to_send, where, (int)strlen("WRONG_USERID_HEX"));
                    ...
                    _dbus_string_free(&uid);
                } else if (_dbus_string_find(&to_send, 0, "USERID_HEX", &where)) {
                    DBusString username;
                    ...
                    if (!_dbus_string_hex_encode(&username, 0, &to_send, where)) {
                    ...
                    _dbus_string_free(&username);
                } else if (_dbus_string_find(&to_send, 0, "WRONG_USERNAME_HEX", &where)) {
                    ...
                    _dbus_string_delete(&to_send, where, (int)strlen("WRONG_USERNAME_HEX"));
                    ...
                    goto out;
#endif
                } else if (_dbus_string_find(&to_send, 0, "USERNAME_HEX", &where)) {
                    DBusString username;
...

            {
                ...
                if (!_dbus_string_copy(&to_send, 0, buffer, _dbus_string_get_length(buffer))) {
                    _dbus_warn(
                        "not enough memory to call bytes_received, or can't add bytes to auth object already in end "
                        "state");
                    _dbus_string_free(&to_send);
            ...
            _dbus_string_free(&to_send);
        } else if (_dbus_string_starts_with_c_str(&line, "EXPECT_STATE")) {
        ...
        } else if (_dbus_string_starts_with_c_str(&line, "EXPECT_COMMAND")) {
            DBusString received;
            ...
            _dbus_string_free(&received);
        } else if (_dbus_string_starts_with_c_str(&line, "EXPECT_UNUSED")) {
            DBusString        expected;
        ...
        } else if (_dbus_string_starts_with_c_str(&line, "EXPECT_HAVE_NO_CREDENTIALS")) {
        ...
        } else if (_dbus_string_starts_with_c_str(&line, "EXPECT_HAVE_SOME_CREDENTIALS")) {
            DBusCredentials* authorized_identity;
        ...
        } else if (_dbus_string_starts_with_c_str(&line, "EXPECT")) {
            DBusString expected;
            ...
            if (_dbus_string_equal_len(&expected, &from_auth, _dbus_string_get_length(&expected))) {
                _dbus_string_delete(&from_auth, 0, _dbus_string_get_length(&expected));
                _dbus_string_free(&expected);
            } else {
                _dbus_warn(
    ...
    parse_failed : {
        _dbus_warn("couldn't process line %d \"%s\"", line_no, _dbus_string_get_const_data(&line));
        goto out;
    }
    }
