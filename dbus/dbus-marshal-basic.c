 ...
 *
 */
static dbus_bool_t set_signature(
    DBusString* str,
    int         pos,
    const char* value,
    int         byte_order,
 ...
 */
dbus_bool_t _dbus_marshal_set_basic(
    DBusString* str,
    int         pos,
    int         type,
    const void* value,
...
static dbus_bool_t marshal_2_octets(
    DBusString*   str,
    int           insert_at,
    dbus_uint16_t value,
    int           byte_order,
...
static dbus_bool_t marshal_4_octets(
    DBusString*   str,
    int           insert_at,
    dbus_uint32_t value,
    int           byte_order,
...
static dbus_bool_t marshal_8_octets(
    DBusString*    str,
    int            insert_at,
...
static dbus_bool_t marshal_len_followed_by_bytes(
    int                  marshal_as,
    DBusString*          str,
    int                  insert_at,
...
static dbus_bool_t marshal_string(DBusString* str, int insert_at, const char* value, int byte_order, int* pos_after) {
    return marshal_len_followed_by_bytes(
        MARSHAL_AS_STRING, str, insert_at, (const unsigned char*)value, strlen(value), byte_order, pos_after);
}
 ...
 **/
dbus_bool_t _dbus_marshal_write_basic(
    DBusString* str,
    int         insert_at,
    int         type,
    const void* value,
    ...
    switch (type) {
        ...
        case DBUS_TYPE_UINT16:
            ...
            break;
        case DBUS_TYPE_BOOLEAN:
            return marshal_4_octets(str, insert_at, vp->u32 != FALSE, byte_order, pos_after);
            break;
        case DBUS_TYPE_INT32:
        case DBUS_TYPE_UINT32:
        case DBUS_TYPE_UNIX_FD:
            return marshal_4_octets(str, insert_at, vp->u32, byte_order, pos_after);
            break;
        case DBUS_TYPE_INT64:
        case DBUS_TYPE_UINT64:
        case DBUS_TYPE_DOUBLE:
...
static dbus_bool_t marshal_1_octets_array(
    DBusString*          str,
    int                  insert_at,
    const unsigned char* value,
    int                  n_elements,
...
static dbus_bool_t marshal_fixed_multi(
    DBusString*           str,
    int                   insert_at,
...
void _dbus_verbose_bytes(const unsigned char* data, int len, int offset) {
    ...
    while (i < len) {
        if (_DBUS_ALIGN_ADDRESS(&data[i], 4) == &data[i]) {
            _dbus_verbose("%4d\t%p: ", offset + i, &data[i]);
        }

        if (data[i] >= 32 && data[i] <= 126)
            _dbus_verbose(" '%c' ", data[i]);
        else
            _dbus_verbose("0x%s%x ", data[i] <= 0xf ? "0" : "", data[i]);
...
#define MARSHAL_BASIC(typename, byte_order, literal)                                                      \
    do {                                                                                                  \
        v_##typename = literal;                                                                           \
        if (!_dbus_marshal_write_basic(&str, pos, DBUS_TYPE_##typename, &v_##typename, byte_order, NULL)) \
...
#define MARSHAL_FIXED_ARRAY(typename, byte_order, literal)                                                           \
    do {                                                                                                             \
        ...
        if (!_dbus_marshal_write_basic(&str, pos, DBUS_TYPE_UINT32, &v_UINT32, byte_order, &next))                   \
            _dbus_assert_not_reached("no memory");                                                                   \
        v_ARRAY_##typename = literal;                                                                                \
        if (!_dbus_marshal_write_fixed_multi(                                                                        \
                &str, next, DBUS_TYPE_##typename, &v_ARRAY_##typename, _DBUS_N_ELEMENTS(literal), byte_order, NULL)) \
            _dbus_assert_not_reached("no memory");                                                                   \
    } while (0)
...
#define DEMARSHAL_FIXED_ARRAY(typename, byte_order)                                                         \
    do {                                                                                                    \
        ...
        _dbus_marshal_read_fixed_multi(                                                                     \
            &str, next, DBUS_TYPE_##typename, &v_ARRAY_##typename, v_UINT32 / alignment, byte_order, NULL); \
...
dbus_bool_t _dbus_marshal_test(void) {
    ...
    dbus_int64_t  array8[3]  = {
        DBUS_INT64_CONSTANT(0x123ffffffff), DBUS_INT64_CONSTANT(0x456ffffffff), DBUS_INT64_CONSTANT(0x789ffffffff)};
    ...
    t_DOUBLE = 3.14;
    if (!_DBUS_DOUBLES_BITWISE_EQUAL(t_DOUBLE, v_DOUBLE)) _dbus_assert_not_reached("got wrong double value");
