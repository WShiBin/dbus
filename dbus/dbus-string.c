...
static void fixup_alignment(DBusRealString* real) {
    ...
    if (old_align_offset != real->align_offset) {
        /* Here comes the suck */
        memmove(real_block + real->align_offset, real_block + old_align_offset, real->len + 1);
    }
...
static dbus_bool_t set_length(DBusRealString* real, int new_length) {
    ...
    if (_DBUS_UNLIKELY(new_length > _DBUS_STRING_MAX_LENGTH))
        ...
        new_length > (real->allocated - _DBUS_STRING_ALLOCATION_PADDING) &&
...
static dbus_bool_t open_gap(int len, DBusRealString* dest, int insert_at) {
    ...
    if (len > _DBUS_STRING_MAX_LENGTH - dest->len) return FALSE; /* detected overflow of dest->len + len below */
...
dbus_bool_t _dbus_string_lengthen(DBusString* str, int additional_length) {
    ...
    if (_DBUS_UNLIKELY(additional_length > _DBUS_STRING_MAX_LENGTH - real->len)) return FALSE; /* would overflow */
...
static dbus_bool_t align_insert_point_then_open_gap(DBusString* str, int* insert_at_p, int alignment, int gap_size) {
    ...
    if (gap_size < delta) {
        memset(&real->str[insert_at], '\0', gap_pos - insert_at);
    }
 ...
 *
 */
dbus_bool_t _dbus_string_replace_len(
 ...
 */
dbus_bool_t _dbus_string_equal_substring(
    const DBusString* a,
    int               a_start,
    int               a_len,
    const DBusString* b,
 ...
 */
dbus_bool_t _dbus_string_hex_decode(
    const DBusString* source,
    int               start,
    int*              end_return,
    DBusString*       dest,
