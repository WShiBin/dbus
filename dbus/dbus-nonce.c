...
static dbus_bool_t do_check_nonce(DBusSocket fd, const DBusString* nonce, DBusError* error) {
    ...
    while (nleft) {
        ...
        if (n == -1 && _dbus_get_is_errno_eintr(saved_errno))
            ...
            dbus_set_error(
                error,
                DBUS_ERROR_IO_ERROR,
                "Could not read nonce from socket (fd=%" DBUS_SOCKET_FORMAT ")",
    ...
    if (!result)
        dbus_set_error(
            ...
            "Nonces do not match, access denied (fd=%" DBUS_SOCKET_FORMAT ")",
            _dbus_socket_printable(fd));
...
dbus_bool_t _dbus_read_nonce(const DBusString* fname, DBusString* nonce, DBusError* error) {
    ...
    if (!nread) {
        dbus_set_error(
            error, DBUS_ERROR_FILE_NOT_FOUND, "Could not read nonce from file %s", _dbus_string_get_const_data(fname));
        return FALSE;
    }
...
static dbus_bool_t do_noncefile_create(DBusNonceFile* noncefile, DBusError* error, dbus_bool_t use_subdir) {
    ...
    if (!generate_and_write_nonce(&noncefile->path, error)) {
        ...
        if (use_subdir)
            _dbus_delete_directory(
                &noncefile->dir,
                NULL);  // we ignore possible errors deleting the dir and return the write error instead
        goto on_error;
    }
    _DBUS_ASSERT_ERROR_IS_CLEAR(error);
    ...
    return TRUE;
on_error:
    if (use_subdir && _dbus_string_get_length(&noncefile->dir) != 0) _dbus_delete_directory(&noncefile->dir, NULL);
