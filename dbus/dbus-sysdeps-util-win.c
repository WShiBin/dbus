 ...
 */
dbus_bool_t _dbus_become_daemon(
    const DBusString* pidfile,
    DBusPipe*         print_pid_pipe,
    DBusError*        error,
    dbus_bool_t       keep_umask) {
    dbus_set_error(error, DBUS_ERROR_NOT_SUPPORTED, "Cannot daemonize on Windows");
    return FALSE;
}
...
static dbus_bool_t _dbus_write_pid_file(const DBusString* filename, unsigned long pid, DBusError* error) {
    ...
    if (hnd == INVALID_HANDLE_VALUE) {
        ...
        dbus_set_error(error, _dbus_win_error_from_last_error(), "Could not create PID file %s: %s", cfilename, emsg);
    ...
    while (total < bytes_to_write) {
        ...
        res = WriteFile(hnd, pidstr + total, bytes_to_write - total, &bytes_written, NULL);

        if (res == 0 || bytes_written <= 0) {
            char* emsg = _dbus_win_error_string(GetLastError());
            dbus_set_error(error, _dbus_win_error_from_last_error(), "Could not write to %s: %s", cfilename, emsg);
            _dbus_win_free_error_string(emsg);
            CloseHandle(hnd);
            return FALSE;
        }
    ...
    if (CloseHandle(hnd) == 0) {
        char* emsg = _dbus_win_error_string(GetLastError());
        dbus_set_error(error, _dbus_win_error_from_last_error(), "Could not close file %s: %s", cfilename, emsg);
        _dbus_win_free_error_string(emsg);
 ...
 */
dbus_bool_t _dbus_write_pid_to_file_and_pipe(
    const DBusString* pidfile,
    DBusPipe*         print_pid_pipe,
    ...
    if (print_pid_pipe != NULL && _dbus_pipe_is_valid(print_pid_pipe)) {
        ...
        if (_dbus_pipe_write(print_pid_pipe, &pid, 0, bytes, error) != bytes) {
            ...
            if (error != NULL && !dbus_error_is_set(error)) {
                dbus_set_error(error, DBUS_ERROR_FAILED, "Printing message bus PID: did not write enough bytes\n");
            }
            _dbus_string_free(&pid);
            return FALSE;
        }
...
dbus_bool_t _dbus_stat(const DBusString* filename, DBusStat* statbuf, DBusError* error) {
    ...
    if (wfad.dwFileAttributes & FILE_ATTRIBUTE_READONLY) statbuf->mode |= _S_IWRITE;

    lastdot = strrchr(filename_c, '.');
    if (lastdot && stricmp(lastdot, ".exe") == 0) statbuf->mode |= _S_IEXEC;
    ...
    statbuf->atime =
        (((dbus_int64_t)wfad.ftLastAccessTime.dwHighDateTime << 32) + wfad.ftLastAccessTime.dwLowDateTime) / 10000000 -
        DBUS_INT64_CONSTANT(116444736000000000);

    statbuf->mtime =
        (((dbus_int64_t)wfad.ftLastWriteTime.dwHighDateTime << 32) + wfad.ftLastWriteTime.dwLowDateTime) / 10000000 -
        DBUS_INT64_CONSTANT(116444736000000000);

    statbuf->ctime =
        (((dbus_int64_t)wfad.ftCreationTime.dwHighDateTime << 32) + wfad.ftCreationTime.dwLowDateTime) / 10000000 -
        DBUS_INT64_CONSTANT(116444736000000000);
...
DBusDirIter* _dbus_directory_open(const DBusString* filename, DBusError* error) {
    ...
    if (_dbus_string_ends_with_c_str(&filespec, "/") || _dbus_string_ends_with_c_str(&filespec, "\\")) {
        if (!_dbus_string_append(&filespec, "*")) {
            dbus_set_error(error, DBUS_ERROR_NO_MEMORY, "Could not append filename wildcard");
            return NULL;
        }
    } else if (!_dbus_string_ends_with_c_str(&filespec, "*")) {
        if (!_dbus_string_append(&filespec, "\\*")) {
            dbus_set_error(error, DBUS_ERROR_NO_MEMORY, "Could not append filename wildcard 2");
            return NULL;
        }
    }
    ...
    if (iter->handle == INVALID_HANDLE_VALUE) {
        if (GetLastError() == ERROR_NO_MORE_FILES)
            ...
            dbus_set_error(
                error,
                _dbus_win_error_from_last_error(),
...
again:
    ...
    if (!iter || iter->finished) return FALSE;
    ...
    if (iter->offset > 0) {
        if (FindNextFileA(iter->handle, &(iter->fileinfo)) == 0) {
            if (GetLastError() == ERROR_NO_MORE_FILES) {
                ...
                char* emsg = _dbus_win_error_string(GetLastError());
                dbus_set_error(error, _dbus_win_error_from_last_error(), "Failed to get next in directory: %s", emsg);
                _dbus_win_free_error_string(emsg);
                return FALSE;
            }
        }
...
dbus_bool_t _dbus_string_get_dirname(const DBusString* filename, DBusString* dirname) {
    ...
    if (sep == 0) return _dbus_string_append(dirname, "."); /* empty string passed in */

    while (sep > 0 &&
           (_dbus_string_get_byte(filename, sep - 1) == '/' || _dbus_string_get_byte(filename, sep - 1) == '\\'))
        --sep;

    _dbus_assert(sep >= 0);

    if (sep == 0 ||
        (sep == 2 && _dbus_string_get_byte(filename, 1) == ':' && isalpha(_dbus_string_get_byte(filename, 0))))
        return _dbus_string_copy_len(filename, 0, sep + 1, dirname, _dbus_string_get_length(dirname));
    ...
    if ((sep == 0 ||
         (sep == 2 && _dbus_string_get_byte(filename, 1) == ':' && isalpha(_dbus_string_get_byte(filename, 0)))) &&
        (_dbus_string_get_byte(filename, sep) == '/' || _dbus_string_get_byte(filename, sep) == '\\'))
        return _dbus_string_copy_len(filename, 0, sep + 1, dirname, _dbus_string_get_length(dirname));
    else
        return _dbus_string_copy_len(filename, 0, sep - 0, dirname, _dbus_string_get_length(dirname));
}
...
dbus_bool_t _dbus_unix_user_is_at_console(dbus_uid_t uid, DBusError* error) {
    dbus_set_error(error, DBUS_ERROR_NOT_SUPPORTED, "UNIX user IDs not supported on Windows\n");
    return FALSE;
}
...
const char* _dbus_lm_strerror(int error_number) {
#ifdef DBUS_WINCE
    // TODO
    return "unknown";
#else
    ...
    switch (error_number) {
        ...
        case NERR_WkstaInconsistentState:
            return "The Workstation service is in an inconsistent state. Restart the computer before restarting the "
                   "Workstation service.";
        ...
        case NERR_PasswordTooShort:
            return "The password does not meet the password policy requirements. Check the minimum password length, "
        ...
        case NERR_NoComputerName:
            return "The computer name could not be added as a message alias. The name may already exist on the "
                   "network.";
...
dbus_bool_t _dbus_replace_install_prefix(DBusString* path) {
    ...
    if (!_dbus_string_init(&runtime_prefix)) return FALSE;
    ...
    if (_dbus_string_starts_with_c_str(path, DBUS_PREFIX "/")) {
        ...
        if (!_dbus_string_replace_len(
                &runtime_prefix, 0, _dbus_string_get_length(&runtime_prefix), path, 0, strlen(DBUS_PREFIX) + 1)) {
            _dbus_string_free(&runtime_prefix);
            return FALSE;
        }
    }
    ...
    for (i = 0; i < _dbus_string_get_length(path); i++) {
        if (_dbus_string_get_byte(path, i) == '\\') _dbus_string_set_byte(path, i, '/');
    }
...
dbus_bool_t _dbus_get_standard_session_servicedirs(DBusList** dirs) {
...
#ifdef DBUS_WINCE
    {
        ...
        if (data_dir != NULL) {
            if (!_dbus_string_append(&servicedir_path, data_dir)) goto oom;

            if (!_dbus_string_append(&servicedir_path, _DBUS_PATH_SEPARATOR)) goto oom;
        }
    }
#else
    {
        ...
        if (!_dbus_string_init(&p)) goto oom;

        /* DBUS_DATADIR is assumed to be absolute; the build systems should
         * ensure that. */
        if (!_dbus_string_append(&p, DBUS_DATADIR) || !_dbus_replace_install_prefix(&p)) {
            _dbus_string_free(&p);
            goto oom;
        }

        if (!_dbus_string_append(&servicedir_path, _dbus_string_get_const_data(&p))) {
            _dbus_string_free(&p);
            goto oom;
        }
    ...
    if (common_progs != NULL) {
        if (!_dbus_string_append(&servicedir_path, common_progs)) goto oom;

        if (!_dbus_string_append(&servicedir_path, _DBUS_PATH_SEPARATOR)) goto oom;
    }

    if (!_dbus_split_paths_and_append(&servicedir_path, DBUS_STANDARD_SESSION_SERVICEDIR, dirs)) goto oom;
...
static dbus_bool_t _dbus_get_config_file_name(DBusString* str, const char* basename) {
    DBusString tmp;

    if (!_dbus_string_append(str, DBUS_DATADIR) || !_dbus_replace_install_prefix(str)) return FALSE;

    _dbus_string_init_const(&tmp, "dbus-1");

    if (!_dbus_concat_dir_and_file(str, &tmp)) return FALSE;
