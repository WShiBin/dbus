...
static void print_validities_seen(dbus_bool_t not_seen) {
    ...
    while (i < _DBUS_N_ELEMENTS(validities_seen)) {
        if ((i - _DBUS_NEGATIVE_VALIDITY_COUNT) == DBUS_VALIDITY_UNKNOWN ||
            ...
            printf("validity %3d seen %d times\n", i - _DBUS_NEGATIVE_VALIDITY_COUNT, validities_seen[i]);
        ++i;
    }
}
...
static void check_memleaks(void) {
    ...
    if (_dbus_get_malloc_blocks_outstanding() != 0) {
        _dbus_warn("%d dbus_malloc blocks were not freed in %s", _dbus_get_malloc_blocks_outstanding(), __FILE__);
...
static dbus_bool_t check_have_valid_message(DBusMessageLoader* loader) {
    ...
    if (_dbus_message_loader_get_is_corrupted(loader)) {
        _dbus_warn(
            "loader corrupted on message that was expected to be valid; invalid reason %d", loader->corruption_reason);
        goto failed;
    }
...
static dbus_bool_t check_loader_results(DBusMessageLoader* loader, DBusValidity expected_validity) {
    if (!_dbus_message_loader_queue_messages(loader)) _dbus_assert_not_reached("no memory to queue messages");
...
dbus_bool_t dbus_internal_do_not_use_try_message_file(const DBusString* filename, DBusValidity expected_validity) {
    ...
    if (!_dbus_string_init(&data)) _dbus_assert_not_reached("could not allocate string");
...
static dbus_bool_t process_test_subdir(
    ...
    if (!_dbus_string_init(&test_directory)) _dbus_assert_not_reached("didn't allocate test_directory");

    _dbus_string_init_const(&filename, subdir);

    if (!_dbus_string_copy(test_base_dir, 0, &test_directory, 0))
        _dbus_assert_not_reached("couldn't copy test_base_dir to test_directory");

    if (!_dbus_concat_dir_and_file(&test_directory, &filename)) _dbus_assert_not_reached("couldn't allocate full path");

    _dbus_string_free(&filename);
    if (!_dbus_string_init(&filename)) _dbus_assert_not_reached("didn't allocate filename string");
...
next:
    while (_dbus_directory_get_next_file(dir, &filename, &error)) {
        ...
        if (!_dbus_string_init(&full_path)) _dbus_assert_not_reached("couldn't init string");

        if (!_dbus_string_copy(&test_directory, 0, &full_path, 0))
            _dbus_assert_not_reached("couldn't copy dir to full_path");

        if (!_dbus_concat_dir_and_file(&full_path, &filename)) _dbus_assert_not_reached("couldn't concat file to dir");
...
dbus_bool_t dbus_internal_do_not_use_foreach_message_file(
    ...
    if (!process_test_subdir(&test_directory, "valid-messages", DBUS_VALID, func, user_data)) goto failed;

    check_memleaks();

    if (!process_test_subdir(&test_directory, "invalid-messages", DBUS_INVALID_FOR_UNKNOWN_REASON, func, user_data))
        goto failed;

    check_memleaks();

    if (!process_test_subdir(&test_directory, "incomplete-messages", DBUS_VALID_BUT_INCOMPLETE, func, user_data))
        goto failed;
...
#define GET_AND_CHECK(iter, typename, literal)                                                      \
    do {                                                                                            \
        ...
        if (v_##typename != literal) _dbus_assert_not_reached("got wrong value from message iter"); \
    } while (0)
...
#define GET_AND_CHECK_STRCMP(iter, typename, literal)                                                          \
    do {                                                                                                       \
        if (dbus_message_iter_get_arg_type(&iter) != DBUS_TYPE_##typename)                                     \
            _dbus_assert_not_reached("got wrong argument type from message iter");                             \
        dbus_message_iter_get_basic(&iter, &v_##typename);                                                     \
        if (strcmp(v_##typename, literal) != 0) _dbus_assert_not_reached("got wrong value from message iter"); \
    } while (0)

#define GET_AND_CHECK_AND_NEXT(iter, typename, literal)                                              \
    do {                                                                                             \
        GET_AND_CHECK(iter, typename, literal);                                                      \
        if (!dbus_message_iter_next(&iter)) _dbus_assert_not_reached("failed to move iter to next"); \
    } while (0)
...
static void verify_test_message(DBusMessage* message) {
    ...
    if (our_int16 != -0x123) _dbus_assert_not_reached("16-bit integers differ!");

    if (our_uint16 != 0x123) _dbus_assert_not_reached("16-bit uints differ!");

    if (our_int != -0x12345678) _dbus_assert_not_reached("integers differ!");

    if (our_uint != 0x12300042) _dbus_assert_not_reached("uints differ!");

    if (our_int64 != DBUS_INT64_CONSTANT(-0x123456789abcd)) _dbus_assert_not_reached("64-bit integers differ!");
    if (our_uint64 != DBUS_UINT64_CONSTANT(0x123456789abcd))
        _dbus_assert_not_reached("64-bit unsigned integers differ!");

    v_DOUBLE = 3.14159;
    if (!_DBUS_DOUBLES_BITWISE_EQUAL(our_double, v_DOUBLE)) _dbus_assert_not_reached("doubles differ!");

    if (strcmp(our_str, "Test string") != 0) _dbus_assert_not_reached("strings differ!");

    if (!our_bool) _dbus_assert_not_reached("booleans differ");

    if (our_byte_1 != 42) _dbus_assert_not_reached("bytes differ!");

    if (our_byte_2 != 24) _dbus_assert_not_reached("bytes differ!");

    if (our_uint32_array_len != 4 || our_uint32_array[0] != 0x12345678 || our_uint32_array[1] != 0x23456781 ||
        our_uint32_array[2] != 0x34567812 || our_uint32_array[3] != 0x45678123)
        _dbus_assert_not_reached("uint array differs");

    if (our_int32_array_len != 4 || our_int32_array[0] != 0x12345678 || our_int32_array[1] != -0x23456781 ||
        our_int32_array[2] != 0x34567812 || our_int32_array[3] != -0x45678123)
        _dbus_assert_not_reached("int array differs");

    if (our_uint64_array_len != 4 || our_uint64_array[0] != 0x12345678 || our_uint64_array[1] != 0x23456781 ||
        our_uint64_array[2] != 0x34567812 || our_uint64_array[3] != 0x45678123)
        _dbus_assert_not_reached("uint64 array differs");

    if (our_int64_array_len != 4 || our_int64_array[0] != 0x12345678 || our_int64_array[1] != -0x23456781 ||
        our_int64_array[2] != 0x34567812 || our_int64_array[3] != -0x45678123)
        _dbus_assert_not_reached("int64 array differs");

    if (our_double_array_len != 3) _dbus_assert_not_reached("double array had wrong length");
    ...
    if (our_byte_array_len != 4) _dbus_assert_not_reached("byte array had wrong length");
    ...
    if (our_boolean_array_len != 5) _dbus_assert_not_reached("bool array had wrong length");

    if (our_boolean_array[0] != TRUE || our_boolean_array[1] != FALSE || our_boolean_array[2] != TRUE ||
        our_boolean_array[3] != TRUE || our_boolean_array[4] != FALSE)
        _dbus_assert_not_reached("bool array had wrong values");

    if (our_string_array_len != 4) _dbus_assert_not_reached("string array was wrong length");

    if (strcmp(our_string_array[0], "Foo") != 0 || strcmp(our_string_array[1], "bar") != 0 ||
        strcmp(our_string_array[2], "") != 0 || strcmp(our_string_array[3], "woo woo woo woo") != 0)
        _dbus_assert_not_reached("string array had wrong values");

    dbus_free_string_array(our_string_array);

    if (dbus_message_iter_next(&iter)) _dbus_assert_not_reached("Didn't reach end of arguments");
}
...
dbus_bool_t _dbus_message_test(const char* test_data_dir) {
    ...
    DBusInitialFDs* initial_fds;
    dbus_bool_t     ok;
    char            basic_types[] = DBUS_TYPE_BYTE_AS_STRING DBUS_TYPE_BOOLEAN_AS_STRING DBUS_TYPE_INT16_AS_STRING
        DBUS_TYPE_INT32_AS_STRING DBUS_TYPE_INT64_AS_STRING DBUS_TYPE_UINT16_AS_STRING DBUS_TYPE_UINT32_AS_STRING
            DBUS_TYPE_UINT64_AS_STRING DBUS_TYPE_DOUBLE_AS_STRING DBUS_TYPE_STRING_AS_STRING;

    initial_fds = _dbus_check_fdleaks_enter();

    message = dbus_message_new_method_call(
        "org.freedesktop.DBus.TestService", "/org/freedesktop/TestPath", "Foo.TestInterface", "TestMethod");
    ...
    if (!dbus_message_set_sender(message, "org.foo.bar1")) _dbus_assert_not_reached("out of memory");
    ...
    if (!dbus_message_set_sender(message, NULL)) _dbus_assert_not_reached("out of memory");
    ...
    if (!dbus_message_set_path(message, "/foo")) _dbus_assert_not_reached("out of memory");
    _dbus_assert(strcmp(dbus_message_get_path(message), "/foo") == 0);

    if (!dbus_message_set_interface(message, "org.Foo")) _dbus_assert_not_reached("out of memory");
    _dbus_assert(strcmp(dbus_message_get_interface(message), "org.Foo") == 0);

    if (!dbus_message_set_member(message, "Bar")) _dbus_assert_not_reached("out of memory");
    _dbus_assert(strcmp(dbus_message_get_member(message), "Bar") == 0);

    /* Set/get them with longer values */
    if (!dbus_message_set_path(message, "/foo/bar")) _dbus_assert_not_reached("out of memory");
    _dbus_assert(strcmp(dbus_message_get_path(message), "/foo/bar") == 0);

    if (!dbus_message_set_interface(message, "org.Foo.Bar")) _dbus_assert_not_reached("out of memory");
    _dbus_assert(strcmp(dbus_message_get_interface(message), "org.Foo.Bar") == 0);

    if (!dbus_message_set_member(message, "BarFoo")) _dbus_assert_not_reached("out of memory");
    ...
    if (_dbus_message_loader_get_is_corrupted(loader)) _dbus_assert_not_reached("message loader corrupted");

    message = _dbus_message_loader_pop_message(loader);
    if (!message) _dbus_assert_not_reached("received a NULL message");

    if (dbus_message_get_reply_serial(message) != 5678) _dbus_assert_not_reached("reply serial fields differ");
    ...
    dbus_message_iter_open_container(
        &iter,
        DBUS_TYPE_ARRAY,
        DBUS_STRUCT_BEGIN_CHAR_AS_STRING DBUS_TYPE_STRING_AS_STRING DBUS_STRUCT_END_CHAR_AS_STRING,
        &array_iter);
    ...
    message = dbus_message_new_method_call(
        "org.freedesktop.DBus.TestService", "/org/freedesktop/TestPath", "Foo.TestInterface", "Method");
    ...
    ok = dbus_message_iter_open_container(
        &iter,
        DBUS_TYPE_ARRAY,
        (DBUS_STRUCT_BEGIN_CHAR_AS_STRING DBUS_TYPE_STRING_AS_STRING DBUS_TYPE_STRING_AS_STRING
    ...
    /* Load all the sample messages from the message factory */
    {
        ...
        while (_dbus_message_data_iter_get_and_next(&diter, &mdata)) {
            if (!dbus_internal_do_not_use_try_message_data(&mdata.data, mdata.expected_validity)) {
                _dbus_warn("expected validity %d and did not get it", mdata.expected_validity);
    ...
    if (!dbus_internal_do_not_use_foreach_message_file(
            test_data_dir, (DBusForeachMessageFileFunc)dbus_internal_do_not_use_try_message_file, NULL))
