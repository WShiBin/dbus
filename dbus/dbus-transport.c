...
static DBusTransport* check_address(const char* address, DBusError* error) {
    ...
    if (!dbus_parse_address(address, &entries, &len, error)) return NULL; /* not a valid address */
...
static DBusTransportOpenResult _dbus_transport_open_autolaunch(
...
static dbus_bool_t auth_via_unix_user_function(DBusTransport* transport) {
    ...
    if (allow) {
        ...
        _dbus_verbose(
            "Client UID " DBUS_UID_FORMAT " was rejected, disconnecting\n",
...
static dbus_bool_t auth_via_default_rules(DBusTransport* transport) {
    ...
    if (transport->allow_anonymous || _dbus_credentials_get_unix_uid(auth_identity) == 0 ||
        ...
        if (_dbus_credentials_include(our_identity, DBUS_CREDENTIAL_WINDOWS_SID))
            ...
            _dbus_verbose(
                "Client authorized as UID " DBUS_UID_FORMAT " matching our UID " DBUS_UID_FORMAT "\n",
                _dbus_credentials_get_unix_uid(auth_identity),
        ...
        if (_dbus_credentials_include(our_identity, DBUS_CREDENTIAL_WINDOWS_SID))
            _dbus_verbose(
                ...
                (_dbus_credentials_get_windows_sid(auth_identity) ? _dbus_credentials_get_windows_sid(auth_identity)
                                                                  : "<null>"),
                (_dbus_credentials_get_windows_sid(our_identity) ? _dbus_credentials_get_windows_sid(our_identity)
                                                                 : "<null>"));
        else
            _dbus_verbose(
                "Client authorized as UID " DBUS_UID_FORMAT " but our UID is " DBUS_UID_FORMAT ", disconnecting\n",
                _dbus_credentials_get_unix_uid(auth_identity),
                _dbus_credentials_get_unix_uid(our_identity));
        _dbus_transport_disconnect(transport);
        allow = FALSE;
...
void _dbus_transport_do_iteration(DBusTransport* transport, unsigned int flags, int timeout_milliseconds) {
    ...
    if ((flags & (DBUS_ITERATION_DO_WRITING | DBUS_ITERATION_DO_READING)) == 0) return; /* Nothing to do */
...
DBusDispatchStatus _dbus_transport_get_dispatch_status(DBusTransport* transport) {
    ...
    if (!_dbus_transport_try_to_authenticate(transport)) {
        if (_dbus_auth_do_work(transport->auth) == DBUS_AUTH_STATE_WAITING_FOR_MEMORY)
...
void _dbus_transport_set_windows_user_function(
    DBusTransport*               transport,
...
void _dbus_transport_get_stats(
    DBusTransport* transport,
