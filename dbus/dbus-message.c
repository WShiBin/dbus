...
#define _DBUS_TYPE_IS_STRINGLIKE(type) \
    (type == DBUS_TYPE_STRING || type == DBUS_TYPE_SIGNATURE || type == DBUS_TYPE_OBJECT_PATH)
    ...
    if (enabled < 0) {
        ...
        if (s && *s) {
            if (*s == '0')
                ...
                _dbus_warn("DBUS_MESSAGE_CACHE should be 0 or 1 if set, not '%s'", s);
        }
    }
...
static void get_const_signature(DBusHeader* header, const DBusString** type_str_p, int* type_pos_p) {
    if (_dbus_header_get_field_raw(header, DBUS_HEADER_FIELD_SIGNATURE, type_str_p, type_pos_p)) {
...
void _dbus_message_add_counter_link(DBusMessage* message, DBusList* link) {
    ...
    if (message->counters == NULL) {
        message->size_counter_delta =
            _dbus_string_get_length(&message->header.data) + _dbus_string_get_length(&message->body);
...
static dbus_bool_t set_or_delete_string_field(DBusMessage* message, int field, int typecode, const char* value) {
    if (value == NULL)
        ...
        return _dbus_header_set_field_basic(&message->header, field, typecode, &value);
}
...
static void dbus_message_cache_shutdown(void* data) {
    ...
    if (!_DBUS_LOCK(message_cache))
        _dbus_assert_not_reached(
            "we would have initialized global locks "
...
static void dbus_message_cache_or_finalize(DBusMessage* message) {
    ...
    if ((_dbus_string_get_length(&message->header.data) + _dbus_string_get_length(&message->body)) >
...
static dbus_bool_t _dbus_message_real_iter_is_zeroed(DBusMessageRealIter* iter) {
    return (
        iter != NULL && iter->message == NULL && iter->changed_stamp == 0 && iter->iter_type == 0 &&
        iter->sig_refcount == 0);
}
...
static dbus_bool_t _dbus_message_iter_check(DBusMessageRealIter* iter) {
    ...
    if (iter->message == NULL || iter->iter_type == 0) {
        _dbus_warn_check_failed(
            "dbus message iterator has already been "
    ...
    if (iter->changed_stamp != iter->message->changed_stamp) {
        _dbus_warn_check_failed(
            "dbus message iterator invalid because the message has been modified (or perhaps the iterator is just "
            "uninitialized)");
        return FALSE;
    }
...
DBusMessage* dbus_message_new_signal(const char* path, const char* iface, const char* name) {
    ...
    if (!_dbus_header_create(
            &message->header, DBUS_COMPILER_BYTE_ORDER, DBUS_MESSAGE_TYPE_SIGNAL, NULL, path, iface, name, NULL)) {
        dbus_message_unref(message);
        return NULL;
    }
...
DBusMessage* dbus_message_new_error_printf(
    ...
    if (_dbus_string_append_printf_valist(&str, error_format, args))
        message = dbus_message_new_error(reply_to, error_name, _dbus_string_get_const_data(&str));
    else
        message = NULL;
 ...
 *
 * @param message the message
 * @returns the new message.or #NULL if not enough memory or Unix file descriptors (in case the message to copy includes
 * Unix file descriptors) can be allocated.
 */
...
dbus_bool_t dbus_message_iter_init(DBusMessage* message, DBusMessageIter* iter) {
    ...
    _dbus_type_reader_init(
        &real->u.reader, _dbus_header_get_byte_order(&message->header), type_str, type_pos, &message->body, 0);
...
void dbus_message_iter_get_basic(DBusMessageIter* iter, void* value) {
    ...
    if (dbus_message_iter_get_arg_type(iter) == DBUS_TYPE_UNIX_FD) {
        ...
        *((int*)value) = -1;
        ...
        _dbus_type_reader_read_basic(&real->u.reader, value);
    }
}
...
static dbus_bool_t _dbus_message_iter_open_signature(DBusMessageRealIter* real) {
    ...
    if (!_dbus_header_get_field_raw(
            &real->message->header, DBUS_HEADER_FIELD_SIGNATURE, &current_sig, &current_sig_pos))
        current_sig = NULL;
...
static dbus_bool_t _dbus_message_iter_close_signature(DBusMessageRealIter* real) {
    ...
    if (!_dbus_header_set_field_basic(
            &real->message->header, DBUS_HEADER_FIELD_SIGNATURE, DBUS_TYPE_SIGNATURE, &v_STRING))
        retval = FALSE;
...
dbus_bool_t dbus_message_iter_open_container(
    ...
    _dbus_return_val_if_fail(
        (type == DBUS_TYPE_STRUCT && contained_signature == NULL) ||
            (type == DBUS_TYPE_DICT_ENTRY && contained_signature == NULL) ||
            (type == DBUS_TYPE_VARIANT && contained_signature != NULL) ||
            (type == DBUS_TYPE_ARRAY && contained_signature != NULL),
        FALSE);
    ...
    if (contained_signature != NULL) {
        ...
        contained_signature_validity =
            _dbus_validate_signature_with_reason(&contained_str, 0, _dbus_string_get_length(&contained_str));
    ...
    _dbus_return_val_if_fail(
        (type == DBUS_TYPE_ARRAY && contained_signature && *contained_signature == DBUS_DICT_ENTRY_BEGIN_CHAR) ||
            contained_signature == NULL || contained_signature_validity == DBUS_VALID,
        FALSE);
    ...
    *real_sub = *real;
    ...
    if (contained_signature != NULL) {
        ...
        ret = _dbus_type_writer_recurse(&real->u.writer, type, NULL, 0, &real_sub->u.writer);
    }
 ...
 */
static dbus_bool_t load_message(
    DBusMessageLoader* loader,
    DBusMessage*       message,
    int                byte_order,
    int                fields_array_len,
  ...
  _dbus_verbose_bytes_of_string (&loader->data, 0, header_len /* + body_len */);
    ...
    if (mode != DBUS_VALIDATION_MODE_WE_TRUST_THIS_DATA_ABSOLUTELY) {
         ...
         */
        validity =
            _dbus_validate_body_with_reason(type_str, type_pos, byte_order, NULL, &loader->data, header_len, body_len);
...
dbus_bool_t dbus_message_set_data(
    ...
    retval = _dbus_data_slot_list_set(
        &slot_allocator, &message->slot_list, slot, data, free_data_func, &old_free_func, &old_data);
 ...
 * @returns -1 if there was no valid data to be demarshalled, 0 if there wasn't enough data to determine how much should
 * be demarshalled. Otherwise returns the number of bytes to be demarshalled
 *
 */
int dbus_message_demarshal_bytes_needed(const char* buf, int len) {
    ...
    validity     = DBUS_VALID;
    have_message = _dbus_header_have_message_untrusted(
        DBUS_MAXIMUM_MESSAGE_LENGTH, &validity, &byte_order, &fields_array_len, &header_len, &body_len, &str, 0, len);
    _dbus_string_free(&str);
...
DBusVariant* _dbus_variant_read(DBusMessageIter* reader) {
    ...
    if (!_dbus_type_writer_recurse(&items_writer, DBUS_TYPE_VARIANT, &contained_signature, 0, &variant_writer))
        goto oom;
