...
static dbus_bool_t _dbus_type_reader_greater_than(const DBusTypeReader* lhs, const DBusTypeReader* rhs);

static void        _dbus_type_writer_set_enabled(DBusTypeWriter* writer, dbus_bool_t enabled);
...
static void free_fixups(DBusList** fixups) {
    DBusList* link;
...
static void apply_and_free_fixups(DBusList** fixups, DBusTypeReader* reader) {
    DBusList* link;
    ...
    while (link != NULL) {
        ...
        if (reader) {
            ...
            _dbus_verbose(
                ...
                f->len_pos_in_reader,
                f->new_len,
                _dbus_marshal_read_uint32(reader->value_str, f->len_pos_in_reader, reader->byte_order, NULL));
#endif

            _dbus_marshal_set_uint32(
                (DBusString*)reader->value_str, f->len_pos_in_reader, f->new_len, reader->byte_order);
        }
...
struct DBusTypeReaderClass {
    ...
    void (*recurse)(DBusTypeReader* sub, DBusTypeReader* parent); /**< recurse with this reader as sub */
    dbus_bool_t (*check_finished)(const DBusTypeReader* reader);  /**< check whether reader is at the end */
    void (*next)(DBusTypeReader* reader, int current_type);       /**< go to the next value */
};

static int element_type_get_alignment(const DBusString* str, int pos) {
...
static void base_reader_recurse(DBusTypeReader* sub, DBusTypeReader* parent) {
    /* point subreader at the same place as parent */
    reader_init(sub, parent->byte_order, parent->type_str, parent->type_pos, parent->value_str, parent->value_pos);
}

static void struct_or_dict_entry_types_only_reader_recurse(DBusTypeReader* sub, DBusTypeReader* parent) {
    base_reader_recurse(sub, parent);

    _dbus_assert(
        _dbus_string_get_byte(sub->type_str, sub->type_pos) == DBUS_STRUCT_BEGIN_CHAR ||
        _dbus_string_get_byte(sub->type_str, sub->type_pos) == DBUS_DICT_ENTRY_BEGIN_CHAR);

    sub->type_pos += 1;
}

static void struct_or_dict_entry_reader_recurse(DBusTypeReader* sub, DBusTypeReader* parent) {
...
static void array_types_only_reader_recurse(DBusTypeReader* sub, DBusTypeReader* parent) {
    base_reader_recurse(sub, parent);
...
static int array_reader_get_array_len(const DBusTypeReader* reader) {
    ...
    array_len =
        _dbus_unpack_uint32(reader->byte_order, _dbus_string_get_const_udata_len(reader->value_str, len_pos, 4));
...
static void array_reader_recurse(DBusTypeReader* sub, DBusTypeReader* parent) {
    int alignment;
    int len_pos;
    ...
    _dbus_verbose(
        ...
        _dbus_type_to_string(_dbus_first_type_in_signature(sub->type_str, sub->type_pos)));
#endif
}

static void variant_reader_recurse(DBusTypeReader* sub, DBusTypeReader* parent) {
    int sig_len;
    ...
    contained_alignment = _dbus_type_get_alignment(_dbus_first_type_in_signature(sub->type_str, sub->type_pos));
...
static dbus_bool_t array_reader_check_finished(const DBusTypeReader* reader) {
    int end_pos;
...
static void skip_one_complete_type(const DBusString* type_str, int* type_pos) {
    _dbus_type_signature_next(_dbus_string_get_const_data(type_str), type_pos);
}
...
static int find_len_of_complete_type(const DBusString* type_str, int type_pos) {
    int end;
...
static void base_reader_next(DBusTypeReader* reader, int current_type) {
    switch (current_type) {
        ...
        case DBUS_TYPE_ARRAY: {
            if (!reader->klass->types_only)
                _dbus_marshal_skip_array(
                    reader->value_str,
                    _dbus_first_type_in_signature(reader->type_str, reader->type_pos + 1),
                    reader->byte_order,
                    &reader->value_pos);
        ...
        default:
            if (!reader->klass->types_only)
                _dbus_marshal_skip_basic(reader->value_str, current_type, reader->byte_order, &reader->value_pos);
...
static void struct_reader_next(DBusTypeReader* reader, int current_type) {
    int t;
...
static void dict_entry_reader_next(DBusTypeReader* reader, int current_type) {
    int t;
...
static void array_types_only_reader_next(DBusTypeReader* reader, int current_type) {
...
static void array_reader_next(DBusTypeReader* reader, int current_type) {
    ...
    switch (_dbus_first_type_in_signature(reader->type_str, reader->type_pos)) {
        case DBUS_TYPE_DICT_ENTRY:
        case DBUS_TYPE_STRUCT:
        case DBUS_TYPE_VARIANT: {
            DBusTypeReader sub;
...
static const DBusTypeReaderClass struct_reader_class =
    {"struct", 2, FALSE, struct_or_dict_entry_reader_recurse, NULL, struct_reader_next};

static const DBusTypeReaderClass struct_types_only_reader_class =
    {"struct types", 3, TRUE, struct_or_dict_entry_types_only_reader_recurse, NULL, struct_reader_next};

static const DBusTypeReaderClass dict_entry_reader_class =
    {"dict_entry", 4, FALSE, struct_or_dict_entry_reader_recurse, NULL, dict_entry_reader_next};

static const DBusTypeReaderClass dict_entry_types_only_reader_class =
    {"dict_entry types", 5, TRUE, struct_or_dict_entry_types_only_reader_recurse, NULL, dict_entry_reader_next};

static const DBusTypeReaderClass array_reader_class =
    {"array", 6, FALSE, array_reader_recurse, array_reader_check_finished, array_reader_next};

static const DBusTypeReaderClass array_types_only_reader_class =
    {"array types", 7, TRUE, array_types_only_reader_recurse, NULL, array_types_only_reader_next};

static const DBusTypeReaderClass variant_reader_class =
    {"variant", 8, FALSE, variant_reader_recurse, NULL, base_reader_next};

#ifndef DBUS_DISABLE_ASSERT
static const DBusTypeReaderClass* const all_reader_classes[] = {
    &body_reader_class,
...
void _dbus_type_reader_init(
    ...
    const DBusString* value_str,
    int               value_pos) {
    reader_init(reader, byte_order, type_str, type_pos, value_str, value_pos);
...
void _dbus_type_reader_init_types_only(DBusTypeReader* reader, const DBusString* type_str, int type_pos) {
    reader_init(
        ...
        type_pos,
        NULL,
        _DBUS_INT_MAX /* crashes if we screw up */);
...
int _dbus_type_reader_get_current_type(const DBusTypeReader* reader) {
    ...
    if (reader->finished || (reader->klass->check_finished && (*reader->klass->check_finished)(reader)))
        t = DBUS_TYPE_INVALID;
    else
...
int _dbus_type_reader_get_element_type(const DBusTypeReader* reader) {
    ...
    element_type = _dbus_first_type_in_signature(reader->type_str, reader->type_pos + 1);
...
void _dbus_type_reader_read_raw(const DBusTypeReader* reader, const unsigned char** value_location) {
    ...
    *value_location = _dbus_string_get_const_udata_len(reader->value_str, reader->value_pos, 0);
}
...
void _dbus_type_reader_read_basic(const DBusTypeReader* reader, void* value) {
    ...
    _dbus_marshal_read_basic(reader->value_str, reader->value_pos, t, value, reader->byte_order, NULL);
...
void _dbus_type_reader_read_fixed_multi(const DBusTypeReader* reader, void* value, int* n_elements) {
    ...
    element_type = _dbus_first_type_in_signature(reader->type_str, reader->type_pos);
    ...
    if (remaining_len == 0)
        ...
        *(const DBusBasicValue**)value =
            (void*)_dbus_string_get_const_data_len(reader->value_str, reader->value_pos, remaining_len);
...
dbus_bool_t _dbus_type_reader_next(DBusTypeReader* reader) {
    ...
    if (t == DBUS_TYPE_INVALID) return FALSE;
...
static dbus_bool_t replacement_block_init(ReplacementBlock* block, DBusTypeReader* reader) {
    if (!_dbus_string_init(&block->replacement)) return FALSE;
    ...
    if (!_dbus_string_lengthen(&block->replacement, block->padding)) goto oom;
...
static dbus_bool_t replacement_block_replace(
...
#endif
    fixups = NULL;
    if (!_dbus_type_writer_write_reader_partial(
            &writer,
            &realign_reader,
            reader,
            block->padding,
    ...
    _dbus_verbose_bytes_of_string(
        &block->replacement, block->padding, _dbus_string_get_length(&block->replacement) - block->padding);
    _dbus_verbose(
        ...
        realign_reader.value_pos);
    _dbus_verbose_bytes_of_string(reader->value_str, reader->value_pos, realign_reader.value_pos - reader->value_pos);
#endif
...
static void replacement_block_free(ReplacementBlock* block) {
    _dbus_string_free(&block->replacement);
}
 ...
 */
static dbus_bool_t reader_set_basic_variable_length(
    DBusTypeReader*       reader,
    int                   current_type,
    const void*           value,
    const DBusTypeReader* realign_root) {
    ...
    if (!replacement_block_init(&block, reader)) return FALSE;
...
#endif
    if (!_dbus_type_writer_write_basic(&writer, current_type, value)) goto out;

    if (!replacement_block_replace(&block, reader, realign_root)) goto out;
...
static void reader_set_basic_fixed_length(DBusTypeReader* reader, int current_type, const void* value) {
    _dbus_marshal_set_basic(
        (DBusString*)reader->value_str, reader->value_pos, current_type, value, reader->byte_order, NULL, NULL);
}
...
dbus_bool_t _dbus_type_reader_set_basic(DBusTypeReader* reader, const void* value, const DBusTypeReader* realign_root) {
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
    _dbus_verbose(
        "  SET BASIC type reader %p type_pos = %d value_pos = %d remaining sig '%s' realign_root = %p with value_pos "
        "%d current_type = %s\n",
        reader,
        reader->type_pos,
        reader->value_pos,
        ...
        _dbus_type_to_string(current_type));
    _dbus_verbose_bytes_of_string(
        realign_root->value_str,
        realign_root->value_pos,
        _dbus_string_get_length(realign_root->value_str) - realign_root->value_pos);
#endif
    ...
    if (dbus_type_is_fixed(current_type)) {
        ...
        _dbus_assert(realign_root != NULL);
        return reader_set_basic_variable_length(reader, current_type, value, realign_root);
    }
}
...
void _dbus_type_writer_init(
    ...
    _dbus_verbose(
        ...
        writer,
        writer->type_str ? _dbus_string_get_const_data_len(writer->type_str, writer->type_pos, 0) : "unknown");
#endif
}
...
void _dbus_type_writer_init_types_delayed(
    ...
    DBusString*     value_str,
    int             value_pos) {
    _dbus_type_writer_init(writer, byte_order, NULL, 0, value_str, value_pos);
}
...
void _dbus_type_writer_init_values_only(
    ...
    DBusString*       value_str,
    int               value_pos) {
    _dbus_type_writer_init(writer, byte_order, (DBusString*)type_str, type_pos, value_str, value_pos);

    writer->type_pos_is_expectation = TRUE;
}

static dbus_bool_t _dbus_type_writer_write_basic_no_typecode(DBusTypeWriter* writer, int type, const void* value) {
    if (writer->enabled)
        return _dbus_marshal_write_basic(
            writer->value_str, writer->value_pos, type, value, writer->byte_order, &writer->value_pos);
    else
        return TRUE;
}
 ...
 */
static void writer_recurse_init_and_check(DBusTypeWriter* writer, int container_type, DBusTypeWriter* sub) {
    _dbus_type_writer_init(
        sub, writer->byte_order, writer->type_str, writer->type_pos, writer->value_str, writer->value_pos);
        ...
        if (expected != sub->container_type) {
            if (expected != DBUS_TYPE_INVALID)
                _dbus_warn_check_failed(
                    ...
                    _dbus_string_get_const_data(writer->type_str),
                    writer->type_pos);
            else
                _dbus_warn_check_failed(
...
static dbus_bool_t write_or_verify_typecode(DBusTypeWriter* writer, int typecode) {
    ...
    if (writer->type_str == NULL) return TRUE;
    ...
    if (writer->type_pos_is_expectation) {
#ifndef DBUS_DISABLE_CHECKS
        {
            ...
            if (expected != typecode) {
                if (expected != DBUS_TYPE_INVALID)
                    ...
                    _dbus_warn_check_failed(
                        "Array or variant type wasn't expecting any more values to be written into it, but a value %s "
                        "was written.\n"
                        "The overall signature expected here was '%s' and we are on byte %d of that signature.",
                        _dbus_type_to_string(typecode),
                        _dbus_string_get_const_data(writer->type_str),
         ...
         */
        if (writer->container_type != DBUS_TYPE_ARRAY) writer->type_pos += 1;
    } else {
        if (!_dbus_string_insert_byte(writer->type_str, writer->type_pos, typecode)) return FALSE;
...
static dbus_bool_t writer_recurse_struct_or_dict_entry(
    DBusTypeWriter*   writer,
    int               begin_char,
    ...
    if (writer->enabled) {
        if (!_dbus_string_alloc_space(sub->value_str, 8)) return FALSE;
    }
    ...
    if (writer->enabled) {
        if (!_dbus_string_insert_bytes(
                sub->value_str, sub->value_pos, _DBUS_ALIGN_VALUE(sub->value_pos, 8) - sub->value_pos, '\0'))
...
static dbus_bool_t writer_recurse_array(
    DBusTypeWriter*   writer,
    const DBusString* contained_type,
    int               contained_type_start,
    int               contained_type_len,
...
#ifndef DBUS_DISABLE_CHECKS
    if (writer->container_type == DBUS_TYPE_ARRAY && writer->type_str) {
        if (!_dbus_string_equal_substring(
            ...
            _dbus_warn_check_failed(
                "Writing an array of '%s' but this is incompatible with the expected type of elements in the parent "
                "array",
                _dbus_string_get_const_data_len(contained_type, contained_type_start, contained_type_len));
    ...
    if (writer->enabled && !is_array_append) {
         ...
         * before array values
         */
        if (!_dbus_string_alloc_space(sub->value_str, 3 + 4 + 4)) return FALSE;
    }
    ...
    if (!writer->type_pos_is_expectation) {
        ...
        if (!_dbus_string_alloc_space(writer->type_str, 1 + contained_type_len)) return FALSE;

        if (!_dbus_string_insert_byte(writer->type_str, writer->type_pos, DBUS_TYPE_ARRAY))
            _dbus_assert_not_reached("failed to insert array typecode after prealloc");

        if (!_dbus_string_copy_len(
                contained_type, contained_type_start, contained_type_len, sub->type_str, sub->u.array.element_type_pos))
    ...
    if (writer->enabled) {
        ...
        if (aligned != sub->value_pos) {
            if (!is_array_append) {
                if (!_dbus_string_insert_bytes(sub->value_str, sub->value_pos, aligned - sub->value_pos, '\0'))
        ...
        if (is_array_append) {
            ...
            _dbus_assert(_DBUS_ALIGN_VALUE(sub->u.array.len_pos, 4) == (unsigned)sub->u.array.len_pos);
            len = _dbus_unpack_uint32(
                sub->byte_order, _dbus_string_get_const_udata_len(sub->value_str, sub->u.array.len_pos, 4));
    ...
    _dbus_verbose(
        ...
        sub,
        sub->type_str ? _dbus_string_get_const_data_len(sub->type_str, sub->type_pos, 0) : "unknown",
        sub->u.array.start_pos,
        sub->u.array.len_pos,
        sub->value_pos);
#endif
 ...
 */
static dbus_bool_t writer_recurse_variant(
    DBusTypeWriter*   writer,
    const DBusString* contained_type,
    int               contained_type_start,
    int               contained_type_len,
    ...
    if (writer->enabled) {
         ...
         * 8-boundary.
         */
        if (!_dbus_string_alloc_space(sub->value_str, contained_type_len + 9)) return FALSE;
    }

    /* write VARIANT typecode to the parent's type string */
    if (!write_or_verify_typecode(writer, DBUS_TYPE_VARIANT)) return FALSE;
    ...
    if (!_dbus_string_insert_byte(sub->value_str, sub->value_pos, contained_type_len))
    ...
    if (!_dbus_string_copy_len(
            contained_type, contained_type_start, contained_type_len, sub->value_str, sub->value_pos))
        _dbus_assert_not_reached("should not have failed to insert variant type sig");

    sub->value_pos += contained_type_len;

    if (!_dbus_string_insert_byte(sub->value_str, sub->value_pos, DBUS_TYPE_INVALID))
...
static dbus_bool_t _dbus_type_writer_recurse_contained_len(
    DBusTypeWriter*   writer,
    int               container_type,
    const DBusString* contained_type,
    int               contained_type_start,
    ...
    switch (container_type) {
        case DBUS_TYPE_STRUCT:
            return writer_recurse_struct_or_dict_entry(
                writer, DBUS_STRUCT_BEGIN_CHAR, contained_type, contained_type_start, contained_type_len, sub);
            break;
        case DBUS_TYPE_DICT_ENTRY:
            return writer_recurse_struct_or_dict_entry(
                writer, DBUS_DICT_ENTRY_BEGIN_CHAR, contained_type, contained_type_start, contained_type_len, sub);
            break;
        case DBUS_TYPE_ARRAY:
            return writer_recurse_array(
                writer, contained_type, contained_type_start, contained_type_len, sub, is_array_append);
            break;
        case DBUS_TYPE_VARIANT:
            return writer_recurse_variant(writer, contained_type, contained_type_start, contained_type_len, sub);
            break;
        default:
 ...
 */
dbus_bool_t _dbus_type_writer_recurse(
    DBusTypeWriter*   writer,
    int               container_type,
    const DBusString* contained_type,
    int               contained_type_start,
    ...
    return _dbus_type_writer_recurse_contained_len(
        writer, container_type, contained_type, contained_type_start, contained_type_len, sub, FALSE);
}
...
dbus_bool_t _dbus_type_writer_append_array(
    ...
    return _dbus_type_writer_recurse_contained_len(
        writer, DBUS_TYPE_ARRAY, contained_type, contained_type_start, contained_type_len, sub, TRUE);
}

static int writer_get_array_len(DBusTypeWriter* writer) {
...
dbus_bool_t _dbus_type_writer_unrecurse(DBusTypeWriter* writer, DBusTypeWriter* sub) {
    ...
    _dbus_assert(!writer->type_pos_is_expectation || (writer->type_pos_is_expectation && sub->type_pos_is_expectation));
    ...
    if (sub->container_type == DBUS_TYPE_STRUCT) {
        if (!write_or_verify_typecode(sub, DBUS_STRUCT_END_CHAR)) return FALSE;
    } else if (sub->container_type == DBUS_TYPE_DICT_ENTRY) {
        if (!write_or_verify_typecode(sub, DBUS_DICT_ENTRY_END_CHAR)) return FALSE;
        ...
        if (sub->u.array.len_pos >= 0) /* len_pos == -1 if we weren't enabled when we passed it */
        {
            ...
            _dbus_marshal_set_uint32(sub->value_str, sub->u.array.len_pos, len, sub->byte_order);
#if RECURSIVE_MARSHAL_WRITE_TRACE
     ...
     */
    if (writer->type_str != NULL) {
        if ((sub->container_type == DBUS_TYPE_STRUCT || sub->container_type == DBUS_TYPE_DICT_ENTRY) &&
...
dbus_bool_t _dbus_type_writer_write_basic(DBusTypeWriter* writer, int type, const void* value) {
    ...
    if (!writer->type_pos_is_expectation && writer->type_str != NULL) {
        if (!_dbus_string_alloc_space(writer->type_str, 1)) return FALSE;
    }

    retval = FALSE;

    if (!_dbus_type_writer_write_basic_no_typecode(writer, type, value)) goto out;

    if (!write_or_verify_typecode(writer, type)) _dbus_assert_not_reached("failed to write typecode after prealloc");
...
dbus_bool_t _dbus_type_writer_write_fixed_multi(
    ...
    if (writer->enabled) {
        if (!_dbus_marshal_write_fixed_multi(
                writer->value_str,
                writer->value_pos,
                element_type,
                value,
...
static void enable_if_after(DBusTypeWriter* writer, DBusTypeReader* reader, const DBusTypeReader* start_after) {
    if (start_after) {
...
static dbus_bool_t append_fixup(DBusList** fixups, const DBusArrayLenFixup* fixup) {
    DBusArrayLenFixup* f;
 ...
 */
static dbus_bool_t writer_write_reader_helper(
    DBusTypeWriter*       writer,
    DBusTypeReader*       reader,
    const DBusTypeReader* start_after,
    int                   start_after_new_pos,
    ...
    while ((current_type = _dbus_type_reader_get_current_type(reader)) != DBUS_TYPE_INVALID) {
        if (dbus_type_is_container(current_type)) {
             ...
             */
            if (!inside_start_after && start_after && reader->value_pos == start_after->value_pos &&
                reader->type_str == start_after->type_str && reader->type_pos == start_after->type_pos)
            ...
            _dbus_type_reader_get_signature(&subreader, &sig_str, &sig_start, &sig_len);

#if RECURSIVE_MARSHAL_WRITE_TRACE
            _dbus_verbose(
                "about to recurse into %s reader at %d subreader at %d writer at %d start_after reader at %d write "
                "target len %d inside_start_after = %d this_is_start_after = %d\n",
                _dbus_type_to_string(current_type),
                reader->value_pos,
                subreader.value_pos,
                writer->value_pos,
            ...
            if (!inside_start_after && !this_is_start_after) enable_if_after(writer, &subreader, start_after);
            enabled_at_recurse = writer->enabled;
            if (!_dbus_type_writer_recurse_contained_len(
                    writer, current_type, sig_str, sig_start, sig_len, &subwriter, FALSE))
                goto oom;
            ...
            if (!_dbus_type_writer_unrecurse(writer, &subwriter)) goto oom;
             ...
             * a fixup.
             */
            if (start_after != NULL && !enabled_at_recurse && past_start_after && current_type == DBUS_TYPE_ARRAY &&
                fixups != NULL) {
                ...
                bytes_before_start_after = start_after->value_pos - reader_array_start_pos;

                fixup.len_pos_in_reader = reader_array_len_pos;
                fixup.new_len = bytes_before_start_after + start_after_new_len + bytes_written_after_start_after;

                _dbus_assert(_DBUS_ALIGN_VALUE(fixup.len_pos_in_reader, 4) == (unsigned)fixup.len_pos_in_reader);

                old_len = _dbus_unpack_uint32(
                    reader->byte_order,
                    _dbus_string_get_const_udata_len(reader->value_str, fixup.len_pos_in_reader, 4));

                if (old_len != fixup.new_len && !append_fixup(fixups, &fixup)) goto oom;

#if RECURSIVE_MARSHAL_WRITE_TRACE
                _dbus_verbose(
                    "Generated fixup len_pos_in_reader = %d new_len = %d reader_array_start_pos = %d "
                    "start_after->value_pos = %d bytes_before_start_after = %d start_after_new_len = %d "
                    "bytes_written_after_start_after = %d\n",
...
#if RECURSIVE_MARSHAL_WRITE_TRACE
            _dbus_verbose("Reading basic value %s at %d\n", _dbus_type_to_string(current_type), reader->value_pos);
#endif
            ...
            _dbus_verbose(
                ...
                inside_start_after);
#endif
            if (!inside_start_after) enable_if_after(writer, reader, start_after);
            if (!_dbus_type_writer_write_basic(writer, current_type, &val)) goto oom;
#if RECURSIVE_MARSHAL_WRITE_TRACE
            _dbus_verbose(
...
oom:
    if (fixups) apply_and_free_fixups(fixups, NULL); /* NULL for reader to apply to */
...
static dbus_bool_t _dbus_type_writer_write_reader_partial(
    ...
    if (start_after) _dbus_type_writer_set_enabled(writer, FALSE);

    if (!writer_write_reader_helper(
            writer, reader, start_after, start_after_new_pos, start_after_new_len, fixups, FALSE))
        goto oom;
