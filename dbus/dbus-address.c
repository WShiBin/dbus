...
void _dbus_set_bad_address(
    ...
    if (address_problem_type != NULL)
        ...
        dbus_set_error(error, DBUS_ERROR_BAD_ADDRESS, "Could not parse server address: %s", address_problem_other);
}
...
static dbus_bool_t append_unescaped_value(
    ...
    while (p != end) {
        if (_DBUS_ADDRESS_OPTIONALLY_ESCAPED_BYTE(*p)) {
            ...
            dbus_set_error(
                error, DBUS_ERROR_BAD_ADDRESS, "In D-Bus address, character '%c' should have been escaped\n", *p);
            goto out;
        }
...
dbus_bool_t dbus_parse_address(
    ...
    while (pos < len) {
        ...
        while (pos < end_pos) {
            ...
            if (!_dbus_string_find_to(&str, pos, comma_pos, "=", &equals_pos) || equals_pos == pos ||
                equals_pos + 1 == comma_pos) {
...
dbus_bool_t _dbus_address_test(void) {
    ...
    while (i < _DBUS_N_ELEMENTS(escape_tests)) {
        ...
        if (escaped == NULL) _dbus_assert_not_reached("oom");
    ...
    if (!dbus_parse_address("unix:path=/tmp/foo;debug:name=test,sliff=sloff;", &entries, &len, &error))
