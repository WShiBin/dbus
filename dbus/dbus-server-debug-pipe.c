...
static DBusServerVTable debug_vtable = {debug_finalize, debug_disconnect};
...
DBusServer* _dbus_server_debug_pipe_new(const char* server_name, DBusError* error) {
    ...
    if (!_dbus_string_append(&address, "debug-pipe:name=") || !_dbus_address_append_escaped(&address, &name_str))
        goto nomem_2;
 ...
 *
 */
DBusServerListenResult _dbus_server_listen_debug_pipe(
 ...
 */
DBusTransportOpenResult _dbus_transport_open_debug_pipe(
    DBusAddressEntry* entry,
