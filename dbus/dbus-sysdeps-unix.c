        ...
        for (cm = CMSG_FIRSTHDR(&m); cm; cm = CMSG_NXTHDR(&m, cm))
            if (cm->cmsg_level == SOL_SOCKET && cm->cmsg_type == SCM_RIGHTS) {
                ...
                if ((m.msg_flags & MSG_CTRUNC) && CMSG_NXTHDR(&m, cm) == NULL &&
                    (char*)payload + payload_len_bytes > (char*)m.msg_control + m.msg_controllen) {
                     ...
                     */
                    payload_len_bytes = m.msg_controllen - ((char*)payload - (char*)m.msg_control);
                }
...
int _dbus_write_socket_with_unix_fds(
    DBusSocket        fd,
...
int _dbus_write_socket_with_unix_fds_two(
    DBusSocket        fd,
...
int _dbus_write_two(
...
#else  /* HAVE_WRITEV */
    {
        ...
        if (ret1 == len1 && buffer2 != NULL) {
            ...
            if (ret2 < 0) ret2 = 0; /* we can't report an error as the first write was OK */
...
int _dbus_connect_unix_socket(const char* path, dbus_bool_t abstract, DBusError* error) {
...
#else  /* !__linux__ */
        dbus_set_error(
            error, DBUS_ERROR_NOT_SUPPORTED, "Operating system does not support abstract socket namespace\n");
        _dbus_close(fd, NULL);
        return -1;
#endif /* !__linux__ */
    } else {
    ...
    if (connect(fd, (struct sockaddr*)&addr, _DBUS_STRUCT_OFFSET(struct sockaddr_un, sun_path) + path_len) < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to connect to socket %s: %s", path, _dbus_strerror(errno));
...
int _dbus_connect_exec(const char* path, char* const argv[], DBusError* error) {
    ...
    if (pid < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to fork() to call %s: %s", path, _dbus_strerror(errno));
        close(fds[0]);
        close(fds[1]);
        return -1;
    }
...
int _dbus_listen_unix_socket(const char* path, dbus_bool_t abstract, DBusError* error) {
    ...
    if (bind(listen_fd, (struct sockaddr*)&addr, _DBUS_STRUCT_OFFSET(struct sockaddr_un, sun_path) + path_len) < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to bind socket \"%s\": %s", path, _dbus_strerror(errno));
        _dbus_close(listen_fd, NULL);
        return -1;
    }
    ...
    if (listen(listen_fd, SOMAXCONN /* backlog */) < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to listen on socket \"%s\": %s", path, _dbus_strerror(errno));
        _dbus_close(listen_fd, NULL);
        return -1;
    }
...
int _dbus_listen_systemd_sockets(DBusSocket** fds, DBusError* error) {
    ...
    for (fd = SD_LISTEN_FDS_START; fd < SD_LISTEN_FDS_START + n; fd++) {
        ...
        if (r < 0) {
            dbus_set_error(
                error, _dbus_error_from_errno(-r), "Failed to verify systemd socket type: %s", _dbus_strerror(-r));
            return -1;
        }
 ...
 * a D-Bus error name. */
static const char* _dbus_error_from_gai(int gai_res, int saved_errno) {
    switch (gai_res) {
#ifdef EAI_FAMILY
        case EAI_FAMILY:
        ...
        if (!_dbus_string_get_length(retport)) {
            ...
            if (!port || !strcmp(port, "0")) {
                ...
                if ((res = getnameinfo(
                    ...
                    dbus_set_error(
                        error,
                        _dbus_error_from_gai(res, saved_errno),
...
again:
    ...
    if (bytes_written < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to write credentials byte: %s", _dbus_strerror(errno));
        return FALSE;
...
again:
    ...
    if (bytes_read < 0) {
        ...
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to read credentials byte: %s", _dbus_strerror(errno));
        return FALSE;
...

    {
        ...
        for (cmsgp = CMSG_FIRSTHDR(&msg); cmsgp != NULL; cmsgp = CMSG_NXTHDR(&msg, cmsgp)) {
            if (cmsgp->cmsg_type == SCM_CREDS && cmsgp->cmsg_level == SOL_SOCKET &&
...
#error Credentials passing not working on this OS is a regression!
    ...
    _dbus_verbose(
        "Credentials:"
        "  pid " DBUS_PID_FORMAT "  uid " DBUS_UID_FORMAT "\n",
        pid_read,
        uid_read);
            ...
            if (getgrouplist(username_c, info->primary_gid, buf, &buf_count) < 0) {
                if (errno == 0) {
                    ...
                    dbus_set_error(
                        error,
                        _dbus_error_from_errno(errno),
                        "Failed to get groups for username \"%s\" primary GID " DBUS_GID_FORMAT ": %s\n",
                        username_c,
                        info->primary_gid,
                        _dbus_strerror(errno));
                    dbus_free(buf);
...
dbus_bool_t _dbus_generate_random_bytes(DBusString* str, int n_bytes, DBusError* error) {
    ...
    if (result != n_bytes) {
        if (result < 0)
            dbus_set_error(
                error, _dbus_error_from_errno(errno), "Could not read /dev/urandom: %s", _dbus_strerror(errno));
        else
            dbus_set_error(error, DBUS_ERROR_IO_ERROR, "Short read from /dev/urandom");
 ...
 */
static dbus_bool_t _read_subprocess_line_argv(
    const char*        progpath,
...
#define WRITE_END 1
    ...
    if (pid < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to fork() to call %s: %s", progpath, _dbus_strerror(errno));
    ...
    if (pid == 0) {
        ...
        if (progpath[0] == '/') {
             ...
             * depends on X11).
             */
            if (path_fallback) /* We must have a slash, because we checked above */
    ...
    if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
        ...
        if (_dbus_string_get_length(&error_message) > 0)
            ...
            dbus_set_error(
                error, DBUS_ERROR_SPAWN_EXEC_FAILED, "%s terminated abnormally without any error message", progpath);
        goto out;
    }
...
dbus_bool_t _dbus_get_autolaunch_address(const char* scope, DBusString* address, DBusError* error) {
    ...
    if (display == NULL || display[0] == '\0') {
        dbus_set_error_const(
            error, DBUS_ERROR_NOT_SUPPORTED, "Unable to autolaunch a dbus-daemon without a $DISPLAY for X11");
        return FALSE;
    }
...
dbus_bool_t _dbus_lookup_launchd_socket(DBusString* socket_path, const char* launchd_env_var, DBusError* error) {
    ...
    dbus_set_error(
        error, DBUS_ERROR_NOT_SUPPORTED, "can't lookup socket from launchd; launchd support not compiled in");
    return FALSE;
#endif
}
...
dbus_bool_t _dbus_append_keyring_directory_for_credentials(DBusString* directory, DBusCredentials* credentials) {
...
#ifdef DBUS_ENABLE_EMBEDDED_TESTS
    {
        ...
        if (override != NULL && *override != '\0') {
            ...
            if (!already_warned) {
                _dbus_warn(
                    "Using %s for testing, set DBUS_TEST_HOMEDIR to avoid", _dbus_string_get_const_data(&homedir));
                already_warned = TRUE;
            }
        }
    }
...
dbus_bool_t _dbus_delete_directory(const DBusString* filename, DBusError* error) {
    ...
    if (rmdir(filename_c) != 0) {
        dbus_set_error(
            error, DBUS_ERROR_FAILED, "Failed to remove directory %s: %s\n", filename_c, _dbus_strerror(errno));
        return FALSE;
    }
    ...
    switch (socket.sa.sa_family) {
        case AF_UNIX:
            if (socket.un.sun_path[0] == '\0') {
                ...
                if (_dbus_string_append(address, "unix:abstract=") && _dbus_address_append_escaped(address, &path_str))
                    return TRUE;
            } else {
                _dbus_string_init_const(&path_str, socket.un.sun_path);
                if (_dbus_string_append(address, "unix:path=") && _dbus_address_append_escaped(address, &path_str))
                    return TRUE;
            }
            break;
        case AF_INET:
            if (inet_ntop(AF_INET, &socket.ipv4.sin_addr, hostip, sizeof(hostip)))
                if (_dbus_string_append_printf(
                        address, "tcp:family=ipv4,host=%s,port=%u", hostip, ntohs(socket.ipv4.sin_port)))
                    return TRUE;
            break;
#ifdef AF_INET6
        case AF_INET6:
            ...
            if (inet_ntop(AF_INET6, &socket.ipv6.sin6_addr, hostip, sizeof(hostip)))
                if (_dbus_string_append_printf(
                        address, "tcp:family=ipv6,port=%u,host=", ntohs(socket.ipv6.sin6_port)) &&
        ...
        default:
            dbus_set_error(
                error, _dbus_error_from_errno(EINVAL), "Failed to read address from socket: Unknown socket type.");
            return FALSE;
    }
err:
    dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to open socket: %s", _dbus_strerror(errno));
