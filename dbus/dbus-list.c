...
static void free_link(DBusList* link) {
    if (!_DBUS_LOCK(list))
        _dbus_assert_not_reached(
            "we should have initialized global locks "
...
dbus_bool_t _dbus_list_test(void) {
    ...
    /* using remove_last */
    if (!_dbus_list_append(&list1, _DBUS_INT_TO_POINTER(2))) _dbus_assert_not_reached("could not allocate for append");
    if (!_dbus_list_append(&list1, _DBUS_INT_TO_POINTER(1))) _dbus_assert_not_reached("could not allocate for append");
    if (!_dbus_list_append(&list1, _DBUS_INT_TO_POINTER(3))) _dbus_assert_not_reached("could not allocate for append");
