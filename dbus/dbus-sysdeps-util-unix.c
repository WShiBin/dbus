...
static dbus_bool_t _dbus_write_pid_file(const DBusString* filename, unsigned long pid, DBusError* error) {
    ...
    if (fd < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to open \"%s\": %s", cfilename, _dbus_strerror(errno));
        return FALSE;
    }
    ...
    if (fprintf(f, "%lu\n", pid) < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to write to \"%s\": %s", cfilename, _dbus_strerror(errno));
    ...
    if (fclose(f) == EOF) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to close \"%s\": %s", cfilename, _dbus_strerror(errno));
        return FALSE;
    }
...
dbus_bool_t _dbus_change_to_daemon_user(const char* user, DBusError* error) {
    ...
    if (setgid(gid) < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to set GID to %lu: %s", gid, _dbus_strerror(errno));
        return FALSE;
    }

    if (setuid(uid) < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to set UID to %lu: %s", uid, _dbus_strerror(errno));
        return FALSE;
    }
...
dbus_bool_t _dbus_rlimit_raise_fd_limit(DBusError* error) {
    ...
    if (getuid() == 0) {
        ...
        if (lim.rlim_cur != RLIM_INFINITY && lim.rlim_cur < ENOUGH_FDS) lim.rlim_cur = ENOUGH_FDS;

        if (lim.rlim_max != RLIM_INFINITY && lim.rlim_max < lim.rlim_cur) lim.rlim_max = lim.rlim_cur;
    }
    ...
    if (lim.rlim_max == RLIM_INFINITY || lim.rlim_cur < lim.rlim_max) lim.rlim_cur = lim.rlim_max;

    /* Early-return if there is nothing to do. */
    if (lim.rlim_max == old.rlim_max && lim.rlim_cur == old.rlim_cur) return TRUE;
...
dbus_bool_t _dbus_rlimit_restore_fd_limit(DBusRLimit* saved, DBusError* error) {
    if (setrlimit(RLIMIT_NOFILE, &saved->lim) < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to restore old fd limit: %s", _dbus_strerror(errno));
        return FALSE;
    }
...
static dbus_bool_t fill_group_info(DBusGroupInfo* info, dbus_gid_t gid, const DBusString* groupname, DBusError* error) {
...
#if defined(HAVE_POSIX_GETPWNAM_R) || defined(HAVE_NONPOSIX_GETPWNAM_R)
    {
        ...
        while (1) {
            ...
            if (group_c_str)
                result = getgrnam_r(group_c_str, &g_str, buf, buflen, &g);
            else
                result = getgrgid_r(gid, &g_str, buf, buflen, &g);
#else
...
dbus_bool_t _dbus_string_get_dirname(const DBusString* filename, DBusString* dirname) {
    ...
    if (sep == 0 && _dbus_string_get_byte(filename, 0) == '/')
...
static dbus_bool_t ensure_owned_directory(
    const char*       label,
    ...
    if ((S_IWOTH | S_IWGRP) & buf.st_mode) {
        dbus_set_error(
            error, DBUS_ERROR_FAILED, "%s \"%s\" can be written by others (mode 0%o)", label, dir, buf.st_mode);
        return FALSE;
    }
...
dbus_bool_t _dbus_set_up_transient_session_servicedirs(DBusList** dirs, DBusError* error) {
    ...
    if (!_dbus_string_append(&xrd, xdg_runtime_dir) ||
        !_dbus_string_append_printf(&dbus1, "%s/dbus-1", xdg_runtime_dir) ||
    ...
    if (!ensure_owned_directory("XDG_RUNTIME_DIR", &xrd, FALSE, error) ||
        !ensure_owned_directory("XDG_RUNTIME_DIR subdirectory", &dbus1, TRUE, error) ||
