...
static void basic_value_zero(DBusBasicValue* value) {
    value->u64 = 0;
}

static dbus_bool_t basic_value_equal(int type, DBusBasicValue* lhs, DBusBasicValue* rhs) {
    if (type == DBUS_TYPE_STRING || type == DBUS_TYPE_SIGNATURE || type == DBUS_TYPE_OBJECT_PATH) {
...
static dbus_bool_t equal_values_helper(DBusTypeReader* lhs, DBusTypeReader* rhs) {
    int lhs_type;
    int rhs_type;
    ...
    if (lhs_type == DBUS_TYPE_INVALID) return TRUE;
...
static dbus_bool_t data_block_init(DataBlock* block, int byte_order, int initial_offset) {
    if (!_dbus_string_init(&block->signature)) return FALSE;
    ...
    if (!_dbus_string_insert_bytes(&block->signature, 0, initial_offset, INITIAL_PADDING_BYTE) ||
        !_dbus_string_insert_bytes(&block->body, 0, initial_offset, INITIAL_PADDING_BYTE) ||
...
static void data_block_save(DataBlock* block, DataBlockState* state) {
...
static void data_block_restore(DataBlock* block, DataBlockState* state) {
    _dbus_string_delete(
        &block->signature,
        state->saved_sig_len,
...
static void data_block_verify(DataBlock* block) {
    if (!_dbus_string_ends_with_c_str(&block->signature, FENCE_BYTES_STR)) {
        int offset;

        offset = _dbus_string_get_length(&block->signature) - N_FENCE_BYTES - 8;
        if (offset < 0) offset = 0;

        _dbus_verbose_bytes_of_string(&block->signature, offset, _dbus_string_get_length(&block->signature) - offset);
        _dbus_assert_not_reached("block did not verify: bad bytes at end of signature");
    }
    if (!_dbus_string_ends_with_c_str(&block->body, FENCE_BYTES_STR)) {
        int offset;

        offset = _dbus_string_get_length(&block->body) - N_FENCE_BYTES - 8;
        if (offset < 0) offset = 0;

        _dbus_verbose_bytes_of_string(&block->body, offset, _dbus_string_get_length(&block->body) - offset);
        _dbus_assert_not_reached("block did not verify: bad bytes at end of body");
    }

    _dbus_assert(_dbus_string_validate_nul(&block->signature, 0, block->initial_offset));
    _dbus_assert(_dbus_string_validate_nul(&block->body, 0, block->initial_offset));
}

static void data_block_free(DataBlock* block) {
...
static void data_block_reset(DataBlock* block) {
    data_block_verify(block);
...
static void data_block_init_reader_writer(DataBlock* block, DBusTypeReader* reader, DBusTypeWriter* writer) {
    if (reader)
        _dbus_type_reader_init(
            reader, block->byte_order, &block->signature, block->initial_offset, &block->body, block->initial_offset);
...
static void real_check_expected_type(DBusTypeReader* reader, int expected, const char* funcname, int line) {
    int t;
...
#define NEXT_EXPECTING_TRUE(reader)                                                                                   \
    do {                                                                                                              \
        if (!_dbus_type_reader_next(reader)) {                                                                        \
            _dbus_warn("_dbus_type_reader_next() should have returned TRUE at %s %d", _DBUS_FUNCTION_NAME, __LINE__); \
            _dbus_assert_not_reached("test failed");                                                                  \
        }                                                                                                             \
    } while (0)

#define NEXT_EXPECTING_FALSE(reader)                                                                                   \
    do {                                                                                                               \
        if (_dbus_type_reader_next(reader)) {                                                                          \
            _dbus_warn("_dbus_type_reader_next() should have returned FALSE at %s %d", _DBUS_FUNCTION_NAME, __LINE__); \
            _dbus_assert_not_reached("test failed");                                                                   \
        }                                                                                                              \
...
struct TestTypeNodeClass {
    ...
    dbus_bool_t (*write_value)(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
    dbus_bool_t (*read_value)(TestTypeNode* node, DBusTypeReader* reader, int seed);
    dbus_bool_t (*set_value)(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed);
    dbus_bool_t (*build_signature)(TestTypeNode* node, DBusString* str);
    dbus_bool_t (*write_multi)(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed, int count);
    dbus_bool_t (*read_multi)(TestTypeNode* node, DBusTypeReader* reader, int seed, int count);
};
 ...
 */
static dbus_bool_t int16_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t int16_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t int16_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed);
static dbus_bool_t int16_write_multi(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed, int count);
static dbus_bool_t int16_read_multi(TestTypeNode* node, DBusTypeReader* reader, int seed, int count);
static dbus_bool_t int32_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t int32_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t int32_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed);
static dbus_bool_t int32_write_multi(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed, int count);
static dbus_bool_t int32_read_multi(TestTypeNode* node, DBusTypeReader* reader, int seed, int count);
static dbus_bool_t int64_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t int64_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t int64_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed);
static dbus_bool_t string_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t string_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t string_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed);
static dbus_bool_t bool_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t bool_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t bool_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed);
static dbus_bool_t byte_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t byte_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t byte_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed);
static dbus_bool_t double_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t double_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t double_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed);
static dbus_bool_t object_path_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t object_path_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t object_path_set_value(
    TestTypeNode*   node,
    DBusTypeReader* reader,
    DBusTypeReader* realign_root,
    int             seed);
static dbus_bool_t signature_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t signature_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t signature_set_value(
    TestTypeNode*   node,
    DBusTypeReader* reader,
    DBusTypeReader* realign_root,
    int             seed);
static dbus_bool_t struct_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t struct_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t struct_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed);
static dbus_bool_t struct_build_signature(TestTypeNode* node, DBusString* str);
static dbus_bool_t dict_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t dict_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t dict_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed);
static dbus_bool_t dict_build_signature(TestTypeNode* node, DBusString* str);
static dbus_bool_t array_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t array_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t array_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed);
static dbus_bool_t array_build_signature(TestTypeNode* node, DBusString* str);
static dbus_bool_t variant_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed);
static dbus_bool_t variant_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed);
static dbus_bool_t variant_set_value(
    TestTypeNode*   node,
    DBusTypeReader* reader,
    DBusTypeReader* realign_root,
    int             seed);
...
static const TestTypeNodeClass* const basic_nodes[] = {
    &int16_class,
    &uint16_class,
    &int32_class,
    &uint32_class,
...
static const TestTypeNodeClass* const container_nodes[] = {
    &struct_1_class,
    &array_1_class,
    &struct_2_class,
    &array_0_class,
...
static TestTypeNode* node_new(const TestTypeNodeClass* klass) {
    TestTypeNode* node;
...
static void node_destroy(TestTypeNode* node) {
    if (node->klass->destroy) (*node->klass->destroy)(node);
    dbus_free(node);
}

static dbus_bool_t node_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed) {
    dbus_bool_t retval;
...
static dbus_bool_t node_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed) {
    /* DBusTypeReader restored; */

    if (!(*node->klass->read_value)(node, reader, seed)) return FALSE;
 ...
 */
static dbus_bool_t node_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed) {
    if (!(*node->klass->set_value)(node, reader, realign_root, seed)) return FALSE;

    return TRUE;
}

static dbus_bool_t node_build_signature(TestTypeNode* node, DBusString* str) {
...
static dbus_bool_t node_append_child(TestTypeNode* node, TestTypeNode* child) {
    ...
    if (!_dbus_list_append(&container->children, child))
        _dbus_assert_not_reached("no memory"); /* we never check the return value on node_append_child anyhow - it's run
                                                  from outside the malloc-failure test code */

    return TRUE;
}

static dbus_bool_t node_write_multi(
    TestTypeNode*   node,
    DataBlock*      block,
...
static dbus_bool_t node_read_multi(TestTypeNode* node, DBusTypeReader* reader, int seed, int n_copies) {
    _dbus_assert(node->klass->read_multi != NULL);

    if (!(*node->klass->read_multi)(node, reader, seed, n_copies)) return FALSE;
...
static dbus_bool_t run_test_copy(NodeIterationData* nid) {
    DataBlock*     src;
    DataBlock      dest;
    dbus_bool_t    retval;
    DBusTypeReader reader;
    ...
    if (!data_block_init(&dest, src->byte_order, src->initial_offset)) return FALSE;
     ...
     */
    if (!_dbus_string_insert_byte(&dest.signature, dest.initial_offset, '\0')) goto out;

    if (!_dbus_type_writer_write_reader(&writer, &reader)) goto out;

    /* Data blocks should now be identical */
    if (!_dbus_string_equal(&src->signature, &dest.signature)) {
        _dbus_verbose("SOURCE\n");
        _dbus_verbose_bytes_of_string(&src->signature, 0, _dbus_string_get_length(&src->signature));
        _dbus_verbose("DEST\n");
        _dbus_verbose_bytes_of_string(&dest.signature, 0, _dbus_string_get_length(&dest.signature));
        _dbus_assert_not_reached("signatures did not match");
    }

    if (!_dbus_string_equal(&src->body, &dest.body)) {
        _dbus_verbose("SOURCE\n");
        _dbus_verbose_bytes_of_string(&src->body, 0, _dbus_string_get_length(&src->body));
        _dbus_verbose("DEST\n");
        _dbus_verbose_bytes_of_string(&dest.body, 0, _dbus_string_get_length(&dest.body));
...
static dbus_bool_t run_test_values_only_write(NodeIterationData* nid) {
    DBusTypeReader reader;
    DBusTypeWriter writer;
    int            i;
    dbus_bool_t    retval;
    ...
    _dbus_type_reader_init(
        &reader, nid->block->byte_order, nid->signature, 0, &nid->block->body, nid->block->initial_offset);

    i = 0;
    while (i < nid->n_nodes) {
        if (!node_write_value(nid->nodes[i], nid->block, &writer, i)) goto out;
    ...
    i = 0;
    while (i < nid->n_nodes) {
        if (!node_read_value(nid->nodes[i], &reader, i)) goto out;
 ...
 */
#define SET_SEED 1
static dbus_bool_t run_test_set_values(NodeIterationData* nid) {
    DBusTypeReader reader;
    DBusTypeReader realign_root;
    dbus_bool_t    retval;
    int            i;
    ...
    data_block_init_reader_writer(nid->block, &reader, NULL);

    realign_root = reader;

    i = 0;
    while (i < nid->n_nodes) {
        if (!node_set_value(nid->nodes[i], &reader, &realign_root, i + SET_SEED)) goto out;
    ...
    i = 0;
    while (i < nid->n_nodes) {
        if (!node_read_value(nid->nodes[i], &reader, i + SET_SEED)) goto out;
...
static dbus_bool_t run_test_delete_values(NodeIterationData* nid) {
    DBusTypeReader reader;
    dbus_bool_t    retval;
    int            t;
        ...
        if (t == DBUS_TYPE_ARRAY) {
            ...
            while ((elem_type = _dbus_type_reader_get_current_type(&array)) != DBUS_TYPE_INVALID) {
                ...
                _dbus_verbose(
                    ...
                    elem,
                    n_elements,
                    _dbus_type_to_string(elem_type),
                    cycle,
                    reader.value_pos,
                    array.value_pos);
                while (elem > 0) {
                    if (!_dbus_type_reader_next(&array)) _dbus_assert_not_reached("should have had another element");
                    --elem;
                }

                if (!_dbus_type_reader_delete(&array, &reader)) goto out;
...
static dbus_bool_t run_test_nodes_iteration(void* data) {
    ...
    data_block_init_reader_writer(nid->block, &reader, &writer);

    /* DBusTypeWriter assumes it's writing into an existing signature,
     * so doesn't add nul on its own. We have to do that.
     */
    if (!_dbus_string_insert_byte(&nid->block->signature, nid->type_offset, '\0')) goto out;
    ...
    if (!_dbus_string_equal_substring(
            nid->signature, 0, _dbus_string_get_length(nid->signature), &nid->block->signature, nid->type_offset)) {
        _dbus_warn(
    ...
    if (n_iterations_expected_this_test <= MAX_ITERATIONS_FOR_EXPENSIVE_TESTS) {
         ...
         */
        if (!run_test_set_values(nid)) goto out;

        if (!run_test_delete_values(nid)) goto out;

        if (!run_test_copy(nid)) goto out;

        if (!run_test_values_only_write(nid)) goto out;
    }
...
static void run_test_nodes_in_one_configuration(
    TestTypeNode**    nodes,
    int               n_nodes,
    const DBusString* signature,
    int               byte_order,
    ...
    if (!data_block_init(&block, byte_order, initial_offset)) _dbus_assert_not_reached("no memory");
    ...
    if (TEST_OOM_HANDLING && n_iterations_expected_this_test <= MAX_ITERATIONS_FOR_EXPENSIVE_TESTS) {
        _dbus_test_oom_handling("running test node", run_test_nodes_iteration, &nid);
    } else {
        if (!run_test_nodes_iteration(&nid)) _dbus_assert_not_reached("no memory");
    }

    data_block_free(&block);
}

static void run_test_nodes(TestTypeNode** nodes, int n_nodes) {
    int        i;
    DBusString signature;

    if (!_dbus_string_init(&signature)) _dbus_assert_not_reached("no memory");

    i = 0;
    while (i < n_nodes) {
        if (!node_build_signature(nodes[i], &signature)) _dbus_assert_not_reached("no memory");
    ...
    while (i <= MAX_INITIAL_OFFSET) {
        run_test_nodes_in_one_configuration(nodes, n_nodes, &signature, DBUS_LITTLE_ENDIAN, i);
        run_test_nodes_in_one_configuration(nodes, n_nodes, &signature, DBUS_BIG_ENDIAN, i);
    ...
    else if ((n_iterations_completed_this_test % (int)(n_iterations_expected_this_test / 10.0)) == 1) {
        fprintf(
            stderr, " %d%% ", (int)(n_iterations_completed_this_test / (double)n_iterations_expected_this_test * 100));
    }
...
static TestTypeNode* value_generator(int* ip) {
    int                      i = *ip;
...
static void build_body(TestTypeNode** nodes, int n_nodes, int byte_order, DBusString* signature, DBusString* body) {
    int            i;
    DataBlock      block;
    DBusTypeReader reader;
    DBusTypeWriter writer;
    ...
    if (!data_block_init(&block, byte_order, 0)) _dbus_assert_not_reached("no memory");

    data_block_init_reader_writer(&block, &reader, &writer);

    /* DBusTypeWriter assumes it's writing into an existing signature,
     * so doesn't add nul on its own. We have to do that.
     */
    if (!_dbus_string_insert_byte(&block.signature, 0, '\0')) _dbus_assert_not_reached("no memory");

    i = 0;
    while (i < n_nodes) {
        if (!node_write_value(nodes[i], &block, &writer, i)) _dbus_assert_not_reached("no memory");

        ++i;
    }

    if (!_dbus_string_copy_len(&block.body, 0, _dbus_string_get_length(&block.body) - N_FENCE_BYTES, body, 0))
        _dbus_assert_not_reached("oom");

    data_block_free(&block);
}

dbus_bool_t dbus_internal_do_not_use_generate_bodies(
    int         sequence,
    int         byte_order,
    DBusString* signature,
    DBusString* body) {
...
static void make_and_run_values_inside_container(const TestTypeNodeClass* container_klass, int n_nested) {
    TestTypeNode* root;
...
static void start_next_test(const char* description, int expected) {
    n_iterations_completed_this_test = 0;
    n_iterations_expected_this_test  = expected;

    fprintf(stderr, ">>> >>> %s %d iterations\n", description, n_iterations_expected_this_test);
}

static void make_and_run_test_nodes(void) {
    int i, j, k, m;
    ...
    start_next_test("All values in one big toplevel", 1);
    {
        ...
        for (i = 0; i < N_VALUES; i++) node_destroy(nodes[i]);
    }

    start_next_test("Each value,value pair combination as toplevel, in both orders", N_VALUES * N_VALUES);
    {
    ...
    start_next_test("Each container containing each value", N_CONTAINERS * N_VALUES);
    ...
    start_next_test("Each container containing each value with arrays as blocks", N_CONTAINERS * N_VALUES);
    arrays_write_fixed_in_blocks = TRUE;
    for (i = 0; i < N_CONTAINERS; i++) {
        const TestTypeNodeClass* container_klass = container_nodes[i];
    ...
    start_next_test("Each container of same container of each value", N_CONTAINERS * N_VALUES);
    for (i = 0; i < N_CONTAINERS; i++) {
        const TestTypeNodeClass* container_klass = container_nodes[i];
    ...
    start_next_test("Each container of same container of same container of each value", N_CONTAINERS * N_VALUES);
    for (i = 0; i < N_CONTAINERS; i++) {
        const TestTypeNodeClass* container_klass = container_nodes[i];
    ...
    start_next_test("Each value,value pair inside a struct", N_VALUES * N_VALUES);
    {
    ...
    start_next_test("All values in one big struct", 1);
    {
        ...
        i = 0;
        while ((child = value_generator(&i))) node_append_child(node, child);
    ...
    if (_dbus_getenv("DBUS_TEST_SLOW") == NULL || atoi(_dbus_getenv("DBUS_TEST_SLOW")) < 1) {
        fprintf(
            stderr,
    ...
    start_next_test("Each container of each container of each value", N_CONTAINERS * N_CONTAINERS * N_VALUES);
    for (i = 0; i < N_CONTAINERS; i++) {
    ...
    start_next_test(
        "Each value,value,value triplet combination as toplevel, in all orders", N_VALUES * N_VALUES * N_VALUES);
    {
...
static dbus_int16_t int16_from_seed(int seed) {
     ...
     */
    static const dbus_int16_t v_of_seed[5] = {SAMPLE_INT16, SAMPLE_INT16_ALTERNATE, -1, _DBUS_INT16_MAX, 1};

    dbus_int16_t v = v_of_seed[seed % _DBUS_N_ELEMENTS(v_of_seed)];

    if (seed > 1) v *= seed; /* wraps around eventually, which is fine */
...
static dbus_bool_t int16_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed) {
    ...
    return _dbus_type_writer_write_basic(writer, node->klass->typecode, &v);
}
...
static dbus_bool_t int16_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed) {
    ...
    _dbus_type_reader_read_basic(reader, (dbus_int16_t*)&v);
...
static dbus_bool_t int16_set_value(TestTypeNode* node, DBusTypeReader* reader, DBusTypeReader* realign_root, int seed) {
    ...
    return _dbus_type_reader_set_basic(reader, &v, realign_root);
}

static dbus_bool_t int16_write_multi(
    TestTypeNode*   node,
    DataBlock*      block,
    DBusTypeWriter* writer,
    int             seed,
    ...
    for (i = 0; i < count; ++i) values[i] = int16_from_seed(seed + i);

    return _dbus_type_writer_write_fixed_multi(writer, node->klass->typecode, &v_ARRAY_INT16, count);
}
...
static dbus_bool_t int16_read_multi(TestTypeNode* node, DBusTypeReader* reader, int seed, int count) {
    ...
    _dbus_type_reader_read_fixed_multi(reader, &values, &n_elements);

    if (n_elements != count) _dbus_warn("got %d elements expected %d", n_elements, count);
    _dbus_assert(n_elements == count);

    for (i = 0; i < count; i++)
        _dbus_assert(
            ((dbus_int16_t)_dbus_unpack_uint16(reader->byte_order, (const unsigned char*)values + (i * 2))) ==
...
static dbus_int32_t int32_from_seed(int seed) {
    /* Generate an integer value that's predictable from seed.  We could
     * just use seed itself, but that would only ever touch one byte of
     * the int so would miss some kinds of bug.
     */
    static const dbus_int32_t v_of_seed[5] = {SAMPLE_INT32, SAMPLE_INT32_ALTERNATE, -1, _DBUS_INT_MAX, 1};
...
static dbus_bool_t int32_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed) {
    ...
    _dbus_type_reader_read_basic(reader, (dbus_int32_t*)&v);
...
static dbus_bool_t int32_write_multi(
    TestTypeNode*   node,
    DataBlock*      block,
    DBusTypeWriter* writer,
    int             seed,
    ...
    for (i = 0; i < count; ++i) values[i] = int32_from_seed(seed + i);

    return _dbus_type_writer_write_fixed_multi(writer, node->klass->typecode, &v_ARRAY_INT32, count);
}
...
static dbus_bool_t int32_read_multi(TestTypeNode* node, DBusTypeReader* reader, int seed, int count) {
    ...
    for (i = 0; i < count; i++)
        _dbus_assert(
            ((int)_dbus_unpack_uint32(reader->byte_order, (const unsigned char*)values + (i * 4))) ==
            int32_from_seed(seed + i));

    return TRUE;
}

static dbus_int64_t int64_from_seed(int seed) {
    dbus_int32_t v32;
    dbus_int64_t v;
...
static dbus_bool_t int64_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed) {
    ...
    _dbus_type_reader_read_basic(reader, (dbus_int64_t*)&v);
...
static void string_from_seed(char* buf, int len, int seed) {
    int           i;
    unsigned char v;
...
static dbus_bool_t string_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed) {
    ...
    string_from_seed(buf, node->klass->subclass_detail, seed);

    return _dbus_type_writer_write_basic(writer, node->klass->typecode, &v_string);
}
...
static dbus_bool_t string_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed) {
    ...
    _dbus_type_reader_read_basic(reader, (const char**)&v);

    string_from_seed(buf, node->klass->subclass_detail, seed);

    if (strcmp(buf, v) != 0) {
        _dbus_warn("read string '%s' expected '%s'", v, buf);
        _dbus_assert_not_reached("test failed");
    }
...
static dbus_bool_t string_set_value(
    TestTypeNode*   node,
    DBusTypeReader* reader,
    DBusTypeReader* realign_root,
    int             seed) {
    ...
    return _dbus_type_reader_set_basic(reader, &v_string, realign_root);
}
...
static dbus_bool_t bool_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed) {
    ...
    _dbus_type_reader_read_basic(reader, (unsigned char*)&v);
...
static double double_from_seed(int seed) {
...
static dbus_bool_t double_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed) {
    ...
    _dbus_type_reader_read_basic(reader, (double*)&v);
...
static dbus_bool_t double_set_value(
    TestTypeNode*   node,
    DBusTypeReader* reader,
    DBusTypeReader* realign_root,
    int             seed) {
...
static void object_path_from_seed(char* buf, int seed) {
    int           i;
    unsigned char v;
    int           len;
...
static dbus_bool_t object_path_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed) {
    ...
    if (strcmp(buf, v) != 0) {
        _dbus_warn("read object path '%s' expected '%s'", v, buf);
        _dbus_assert_not_reached("test failed");
    }
...
static void signature_from_seed(char* buf, int seed) {
    /* try to avoid ascending, descending, or alternating length to help find bugs */
    const char* sample_signatures[] = {"asax", "", "asau(xxxx)", "x", "ai", "a(ii)"};
...
static dbus_bool_t signature_read_value(TestTypeNode* node, DBusTypeReader* reader, int seed) {
    ...
    if (strcmp(buf, v) != 0) {
        _dbus_warn("read signature value '%s' expected '%s'", v, buf);
        _dbus_assert_not_reached("test failed");
    }
...
static dbus_bool_t struct_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed) {
    ...
    if (!_dbus_type_writer_recurse(writer, DBUS_TYPE_STRUCT, NULL, 0, &sub)) return FALSE;
...
static dbus_bool_t struct_read_or_set_value(
    TestTypeNode*   node,
    DBusTypeReader* reader,
    DBusTypeReader* realign_root,
    int             seed) {
    ...
    while (i < n_copies) {
        ...
        while (link != NULL) {
            ...
            if (realign_root == NULL) {
                if (!node_read_value(child, &sub, seed + i)) return FALSE;
            } else {
                if (!node_set_value(child, &sub, realign_root, seed + i)) return FALSE;
            }
...
static dbus_bool_t struct_set_value(
    TestTypeNode*   node,
    DBusTypeReader* reader,
    DBusTypeReader* realign_root,
    int             seed) {
...
static dbus_bool_t struct_build_signature(TestTypeNode* node, DBusString* str) {
    ...
    if (!_dbus_string_append_byte(str, DBUS_STRUCT_BEGIN_CHAR)) goto oom;
    ...
    while (i < n_copies) {
        ...
        while (link != NULL) {
            ...
            if (!node_build_signature(child, str)) goto oom;
    ...
    if (!_dbus_string_append_byte(str, DBUS_STRUCT_END_CHAR)) goto oom;
...
static dbus_bool_t array_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed) {
    ...
    if (!_dbus_string_init(&element_signature)) return FALSE;

    child = _dbus_list_get_first(&container->children);

    if (!node_build_signature(child, &element_signature)) goto oom;

    element_type = _dbus_first_type_in_signature(&element_signature, 0);

    if (!_dbus_type_writer_recurse(writer, DBUS_TYPE_ARRAY, &element_signature, 0, &sub)) goto oom;

    if (arrays_write_fixed_in_blocks && dbus_type_is_fixed(element_type) && child->klass->write_multi) {
        if (!node_write_multi(child, block, &sub, seed, n_copies)) goto oom;
    } else {
        i = 0;
        while (i < n_copies) {
            DBusList* link;
            ...
            while (link != NULL) {
                ...
                if (!node_write_value(child2, block, &sub, seed + i)) goto oom;
    ...
    if (!_dbus_type_writer_unrecurse(writer, &sub)) goto oom;
...
static dbus_bool_t array_read_or_set_value(
    TestTypeNode*   node,
    DBusTypeReader* reader,
    DBusTypeReader* realign_root,
    int             seed) {
    ...
    if (n_copies > 0) {
        ...
        if (realign_root == NULL && arrays_write_fixed_in_blocks &&
            dbus_type_is_fixed(_dbus_type_reader_get_element_type(reader)) && child->klass->read_multi) {
            if (!node_read_multi(child, &sub, seed, n_copies)) return FALSE;
        } else {
            i = 0;
            while (i < n_copies) {
                DBusList* link;
                ...
                while (link != NULL) {
                    ...
                    _dbus_assert(child2->klass->typecode == _dbus_type_reader_get_element_type(reader));

                    if (realign_root == NULL) {
                        if (!node_read_value(child2, &sub, seed + i)) return FALSE;
                    } else {
                        if (!node_set_value(child2, &sub, realign_root, seed + i)) return FALSE;
                    }
...
static dbus_bool_t array_build_signature(TestTypeNode* node, DBusString* str) {
    ...
    if (!node_build_signature(_dbus_list_get_first(&container->children), str)) goto oom;
...
static dbus_bool_t variant_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed) {
    ...
    if (!_dbus_string_init(&content_signature)) return FALSE;

    if (!node_build_signature(child, &content_signature)) goto oom;

    if (!_dbus_type_writer_recurse(writer, DBUS_TYPE_VARIANT, &content_signature, 0, &sub)) goto oom;

    if (!node_write_value(child, block, &sub, seed + VARIANT_SEED)) goto oom;
...
static dbus_bool_t variant_read_or_set_value(
    TestTypeNode*   node,
    DBusTypeReader* reader,
    DBusTypeReader* realign_root,
    int             seed) {
    ...
    if (realign_root == NULL) {
        if (!node_read_value(child, &sub, seed + VARIANT_SEED)) return FALSE;
    } else {
        if (!node_set_value(child, &sub, realign_root, seed + VARIANT_SEED)) return FALSE;
    }
...
static dbus_bool_t dict_write_value(TestTypeNode* node, DataBlock* block, DBusTypeWriter* writer, int seed) {
    ...
    if (!_dbus_string_init(&entry_value_signature)) return FALSE;
    ...
    if (!node_build_signature(child, &entry_value_signature)) goto oom;

    if (!_dbus_string_append(&dict_entry_signature, DBUS_DICT_ENTRY_BEGIN_CHAR_AS_STRING DBUS_TYPE_INT32_AS_STRING))
        goto oom;

    if (!_dbus_string_copy(
            &entry_value_signature, 0, &dict_entry_signature, _dbus_string_get_length(&dict_entry_signature)))
        goto oom;

    if (!_dbus_string_append_byte(&dict_entry_signature, DBUS_DICT_ENTRY_END_CHAR)) goto oom;

    if (!_dbus_type_writer_recurse(writer, DBUS_TYPE_ARRAY, &dict_entry_signature, 0, &sub)) goto oom;
    ...
    while (i < n_entries) {
        ...
        if (!_dbus_type_writer_recurse(&sub, DBUS_TYPE_DICT_ENTRY, NULL, 0, &entry_sub)) goto oom;

        key = int32_from_seed(seed + i);

        if (!_dbus_type_writer_write_basic(&entry_sub, DBUS_TYPE_INT32, &key)) goto oom;

        if (!node_write_value(child, block, &entry_sub, seed + i)) goto oom;

        if (!_dbus_type_writer_unrecurse(&sub, &entry_sub)) goto oom;
...
static dbus_bool_t dict_read_or_set_value(
    TestTypeNode*   node,
    DBusTypeReader* reader,
    DBusTypeReader* realign_root,
    int             seed) {
    ...
    if (n_entries > 0) {
        ...
        while (i < n_entries) {
            ...
            if (realign_root == NULL) {
                ...
                _dbus_type_reader_read_basic(&entry_sub, (dbus_int32_t*)&v);

                _dbus_assert(v == int32_from_seed(seed + i));

                NEXT_EXPECTING_TRUE(&entry_sub);

                if (!node_read_value(child, &entry_sub, seed + i)) return FALSE;
                ...
                if (!_dbus_type_reader_set_basic(&entry_sub, &v, realign_root)) return FALSE;

                NEXT_EXPECTING_TRUE(&entry_sub);

                if (!node_set_value(child, &entry_sub, realign_root, seed + i)) return FALSE;
...
static dbus_bool_t dict_build_signature(TestTypeNode* node, DBusString* str) {
    ...
    if (!_dbus_string_append(str, DBUS_DICT_ENTRY_BEGIN_CHAR_AS_STRING DBUS_TYPE_INT32_AS_STRING)) goto oom;
