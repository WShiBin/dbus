...
DBUS_PRIVATE_EXPORT
void _dbus_warn(const char* format, ...) _DBUS_GNUC_PRINTF(1, 2);

DBUS_PRIVATE_EXPORT
void _dbus_warn_check_failed(const char* format, ...) _DBUS_GNUC_PRINTF(1, 2);
DBUS_PRIVATE_EXPORT
...
DBUS_PRIVATE_EXPORT
void _dbus_verbose_real(const char* file, const int line, const char* function, const char* format, ...)
    _DBUS_GNUC_PRINTF(4, 5);
#define _dbus_verbose(fmt, ...) _dbus_verbose_real(__FILE__, __LINE__, __FUNCTION__, fmt, ##__VA_ARGS__)
#else
DBUS_PRIVATE_EXPORT
void _dbus_verbose_real(const char* format, ...) _DBUS_GNUC_PRINTF(1, 2);
...
#else
DBUS_PRIVATE_EXPORT
void _dbus_real_assert_not_reached(const char* explanation, const char* file, int line) _DBUS_GNUC_NORETURN;
...
#define _DBUS_STRUCT_OFFSET(struct_type, member) ((intptr_t)((unsigned char*)&((struct_type*)0)->member))
...
static inline void _dbus_assert_error_is_clear(const DBusError* error) {
    _dbus_assert(error == NULL || !dbus_error_is_set(error));
}
...
#define _dbus_return_val_if_error_is_set(error, val) \
    _dbus_return_val_if_fail((error) == NULL || !dbus_error_is_set((error)), (val))
...
#define _DBUS_ALIGN_ADDRESS(this, boundary) ((void*)_DBUS_ALIGN_VALUE(this, boundary))
...
dbus_bool_t _dbus_register_shutdown_func_unlocked(DBusShutdownFunction function, void* data);
...
dbus_bool_t _dbus_read_uuid_file(
