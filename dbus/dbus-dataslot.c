 ...
 */
dbus_bool_t _dbus_data_slot_list_set(
...
#define N_SLOTS 100
    ...
    while (i < N_SLOTS) {
        ...
        if (tmp != i) _dbus_assert_not_reached("did not allocate slots in numeric order");
    ...
    i = 0;
    while (i < N_SLOTS) {
        if (!_dbus_data_slot_list_set(
                &allocator, &list, i, _DBUS_INT_TO_POINTER(i), test_free_slot_data_func, &old_free_func, &old_data))
