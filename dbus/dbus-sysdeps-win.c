...
static BOOL load_ex_ip_helper_procedures(void) {
    ...
    lpfnAllocateAndGetTcpExTableFromStack =
        (ProcAllocateAndGetTcpExtTableFromStack)GetProcAddress(hModule, "AllocateAndGetTcpExTableFromStack");
 ...
 */
static dbus_pid_t get_pid_from_extended_tcp_table(int peer_port) {
    dbus_pid_t              result;
    DWORD                   errorCode, size = 0, i;
    MIB_TCPTABLE_OWNER_PID* tcp_table;

    if ((errorCode = GetExtendedTcpTable(NULL, &size, TRUE, AF_INET, TCP_TABLE_OWNER_PID_ALL, 0)) ==
    ...
    for (i = 0; i < tcp_table->dwNumEntries; i++) {
        ...
        if (p->dwState == MIB_TCP_STATE_ESTAB && local_address == INADDR_LOOPBACK && local_port == peer_port)
 ...
 * @return process id or 0 if connection has not been found
 */
static dbus_pid_t get_pid_from_tcp_ex_table(int peer_port) {
    dbus_pid_t       result;
    DWORD            errorCode, i;
    PMIB_TCPTABLE_EX tcp_table = NULL;

    if (!load_ex_ip_helper_procedures()) {
        _dbus_verbose("Error not been able to load iphelper procedures\n");
        return 0;
    }
    ...
    if (errorCode != NO_ERROR) {
        _dbus_verbose("Error not been able to call AllocateAndGetTcpExTableFromStack()\n");
        return 0;
    }
 ...
 */
static dbus_pid_t _dbus_get_peer_pid_from_tcp_handle(int handle) {
...
char* _dbus_win_error_string(int error_number) {
    ...
    if (msg[strlen(msg) - 1] == '\n') msg[strlen(msg) - 1] = '\0';
    if (msg[strlen(msg) - 1] == '\r') msg[strlen(msg) - 1] = '\0';
 ...
 */
static void _dbus_win_handle_set_close_on_exec(HANDLE handle) {
    if (!SetHandleInformation(
            (HANDLE)handle, HANDLE_FLAG_INHERIT | HANDLE_FLAG_PROTECT_FROM_CLOSE, 0 /*disable both flags*/)) {
...
again:
    ...
    rc = WSASend(fd.sock, vectors, data2 ? 2 : 1, &bytes_written, 0, NULL, NULL);
    ...
    if (bytes_written == (DWORD)-1 && errno == EINTR) goto again;
...
dbus_bool_t _dbus_win_startup_winsock(void) {
    ...
    if (!_DBUS_LOCK(sysdeps)) return FALSE;

    if (beenhere) goto out;
...
dbus_bool_t _dbus_win_account_to_sid(const wchar_t* waccount, void** ppsid, DBusError* error) {
    ...
    *ppsid = NULL;
    ...
    sid_length     = 0;
    wdomain_length = 0;
    if (!LookupAccountNameW(NULL, waccount, NULL, &sid_length, NULL, &wdomain_length, &use) &&
    ...
    *ppsid = dbus_malloc(sid_length);
    ...
    if (!LookupAccountNameW(NULL, waccount, (PSID)*ppsid, &sid_length, wdomain, &wdomain_length, &use)) {
        _dbus_win_set_error_from_win_error(error, GetLastError());
        goto out2;
    }
...
dbus_bool_t _dbus_getsid(char** sid, dbus_pid_t process_id) {
    ...
    if ((!GetTokenInformation(process_token, TokenUser, NULL, 0, &n) && GetLastError() != ERROR_INSUFFICIENT_BUFFER) ||
        (token_user = alloca(n)) == NULL || !GetTokenInformation(process_token, TokenUser, token_user, n, &n)) {
...
out0:
    ...
    dbus_set_error(
        error, _dbus_error_from_errno(errno), "Could not setup socket pair: %s", _dbus_strerror_from_errno());
...
#define DBUS_STACK_WSAEVENTS 256
    ...
    for (i = 0; i < n_fds; i++) {
        ...
        if (fdp->events & _DBUS_POLLIN) msgp += sprintf(msgp, "R:%Iu ", fdp->fd.sock);

        if (fdp->events & _DBUS_POLLOUT) msgp += sprintf(msgp, "W:%Iu ", fdp->fd.sock);
    ...
    for (i = 0; i < n_fds; i++) {
        ...
        if (fdp->events & _DBUS_POLLIN) lNetworkEvents |= FD_READ | FD_ACCEPT | FD_CLOSE;

        if (fdp->events & _DBUS_POLLOUT) lNetworkEvents |= FD_WRITE | FD_CONNECT;
    ...
    if (DBUS_SOCKET_API_RETURNS_ERROR(ready)) {
        ...
        for (i = 0; i < n_fds; i++) {
            ...
            if (ne.lNetworkEvents & (FD_READ | FD_ACCEPT | FD_CLOSE)) fdp->revents |= _DBUS_POLLIN;

            if (ne.lNetworkEvents & (FD_WRITE | FD_CONNECT)) fdp->revents |= _DBUS_POLLOUT;

            if (ne.lNetworkEvents & (FD_OOB)) fdp->revents |= _DBUS_POLLERR;

            if (ne.lNetworkEvents & (FD_READ | FD_ACCEPT | FD_CLOSE)) msgp += sprintf(msgp, "R:%Iu ", fdp->fd.sock);

            if (ne.lNetworkEvents & (FD_WRITE | FD_CONNECT)) msgp += sprintf(msgp, "W:%Iu ", fdp->fd.sock);

            if (ne.lNetworkEvents & (FD_OOB)) msgp += sprintf(msgp, "E:%Iu ", fdp->fd.sock);

            msgp += sprintf(msgp, "lNetworkEvents:%d ", ne.lNetworkEvents);

            if (ne.lNetworkEvents) ret++;
    ...
    if (n_fds > DBUS_STACK_WSAEVENTS) free(pEvents);
...
#define DBUS_POLL_CHAR_BUFFER_SIZE 2000
    ...
    for (i = 0; i < n_fds; i++) {
        ...
        if (fdp->events & _DBUS_POLLIN) FD_SET(fdp->fd.sock, &read_set);

        if (fdp->events & _DBUS_POLLOUT) FD_SET(fdp->fd.sock, &write_set);
    ...
    if (DBUS_SOCKET_API_RETURNS_ERROR(ready)) {
        DBUS_SOCKET_SET_ERRNO();
        if (errno != WSAEWOULDBLOCK) _dbus_verbose("select: failed: %s\n", _dbus_strerror_from_errno());
    } else if (ready == 0)
        ...
        for (i = 0; i < n_fds; i++) {
            ...
            if (FD_ISSET(fdp->fd.sock, &read_set)) msgp += sprintf(msgp, "R:%Iu ", fdp->fd.sock);

            if (FD_ISSET(fdp->fd.sock, &write_set)) msgp += sprintf(msgp, "W:%Iu ", fdp->fd.sock);

            if (FD_ISSET(fdp->fd.sock, &err_set)) msgp += sprintf(msgp, "E:%Iu\n\t", fdp->fd.sock);
        }
        msgp += sprintf(msgp, "\n");
        _dbus_verbose("%s", msg);
#endif
        ...
        for (i = 0; i < n_fds; i++) {
            ...
            if (FD_ISSET(fdp->fd.sock, &read_set)) fdp->revents |= _DBUS_POLLIN;

            if (FD_ISSET(fdp->fd.sock, &write_set)) fdp->revents |= _DBUS_POLLOUT;

            if (FD_ISSET(fdp->fd.sock, &err_set)) fdp->revents |= _DBUS_POLLERR;
        }
    }
    return ready;
...
DBusSocket _dbus_connect_tcp_socket_with_nonce(
    const char* host,
    const char* port,
    const char* family,
    ...
    if (!family)
        ...
        dbus_set_error(error, DBUS_ERROR_INVALID_ARGS, "Unknown address family %s", family);
        return _dbus_socket_get_invalid();
    }
    ...
    while (tmp) {
        if ((fd.sock = socket(tmp->ai_family, SOCK_STREAM, 0)) == INVALID_SOCKET) {
            DBUS_SOCKET_SET_ERRNO();
            dbus_set_error(
                error, _dbus_error_from_errno(errno), "Failed to open socket: %s", _dbus_strerror_from_errno());
            freeaddrinfo(ai);
            return _dbus_socket_get_invalid();
        }
        _DBUS_ASSERT_ERROR_IS_CLEAR(error);
    ...
    if (noncefile != NULL) {
        ...
        dbus_bool_t ret;
        if (!_dbus_string_init(&noncefileStr) || !_dbus_string_append(&noncefileStr, noncefile)) {
            closesocket(fd.sock);
        ...
        if (!_dbus_string_get_length(retport)) {
            ...
            if (!port || !strcmp(port, "0")) {
                ...
                if (getsockname(fd.sock, &addr.Address, &addrlen) == SOCKET_ERROR ||
                    (res = getnameinfo(&addr.Address, addrlen, NULL, 0, portbuf, sizeof(portbuf), NI_NUMERICSERV)) !=
                        0) {
                    DBUS_SOCKET_SET_ERRNO();
                    dbus_set_error(
                        error,
...
failed:
    ...
    for (i = 0; i < nlisten_fd; i++) closesocket(listen_fd[i].sock);
    dbus_free(listen_fd);
    return -1;
}
...
again:
    ...
    if (bytes_written < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to write credentials byte: %s", _dbus_strerror_from_errno());
        return FALSE;
...
dbus_bool_t _dbus_read_credentials_socket(DBusSocket handle, DBusCredentials* credentials, DBusError* error) {
    ...
    if (_dbus_string_init(&buf)) {
        ...
        if (bytes_read > 0) _dbus_verbose("got one zero byte from server\n");
    ...
    if (_dbus_getsid(&sid, pid)) {
        if (!_dbus_credentials_add_windows_sid(credentials, sid)) goto out;
    }
...
dbus_bool_t _dbus_concat_dir_and_file(DBusString* dir, const DBusString* next_component) {
    ...
    file_starts_with_slash =
        ('/' == _dbus_string_get_byte(next_component, 0) || '\\' == _dbus_string_get_byte(next_component, 0));
...
dbus_bool_t _dbus_credentials_add_from_user(DBusCredentials* credentials, const DBusString* username) {
    return _dbus_credentials_add_windows_sid(credentials, _dbus_string_get_const_data(username));
}
...
dbus_bool_t _dbus_credentials_add_from_current_process(DBusCredentials* credentials) {
    ...
    if (!_dbus_getsid(&sid, _dbus_getpid())) goto failed;

    if (!_dbus_credentials_add_pid(credentials, _dbus_getpid())) goto failed;

    if (!_dbus_credentials_add_windows_sid(credentials, sid)) goto failed;
...
dbus_bool_t _dbus_append_user_from_current_process(DBusString* str) {
    ...
    if (!_dbus_getsid(&sid, _dbus_getpid())) return FALSE;
...
void _dbus_get_real_time(long* tv_sec, long* tv_usec) {
    ...
    if (tv_sec) *tv_sec = time64 / 1000000;

    if (tv_usec) *tv_usec = time64 % 1000000;
}
...
dbus_bool_t _dbus_create_directory(const DBusString* filename, DBusError* error) {
    ...
    if (!CreateDirectoryA(filename_c, NULL)) {
        dbus_set_error(
            error, DBUS_ERROR_FAILED, "Failed to create directory %s: %s\n", filename_c, _dbus_strerror_from_errno());
        return FALSE;
    } else
        return TRUE;
}
...
const char* _dbus_get_tmpdir(void) {
    ...
    if (tmpdir == NULL) {
        ...
        if (last_slash > p && last_slash[1] == '\0') last_slash[0] = '\0';
        last_slash = _mbsrchr(p, '/');
        if (last_slash > p && last_slash[1] == '\0') last_slash[0] = '\0';
...
dbus_bool_t _dbus_delete_file(const DBusString* filename, DBusError* error) {
    ...
    if (DeleteFileA(filename_c) == 0) {
        dbus_set_error(
            error, DBUS_ERROR_FAILED, "Failed to delete file %s: %s\n", filename_c, _dbus_strerror_from_errno());
        return FALSE;
    } else
        return TRUE;
}
...
static void dump_backtrace_for_thread(HANDLE hThread) {
    ...
    if (hThread == GetCurrentThread()) return;
...
static void dump_backtrace(void) {
    ...
    DWORD  dwThreadId;
    DuplicateHandle(
        GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(), &hCurrentThread, 0, FALSE, DUPLICATE_SAME_ACCESS);
    hThread = CreateThread(NULL, 0, dump_thread_proc, (LPVOID)hCurrentThread, 0, &dwThreadId);
...
dbus_bool_t _dbus_read_local_machine_uuid(DBusGUID* machine_id, dbus_bool_t create_if_not_found, DBusError* error) {
    ...
    // 12340001
    u =
        ((fromAscii(lpc[0]) << 0) | (fromAscii(lpc[1]) << 4) | (fromAscii(lpc[2]) << 8) | (fromAscii(lpc[3]) << 12) |
         (fromAscii(lpc[4]) << 16) | (fromAscii(lpc[5]) << 20) | (fromAscii(lpc[6]) << 24) | (fromAscii(lpc[7]) << 28));
    ...
    u =
        ((fromAscii(lpc[0]) << 0) | (fromAscii(lpc[1]) << 4) | (fromAscii(lpc[2]) << 8) | (fromAscii(lpc[3]) << 12) |
         (fromAscii(lpc[5]) << 16) | (fromAscii(lpc[6]) << 20) | (fromAscii(lpc[7]) << 24) | (fromAscii(lpc[8]) << 28));
...
static void _dbus_global_unlock(HANDLE mutex) {
    ReleaseMutex(mutex);
    CloseHandle(mutex);
}
...
// sync _dbus_daemon_publish_session_bus_address, _dbus_daemon_unpublish_session_bus_address and
...
static dbus_bool_t _dbus_get_install_root_as_hash(DBusString* out) {
    DBusString install_path;

    _dbus_string_init(&install_path);

    if (!_dbus_get_install_root(&install_path) || _dbus_string_get_length(&install_path) == 0) return FALSE;

    _dbus_string_init(out);
    _dbus_string_tolower_ascii(&install_path, 0, _dbus_string_get_length(&install_path));

    if (!_dbus_sha_compute(&install_path, out)) return FALSE;

    return TRUE;
}

static dbus_bool_t _dbus_get_address_string(DBusString* out, const char* basestring, const char* scope) {
    _dbus_string_init(out);
...
static dbus_bool_t _dbus_get_shm_name(DBusString* out, const char* scope) {
    return _dbus_get_address_string(out, cDBusDaemonAddressInfo, scope);
}

static dbus_bool_t _dbus_get_mutex_name(DBusString* out, const char* scope) {
...
dbus_bool_t _dbus_daemon_is_session_bus_address_published(const char* scope) {
    ...
    if (hDBusDaemonMutex) return TRUE;
    ...
    if (hDBusDaemonMutex == NULL) return FALSE;
...
dbus_bool_t _dbus_daemon_publish_session_bus_address(const char* address, const char* scope) {
    HANDLE        lock;
    ...
    hDBusSharedMem = CreateFileMappingA(
...
static dbus_bool_t _dbus_get_autolaunch_shm(DBusString* address, DBusString* shm_name) {
    HANDLE sharedMem;
    char*  shared_addr;
    int    i;
    ...
    for (i = 0; i < 20; ++i) {
        ...
        if (sharedMem == 0) Sleep(100);
        if (sharedMem != 0) break;
    }

    if (sharedMem == 0) return FALSE;

    shared_addr = MapViewOfFile(sharedMem, FILE_MAP_READ, 0, 0, 0);

    if (!shared_addr) return FALSE;
...
static dbus_bool_t _dbus_daemon_already_runs(DBusString* address, DBusString* shm_name, const char* scope) {
    HANDLE      lock;
    HANDLE      daemon;
    DBusString  mutex_name;
    ...
    if (CreateProcessA(dbus_exe_path, dbus_args, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
        ...
        if (retval == FALSE)
            dbus_set_error_const(
                error, DBUS_ERROR_FAILED, "Failed to get autolaunch address from launched dbus-daemon");
    } else {
...
dbus_bool_t _dbus_get_install_root(DBusString* str) {
    ...
    do {
        ...
        SetLastError(0);
        pathLength =
            GetModuleFileNameA(_dbus_win_get_dll_hmodule(), _dbus_string_get_data(str), _dbus_string_get_length(str));
...
#define KEYRING_DIR ".dbus-keyrings"
    ...
    if (!_dbus_string_copy(&homedir, 0, directory, _dbus_string_get_length(directory))) {
        goto failed;
    }
...
dbus_bool_t _dbus_delete_directory(const DBusString* filename, DBusError* error) {
    ...
    if (RemoveDirectoryA(filename_c) == 0) {
        ...
        dbus_set_error(error, _dbus_win_error_from_last_error(), "Failed to remove directory %s: %s", filename_c, emsg);
...
dbus_bool_t _dbus_path_is_absolute(const DBusString* filename) {
    if (_dbus_string_get_length(filename) > 0)
        return _dbus_string_get_byte(filename, 1) == ':' || _dbus_string_get_byte(filename, 0) == '\\' ||
