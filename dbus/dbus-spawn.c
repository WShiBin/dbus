...
static ReadStatus read_ints(int fd, int* buf, int n_ints_in_buf, int* n_ints_read, DBusError* error) {
    ...
    again:
        ...
        if (chunk < 0) {
            dbus_set_error(
                error, DBUS_ERROR_SPAWN_FAILED, "Failed to read from child pipe (%s)", _dbus_strerror(errno));
        ...
        if (sitter->sitter_pid > 0) {
            ...
            if (ret < 0) {
                if (errno == ECHILD)
                    ...
                    _dbus_warn("Unexpected error %d in waitpid() for babysitter: %s", errno, _dbus_strerror(errno));
            } else {
...
void _dbus_babysitter_kill_child(DBusBabysitter* sitter) {
    ...
    if (sitter->grandchild_pid == -1) return; /* child is already dead, or we're so hosed we'll never recover */
...
void _dbus_babysitter_set_child_exit_error(DBusBabysitter* sitter, DBusError* error) {
    ...
    if (sitter->have_exec_errnum) {
        ...
        if (WIFEXITED(sitter->status))
            ...
            dbus_set_error(error, DBUS_ERROR_FAILED, "Process %s exited abnormally", sitter->log_name);
    } else {
        dbus_set_error(error, DBUS_ERROR_FAILED, "Process %s exited, reason unknown", sitter->log_name);
    }
}
 ...
 */
dbus_bool_t _dbus_babysitter_set_watch_functions(
    DBusBabysitter*          sitter,
    ...
    return _dbus_watch_list_set_functions(
        sitter->watches, add_function, remove_function, toggled_function, data, free_data_function);
}
...
dbus_bool_t _dbus_spawn_async_with_babysitter(
     ...
     */
    sitter->error_watch =
        _dbus_watch_new(child_err_report_pipe[READ_END], DBUS_WATCH_READABLE, TRUE, handle_watch, sitter, NULL);
    if (sitter->error_watch == NULL) {
        dbus_set_error(error, DBUS_ERROR_NO_MEMORY, NULL);
        goto cleanup_and_fail;
    }
    ...
    sitter->sitter_watch =
        _dbus_watch_new(babysitter_pipe[0].fd, DBUS_WATCH_READABLE, TRUE, handle_watch, sitter, NULL);
    if (sitter->sitter_watch == NULL) {
        dbus_set_error(error, DBUS_ERROR_NO_MEMORY, NULL);
        goto cleanup_and_fail;
    }
...
void _dbus_babysitter_set_result_function(
    DBusBabysitter*            sitter,
