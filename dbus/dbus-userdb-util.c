...
static DBusGroupInfo* _dbus_group_info_ref(DBusGroupInfo* info) {
 ...
 */
const DBusGroupInfo* _dbus_user_database_lookup_group(
    DBusUserDatabase* db,
    dbus_gid_t        gid,
    ...
    if (gid != DBUS_GID_UNSET)
        ...
        info = _dbus_hash_table_lookup_string(db->groups_by_name, _dbus_string_get_const_data(groupname));
    if (info) {
 ...
 */
dbus_bool_t _dbus_user_database_get_groupname(
    DBusUserDatabase*     db,
    const DBusString*     groupname,
    const DBusGroupInfo** info,
    DBusError*            error) {
 ...
 * @returns #FALSE if error is set
 */
dbus_bool_t _dbus_user_database_get_gid(
    DBusUserDatabase*     db,
    dbus_gid_t            gid,
    const DBusGroupInfo** info,
    DBusError*            error) {
...
dbus_bool_t _dbus_userdb_test(const char* test_data_dir) {
    ...
    if (!_dbus_username_from_current_process(&username)) _dbus_assert_not_reached("didn't get username");

    if (!_dbus_homedir_from_current_process(&homedir)) _dbus_assert_not_reached("didn't get homedir");

    if (!_dbus_get_user_id(username, &uid)) _dbus_assert_not_reached("didn't get uid");

    if (!_dbus_groups_from_uid(uid, &group_ids, &n_group_ids)) _dbus_assert_not_reached("didn't get groups");
    ...
    for (i = 0; i < n_group_ids; i++) printf(" %ld", group_ids[i]);
