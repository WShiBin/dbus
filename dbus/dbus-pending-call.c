...
static void _dbus_pending_call_trace_ref(
    ...
    _dbus_trace_ref(
        "DBusPendingCall", pending_call, old_refcount, new_refcount, why, "DBUS_PENDING_CALL_TRACE", &enabled);
#endif
}
 ...
 */
DBusPendingCall* _dbus_pending_call_new_unlocked(
...
void _dbus_pending_call_set_reply_unlocked(DBusPendingCall* pending, DBusMessage* message) {
    ...
    _dbus_verbose(
        ...
        message,
        dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_METHOD_RETURN ? "method return"
 ...
 * @return #FALSE on OOM
 */
dbus_bool_t _dbus_pending_call_set_timeout_error_unlocked(
    DBusPendingCall* pending,
    DBusMessage*     message,
 ...
 */
dbus_bool_t _dbus_pending_call_set_data_unlocked(
    DBusPendingCall* pending,
    dbus_int32_t     slot,
    void*            data,
    ...
    retval = _dbus_data_slot_list_set(
        &slot_allocator, &pending->slot_list, slot, data, free_data_func, &old_free_func, &old_data);
