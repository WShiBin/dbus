...
struct DBusAuth {
    ...
    unsigned int
        already_asked_for_initial_response : 1; /**< Already sent a blank challenge to get an initial response */
...
static void        goto_state(DBusAuth* auth, const DBusAuthStateData* new_state);
static dbus_bool_t send_auth(DBusAuth* auth, const DBusAuthMechanismHandler* mech);
static dbus_bool_t send_data(DBusAuth* auth, DBusString* data);
static dbus_bool_t send_rejected(DBusAuth* auth);
static dbus_bool_t send_error(DBusAuth* auth, const char* message);
...
static const DBusAuthStateData server_state_waiting_for_auth = {"WaitingForAuth", handle_server_state_waiting_for_auth};
static const DBusAuthStateData server_state_waiting_for_data = {"WaitingForData", handle_server_state_waiting_for_data};
...
static dbus_bool_t handle_client_state_waiting_for_data(
    ...
    DBusAuthCommand   command,
    const DBusString* args);
static dbus_bool_t handle_client_state_waiting_for_ok(DBusAuth* auth, DBusAuthCommand command, const DBusString* args);
static dbus_bool_t handle_client_state_waiting_for_reject(
    DBusAuth*         auth,
    DBusAuthCommand   command,
    const DBusString* args);
static dbus_bool_t handle_client_state_waiting_for_agree_unix_fd(
    DBusAuth*         auth,
    DBusAuthCommand   command,
    const DBusString* args);

static const DBusAuthStateData client_state_need_send_auth   = {"NeedSendAuth", NULL};
static const DBusAuthStateData client_state_waiting_for_data = {"WaitingForData", handle_client_state_waiting_for_data};
/* The WaitingForOK state doesn't appear to be used.
 * See https://bugs.freedesktop.org/show_bug.cgi?id=97298 */
_DBUS_GNUC_UNUSED
static const DBusAuthStateData client_state_waiting_for_ok     = {"WaitingForOK", handle_client_state_waiting_for_ok};
...
static const DBusAuthStateData client_state_waiting_for_agree_unix_fd = {
    ...
    handle_client_state_waiting_for_agree_unix_fd};

/**
 * Common terminal states.  Terminal states have handler == NULL.
 */

static const DBusAuthStateData common_state_authenticated = {"Authenticated", NULL};

static const DBusAuthStateData common_state_need_disconnect = {"NeedDisconnect", NULL};
...
static DBusAuth* _dbus_auth_new(int size) {
    DBusAuth* auth;
    ...
    if (!_dbus_string_init(&auth->incoming)) goto enomem_0;

    if (!_dbus_string_init(&auth->outgoing)) goto enomem_1;

    if (!_dbus_string_init(&auth->identity)) goto enomem_2;

    if (!_dbus_string_init(&auth->context)) goto enomem_3;

    if (!_dbus_string_init(&auth->challenge)) goto enomem_4;

    /* default context if none is specified */
    if (!_dbus_string_append(&auth->context, "org_freedesktop_general")) goto enomem_5;

    auth->credentials = _dbus_credentials_new();
    if (auth->credentials == NULL) goto enomem_6;

    auth->authorized_identity = _dbus_credentials_new();
    if (auth->authorized_identity == NULL) goto enomem_7;

    auth->desired_identity = _dbus_credentials_new();
    if (auth->desired_identity == NULL) goto enomem_8;
...
static void shutdown_mech(DBusAuth* auth) {
    /* Cancel any auth */
 ...
 */
static dbus_bool_t sha1_compute_hash(
    DBusAuth*         auth,
    int               cookie_id,
    ...
    if (!_dbus_string_init(&cookie)) return FALSE;

    if (!_dbus_keyring_get_hex_key(auth->keyring, cookie_id, &cookie)) goto out_0;
    ...
    if (!_dbus_string_init(&to_hash)) goto out_0;

    if (!_dbus_string_copy(server_challenge, 0, &to_hash, _dbus_string_get_length(&to_hash))) goto out_1;

    if (!_dbus_string_append(&to_hash, ":")) goto out_1;

    if (!_dbus_string_copy(client_challenge, 0, &to_hash, _dbus_string_get_length(&to_hash))) goto out_1;

    if (!_dbus_string_append(&to_hash, ":")) goto out_1;

    if (!_dbus_string_copy(&cookie, 0, &to_hash, _dbus_string_get_length(&to_hash))) goto out_1;

    if (!_dbus_sha_compute(&to_hash, hash)) goto out_1;
...
static dbus_bool_t sha1_handle_first_client_response(DBusAuth* auth, const DBusString* data) {
    ...
    if (_dbus_string_get_length(data) > 0) {
        if (_dbus_string_get_length(&auth->identity) > 0) {
            ...
            if (!_dbus_string_copy(data, 0, &auth->identity, 0)) return FALSE;
        }
    }
     ...
     */
    if (auth->keyring && !_dbus_keyring_is_for_credentials(auth->keyring, auth->desired_identity)) {
        _dbus_keyring_unref(auth->keyring);
        auth->keyring = NULL;
    }

    if (auth->keyring == NULL) {
        auth->keyring = _dbus_keyring_new_for_credentials(auth->desired_identity, &auth->context, &error);
        ...
        if (auth->keyring == NULL) {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
                ...
                if (send_rejected(auth)) retval = TRUE; /* retval is only about mem */
                dbus_error_free(&error);
                goto out;
            }
        } else {
    ...
    if (auth->cookie_id < 0) {
        ...
        if (send_rejected(auth)) retval = TRUE;
        dbus_error_free(&error);
        goto out;
    } else {
        _dbus_assert(!dbus_error_is_set(&error));
    ...
    if (!_dbus_string_copy(&auth->context, 0, &tmp2, _dbus_string_get_length(&tmp2))) goto out;

    if (!_dbus_string_append(&tmp2, " ")) goto out;

    if (!_dbus_string_append_int(&tmp2, auth->cookie_id)) goto out;
    ...
    _dbus_string_set_length(&auth->challenge, 0);
    if (!_dbus_string_hex_encode(&tmp, 0, &auth->challenge, 0)) goto out;

    if (!_dbus_string_hex_encode(&tmp, 0, &tmp2, _dbus_string_get_length(&tmp2))) goto out;

    if (!send_data(auth, &tmp2)) goto out;
...
out:
    ...
    if (myself != NULL) _dbus_credentials_unref(myself);

    return retval;
}

static dbus_bool_t sha1_handle_second_client_response(DBusAuth* auth, const DBusString* data) {
    ...
    if (!_dbus_string_init(&client_challenge)) goto out_0;

    if (!_dbus_string_init(&client_hash)) goto out_1;

    if (!_dbus_string_copy_len(data, 0, i, &client_challenge, 0)) goto out_2;

    _dbus_string_skip_blank(data, i, &i);

    if (!_dbus_string_copy_len(data, i, _dbus_string_get_length(data) - i, &client_hash, 0)) goto out_2;

    if (_dbus_string_get_length(&client_challenge) == 0 || _dbus_string_get_length(&client_hash) == 0) {
        _dbus_verbose("%s: zero-length client challenge or hash\n", DBUS_AUTH_NAME(auth));
        if (send_rejected(auth)) retval = TRUE;
        goto out_2;
    }

    if (!_dbus_string_init(&correct_hash)) goto out_2;

    if (!sha1_compute_hash(auth, auth->cookie_id, &auth->challenge, &client_challenge, &correct_hash)) goto out_3;
    ...
    if (!_dbus_credentials_add_credentials(auth->authorized_identity, auth->desired_identity)) goto out_3;
...
static dbus_bool_t handle_server_data_cookie_sha1_mech(DBusAuth* auth, const DBusString* data) {
    if (auth->cookie_id < 0)
...
static void handle_server_shutdown_cookie_sha1_mech(DBusAuth* auth) {
    auth->cookie_id = -1;
    _dbus_string_set_length(&auth->challenge, 0);
}

static dbus_bool_t handle_client_initial_response_cookie_sha1_mech(DBusAuth* auth, DBusString* response) {
    DBusString  username;
    dbus_bool_t retval;

    retval = FALSE;

    if (!_dbus_string_init(&username)) return FALSE;

    if (!_dbus_append_user_from_current_process(&username)) goto out_0;

    if (!_dbus_string_hex_encode(&username, 0, response, _dbus_string_get_length(response))) goto out_0;
...
static dbus_bool_t handle_client_data_cookie_sha1_mech(DBusAuth* auth, const DBusString* data) {
    ...
    if (!_dbus_string_find_blank(data, 0, &i)) {
        if (send_error(auth, "Server did not send context/ID/challenge properly")) retval = TRUE;
        goto out_0;
    }

    if (!_dbus_string_init(&context)) goto out_0;

    if (!_dbus_string_copy_len(data, 0, i, &context, 0)) goto out_1;
    ...
    if (!_dbus_string_init(&cookie_id_str)) goto out_1;

    if (!_dbus_string_copy_len(data, i, j - i, &cookie_id_str, 0)) goto out_2;

    if (!_dbus_string_init(&server_challenge)) goto out_2;

    i = j;
    _dbus_string_skip_blank(data, i, &i);
    j = _dbus_string_get_length(data);

    if (!_dbus_string_copy_len(data, i, j - i, &server_challenge, 0)) goto out_3;

    if (!_dbus_keyring_validate_context(&context)) {
        if (send_error(auth, "Server sent invalid cookie context")) retval = TRUE;
        goto out_3;
    }

    if (!_dbus_string_parse_int(&cookie_id_str, 0, &val, NULL)) {
        if (send_error(auth, "Could not parse cookie ID as an integer")) retval = TRUE;
        goto out_3;
    }

    if (_dbus_string_get_length(&server_challenge) == 0) {
        if (send_error(auth, "Empty server challenge string")) retval = TRUE;
        goto out_3;
    }

    if (auth->keyring == NULL) {
        auth->keyring = _dbus_keyring_new_for_credentials(NULL, &context, &error);
        ...
        if (auth->keyring == NULL) {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
                ...
                if (send_error(auth, "Could not load cookie file")) retval = TRUE; /* retval is only about mem */
    ...
    if (!_dbus_string_init(&tmp)) goto out_3;
    ...
    if (!_dbus_generate_random_bytes(&tmp, N_CHALLENGE_BYTES, &error)) {
        if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
            ...
            if (send_error(auth, "Failed to generate challenge")) retval = TRUE; /* retval is only about mem */
    ...
    if (!_dbus_string_init(&client_challenge)) goto out_4;

    if (!_dbus_string_hex_encode(&tmp, 0, &client_challenge, 0)) goto out_5;

    if (!_dbus_string_init(&correct_hash)) goto out_5;

    if (!sha1_compute_hash(auth, val, &server_challenge, &client_challenge, &correct_hash)) goto out_6;

    if (_dbus_string_get_length(&correct_hash) == 0) {
        /* couldn't find the cookie ID or something */
        if (send_error(auth, "Don't have the requested cookie ID")) retval = TRUE;
        goto out_6;
    }

    _dbus_string_set_length(&tmp, 0);

    if (!_dbus_string_copy(&client_challenge, 0, &tmp, _dbus_string_get_length(&tmp))) goto out_6;

    if (!_dbus_string_append(&tmp, " ")) goto out_6;

    if (!_dbus_string_copy(&correct_hash, 0, &tmp, _dbus_string_get_length(&tmp))) goto out_6;

    if (!send_data(auth, &tmp)) goto out_6;
...
static void handle_client_shutdown_cookie_sha1_mech(DBusAuth* auth) {
    auth->cookie_id = -1;
    _dbus_string_set_length(&auth->challenge, 0);
}
...
static dbus_bool_t handle_server_data_external_mech(DBusAuth* auth, const DBusString* data) {
    ...
    if (_dbus_string_get_length(&auth->identity) == 0 && !auth->already_asked_for_initial_response) {
     ...
     */
    if (_dbus_string_get_length(&auth->identity) == 0) {
        if (!_dbus_credentials_add_credentials(auth->desired_identity, auth->credentials)) {
            return FALSE; /* OOM */
        }
    } else {
        if (!_dbus_credentials_add_from_user(auth->desired_identity, &auth->identity)) {
    ...
    if (_dbus_credentials_are_anonymous(auth->desired_identity)) {
        _dbus_verbose(
            "%s: desired user %s is no good\n", DBUS_AUTH_NAME(auth), _dbus_string_get_const_data(&auth->identity));
        return send_rejected(auth);
    }

    if (_dbus_credentials_are_superset(auth->credentials, auth->desired_identity)) {
        /* client has authenticated */
        if (!_dbus_credentials_add_credentials(auth->authorized_identity, auth->desired_identity)) return FALSE;
...
static void handle_server_shutdown_external_mech(DBusAuth* auth) {
}

static dbus_bool_t handle_client_initial_response_external_mech(DBusAuth* auth, DBusString* response) {
    ...
    if (!_dbus_string_init(&plaintext)) return FALSE;

    if (!_dbus_append_user_from_current_process(&plaintext)) goto failed;

    if (!_dbus_string_hex_encode(&plaintext, 0, response, _dbus_string_get_length(response))) goto failed;
...
static dbus_bool_t handle_client_data_external_mech(DBusAuth* auth, const DBusString* data) {
    return TRUE;
}

static void handle_client_shutdown_external_mech(DBusAuth* auth) {
}
...
static dbus_bool_t handle_server_data_anonymous_mech(DBusAuth* auth, const DBusString* data) {
    if (_dbus_string_get_length(data) > 0) {
        ...
        _dbus_verbose(
            "%s: ANONYMOUS client sent trace string: '%s'\n", DBUS_AUTH_NAME(auth), _dbus_string_get_const_data(data));
    }
...
static void handle_server_shutdown_anonymous_mech(DBusAuth* auth) {
}

static dbus_bool_t handle_client_initial_response_anonymous_mech(DBusAuth* auth, DBusString* response) {
    ...
    if (!_dbus_string_append(&plaintext, "libdbus " DBUS_VERSION_STRING)) goto failed;
...
static dbus_bool_t handle_client_data_anonymous_mech(DBusAuth* auth, const DBusString* data) {
    return TRUE;
}

static void handle_client_shutdown_anonymous_mech(DBusAuth* auth) {
}
 ...
 *
 */
static const DBusAuthMechanismHandler all_mechanisms[] = {
    {"EXTERNAL",
...
static const DBusAuthMechanismHandler* find_mech(const DBusString* name, char** allowed_mechs) {
    int i;

    if (allowed_mechs != NULL &&
        !_dbus_string_array_contains((const char**)allowed_mechs, _dbus_string_get_const_data(name)))
        return NULL;

    i = 0;
    while (all_mechanisms[i].mechanism != NULL) {
        if (_dbus_string_equal_c_str(name, all_mechanisms[i].mechanism)) return &all_mechanisms[i];
...
static dbus_bool_t send_auth(DBusAuth* auth, const DBusAuthMechanismHandler* mech) {
    ...
    if (!_dbus_string_init(&auth_command)) return FALSE;

    if (!_dbus_string_append(&auth_command, "AUTH ")) {
        _dbus_string_free(&auth_command);
        return FALSE;
    }

    if (!_dbus_string_append(&auth_command, mech->mechanism)) {
        _dbus_string_free(&auth_command);
        return FALSE;
    }
    ...
    if (!_dbus_string_copy(&auth_command, 0, &auth->outgoing, _dbus_string_get_length(&auth->outgoing))) {
        _dbus_string_free(&auth_command);
        return FALSE;
    }
...
static dbus_bool_t send_data(DBusAuth* auth, DBusString* data) {
    ...
    if (data == NULL || _dbus_string_get_length(data) == 0)
        ...
        if (!_dbus_string_append(&auth->outgoing, "DATA ")) goto out;

        if (!_dbus_string_hex_encode(data, 0, &auth->outgoing, _dbus_string_get_length(&auth->outgoing))) goto out;
...
static dbus_bool_t send_rejected(DBusAuth* auth) {
    ...
    if (!_dbus_string_init(&command)) return FALSE;

    if (!_dbus_string_append(&command, "REJECTED")) goto nomem;

    for (i = 0; all_mechanisms[i].mechanism != NULL; i++) {
        /* skip mechanisms that aren't allowed */
        if (auth->allowed_mechs != NULL &&
            !_dbus_string_array_contains((const char**)auth->allowed_mechs, all_mechanisms[i].mechanism))
            continue;

        if (!_dbus_string_append(&command, " ")) goto nomem;

        if (!_dbus_string_append(&command, all_mechanisms[i].mechanism)) goto nomem;
    }

    if (!_dbus_string_append(&command, "\r\n")) goto nomem;

    if (!_dbus_string_copy(&command, 0, &auth->outgoing, _dbus_string_get_length(&auth->outgoing))) goto nomem;
...
static dbus_bool_t send_error(DBusAuth* auth, const char* message) {
    return _dbus_string_append_printf(&auth->outgoing, "ERROR \"%s\"\r\n", message);
}
...
static dbus_bool_t send_ok(DBusAuth* auth) {
    ...
    if (_dbus_string_append(&auth->outgoing, "OK ") &&
        _dbus_string_copy(
            &DBUS_AUTH_SERVER(auth)->guid, 0, &auth->outgoing, _dbus_string_get_length(&auth->outgoing)) &&
...
static dbus_bool_t process_ok(DBusAuth* auth, const DBusString* args_from_ok) {
    int end_of_hex;
    ...
    end_of_hex = 0;
    if (!_dbus_string_hex_decode(args_from_ok, 0, &end_of_hex, &DBUS_AUTH_CLIENT(auth)->guid_from_server, 0))
        return FALSE;

    /* now clear out the scratch */
    _dbus_string_set_length(&DBUS_AUTH_CLIENT(auth)->guid_from_server, 0);

    if (end_of_hex != _dbus_string_get_length(args_from_ok) || end_of_hex == 0) {
        _dbus_verbose(
            "Bad GUID from server, parsed %d bytes and had %d bytes from server\n",
            end_of_hex,
            _dbus_string_get_length(args_from_ok));
        goto_state(auth, &common_state_need_disconnect);
        return TRUE;
    }
    ...
    if (auth->unix_fd_possible) return send_negotiate_unix_fd(auth);
...
static dbus_bool_t process_data(DBusAuth* auth, const DBusString* args, DBusAuthDataFunction data_func) {
    int        end;
    DBusString decoded;

    if (!_dbus_string_init(&decoded)) return FALSE;
    ...
    if (_dbus_string_get_length(args) != end) {
        _dbus_string_free(&decoded);
        if (!send_error(auth, "Invalid hex encoding")) return FALSE;

        return TRUE;
    }

#ifdef DBUS_ENABLE_VERBOSE_MODE
    if (_dbus_string_validate_ascii(&decoded, 0, _dbus_string_get_length(&decoded)))
        _dbus_verbose("%s: data: '%s'\n", DBUS_AUTH_NAME(auth), _dbus_string_get_const_data(&decoded));
#endif
...
static dbus_bool_t send_negotiate_unix_fd(DBusAuth* auth) {
    if (!_dbus_string_append(&auth->outgoing, "NEGOTIATE_UNIX_FD\r\n")) return FALSE;
...
static dbus_bool_t handle_auth(DBusAuth* auth, const DBusString* args) {
    if (_dbus_string_get_length(args) == 0) {
        /* No args to the auth, send mechanisms */
        if (!send_rejected(auth)) return FALSE;
        ...
        if (!_dbus_string_init(&mech)) return FALSE;
        ...
        if (!_dbus_string_copy_len(args, 0, i, &mech, 0)) goto failed;

        _dbus_string_skip_blank(args, i, &i);
        if (!_dbus_string_copy(args, i, &hex_response, 0)) goto failed;

        auth->mech = find_mech(&mech, auth->allowed_mechs);
        if (auth->mech != NULL) {
            _dbus_verbose("%s: Trying mechanism %s\n", DBUS_AUTH_NAME(auth), auth->mech->mechanism);

            if (!process_data(auth, &hex_response, auth->mech->server_data_func)) goto failed;
        } else {
            /* Unsupported mechanism */
            _dbus_verbose("%s: Unsupported mechanism %s\n", DBUS_AUTH_NAME(auth), _dbus_string_get_const_data(&mech));

            if (!send_rejected(auth)) goto failed;
        }
...
static dbus_bool_t get_word(const DBusString* str, int* start, DBusString* word) {
    int i;
    ...
    if (i > *start) {
        if (!_dbus_string_copy_len(str, *start, i - *start, word, 0)) return FALSE;
...
static dbus_bool_t record_mechanisms(DBusAuth* auth, const DBusString* args) {
    int next;
    int len;

    if (auth->already_got_mechanisms) return TRUE;
    ...
    while (next < len) {
        ...
        if (!_dbus_string_init(&m)) goto nomem;
...
static dbus_bool_t process_rejected(DBusAuth* auth, const DBusString* args) {
    const DBusAuthMechanismHandler* mech;
    ...
    if (!auth->already_got_mechanisms) {
        if (!record_mechanisms(auth, args)) return FALSE;
    }

    if (DBUS_AUTH_CLIENT(auth)->mechs_to_try != NULL) {
        mech = client->mechs_to_try->data;

        if (!send_auth(auth, mech)) return FALSE;
...
static DBusAuthCommand lookup_command_from_name(DBusString* command) {
    int i;

    for (i = 0; i < _DBUS_N_ELEMENTS(auth_command_names); i++) {
        if (_dbus_string_equal_c_str(command, auth_command_names[i].name)) return auth_command_names[i].command;
    }

    return DBUS_AUTH_COMMAND_UNKNOWN;
}

static void goto_state(DBusAuth* auth, const DBusAuthStateData* state) {
    _dbus_verbose("%s: going from state %s to state %s\n", DBUS_AUTH_NAME(auth), auth->state->name, state->name);

    auth->state = state;
}

/* returns whether to call it again right away */
static dbus_bool_t process_command(DBusAuth* auth) {
    DBusAuthCommand command;
    DBusString      line;
    DBusString      args;
    int             eol;
    ...
    if (!_dbus_string_copy_len(&auth->incoming, 0, eol, &line, 0)) goto out;

    if (!_dbus_string_validate_ascii(&line, 0, _dbus_string_get_length(&line))) {
    ...
    _dbus_verbose("%s: got command \"%s\"\n", DBUS_AUTH_NAME(auth), _dbus_string_get_const_data(&line));

    _dbus_string_find_blank(&line, 0, &i);
    _dbus_string_skip_blank(&line, i, &j);

    if (j > i) _dbus_string_delete(&line, i, j - i);

    if (!_dbus_string_move(&line, i, &args, 0)) goto out;
    ...
    if (!(*auth->state->handler)(auth, command, &args)) goto out;
...
DBusAuth* _dbus_auth_server_new(const DBusString* guid) {
    ...
    if (!_dbus_string_init(&guid_copy)) return NULL;
...
DBusAuth* _dbus_auth_client_new(void) {
    ...
    if (!_dbus_string_init(&guid_str)) return NULL;
...
void _dbus_auth_unref(DBusAuth* auth) {
    ...
    if (auth->refcount == 0) {
        ...
        if (auth->keyring) _dbus_keyring_unref(auth->keyring);
...
#define MAX_BUFFER (16 * _DBUS_ONE_KILOBYTE)
    ...
    do {
        if (DBUS_AUTH_IN_END_STATE(auth)) break;
...
dbus_bool_t _dbus_auth_get_bytes_to_send(DBusAuth* auth, const DBusString** str) {
    ...
    *str = NULL;
    ...
    if (_dbus_string_get_length(&auth->outgoing) == 0) return FALSE;
...
void _dbus_auth_bytes_sent(DBusAuth* auth, int bytes_sent) {
    _dbus_verbose(
        "%s: Sent %d bytes of: %s\n", DBUS_AUTH_NAME(auth), bytes_sent, _dbus_string_get_const_data(&auth->outgoing));

    _dbus_string_delete(&auth->outgoing, 0, bytes_sent);
}
...
void _dbus_auth_get_unused_bytes(DBusAuth* auth, const DBusString** str) {
    if (!DBUS_AUTH_IN_END_STATE(auth)) return;
...
dbus_bool_t _dbus_auth_needs_encoding(DBusAuth* auth) {
    if (auth->state != &common_state_authenticated) return FALSE;
...
dbus_bool_t _dbus_auth_encode_data(DBusAuth* auth, const DBusString* plaintext, DBusString* encoded) {
    ...
    if (_dbus_auth_needs_encoding(auth)) {
        ...
        return _dbus_string_copy(plaintext, 0, encoded, _dbus_string_get_length(encoded));
    }
}
...
dbus_bool_t _dbus_auth_decode_data(DBusAuth* auth, const DBusString* encoded, DBusString* plaintext) {
    ...
    if (_dbus_auth_needs_decoding(auth)) {
        ...
        return _dbus_string_copy(encoded, 0, plaintext, _dbus_string_get_length(plaintext));
    }
}
...
dbus_bool_t _dbus_auth_set_credentials(DBusAuth* auth, DBusCredentials* credentials) {
    ...
    return _dbus_credentials_add_credentials(auth->credentials, credentials);
}
...
dbus_bool_t _dbus_auth_set_context(DBusAuth* auth, const DBusString* context) {
    return _dbus_string_replace_len(
        context, 0, _dbus_string_get_length(context), &auth->context, 0, _dbus_string_get_length(context));
}
...
dbus_bool_t _dbus_auth_dump_supported_mechanisms(DBusString* buffer) {
    ...
    for (i = 0; all_mechanisms[i].mechanism != NULL; i++) {
        if (i > 0) {
            if (!_dbus_string_append(buffer, ", ")) return FALSE;
        }
        if (!_dbus_string_append(buffer, all_mechanisms[i].mechanism)) return FALSE;
    }
    return TRUE;
}
