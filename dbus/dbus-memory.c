 ...
 */
typedef enum { SOURCE_UNKNOWN, SOURCE_MALLOC, SOURCE_REALLOC, SOURCE_MALLOC_ZERO, SOURCE_REALLOC_NULL } BlockSource;
...
void* dbus_realloc(void* memory, size_t bytes) {
    ...
    if (bytes == 0) /* guarantee this is safe */
    {
        ...
        if (memory) {
            ...
            if (bytes >= old_bytes) /* old guards shouldn't have moved */
...
dbus_bool_t _dbus_memory_test(void) {
    ...
    guards     = TRUE;
    p          = dbus_malloc(4);
    if (p == NULL) _dbus_assert_not_reached("no memory");
