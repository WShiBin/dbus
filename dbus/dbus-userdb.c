...
static DBusUserInfo* _dbus_user_info_ref(DBusUserInfo* info) {
...
dbus_bool_t _dbus_is_a_number(const DBusString* str, unsigned long* num) {
    ...
    if (_dbus_string_parse_uint(str, 0, num, &end) && end == _dbus_string_get_length(str))
        return TRUE;
    else
        return FALSE;
}
 ...
 */
const DBusUserInfo* _dbus_user_database_lookup(
    DBusUserDatabase* db,
    dbus_uid_t        uid,
...
static dbus_bool_t init_system_db(void) {
    ...
    if (system_db == NULL) {
        ...
        if (!_dbus_string_append(&process_username, info->username) ||
            !_dbus_string_append(&process_homedir, info->homedir) ||
...
DBusUserDatabase* _dbus_user_database_new(void) {
    ...
    db->users = _dbus_hash_table_new(DBUS_HASH_UINTPTR, NULL, (DBusFreeFunction)_dbus_user_info_unref);

    if (db->users == NULL) goto failed;

    db->groups = _dbus_hash_table_new(DBUS_HASH_UINTPTR, NULL, (DBusFreeFunction)_dbus_group_info_unref);

    if (db->groups == NULL) goto failed;

    db->users_by_name = _dbus_hash_table_new(DBUS_HASH_STRING, NULL, (DBusFreeFunction)_dbus_user_info_unref);
    if (db->users_by_name == NULL) goto failed;

    db->groups_by_name = _dbus_hash_table_new(DBUS_HASH_STRING, NULL, (DBusFreeFunction)_dbus_group_info_unref);
 ...
 * @returns #FALSE if error is set
 */
dbus_bool_t _dbus_user_database_get_username(
    DBusUserDatabase*    db,
    const DBusString*    username,
    const DBusUserInfo** info,
    DBusError*           error) {
