...
static dbus_bool_t return_uuid(DBusGUID* uuid, char** uuid_p, DBusError* error) {
    if (uuid_p) {
        DBusString encoded;
        ...
        if (!_dbus_uuid_encode(uuid, &encoded) || !_dbus_string_steal_data(&encoded, uuid_p)) {
            _DBUS_SET_OOM(error);
 ...
 */
dbus_bool_t dbus_internal_do_not_use_get_uuid(
    const char* filename,
    char**      uuid_p,
    ...
    if (filename) {
        ...
        if (!_dbus_read_uuid_file(&filename_str, &uuid, create_if_not_found, error)) goto error;
    } else {
        if (!_dbus_read_local_machine_uuid(&uuid, create_if_not_found, error)) goto error;
    }

    if (!return_uuid(&uuid, uuid_p, error)) goto error;
