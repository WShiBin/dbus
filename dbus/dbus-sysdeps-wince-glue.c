...
static char* read_w32_registry_string(const char* root, const char* dir, const char* name) {
    ...
    if (RegOpenKeyExA(root_key, dir, 0, KEY_READ, &key_handle)) {
        if (root) return NULL; /* no need for a RegClose, so return direct */
    ...
    if (RegQueryValueExA(key_handle, name, 0, NULL, NULL, &nbytes)) {
        ...
        RegCloseKey(key_handle);
        if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, dir, 0, KEY_READ, &key_handle)) return NULL; /* Nope.  */
...
FormatMessageA(
    ...
    len = FormatMessageW(
        dwFlags | FORMAT_MESSAGE_ALLOCATE_BUFFER, lpSource, dwMessageId, dwLanguageId, (LPWSTR)&buffer_w, 0, Arguments);
    if (len == 0) return 0;
