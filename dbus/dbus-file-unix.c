...
dbus_bool_t _dbus_file_get_contents(DBusString* str, const DBusString* filename, DBusError* error) {
    ...
    if (fd < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to open \"%s\": %s", filename_c, _dbus_strerror(errno));
        return FALSE;
    }
    ...
    if (fstat(fd, &sb) < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Failed to stat \"%s\": %s", filename_c, _dbus_strerror(errno));
    ...
    if (sb.st_size > _DBUS_ONE_MEGABYTE) {
        dbus_set_error(
            error, DBUS_ERROR_FAILED, "File size %lu of \"%s\" is too large.", (unsigned long)sb.st_size, filename_c);
        _dbus_close(fd, NULL);
        return FALSE;
    }
 ...
 */
dbus_bool_t _dbus_string_save_to_file(
...
#define N_TMP_FILENAME_RANDOM_BYTES 8
    ...
    if (fd < 0) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Could not create %s: %s", tmp_filename_c, _dbus_strerror(errno));
        goto out;
    }
    if (world_readable) {
        ...
        if (fchmod(fd, 0644) < 0) {
            dbus_set_error(
                error, _dbus_error_from_errno(errno), "Could not chmod %s: %s", tmp_filename_c, _dbus_strerror(errno));
            goto out;
        }
    }
    ...
    if (!_dbus_close(fd, NULL)) {
        dbus_set_error(
            error, _dbus_error_from_errno(errno), "Could not close file %s: %s", tmp_filename_c, _dbus_strerror(errno));
