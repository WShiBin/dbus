...
static DBusHandlerResult echo_filter(DBusConnection* connection, DBusMessage* message, void* user_data) {
    ...
    if (reply == NULL || !dbus_connection_send(connection, reply, NULL)) g_error("OOM");
 ...
 */
static gpointer service_thread_cb(gpointer user_data) {
    Fixture* f = user_data;
 ...
 */
static gpointer client_dispatcher_thread_cb(gpointer user_data) {
    Fixture* f = user_data;
 ...
 * threads.
 */
static gpointer client_caller_thread_cb(gpointer user_data) {
    Fixture* f = user_data;
    gsize    i;
    ...
    for (i = 0; i < f->calls_per_thread; i++) {
        ...
        if (i % 100 == 0)
            g_printerr(
                "# thread %p: %" G_GSIZE_FORMAT "/%" G_GSIZE_FORMAT "\n", g_thread_self(), i, f->calls_per_thread);

        call = dbus_message_new_method_call(f->service_name, "/", "com.example.Echo", "Echo");
        g_assert_nonnull(call);

        time_before = g_get_monotonic_time();
        reply       = dbus_connection_send_with_reply_and_block(f->client_conn, call, 30000, &error);
...
static void setup(Fixture* f, gconstpointer context) {
    ...
    if (!dbus_connection_add_filter(f->service_conn, echo_filter, f, NULL)) g_error("OOM");

    f->service_thread           = g_thread_new("service", service_thread_cb, f);
    f->client_dispatcher_thread = g_thread_new("client dispatcher", client_dispatcher_thread_cb, f);
}
 ...
 */
static void test_threads(Fixture* f, gconstpointer context) {
    gsize i;
    ...
    for (i = 0; i < f->n_caller_threads; i++) {
        ...
        f->client_caller_threads[i] = g_thread_new(name, client_caller_thread_cb, f);
    }
    ...
    while (f->callers_remaining > 0) g_cond_wait(&f->callers_remaining_cond, &f->callers_remaining_mutex);

    g_mutex_unlock(&f->callers_remaining_mutex);

    for (i = 0; i < f->n_caller_threads; i++) g_thread_join(g_steal_pointer(&f->client_caller_threads[i]));
...
static void teardown(Fixture* f, gconstpointer context G_GNUC_UNUSED) {
    ...
    if (f->service_thread != NULL) g_thread_join(g_steal_pointer(&f->service_thread));

    if (f->client_dispatcher_thread != NULL) g_thread_join(g_steal_pointer(&f->client_dispatcher_thread));
...
int main(int argc, char** argv) {
    ...
    g_test_add("/thread-blocking", Fixture, NULL, setup, test_threads, teardown);
