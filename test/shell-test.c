...
static dbus_bool_t test_command_line_internal(dbus_bool_t should_work, const char* arg1, va_list var_args) {
    ...
    if (!_dbus_list_append(&list, (char*)arg1)) return FALSE;
    ...
    for (i = 0, node = _dbus_list_get_first_link(&list); i < original_argc && node;
        ...
        if ((i > 0 && !_dbus_string_append_byte(&str, ' ')) || !_dbus_string_append(&str, original_argv[i])) {
            _dbus_list_clear(&list);
            dbus_free(original_argv);
    ...
    if (!_dbus_shell_parse_argv(command_line, &shell_argc, &shell_argv, &error)) {
        printf(
            "# Error%s parsing command line: %s\n",
        ...
        for (i = 0; i < shell_argc; i++) {
            ...
            if (strcmp(unquoted ? unquoted : "", shell_argv[i] ? shell_argv[i] : "")) {
                printf(
                    "Position %d, returned argument (%s) does not match original (%s)\n", i, shell_argv[i], unquoted);
                dbus_free(unquoted);
                dbus_free(original_argv);
                dbus_free_string_array(shell_argv);
                return FALSE;
            ...
            dbus_free(unquoted);
            if (shell_argv[i]) printf("Argument %d = %s\n", i, shell_argv[i]);
        }
...
static void test_command_line(const char* arg1, ...) {
    va_list var_args;
...
static void test_command_line_fails(const char* arg1, ...) {
    va_list var_args;
