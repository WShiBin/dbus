...
int main(int argc, char** argv) {
    ...
    if (argc != 2) die("syntax: manual-dir-iter <path>");

    dbus_error_init(&tmp_error);

    if (!_dbus_string_init(&filename)) oom("init filename");

    if (!_dbus_string_init(&dirname)) oom("init dirname");

    if (!_dbus_string_append(&dirname, argv[1])) oom("append argv[1]");
    ...
    if (dbus_error_is_set(&tmp_error)) die(tmp_error.message);
