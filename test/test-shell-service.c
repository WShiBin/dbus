...
static DBusHandlerResult handle_echo(DBusConnection* connection, DBusMessage* message) {
    ...
    if (!dbus_connection_get_object_path_data(connection, echo_path, (void**)&d)) die("No memory");
    ...
    for (i = 0; i < d->argc; ++i)
        if (!dbus_message_iter_append_basic(&iter, DBUS_TYPE_STRING, &(d->argv[i]))) die("No memory\n");
...
int main(int argc, char** argv) {
    ...
    if (!dbus_connection_register_object_path(connection, echo_path, &echo_vtable, (void*)&echo_data)) die("No memory");

    {
        void* d;
        if (!dbus_connection_get_object_path_data(connection, echo_path, &d)) die("No memory");
        if (d != (void*)&echo_data) die("dbus_connection_get_object_path_data() doesn't seem to work right\n");
    }

    result = dbus_bus_request_name(connection, "org.freedesktop.DBus.TestSuiteShellEchoServiceSuccess", 0, &error);
