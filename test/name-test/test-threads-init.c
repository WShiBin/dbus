...
static void _run_iteration(DBusConnection* conn) {
    ...
    method = dbus_message_new_method_call(
        "org.freedesktop.DBus.TestSuiteEchoService", "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Echo");
    ...
    method =
        dbus_message_new_method_call(DBUS_SERVICE_DBUS, DBUS_PATH_DBUS, "org.freedesktop.Introspectable", "Introspect");
...
static void check_mutex_lock(DBusMutex* mutex1, DBusMutex* mutex2, dbus_bool_t is_same) {
...
static void check_condvar_lock(DBusCondVar* condvar1, DBusCondVar* condvar2, dbus_bool_t is_same) {
...
int main(int argc, char* argv[]) {
    ...
    _dbus_connection_test_get_locks(conn, &mutex1, &dispatch_mutex1, &io_path_mutex1, &dispatch_cond1, &io_path_cond1);
    _run_iteration(conn);
    _dbus_connection_test_get_locks(conn, &mutex2, &dispatch_mutex2, &io_path_mutex2, &dispatch_cond2, &io_path_cond2);
    ...
    method = dbus_message_new_method_call(
        "org.freedesktop.TestSuiteEchoService", "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Exit");
