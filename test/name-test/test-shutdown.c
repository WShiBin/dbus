...
static void open_destroy_shared_session_bus_connection(void) {
    DBusError       error;
    ...
    if (comma == NULL) die("Couldn't find GUID in session bus address");
    *comma = '\0';

    connection = dbus_connection_open(session_addr_no_guid, &error);
    free(session_addr_no_guid);
    if (connection == NULL) die("Failed to open connection to temp session bus\n");
