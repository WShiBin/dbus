...
int main(int argc, char** argv) {
    ...
    if (server_id == NULL) die("No bus server ID retrieved\n");

    printf("ok %d - session bus server ID is %s\n", ++test_num, server_id);

    if (strlen(server_id) != 32) die("Bus server id should have length 32\n");
    ...
    if (id == NULL) die("No bus ID retrieved\n");

    printf("ok %d - session bus ID is %s\n", ++test_num, id);

    if (strlen(id) != 32) die("Bus ID should have length 32\n");
