...
static void die(const char* message, ...) _DBUS_GNUC_NORETURN _DBUS_GNUC_PRINTF(1, 2);
...
static DBusHandlerResult filter_session_message(DBusConnection* connection, DBusMessage* message, void* user_data) {
    ...
    if (dbus_message_is_signal(message, DBUS_INTERFACE_DBUS, "NameOwnerChanged") &&
        ...
        dbus_message_get_args(
            ...
            &new_owner,
            DBUS_TYPE_INVALID) &&
        strcmp(name, PRIVSERVER_SERVICE) == 0 && old_owner[0] != '\0' && new_owner[0] == '\0') {
        *service_died_p = TRUE;
    }
...
static DBusHandlerResult filter_private_message(DBusConnection* connection, DBusMessage* message, void* user_data) {
...
static void open_shutdown_private_connection(dbus_bool_t use_guid) {
    DBusError       error;
    DBusLoop*       loop;
    ...
    if (!session) die("couldn't access session bus\n");
    ...
    if (dbus_error_is_set(&error))
        die("couldn't add match rule \"%s\": %s: %s", PRIVSERVER_DIED_RULE, error.name, error.message);

    if (!dbus_connection_add_filter(session, filter_session_message, &service_died, NULL))
        die("couldn't add filter to session bus\n");

    msg = dbus_message_new_method_call(PRIVSERVER_SERVICE, "/", PRIVSERVER_INTERFACE, "GetPrivateAddress");
    if (!(reply = dbus_connection_send_with_reply_and_block(session, msg, -1, &error)))
        die("couldn't send message: %s\n", error.message);
    dbus_message_unref(msg);

    if (dbus_set_error_from_message(&error, reply)) die("%s: %s", error.name, error.message);
    ...
    free(addr);
    if (!privconn) die("couldn't connect to server direct connection: %s\n", error.message);
    dbus_message_unref(reply);

    dbus_connection_set_exit_on_disconnect(privconn, FALSE);
    if (!dbus_connection_add_filter(privconn, filter_private_message, &private_conn_lost, NULL))
        die("couldn't add filter to private connection\n");
    test_connection_setup(loop, privconn);

    msg = dbus_message_new_method_call(PRIVSERVER_SERVICE, "/", PRIVSERVER_INTERFACE, "Quit");
    if (!dbus_connection_send(session, msg, NULL)) die("couldn't send Quit message\n");
    dbus_message_unref(msg);

    while (!service_died || !private_conn_lost) _dbus_loop_iterate(loop, TRUE);

    dbus_connection_remove_filter(session, filter_session_message, &service_died);
    ...
    dbus_connection_remove_filter(privconn, filter_private_message, &private_conn_lost);
