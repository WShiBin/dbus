...
dbus_bool_t test_connection_setup(TestMainContext* ctx, DBusConnection* connection) {
    ...
    if (!dbus_connection_set_watch_functions(connection, add_watch, remove_watch, toggle_watch, cd, cdata_free))
        goto nomem;

    cd = cdata_new(loop, connection);
    if (cd == NULL) goto nomem;

    if (!dbus_connection_set_timeout_functions(connection, add_timeout, remove_timeout, NULL, cd, cdata_free))
        goto nomem;
...
dbus_bool_t test_server_setup(TestMainContext* ctx, DBusServer* server) {
    ...
    if (!dbus_server_set_watch_functions(
            server, add_server_watch, remove_server_watch, toggle_server_watch, sd, serverdata_free)) {
        goto nomem;
    }
    ...
    if (!dbus_server_set_timeout_functions(
            server, add_server_timeout, remove_server_timeout, NULL, sd, serverdata_free)) {
        goto nomem;
    }
    return TRUE;
