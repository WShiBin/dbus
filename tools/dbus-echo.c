...
static void usage_echo(int exit_with) {
    fprintf(
        stderr,
...
static void usage_black_hole(int exit_with) {
    fprintf(
        stderr,
...
static DBusHandlerResult filter(DBusConnection* connection, DBusMessage* message, void* user_data) {
    ...
    if (!noreply) {
        ...
        if (reply == NULL) tool_oom("allocating reply");

        if (!dbus_connection_send(connection, reply, NULL)) tool_oom("sending reply");
...
static DBusConnection* init_connection(DBusBusType type, const char* name) {
    ...
    if (name != NULL) {
        if (dbus_bus_request_name(connection, name, DBUS_NAME_FLAG_DO_NOT_QUEUE, NULL) !=
    ...
    if (!dbus_connection_add_filter(connection, filter, NULL, NULL)) tool_oom("adding message filter");
...
int dbus_test_tool_black_hole(int argc, char** argv) {
    ...
    if (noread) {
        while (1) _dbus_sleep_milliseconds(3600);
    }
