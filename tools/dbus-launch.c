...
static void save_machine_uuid(const char* uuid_arg) {
    if (strlen(uuid_arg) != 32) {
        fprintf(stderr, "machine ID '%s' looks like it's the wrong length, should be 32 hex digits", uuid_arg);
        exit(1);
    }
...
static int read_machine_uuid_if_needed(DBusError* error) {
    if (machine_uuid != NULL) return TRUE;

    machine_uuid = dbus_try_get_local_machine_id(error);

    if (machine_uuid == NULL) return FALSE;
...
static ReadStatus read_line(int fd, char* buf, size_t maxlen) {
    size_t     bytes = 0;
    ReadStatus retval;
    ...
    again:
        ...
        chunk = read(fd, buf + bytes, to_read);
    ...
    if (retval == READ_STATUS_EOF && bytes > 0) retval = READ_STATUS_OK;

    /* whack newline */
    if (retval != READ_STATUS_ERROR && bytes > 0 && buf[bytes - 1] == '\n') buf[bytes - 1] = '\0';

    return retval;
}

static ReadStatus read_pid(int fd, pid_t* buf) {
    size_t     bytes = 0;
    ReadStatus retval;
...
static void print_variables(
    ...
    if (binary_syntax) {
        ...
        if (bus_pid) printf("set DBUS_SESSION_BUS_PID=%ld;\n", (long)bus_pid);
        if (bus_wid) printf("set DBUS_SESSION_BUS_WINDOWID=%ld;\n", (long)bus_wid);
        fflush(stdout);
    } else if (bourne_shell_syntax) {
        printf("DBUS_SESSION_BUS_ADDRESS='%s';\n", bus_address);
        printf("export DBUS_SESSION_BUS_ADDRESS;\n");
        if (bus_pid) printf("DBUS_SESSION_BUS_PID=%ld;\n", (long)bus_pid);
        if (bus_wid) printf("DBUS_SESSION_BUS_WINDOWID=%ld;\n", (long)bus_wid);
        fflush(stdout);
    } else {
        printf("DBUS_SESSION_BUS_ADDRESS=%s\n", bus_address);
        if (bus_pid) printf("DBUS_SESSION_BUS_PID=%ld\n", (long)bus_pid);
...
static void kill_bus_when_session_ends(void) {
    ...
    if (tty_fd < 0 && x_fd < 0) {
        fprintf(
            stderr, "No terminal on standard input and no X display; cannot attach message bus to session lifetime\n");
        kill_bus_and_exit(1);
    }
    ...
    while (TRUE) {
        ...
        select(MAX(tty_fd, x_fd) + 1, &read_set, NULL, &err_set, NULL);
         ...
         */
        if (x_fd >= 0)
            verbose("X fd condition reading = %d error = %d\n", FD_ISSET(x_fd, &read_set), FD_ISSET(x_fd, &err_set));
#endif
...
_DBUS_GNUC_NORETURN static void babysit(
    ...
    int   read_bus_pid_fd) /* read pid from here */
{
    ...
    verbose(
        "babysitting, exit_with_session = %d, child_pid = %ld, read_bus_pid_fd = %d\n",
        exit_with_session,
        (long)child_pid,
        read_bus_pid_fd);
    ...
    if (!exit_with_session) flags |= DBUS_FORCE_STDIN_NULL;
...
static void pass_info(
    ...
    if (runprog) {
        ...
        if (envvar == NULL || args == NULL) goto oom;
        ...
        print_variables(bus_address, bus_pid, bus_wid, c_shell_syntax, bourne_shell_syntax, binary_syntax);
    }
...
int main(int argc, char** argv) {
    ...
    if (!_dbus_ensure_standard_fds(0, &error_str)) {
        fprintf(stderr, "dbus-launch: fatal error setting up standard fds: %s: %s\n", error_str, _dbus_strerror(errno));
        return 1;
    }
    ...
    while (i < argc) {
        ...
        if (strcmp(arg, "--help") == 0 || strcmp(arg, "-h") == 0 || strcmp(arg, "-?") == 0)
        ...
        else if (strcmp(arg, "-c") == 0 || strcmp(arg, "--csh-syntax") == 0)
            c_shell_syntax = TRUE;
        else if (strcmp(arg, "-s") == 0 || strcmp(arg, "--sh-syntax") == 0)
            ...
            save_machine_uuid(s);
        } else if (prev_arg && strcmp(prev_arg, "--autolaunch") == 0) {
            if (autolaunch) {
                fprintf(stderr, "--autolaunch given twice\n");
                exit(1);
            }
    ...
    if (autolaunch) {
        ...
        if (!_dbus_string_init(&user_bus)) tool_oom("initializing");
        ...
        if (!read_machine_uuid_if_needed(&error)) {
            fprintf(
                stderr, "Session lifetime based on X11 requested, but machine UUID unavailable: %s.\n", error.message);
    ...
    if (pipe(bus_pid_to_launcher_pipe) < 0 || pipe(bus_address_to_launcher_pipe) < 0 ||
...
#define MAX_FD_LEN 64
        ...
        if (close_stderr) do_close_stderr();
        ...
        if (ret > 0) {
             ...
             * daemon
             */
            babysit(exit_with_session || exit_with_x11, ret, bus_pid_to_babysitter_pipe[READ_END]);
            exit(0);
        }
        ...
        if (autolaunch && user_bus_supported) {
            ...
            do_write(
                bus_address_to_launcher_pipe[WRITE_END],
                _dbus_string_get_const_data(&user_bus),
        ...
        sprintf(write_pid_fd_as_string, "%d", bus_pid_to_launcher_pipe[WRITE_END]);

        sprintf(write_address_fd_as_string, "%d", bus_address_to_launcher_pipe[WRITE_END]);
...
#ifdef DBUS_ENABLE_EMBEDDED_TESTS
        {
            ...
            if (test_daemon != NULL) {
                if (config_file == NULL && getenv("DBUS_TEST_DATA") != NULL) {
                    config_file = concat2(getenv("DBUS_TEST_DATA"), "/valid-config-files/session.conf");
                ...
                execl(
                    ...
                    "--fork",
                    "--print-pid",
                    write_pid_fd_as_string,
                    "--print-address",
                    write_address_fd_as_string,
                    config_file ? "--config-file" : "--session",
                    config_file, /* has to be last in this varargs list */
                    NULL);

                fprintf(stderr, "Failed to execute test message bus daemon %s: %s.\n", test_daemon, strerror(errno));
                exit(1);
            }
        }
#endif /* DBUS_ENABLE_EMBEDDED_TESTS */
        ...
        fprintf(stderr, "Failed to execute message bus daemon: %s\n", strerror(errno));
        exit(1);
    } else {
        /* Parent */
#define MAX_PID_LEN 64
        ...
        switch (read_line(bus_address_to_launcher_pipe[READ_END], bus_address, MAX_ADDR_LEN)) {
            case READ_STATUS_OK:
                break;
            case READ_STATUS_EOF:
        ...
        if (xdisplay != NULL) {
            ...
            if (autolaunch) {
                if (ret2 == 0) {
                    ...
                    if (x11_get_address(&address, &bus_pid, &wid) && address != NULL) {
                        verbose("dbus-daemon is already running. Returning existing parameters.\n");
                        /* Kill the old bus */
                        kill_bus();
                        pass_info(
