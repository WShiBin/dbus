...
static void exec_dbus_daemon(const char* dbus_daemon, int bus_address_pipe[2], const char* config_file) {
...
static void exec_app(int prog_arg, char** argv) {
    execvp(argv[prog_arg], argv + prog_arg);

    fprintf(stderr, "%s: failed to exec '%s': %s\n", me, argv[prog_arg], strerror(errno));
    exit(1);
}
...
int main(int argc, char** argv) {
    ...
    while (i < argc) {
        ...
        if (strcmp(arg, "--help") == 0 || strcmp(arg, "-h") == 0 || strcmp(arg, "-?") == 0) {
            ...
            dbus_daemon = file;
        } else if (strcmp(arg, "--config-file") == 0 || strcmp(arg, "--dbus-daemon") == 0) {
            requires_arg = 1;
        } else if (arg[0] == '-') {
            if (strcmp(arg, "--") != 0) {
    ...
    if (dbus_daemon == NULL) dbus_daemon = "dbus-daemon";
    ...
    if (!dbus_setenv("DBUS_SESSION_BUS_ADDRESS", bus_address) || !dbus_setenv("DBUS_SESSION_BUS_PID", NULL) ||
        !dbus_setenv("DBUS_SESSION_BUS_WINDOWID", NULL) || !dbus_setenv("DBUS_STARTER_ADDRESS", NULL) ||
