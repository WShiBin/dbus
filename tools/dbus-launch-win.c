...
static errno_t wcscat_s(wchar_t* dest, size_t size, const wchar_t* src) {
    assert(sizeof(wchar_t) * (wcslen(dest) + wcslen(src) + 1) <= size);
    wcscat(dest, src);
    return 0;
}

static errno_t wcscpy_s(wchar_t* dest, size_t size, const wchar_t* src) {
...
int main(int argc, char** argv) {
    ...
    if ((p = wcsrchr(dbusDaemonPath, L'\\'))) {
        ...
        if (verbose)
            fprintf(
                stderr,
                "error: could not extract path from current "
    ...
    if (verbose) fprintf(stderr, "%ls %ls\n", dbusDaemonPath, command);
#else
    command[0] = L'\0';
    ...
    if (verbose) flags |= CREATE_NEW_CONSOLE;

#ifdef DBUS_WINCE
    inherit = FALSE;
#else
    flags |= NORMAL_PRIORITY_CLASS;
    if (!verbose) flags |= DETACHED_PROCESS;
#endif

    result = CreateProcessW(dbusDaemonPath, command, 0, 0, inherit, flags, 0, 0, &si, &pi);

    if (result == 0) {
        if (verbose) fprintf(stderr, "Could not start " DBUS_DAEMON_NAME ". error=%u\n", (unsigned)GetLastError());
        return 4;
    }
