...
static void pc_notify(DBusPendingCall* pc, void* data) {
    DBusMessage* message;
    int*         received_p = data;
    DBusError    error;
...
static void consume_stdin(char** payload_p, size_t* len_p) {
    ...
    if (buf == NULL) tool_oom("reading payload from stdin");

    while (1) {
        if (len - pos < BLOCK_SIZE) {
            char* tmp = dbus_realloc(buf, len + BLOCK_SIZE);

            if (tmp == NULL) tool_oom("reading payload from stdin");
...
int dbus_test_tool_spam(int argc, char** argv) {
    ...
    for (i = 2; i < argc; i++) {
        ...
        if (payload != NULL && (strstr(arg, "--payload=") == arg || strcmp(arg, "--stdin") == 0 ||
                                strcmp(arg, "--message-stdin") == 0 || strcmp(arg, "--random-size") == 0)) {
            fprintf(
                stderr,
                "At most one of --payload, --stdin, --message-stdin "
        ...
        if (strcmp(arg, "--system") == 0) {
            ...
            if (template == NULL) {
                fprintf(stderr, "Unable to demarshal template message: %s: %s", error.name, error.message);
                exit(1);
            }
            ...
            for (p = payload_buf; p < payload_buf + payload_len; p += consumed) {
                ...
                if (sscanf(p, " %u %n", &len, &consumed) == 0) break;
            ...
            if (random_sizes == NULL) tool_oom("allocating array of message lengths");

            for (p = payload_buf, j = 0; p < payload_buf + payload_len && j < n_random_sizes; p += consumed, j++) {
            ...
            if (payload_buf == NULL) tool_oom("allocating maximum-sized payload");
            ...
            if (queue_len > 1) usage(2);

            if (messages_per_conn > 0) usage(2);
            ...
            if (messages_per_conn > 0 && flood) usage(2);
        } else {
            usage(2);
        }
    }
    ...
    VERBOSE(
        stderr,
        "Will send up to %d messages, with up to %d queued, max %d per connection\n",
        count,
        queue_len,
        messages_per_conn);
    ...
    while (no_reply ? sent < count : received < count) {
         ...
         */
        if (connection == NULL || (messages_per_conn > 0 && sent_in_this_conn == messages_per_conn &&
         ...
         * - queue
         */
        while (sent < count && (messages_per_conn == 0 || sent_in_this_conn < messages_per_conn) &&
            ...
            if (template != NULL) {
                ...
                if (message == NULL) tool_oom("copying message");
                ...
                message = dbus_message_new_method_call(destination, "/", "com.example", "Spam");

                if (message == NULL) tool_oom("allocating message");
                ...
                switch (payload_type) {
                    case DBUS_TYPE_STRING:
                        ...
                        mem = dbus_message_append_args(message, DBUS_TYPE_STRING, &payload, DBUS_TYPE_INVALID);
                    ...
                    case DBUS_TYPE_ARRAY:
                        ...
                        if (random_sizes != NULL) len = random_sizes[rand() % n_random_sizes];

                        mem = dbus_message_append_args(
                            message, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE, &payload, (dbus_uint32_t)len, DBUS_TYPE_INVALID);
                        break;
                ...
                if (!mem) tool_oom("building message");
            }

            if (no_reply) {
                if (!dbus_connection_send(connection, message, NULL)) tool_oom("sending message");
                ...
                if (!dbus_connection_send_with_reply(connection, message, &pc, DBUS_TIMEOUT_INFINITE))
                ...
                if (pc == NULL) tool_oom("sending message");

                if (dbus_pending_call_get_completed(pc))
                    pc_notify(pc, &received);
                else if (!dbus_pending_call_set_notify(pc, pc_notify, &received, NULL))
    ...
    if (template != NULL) dbus_message_unref(template);
