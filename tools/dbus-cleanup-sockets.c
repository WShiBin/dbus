...
typedef enum { SOCKET_UNKNOWN, SOCKET_FAILED_TO_HANDLE, SOCKET_DEAD, SOCKET_ALIVE, SOCKET_UNLINKED } SocketStatus;
...
static SocketEntry* socket_entry_new(const char* dir, const char* fname) {
    SocketEntry* se;
    int          len;
...
static void free_socket_entry(SocketEntry* se) {
    if (se) {
        free(se->name);
...
static void free_socket_entries(SocketEntry** entries, int n_entries) {
    int i;

    if (entries) {
        for (i = 0; i < n_entries; ++i) free_socket_entry(entries[i]);
        free(entries);
    }
}

static void read_sockets(const char* dir, SocketEntry*** entries_p, int* n_entries_p) {
    DIR*           dirh;
    struct dirent* dent;
    SocketEntry**  entries;
    int            n_entries;
    ...
    while ((dent = readdir(dirh))) {
        ...
        if (strncmp(dent->d_name, "dbus-", 5) != 0) continue;
...
static SocketStatus open_socket(SocketEntry* se) {
    int                ret;
    ...
    if (se->n_retries > 5) {
        fprintf(
            stderr,
            "Warning: giving up on socket %s after several retries; unable to determine socket's status\n",
            se->name);
...
static int handle_sockets(SocketEntry** entries, int n_entries) {
    int i;
    int n_unknown;
    ...
    while (i < n_entries) {
        ...
        if (se->status != SOCKET_UNKNOWN) continue;
        ...
        if (se->status == SOCKET_UNKNOWN) n_unknown += 1;
    }
...
static void version(void) {
    printf(
        "D-Bus Socket Cleanup Utility %s\n"
        "Copyright (C) 2003 Red Hat, Inc.\n"
        "Copyright (C) 2002 Michael Meeks\n"
...
int main(int argc, char** argv) {
    ...
    if (dirname == NULL) dirname = DBUS_SESSION_SOCKET_DIR;

#ifdef AF_UNIX
    clean_dir(dirname);

    printf(
        "Cleaned up %d sockets in %s; %d sockets are still in use; %d in unknown state\n",
        cleaned_count,
        dirname,
        alive_count,
        unhandled_count);
