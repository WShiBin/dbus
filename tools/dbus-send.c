...
static void usage(int ecode) {
    fprintf(
        stderr,
        "Usage: %s [--help] [--system | --session | --bus=ADDRESS | --peer=ADDRESS] [--dest=NAME] [--type=TYPE] "
...
int main(int argc, char* argv[]) {
    ...
    for (i = 1; i < argc && name == NULL; i++) {
        ...
        if (strcmp(arg, "--system") == 0) {
            ...
            (strstr(arg, "--bus=") == arg) || (strstr(arg, "--peer=") == arg) || (strstr(arg, "--address=") == arg)) {
            ...
            reply_timeout = strtol(strchr(arg, '=') + 1, NULL, 10);
            if (reply_timeout <= 0) {
    ...
    if (type_str != NULL) {
        ...
        if (!(message_type == DBUS_MESSAGE_TYPE_METHOD_CALL || message_type == DBUS_MESSAGE_TYPE_SIGNAL)) {
    ...
    if (connection == NULL) {
        fprintf(
            ...
            (address != NULL) ? address : ((type == DBUS_BUS_SYSTEM) ? "system" : "session"),
            error.message);
        ...
        *(c++) = 0;
        ...
        if (container_type == DBUS_TYPE_DICT_ENTRY) {
            ...
            *(c++)         = 0;
            ...
            sig[4]         = '\0';
            handle_oom(dbus_message_iter_open_container(&iter, DBUS_TYPE_ARRAY, sig, &container_iter));
            target_iter = &container_iter;
        } else if (container_type != DBUS_TYPE_INVALID) {
            char sig[2];
            sig[0] = type2;
            sig[1] = '\0';
            handle_oom(dbus_message_iter_open_container(&iter, container_type, sig, &container_iter));
            target_iter = &container_iter;
        } else
            target_iter = &iter;
        ...
        if (container_type != DBUS_TYPE_INVALID) {
            handle_oom(dbus_message_iter_close_container(&iter, &container_iter));
        }
    }
