...
static int x_io_error_handler(Display* local_xdisplay) {
    verbose("X IO error\n");
    kill_bus_and_exit(0);
    return 0;
}
...
static char* get_session_file(void) {
    ...
    result = malloc(strlen(home) + strlen(prefix) + strlen(machine) + strlen(display) + 2);
    if (result == NULL) {
        /* out of memory */
        free(display);
        return NULL;
...
static void ensure_session_directory(void) {
    const char* home;
    char*       dir;
...
static Display* open_x11(void) {
    if (xdisplay != NULL) return xdisplay;
...
static int init_x_atoms(Display* display) {
    ...
    atom_name = malloc(
        strlen(machine) + strlen(user_name) + 2 +
        MAX(strlen(selection_prefix), MAX(strlen(address_prefix), strlen(pid_prefix))));
    if (atom_name == NULL) {
...
int x11_get_address(char** paddress, pid_t* pid, long* wid) {
    ...
    *paddress = NULL;
    ...
    if (owner == None) return TRUE; /* no owner */
    if (wid != NULL) *wid = (long)owner;
    ...
    result = XGetWindowProperty(
        ...
        &after,
        (unsigned char**)&data);
    if (result != Success || type == None || after != 0 || data == NULL || format != 8) return FALSE; /* error */
 ...
 */
static Window set_address_in_x11(char* address, pid_t pid) {
    ...
    /* Create our window */
    wid = XCreateWindow(
        xdisplay, RootWindow(xdisplay, 0), -20, -20, 10, 10, 0, CopyFromParent, InputOnly, CopyFromParent, 0, NULL);
    /* The type of a Window varies, so cast it to something reasonable */
    verbose("Created window %lu\n", (unsigned long)wid);

    /* Save the property in the window */
    XChangeProperty(
        xdisplay, wid, address_atom, XA_STRING, 8, PropModeReplace, (unsigned char*)address, strlen(address));
    pid32 = pid;
    XChangeProperty(xdisplay, wid, pid_atom, XA_CARDINAL, 32, PropModeReplace, (unsigned char*)&pid32, 1);
 ...
 */
static int set_address_in_file(char* address, pid_t pid, Window wid) {
    char* session_file;
    FILE* f;
    ...
    free(session_file);
    if (f == NULL) return FALSE; /* some kind of error */
    fprintf(
        f,
...
int x11_save_address(char* address, pid_t pid, long* wid) {
    ...
    if (id != None) {
        if (!set_address_in_file(address, pid, id)) return FALSE;

        if (wid != NULL) *wid = (long)id;
        return TRUE;
    }
    return FALSE;
}
