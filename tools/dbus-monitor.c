...
static void profile_print_with_attrs(
    const char*           type,
    DBusMessage*          message,
    long                  sec,
    long                  usec,
    ...
    if (attrs & PROFILE_ATTRIBUTE_FLAG_SERIAL) printf("\t%u", dbus_message_get_serial(message));

    if (attrs & PROFILE_ATTRIBUTE_FLAG_SENDER) printf("\t%s", TRAP_NULL_STRING(dbus_message_get_sender(message)));

    if (attrs & PROFILE_ATTRIBUTE_FLAG_DESTINATION)
        printf("\t%s", TRAP_NULL_STRING(dbus_message_get_destination(message)));

    if (attrs & PROFILE_ATTRIBUTE_FLAG_REPLY_SERIAL) printf("\t%u", dbus_message_get_reply_serial(message));

    if (attrs & PROFILE_ATTRIBUTE_FLAG_PATH) printf("\t%s", TRAP_NULL_STRING(dbus_message_get_path(message)));

    if (attrs & PROFILE_ATTRIBUTE_FLAG_INTERFACE) printf("\t%s", TRAP_NULL_STRING(dbus_message_get_interface(message)));

    if (attrs & PROFILE_ATTRIBUTE_FLAG_MEMBER) printf("\t%s", TRAP_NULL_STRING(dbus_message_get_member(message)));
...
static void print_message_profile(DBusMessage* message) {
    ...
    switch (dbus_message_get_type(message)) {
        case DBUS_MESSAGE_TYPE_METHOD_CALL:
            profile_print_with_attrs(
                ...
                sec,
                usec,
                PROFILE_ATTRIBUTE_FLAG_SERIAL | PROFILE_ATTRIBUTE_FLAG_SENDER | PROFILE_ATTRIBUTE_FLAG_DESTINATION |
...
static DBusHandlerResult profile_filter_func(DBusConnection* connection, DBusMessage* message, void* user_data) {
...
static DBusHandlerResult binary_filter_func(DBusConnection* connection, DBusMessage* message, void* user_data) {
     ...
     */
    if (!dbus_message_marshal(message, &blob, &len)) tool_oom("retrieving message");
...
static void usage(char* name, int ecode) {
    fprintf(
        stderr,
        "Usage: %s [--system | --session | --address ADDRESS] [--monitor | --profile | --pcap | --binary ] [watch "
        "expressions]\n",
        name);
    exit(ecode);
}
...
static dbus_bool_t become_monitor(DBusConnection* connection, int numFilters, const char* const* filters) {
    ...
    if (m == NULL) tool_oom("becoming a monitor");
...
int main(int argc, char* argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (!strcmp(arg, "--system")) {
            ...
            if (filters == NULL) tool_oom("adding a new filter slot");
            filters[j] = (char*)malloc(filter_len);
            if (filters[j] == NULL) tool_oom("adding a new filter");
    ...
    if (!dbus_connection_add_filter(connection, filter_func, _DBUS_INT_TO_POINTER(binary_mode), NULL)) {
    ...
    if (become_monitor(connection, numFilters, (const char* const*)filters)) {
        ...
        if (dbus_error_is_set(&error)) {
            ...
            if (dbus_error_is_set(&error)) goto lose;
        }
    }
