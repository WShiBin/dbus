...
static void indent(int depth) {
    while (depth-- > 0) printf("   "); /* INDENT spaces. */
}

static void print_hex(const unsigned char* bytes, unsigned int len, int depth) {
...
static void print_ay(DBusMessageIter* iter, int depth) {
    ...
    for (i = 0; i < len; i++) {
        if ((bytes[i] < 32 || bytes[i] > 126) && (i < len - 1 || bytes[i] != '\0')) {
            all_ascii = FALSE;
            break;
        }
    }
    ...
    if (all_ascii && len > 0 && bytes[len - 1] == '\0') {
        ...
        if (copy == NULL) tool_oom("copying bytestring");
...
static void print_fd(int fd, int depth) {
    ...
    indent(depth + 1);
    printf("type: ");
    if (S_ISREG(statbuf.st_mode)) printf("file\n");
    ...
    if (getsockname(fd, &addr.sa, &addrlen)) return;
    ...
    switch (addr.sa.sa_family) {
        case AF_UNIX:
            ...
            if (addr.un.sun_path[0] == '\0') {
                 ...
                 */
                indent(depth + 1);
                printf("name @%.*s\n", (int)(addrlen - sizeof(sa_family_t) - 1), &(addr.un.sun_path[1]));
        ...
        switch (type) {
            ...
            case DBUS_TYPE_ARRAY: {
                ...
                while (current_type != DBUS_TYPE_INVALID) {
                    ...
                    if (current_type != DBUS_TYPE_INVALID) printf(",");
                }
                indent(depth);
                printf("]\n");
                break;
            ...
            case DBUS_TYPE_STRUCT: {
                ...
                while ((current_type = dbus_message_iter_get_arg_type(&subiter)) != DBUS_TYPE_INVALID) {
                    print_iter(&subiter, literal, depth + 1);
                    dbus_message_iter_next(&subiter);
                    if (dbus_message_iter_get_arg_type(&subiter) != DBUS_TYPE_INVALID) printf(",");
                }
                indent(depth);
                printf("}\n");
                break;
...
void print_message(DBusMessage* message, dbus_bool_t literal, long sec, long usec) {
    ...
    if (!literal) {
        if (sec != 0 || usec != 0) {
            printf(
                "%s time=%ld.%06ld sender=%s -> destination=%s",
